...
<lz:DefaultCtrl>{
	...
	editables:"csugId,csugName",
	${lz:set("注释","当处于添加数据时哪些字段可显示。为什么？因为有些字段可能是只读的")}
	visibles:"csugId,csugName",
	</s:if>
	<s:else>
	${lz:set("注释","当处于编辑数据时哪些字段可编辑")}
	editables:"csugId,csugName,${unit_login.$csuuUnitInfo.csuiType==2?"csugPerson,":""}csugMemo",
	${lz:set("注释","当处于编辑数据时哪些字段可显示。为什么？因为有些字段可能是只读的")}
	visibles:"csugId,csugName,${unit_login.$csuuUnitInfo.csuiType==2?"csugPerson,":""}csugMemo",
	</s:else>
}</lz:DefaultCtrl>
${lz:set("注释","***************************************************")}
...
<s:else>
	<form class="form editform" ref="CsUnitGroup" id="groupForm" name="groupForm" action="${empty CTRL.action?"group.save.do":CTRL.action}" method="post">
		<div class="head"></div>
		<div class="body">
			<div class="content">
	...
	${before$csugId}
	<dl class="${CTRL.e.csugId?"hidden":""}" major  ref="csugId" >
		<dt>编　　号:</dt>
		<s:if test="#request.CTRL.e.csugId==true">
		${lz:set("haveEditable",true)}
	...
	${before$csugInfo}
	<dl class="" major  ref="csugInfo" >
		<dt>所属单位:</dt>
		<s:if test="#request.CTRL.e.csugInfo==true">
		${lz:set("haveEditable",true)}
		<dd input="query">
		<s:if test="#request.csUnitGroup$csugInfo!=null">${csUnitGroup$csugInfo}</s:if><s:else>
		 			<input title="请输入单位信息名称进行查询" class="combox narrow" action="${basePath}${proname}/unit/info_query.do?value={param}&csuiHost={csugHost}" type="text" id="csugInfo" name="csUnitGroup.csugInfo" text="${get:CsUnitInfo(csUnitGroup.csugInfo).csuiName}" value="${csUnitGroup.csugInfo}" />
	...
	${before$csugRemark}
	<dl class="" major  ref="csugRemark" style="width:98%;">
		<dt>备　　注:</dt>
		<s:if test="#request.CTRL.e.csugRemark==true">
		${lz:set("haveEditable",true)}
		<dd input="textarea">
	...
	${before$csugMemo}
	<dl class="" minor  ref="csugMemo" style="width:98%;">
		<dt>备忘信息:</dt>
		<s:if test="#request.CTRL.e.csugMemo==true">
		${lz:set("haveEditable",true)}
		<dd input="textarea">
	...
	${before$csugData}
	<dl class="" minor  ref="csugData" style="width:98%;">
		<dt>程序数据:</dt>
		<s:if test="#request.CTRL.e.csugData==true">
		${lz:set("haveEditable",true)}
		<dd input="textarea">
	...
	<dl class="" major  ref="csugUpdateTime" >
		<dt>修改时间:</dt>
		<s:if test="#request.CTRL.e.csugUpdateTime==true">
		${lz:set("haveEditable",true)}
		<dd input="datetime">
	...
	${before$csugAddTime}
	<dl class="" major  ref="csugAddTime" >
		<dt>添加时间:</dt>
		<s:if test="#request.CTRL.e.csugAddTime==true">
		${lz:set("haveEditable",true)}
		<dd input="datetime">
