...
<div class="plate">
	<form class="query" id="queryForm" action="${basePath}unit/car.do" method="post">
		<table align="center" width="90%" border="0" cellspacing="0" cellpadding="0">
		  <tr>
		    <td>车牌号:</td>
		    <td><input type="text" class="input" size="16"  maxlength="32" name="cscNumber" id="cscNumber"  value="${cscNumber}"/></td>
		    <td>车型:</td>
		    <td><select class="combox"  action="${basePath}unit/object/carmodel_query.do?size=-1" id="cscModel" name="cscModel">
		    ...
		    <td>网点:</td>
		    <td><select class="combox"  action="${basePath}unit/object/outlets_tree.do?parent={param}" id="cscOutlets" name="cscOutlets">
		...
		<s:iterator value="#request.cars" id="item" status="i">
			<tr>
		  	 	...
		  	 	<td align="center">
					...
					<s:else>
						<a href="javascript:addAllOrder(${item.cscId})">
			  	 		...
			  	 		<a href="javascript:addProxyOrder(${item.cscId})">
...
<script>
	...
	function addAllOrder(id){
		var url = "${basePath}unit/order.edit.do";
		var params = {entrypoint:"${entrypoint}","csOrder.csoCar":id,controller:{title:"公务用车"}};
		href(url,params);
	}
