...
<div class="body">
	<div class="content">	
		...
		<dl class="${(canEditable && false)?"hidden":""} ${canEditable && true ? "" : " minor"}"  ref="csuoNumber">
			<dt>订单车辆:</dt>
			<dd>			
			<div class="state-input narrow">
			 		<input type="hidden" readOnly="true" value="${csUnitOrder.csuoNumber}" id="csuoNumber" />
			 		<span>
		...
		<dl class="${(canEditable && false)?"hidden":""} ${canEditable && false ? "" : " minor"}"  ref="csoRemark" style="width:98%;">
			<dt>备注:</dt>
			<dd>
			<input type="hidden" readOnly="true" value="${csUnitOrder.csuoRemark}" id="csuoRemark" />
			 		<span>
			 		${csUnitOrder.csuoRemark==1?"机要任务":""}	
					${csUnitOrder.csuoRemark==2?"应急勤务":""}
				 	${csUnitOrder.csuoRemark==3?"文件交换":""}	
				 	${csUnitOrder.csuoRemark==4?"跨区交通":""}	
			 		</span>
			 </dd>
		</dl>
		...
		<div class="line"></div>
		<s:if test="#request.canVisible==2">
		...
		<s:elseif test="#request.canVisible==1">
		<center class="buttons">			
			<s:if test="#request.havePrimary==false">
			<input type="hidden" value="${csUnitOrder.csuoId}" name="csUnitOrder.csuoId" id="csuoId" />
			</s:if>			
