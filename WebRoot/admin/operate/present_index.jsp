...
<lz:DefaultCtrl>{
	...
	queryables:"cspId,cspHost,cspName,cspType,cspCount,cspUpdateTime,cspAddTime,cspStatus",
	${lz:set("注释","当用户选择显示全部字段时，哪些字段可显示在表格中")}
	listables:"cspId,cspHost,cspName,cspType,cspCount,cspUpdateTime,cspAddTime,cspStatus",
	</s:if>
	<s:else>
...
<div class="title">
	...
	<span class="caption">${empty title?"礼品赠品管理":title}</span>
</div>
  	</s:if>
...
<div class="plate" style="${((hideQuery==true && window.imize!="maximize")||(hideQuery!=true && window.imize=="minimize")) ? "display:none;":""}">
	<form ref="CsPresent" class="query" id="queryForm" action="${basePath}${lz:replace(servletPath,"^/","")}?${lz:querys("UTF-8","page")}" method="post">
		${lz:set("haveQuery",false)}
		
		${lz:set("注释","****导入查询表单各字段****")}
		<%@include file="present.query.jsp"%>
	...
	<div class="content">
	    <div class="toolbar">
	      ...
	      <s:if test="#request.alias==null">
			...
			<button type="button" class="add" onclick="AddPresent();">
				添加
			</button>							
			</s:if>
		  </s:if>
		  ...
		  <s:iterator value="#request.items" id="handle" status="i">
			...
			<dl ref="com.ccclubs.model.CsPresent" class="display dropdowan" style="margin-left:30px;">
				<dt>
			...
			<dl ref="com.ccclubs.model.CsPresent" class="batch dropdowan">
				<dt>
		...
		${lz:set("注释","*****************数据列表开始*****************")}
		<table id="present_table" ref="CsPresent" class="table" width="100%" border="0" cellspacing="1" cellpadding="0">
		<thead class="state-header">
		  ...
		  ${lz:set("注释","****数据列表列头开始****")}
			 ...
			 <td <s:if test="#request.defines['cspName']>0">colspan="${defines["cspName"]}" ${all?"width":"iwidth"}="${defines["cspName"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=200":""} </s:else> tdid="3" ref="cspName" flagKey  title="礼品名称">
			 ...
			 <td <s:if test="#request.defines['cspType']>0">colspan="${defines["cspType"]}" ${all?"width":"iwidth"}="${defines["cspType"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=120":""} </s:else> tdid="4" ref="cspType"   title="礼品分类">
			 ...
			 <s:if test="#request.defines==null && #request.CTRL.l.cspCount || #request.defines['cspCount']!=null">
			 <td <s:if test="#request.defines['cspCount']>0">colspan="${defines["cspCount"]}" ${all?"width":"iwidth"}="${defines["cspCount"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=120":""} </s:else> tdid="5" ref="cspCount"   title="库存数">
			 	<a class="${desc=="csp_count" ? "desc" : ""}${asc=="csp_count" ? "asc" : ""}" href="?${desc=="csp_count" ? "asc=csp_count" : ""}${(asc=="csp_count" || desc!="csp_count" )? "desc=csp_count" : ""}&${lz:queryss("UTF-8","desc","asc")}">
			 	库存数
			 	</a>
			 	${lz:set("checkeds[]","cspCount")}
			 </td>
			 </s:if>
				
			 <s:if test="#request.defines==null && #request.CTRL.l.cspUpdateTime || #request.defines['cspUpdateTime']!=null">
			 <td <s:if test="#request.defines['cspUpdateTime']>0">colspan="${defines["cspUpdateTime"]}" ${all?"width":"iwidth"}="${defines["cspUpdateTime"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=140":""} </s:else> tdid="6" ref="cspUpdateTime"   title="修改时间">
			 ...
			 <td <s:if test="#request.defines['cspAddTime']>0">colspan="${defines["cspAddTime"]}" ${all?"width":"iwidth"}="${defines["cspAddTime"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=140":""} </s:else> tdid="7" ref="cspAddTime"   title="添加时间">
			 ...
			 <s:if test="#request.defines==null && #request.CTRL.l.cspStatus || #request.defines['cspStatus']!=null">
			 <td <s:if test="#request.defines['cspStatus']>0">colspan="${defines["cspStatus"]}" ${all?"width":"iwidth"}="${defines["cspStatus"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=120":""} </s:else> tdid="8" ref="cspStatus"   title="状态">
			...
			<td rowspan="2" width="105" class="operation" tdid="10" ref="operation">
				<s:if test="#request.CTRL.canAdd==true">
					<a href="javascript:AddPresent()" style="text-decoration:underline">添加</a>
				</s:if>
				<s:else>&nbsp;</s:else>
			</td>
		  </tr>
		  ...
		  <tr id="${item.cspId}" >
			...
			<s:if test="#request.defines==null && #request.CTRL.l.cspType || #request.defines['cspType']!=null">
				...
				<s:else>
					...
					<td ref="cspType" class="td " relate="${item.cspType}">
						 
						 	<a <s:if test="#item.cspType!=null && #item.cspType!=''"> onclick="window.href('${basePath}${proname}/configurator/property_details.do?key=${item.cspType}',{ctrl:{editable:false,visible:true}})" href="javascript:void(0);"</s:if>>
						 	${lz:or(item$cspType[i.count-1],lz:left(item.cspType$,100))}</a>
					</td>
				</s:else>
			</s:if>
			 
			<s:if test="#request.defines==null && #request.CTRL.l.cspCount || #request.defines['cspCount']!=null">
				<s:if test="#request.defines['cspCount']>0">
					${lz:set("注释","****cspCount关联表的子级字段：如果用户勾选了要显示的话****")}
				  	<s:iterator value="#request.childes" id="fieldName" status="j">
				  		${lz:set("isList",lz:indexOf(fieldName,"[]")>-1)}
				 		<s:if test="#request.isList==true">
				 			${lz:set("sizeList",lz:size(item.cspCount))}
				 			${lz:set("rowspan",rowspan>sizeList?rowspan:sizeList)}
				 		</s:if>
				  		${lz:set("atCspCount",lz:indexOf(fieldName,"cspCount")>-1)}
				  		<s:if test="#request.atCspCount==true">
				 			<td ${isList?"class='onemore'":""}>${lz:left(lz:el(item,fieldName),100)}</td>
				 		</s:if>
				 	</s:iterator>
				</s:if>
				<s:else>
					${lz:set("注释","****cspCount字段的字串格式化输出****")}
					<td ref="cspCount" class="td ">
						 
						 	${lz:or(item$cspCount[i.count-1],lz:left(item.cspCount$,100))}
					</td>
				</s:else>
			</s:if>
		   ...
		   ${lz:set("注释","*****************数据列表数据部分结束*****************")}		  
			...
			<s:if test="#request.alias=='select'">
				<a href="javascript:SelectPresent('${item.cspId}')">选择</a>
			</s:if>
			<s:if test="#request.alias==null">
				<s:if test="#request.CTRL.canEdit==true">
				<a href="javascript:EditPresent('${item.cspId}')">修改</a></s:if>
				<s:if test="#request.CTRL.canDel==true">
				<a href="javascript:DelPresent('${item.cspId}','${item.cspName}')">删除</a></s:if>
			</s:if>
			<s:if test="#request.CTRL.canView==true">
			<a href="javascript:DetailsPresent('${item.cspId}')">详情</a></s:if>
			</td>
		  </tr>
					...
					${lz:set("注释","****cspCount关联表的子级字段：如果用户勾选了要显示的话****")}
				  	<s:iterator value="#request.childes" id="fieldName" status="j">
				  		...
				  		<s:if test="#request.atCspCount==true">
				  			${lz:set("isList",lz:indexOf(fieldName,"[]")>-1)}
				  			<s:if test="#request.isList==true">
				  				<lz:set name="arrFieldName">cspCount[${k.count}]${lz:split(fieldName,"\\[\\]")[1]}</lz:set>
				 				<td class="onemore">${lz:left(lz:el(item,arrFieldName),100)}</td>
				 			</s:if>
				 			<s:else>
								<s:if test="#k.count==1"><td class="blank" rowspan="${rowspan-1}">&nbsp;</td></s:if>
...
<s:if test="#request.CTRL.canAdd==true">
	...
	* 添加礼品赠品
	**/
	function AddPresent(parent){		
		var url = "${basePath}${namespace}present_edit.do?edittype=save";
		var params = {entrypoint:"${entrypoint}",parent:(parent?parent:""),ctrl:${ctrl==null?"{title:'添加礼品赠品'}":lz:json(ctrl)}};
		href(url,params);
	}
</s:if>
<s:if test="#request.alias=='select'">
...
<s:if test="#request.CTRL.canEdit==true">
	...
	**/
	function EditPresent(id){
		var url = "${basePath}${namespace}present_edit.do?edittype=update";
		var params = {entrypoint:"${entrypoint}",id:id,ctrl:${ctrl==null?"{title:'修改礼品赠品'}":lz:json(ctrl)}};
		href(url,params);
	}
	/*
	* 修改所选
	...
	function UpdateSel(flag)
	{
		...
		if(checker.length==0){
			...
			$.alert("确定要对当前查询的${objects.count}条记录进行批量修改吗?",function(){
				var url = "${basePath}${namespace}present_edit.do";
				var params = {entrypoint:"${entrypoint}",method:flag,PARAMS:${lz:json(PARAMS)},ctrl:{title:"批量更新礼品赠品",visible:false,editable:false,fields:{}}};
			...
			var url = "${basePath}${namespace}present_edit.do";
			var params = {entrypoint:"${entrypoint}",method:flag,ids:ids,ctrl:{title:"批量更新礼品赠品",visible:false,editable:false,fields:{}}};
			for(var o in fieldNames[flag])
				params.ctrl["fields"][o]={visible:true,editable:true};
			href(url,params);
		}
	...
	**/
	...
	{
		var url = "${basePath}${namespace}present_edit.do";
		var params = {entrypoint:"${entrypoint}",id:id,method:flag,ctrl:{title:"更新礼品赠品",visible:false,editable:false,fields:{}}};
		for(var o in fieldNames[flag])
			params.ctrl["fields"][o]={visible:true,editable:true};
		href(url,params);
	}
	...
	$(function(){
		...
		$(".table tbody td.td").dblclick(function(){
			var url = "${basePath}${namespace}present_edit.do";
			var params = {entrypoint:"${entrypoint}",id:$(this).parents("tr:eq(0)").attr("id"),method:"any",ctrl:{title:"更新礼品赠品",visible:false,editable:false,fields:{}}};
...
<s:if test="#request.CTRL.canDel==true">
	...
	* 删除礼品赠品
	**/
	function DelPresent(id,flag)
	{
		var dialog=top.$.dialog({
				html:"<span style='white-space:nowrap;'>确定要删除礼品赠品["+flag+"]吗？</span>",
				title:"删除礼品赠品",
				modal: true,			
				onOk:function(){
					dialog.closeDialog();
					var url = "${basePath}${namespace}present_del.do";
					var params = {entrypoint:"${entrypoint}",id:id};
					$.submit(url,params);
				}
		 });	
	...
	*/
	function DelSel(){
		...
		var dialog=top.$.dialog({
				html:"<span style='white-space:nowrap;'>确定要删除所选礼品赠品吗？</span>",
				title:"批量删除礼品赠品",
				modal: true,			
				onOk:function(){
					var ids = "";
	...
	* 显示礼品赠品详情
	**/
	function DetailsPresent(id)
	{
		var url = "${basePath}${namespace}present_details.do";
		var params = {entrypoint:"${entrypoint}",id:id,ctrl:{title:"礼品赠品详情",editable:false,visible:true}};
		href(url,params);
	}
	/*
	* 显示所选记录详情
