...
<div class="title">
	...
	<a class="linker export" href="operecord_stats.do?export=true">
		<img align="absmiddle" src="${basePath}admin/images/icons/20088256427828778018.png"/>
		数据导出
	</a>
	<a href="operecord.do?${lz:querys("UTF-8","")}" class="linker">
		<img align="absmiddle" src="${basePath}admin/images/icons/20088256421631277806.png"/>
		数据管理
	</a>
	...
	<span class="caption">收支记录统计</span>
...
<div class="plate" style="${(hideQuery || (lz:or(window.imize,"maximize"))=="minimize") ? "display:none;":""}">
	<form ref="CsOperRecord" class="query statsForm" id="queryForm" action="operecord_stats.do" method="post">
		...
		<div class="group condition">
			...
			<ul>
				<li>
					<input ${csorHostGroup==true?"checked='checked'":""} class="groupBy" type="checkbox" name="groupBy" value="csorHost"/>城市
				 	&nbsp;&nbsp;
				</li>
				<li>
					<input ${csorMoneyTypeGroup==true?"checked='checked'":""} class="groupBy" type="checkbox" name="groupBy" value="csorMoneyType"/>金额类型
				 	&nbsp;&nbsp;
				</li>
				<li>
					<input ${csorRecordTypeGroup==true?"checked='checked'":""} class="groupBy" type="checkbox" name="groupBy" value="csorRecordType"/>科目类型
				 	&nbsp;&nbsp;
				</li>
				<li>
					<input ${csorDateTimeGroup==true?"checked='checked'":""} class="groupBy" type="checkbox" name="groupBy" value="csorDateTime"/>
				 	<select name="csorDateTimeTimeType">
				 		<option value="%Y-%m,yyyy年MM月">发生时间</option>
				 		
				 		<option ${csorDateTimeTimeType=="%Y,yyyy年"?"selected":""} value="%Y,yyyy年">
					 	每年(发生时间)</option>
					 	<option ${csorDateTimeTimeType=="%Y-%m,yyyy年MM月"?"selected":""} value="%Y-%m,yyyy年MM月">
					 	每月(发生时间)</option>
					 	<option ${csorDateTimeTimeType=="%Y%U,yyyy年第ww周"?"selected":""} value="%Y%U,yyyy年第ww周">
					 	每周(发生时间)</option>
					 	<option ${csorDateTimeTimeType=="%Y-%m-%d,yyyy年MM月dd日"?"selected":""} value="%Y-%m-%d,yyyy年MM月dd日">
					 	每日(发生时间)</option>
					 	<option ${csorDateTimeTimeType=="%Y-%m-%d %H,yyyy年MM月dd日 HH点"?"selected":""} value="%Y-%m-%d %H,yyyy年MM月dd日 HH点">
					 	每时(发生时间)</option>
					 	
					 	<option ${csorDateTimeTimeType=="%m,MM月"?"selected":""} value="%m,MM月">
					 	月份(发生时间)</option>
					 	<option ${csorDateTimeTimeType=="%U,第ww周"?"selected":""} value="%U,第ww周">
					 	周份(发生时间)</option>
					 	<option ${csorDateTimeTimeType=="%w,EEEE"?"selected":""} value="%w,EEEE">
					 	星期(发生时间)</option>
					 	<option ${csorDateTimeTimeType=="%d,dd日"?"selected":""} value="%d,dd日">
					 	天份(发生时间)</option>
					 	<option ${csorDateTimeTimeType=="%H,HH点"?"selected":""} value="%H,HH点">
					 	时份(发生时间)</option>
		...
		<div class="sum condition">
			...
			<ul>
				...
				<li>
				 	<input ${csorInSum==true?"checked='checked'":""} class="sumBy" type="checkbox" name="sumBy" value="csorIn"/>
					收入&nbsp;&nbsp;
				</li>
				<li>
				 	<input ${csorOutSum==true?"checked='checked'":""} class="sumBy" type="checkbox" name="sumBy" value="csorOut"/>
					支出&nbsp;&nbsp;
				</li>
	...
	<div class="content">
		...
		stats.datas=[ 
			  ...
			  <lz:set name="itemValue">
					 	 ${get:SrvHost(item.csorHost).shName}			 	 
		  	 </lz:set>
		  	 ,"csorHost":"${lz:js(lz:trim(itemValue))}"
			  <lz:set name="itemValue">
						 ${item.csorMoneyType==0?"保证金":""}
						 ${item.csorMoneyType==1?"优惠券":""}
		  	 </lz:set>
		  	 ,"csorMoneyType":"${lz:js(lz:trim(itemValue))}"
			  <lz:set name="itemValue">
					 	 ${get:CsRecordSubject(item.csorRecordType).csrsName}			 	 
		  	 </lz:set>
		  	 ,"csorRecordType":"${lz:js(lz:trim(itemValue))}"
			  <lz:set name="itemValue">
					 	 ${lz:date(item.csorDateTime,lz:split(csorDateTimeTimeType,",")[1])}		 	 
		  	 </lz:set>
		  	 ,"csorDateTime":"${lz:js(lz:trim(itemValue))}"
		  	 ,"csorIn":"${lz:digit(item.csorIn,2)}"
		  	 ,"csorOut":"${lz:digit(item.csorOut,2)}"
			  <s:iterator value="#request.sumBy" id="sumName" status="i">
			  ${lz:set("array",lz:split(sumName,":"))}${lz:set("arrayCount",lz:size(array))}
