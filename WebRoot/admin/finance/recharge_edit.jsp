...
<s:if test="#request.CTRL.title=='审核'">
	<s:if test="csArtifRecord.csarStatus!=0">
		${lz:set("message","只有待审核的记录才能审核")}
	...
	<s:else>
		<lz:set name="message">
			审核通过后,会员${csArtifRecord.csarMoneyType$}账户将会${csArtifRecord.$csarRecordSubject.csrsIsUserIn?"增加":"扣除"}${csArtifRecord.csarCount>0?csArtifRecord.csarCount:-csArtifRecord.csarCount}元
			<s:if test="csArtifRecord.$csarRecordSubject.csrsIsOperIn||csArtifRecord.$csarRecordSubject.csrsIsOperOut">
			,商家${csArtifRecord.csarHost$}账户将会${csArtifRecord.$csarRecordSubject.csrsIsOperIn?"增加":"扣除"}${csArtifRecord.csarCount>0?csArtifRecord.csarCount:-csArtifRecord.csarCount}元
			</s:if>
		</lz:set>
		<lz:set name="jsonString">
			{
				title:"审核",
				editable:false,
				action:"recharge_check.do",
				fields:{
					csarRemark:{editable:true,visible:true}
					,csarReceived:{visible:false}
					,csarVoucher:{visible:false}
				},
				submits:[
...
<s:if test="#request.CTRL.title=='收讫'">
	<s:if test="csArtifRecord.csarStatus!=1">
		${lz:set("message","只有审核通过记录才能收讫")}
		<lz:set name="jsonString">
			{
				title:"收讫",
				editable:false,				
...
<s:if test="#request.CTRL.title=='入账'">
	<s:if test="csArtifRecord.csarStatus!=0 || csArtifRecord.csarReceived!=true">
		${lz:set("message","只有审核通过并且已收讫的记录才能入账")}
		<lz:set name="jsonString">
			{
				title:"入账",
				editable:false,
		...
		<lz:set name="jsonString">
			{
				title:"入账",
				editable:false,
				action:"recharge_voucher.do",
				fields:{
					csarVoucher:{editable:true,visible:true}
				},
				submits:[
					{name:"入账",value="入账"}
				]
...
<s:else>
	<form class="form editform" ref="CsArtifRecord" id="rechargeForm" name="rechargeForm" action="${empty CTRL.action?"recharge_save.do":CTRL.action}" method="post">
		<div class="head"></div>
		<div class="body">
			<div class="content">
	...
	<dl class="csarStatus " major  ref="csarStatus" >
		...
		<s:if test="#request.csArtifRecord$csarStatus!=null">${csArtifRecord$csarStatus}</s:if><s:else>
		 	<select class="narrow" id="csarStatus" name="csArtifRecord.csarStatus">
		 		<option value="">请选择</option>
				<option value="0" ${csArtifRecord.csarStatus==0?"selected":""}>待审核</option>
