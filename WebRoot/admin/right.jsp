...
<table id="right-panel" width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
	<tr height="32">
		<td class="state-title">
			<div id="tabframe">
				<table id="tabs" border="0" cellspacing="0" cellpadding="0" style="">
				  <tr id="tabstr"></tr>
				</table>
			</div>		
			<div id="tabmore">
...
var href=function(url,title,ico){
	if(url.indexOf("#")==0)
		return false;
	var flag=url;
	if(url.indexOf("?")!=-1)
	...
	if($.trim(title)=="系统导航")
		newHref.td.prependTo($("#tabstr")).hide();	
	else
		newHref.td.appendTo($("#tabstr"));
	newHref.table=$("<table class='item state-focus' border='0' cellspacing='0' cellpadding='0'><tr><td class='btn'>"+(ico ? ("<img width='20' height='20' align='absmiddle' src='"+ico+"'/>&nbsp;&nbsp;") : "")+""+title+"</td><td class='cls'>x</td></tr></table>");
	...
	newHref.active=function(){
		if(current){
			current.table.attr("class","item state-header");
			current.iframe.hide();
			top.$(current.iframe.get(0).contentWindow).trigger("visible",false);
		}
		current=newHref;
		current.table.attr("class","item state-focus");
		current.iframe.show();
		top.$(current.iframe.get(0).contentWindow).trigger("visible",true);		
	}
...
var onResize=function(){
	$("#iframes>iframe").attr("height",$(window).height()-32).height($(window).height()-32);
...
$(function(){
	...
	parent.href("welcome.do","欢迎页面","images/icons/home.png");
	//if(${lz:config("debug")=="true"})	
		//parent.href("example.do","开发者示例");	
});
</script>
