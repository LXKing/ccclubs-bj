...
<lz:DefaultCtrl>{
	...
	listables:"cscId,cscTitle,cscParent,cscFlag,cscRecommend,cscThum,cscKeywords,cscImages,cscFile,cscAddTime,cscUpdateTime,cscViewS,cscReplyS,cscCollectS,cscPraiseS,cscIsActivity,cscActivityEvent,cscStatus",
	</s:if>
	<s:else>
	${lz:set("注释","当用户选择显示部分字段时，哪些字段可查询")}
	queryables:"cscId",
	${lz:set("注释","当用户选择显示部分字段时，哪些字段可显示在表格中")}
	listables:"cscId,cscTitle,cscParent,cscFlag,cscRecommend,cscAddTime,cscIsActivity,cscActivityEvent,cscStatus",
	</s:else>
}</lz:DefaultCtrl>
${lz:set("注释","***************************************************")}
	...
	<div class="content">
		...
		<thead class="state-header">
		  ...
		  ${lz:set("注释","****数据列表列头开始****")}
			 ...
			 <s:if test="#request.defines==null && #request.CTRL.l.cscIsActivity || #request.defines['cscIsActivity']!=null">
			 <td <s:if test="#request.defines['cscTitle']>0">colspan="${defines["cscTitle"]}" ${all?"width":"iwidth"}="${defines["cscIsActivity"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=200":""} </s:else> tdid="1" ref="cscIsActivity" flagKey  title="活动盒子">
			 	<a class="${desc=="csc_is_activity" ? "desc" : ""}${asc=="csc_is_activity" ? "asc" : ""}" href="?${desc=="csc_is_activity" ? "asc=csc_is_activity" : ""}${(asc=="csc_is_activity" || desc!="csc_is_activity" )? "desc=csc_is_activity" : ""}&${lz:queryss("UTF-8","desc","asc")}">
			 	活动盒子
			 	</a>
			 	${lz:set("checkeds[]","cscIsActivity")}
			 </td>
			 </s:if>
			 
			 <s:if test="#request.defines==null && #request.CTRL.l.cscActivityEvent || #request.defines['cscActivityEvent']!=null">
			 <td <s:if test="#request.defines['cscActivityEvent']>0">colspan="${defines["cscActivityEvent"]}" ${all?"width":"iwidth"}="${defines["cscActivityEvent"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=200":""} </s:else> tdid="1" ref="cscActivityEvent" flagKey  title="活动事件">
			 	<a class="${desc=="csc_activity_event" ? "desc" : ""}${asc=="csc_activity_event" ? "asc" : ""}" href="?${desc=="csc_activity_event" ? "asc=csc_activity_event" : ""}${(asc=="csc_activity_event" || desc!="csc_activity_event" )? "desc=csc_activity_event" : ""}&${lz:queryss("UTF-8","desc","asc")}">
			 	活动事件
			 	</a>
			 	${lz:set("checkeds[]","cscActivityEvent")}
			 </td>
			 </s:if>
		 ...
		 <tbody>
		  ...
		  <tr id="${item.cscId}"  tree="${item.cscParent}">
			...
			<s:if test="#request.defines==null && #request.CTRL.l.cscIsActivity || #request.defines['cscIsActivity']!=null">
				<s:if test="#request.defines['cscIsActivity']>0">
					${lz:set("注释","****cscIsActivity关联表的子级字段：如果用户勾选了要显示的话****")}
				  	<s:iterator value="#request.childes" id="fieldName" status="j">
				  		${lz:set("isList",lz:indexOf(fieldName,"[]")>-1)}
				 		<s:if test="#request.isList==true">
				 			${lz:set("sizeList",lz:size(item.cscIsActivity))}
				 			${lz:set("rowspan",rowspan>sizeList?rowspan:sizeList)}
				 		</s:if>
				  		${lz:set("atCscIsActivity",lz:indexOf(fieldName,"cscIsActivity")>-1)}
				  		<s:if test="#request.atCscIsActivity==true">
				 			<td ${isList?"class='onemore'":""}>${lz:left(lz:el(item,fieldName),100)}</td>
				 		</s:if>
				 	</s:iterator>
				</s:if>
				<s:else>
					${lz:set("注释","****cscIsActivity字段的字串格式化输出****")}
					<td ref="cscIsActivity" class="td ">
						 <s:if test="#request.alias==null">
							 <s:if test="#request.CTRL.canEdit==true">
							 	<a class="modify" href="javascript:Update('${item.cscId}','cscIsActivity')"></a>
							 </s:if>
					     </s:if>
						 
						 	${lz:or(item$cscIsActivity[i.count-1],lz:left(item.cscIsActivity$,100))}
					</td>
				</s:else>
			</s:if>
			
			<s:if test="#request.defines==null && #request.CTRL.l.cscActivityEvent || #request.defines['cscActivityEvent']!=null">
				<s:if test="#request.defines['cscActivityEvent']>0">
					${lz:set("注释","****cscActivityEvent关联表的子级字段：如果用户勾选了要显示的话****")}
				  	<s:iterator value="#request.childes" id="fieldName" status="j">
				  		${lz:set("isList",lz:indexOf(fieldName,"[]")>-1)}
				 		<s:if test="#request.isList==true">
				 			${lz:set("sizeList",lz:size(item.cscActivityEvent))}
				 			${lz:set("rowspan",rowspan>sizeList?rowspan:sizeList)}
				 		</s:if>
				  		${lz:set("atCscActivityEvent",lz:indexOf(fieldName,"cscActivityEvent")>-1)}
				  		<s:if test="#request.atCscActivityEvent==true">
				 			<td ${isList?"class='onemore'":""}>${lz:left(lz:el(item,fieldName),100)}</td>
				 		</s:if>
				 	</s:iterator>
				</s:if>
				<s:else>
					${lz:set("注释","****cscActivityEvent字段的字串格式化输出****")}
					<td ref="cscActivityEvent" class="td ">
						 <s:if test="#request.alias==null">
							 <s:if test="#request.CTRL.canEdit==true">
							 	<a class="modify" href="javascript:Update('${item.cscId}','cscActivityEvent')"></a>
							 </s:if>
					     </s:if>
						 
						 	${lz:or(item$cscActivityEvent[i.count-1],lz:left(item.cscActivityEvent$,100))}
					</td>
				</s:else>
			</s:if>
			...
			<s:if test="#request.defines==null && #request.CTRL.l.cscFile || #request.defines['cscFile']!=null">
					...
					${lz:set("注释","****cscIsActivity关联表的子级字段：如果用户勾选了要显示的话****")}
				  	<s:iterator value="#request.childes" id="fieldName" status="j">
				  		...
				  		<s:if test="#request.atCscIsActivity==true">
				  			${lz:set("isList",lz:indexOf(fieldName,"[]")>-1)}
				  			<s:if test="#request.isList==true">
				  				<lz:set name="arrFieldName">cscIsActivity[${k.count}]${lz:split(fieldName,"\\[\\]")[1]}</lz:set>
				 				<td class="onemore">${lz:left(lz:el(item,arrFieldName),100)}</td>
				 			</s:if>
				 			<s:else>
								<s:if test="#k.count==1"><td class="blank" rowspan="${rowspan-1}">&nbsp;</td></s:if>
			 ...
			 <s:if test="#request.defines==null && #request.CTRL.l.cscActivityEvent || #request.defines['cscActivityEvent']!=null">
				<s:if test="#request.defines['cscActivityEvent']>0">
					${lz:set("注释","****cscFile关联表的子级字段：如果用户勾选了要显示的话****")}
				  	<s:iterator value="#request.childes" id="fieldName" status="j">
				  		${lz:set("atCscActivityEvent",lz:indexOf(fieldName,"cscFile")>-1)}				  		
				  		<s:if test="#request.atCscActivityEvent==true">
				  			${lz:set("isList",lz:indexOf(fieldName,"[]")>-1)}
				  			<s:if test="#request.isList==true">
				  				<lz:set name="arrFieldName">cscActivityEvent[${k.count}]${lz:split(fieldName,"\\[\\]")[1]}</lz:set>
				 				<td class="onemore">${lz:left(lz:el(item,arrFieldName),100)}</td>
				 			</s:if>
				 			<s:else>
								<s:if test="#k.count==1"><td class="blank" rowspan="${rowspan-1}">&nbsp;</td></s:if>
