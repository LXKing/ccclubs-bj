...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8;" />  
	...
	<style type="text/css">
	#allmap{position: absolute;top: 0;left: 0;right: 0;bottom: 0;width:100%;height:100%;}
	#searcher{}
	...
	#elementTypes{position: absolute;left:10px;top:10px;width:230px;}
...
var latlngArray = [${lz:or(defaultHost.shLocation,"30.276611,120.161665")}];
//地图对象
var map = new AMap.Map(document.getElementById('allmap'),{
	        center: new AMap.LngLat(latlngArray[1],latlngArray[0]),//没有配置位置就定位到杭州
	        zoom: 11,
	        scrollWheel: true	        
	    });
...
//信息窗口
var infoWin = new AMap.InfoWindow({offset: new AMap.Pixel(5, -28)});
function onMarker(who){	
	var marker = this.info ? this : who;
	infoWin.setPosition(marker.getPosition());
	map.setCenter(marker.getPosition());
    $.ajax({
    	url:"${basePath}admin/control/map_info.do",
	...
	setTimeout(function(){
		...
		for(var o in markers){
			if(pThis.attr("ref")=="cars"){
				if(parent.cars[markers[o].info.number].show && bShow){
					markers[o].show();
				}else{
					markers[o].hide();
				}
...
function getIcon(bOrder,bOnlie,bPower,bCharging,bBattery){
	...
	var imageOffset = new AMap.Pixel(0, 0);
    var size = new AMap.Size(36, 39);
	var icon = window.icons[sIcon] = new AMap.Icon({image:"/admin/images/state/v2/marker-"+sIcon+".png?v=1", size:size, imageOffset:imageOffset});
	return icon;	
}
function getShadow(bOrder,bOnlie,bPower,bCharging,bBattery){
	...
	var imageOffset = new AMap.Size(36, 0);
    var size = new AMap.Size(36, 39);
	var shadow = window.shadows[sShadow] = new AMap.Icon({image:"/admin/images/state/v2/marker-"+sShadow+".png?v=1", size:size, imageOffset:imageOffset});
	return shadow;	
}
var outletIcon=new AMap.Icon({
   	image:"/admin/images/state/outlet-red.png", 
	size:new AMap.Size(32, 32), 
	imageOffset:new AMap.Size(0, 0)}
);
var outletAndPowerIcon = new AMap.Icon({
   	image:"/admin/images/state/pin-blue.png?v=2", 
	size:new AMap.Size(32, 32),  
	imageOffset:new AMap.Size(0, 0)}
); 
var gasIcon=new AMap.Icon({
   	image:"/admin/images/state/gas.png", 
	size:new AMap.Size(32, 32), 
	imageOffset:new AMap.Size(0, 0)}
);
var pileIcon=new AMap.Icon({
   	image:"/admin/images/state/pile.png", 
	size:new AMap.Size(32, 32), 
	imageOffset:new AMap.Size(0, 0)}
);
...
function remote(operate,id){
	...
	top.$(input).dialog({
		...
		onOk:function(){
			...
			$.ajax({
				url:"${basePath}admin/control/remote_save.do",
				cache:false,
				type:"post",
				data:{
					"csRemote.csrCar":id,
					"csRemote.csrType":controls[operate],
					"csRemote.csrRemark":input.val()
				}
			});	
			$.tips("远程控制提交成功");		
		}
...
function update(){
	...
	$.ajax({
				url:"${basePath}admin/control/map_states.do",
				cache:false,
				type:"get",
				data:{},
				success:function(data){
						...
						for(var num in states){
							...
							if(state.cssLongitude==null||state.cssLatitude==null)
								return;
							var wgs84togcj02 = coordtransform.wgs84togcj02(parseFloat(state.cssLongitude), parseFloat(state.cssLatitude));

							cars[number].setPosition(new AMap.LngLat( wgs84togcj02[0],wgs84togcj02[1]));
							//if(state.cssHost==2)
	...
	//$.getObject("${basePath}admin/control/map_states.do",{},function(data){
	//	
	//});
}
...
function init(){
	...
	for(var o in parent.outlets){
		...
		var m = outlets[outlets.length]=new AMap.Marker({
		    position:new AMap.LngLat(outlet.csoLongitude,outlet.csoLatitude),
		    map:map,
		    cursor:"pointer",
		    title:outlet.csoName,
		    icon:(piles && piles.length>0 ? outletAndPowerIcon : outletIcon)
		});	
		if(shows["showOutlets"]){
			m.show();
		}else{
			m.hide()
		}
...
// 		}
		...
		AMap.event.addListener(m,"click", function(who){
				var marker = this.info ? this : who;
				infoWin.setPosition(marker.getPosition());
				
			    $.ajax({
			    	url:"${basePath}admin/control/map_powerInfo.do",
	...
	for(var o in parent.gases){
		...
		var m = gases[gases.length]=new AMap.Marker({
		    position:new AMap.LngLat(gas.csgsLocation.split(",")[1],gas.csgsLocation.split(",")[0]),
		    map:map,
		    cursor:"pointer",
		    title:gas.csgsName,
		    icon:gasIcon
...
// 	}
	...
	for(var o in parent.cars){
		...
		var marker = cars[car.cscId]=cars[car.cscNumber]=new AMap.Marker({
		    position:new AMap.LngLat(120.161665,30.276611),//定位到杭州
		    //map:map,
		    cursor:"pointer",
		...
		AMap.event.addListener(marker,"click",onMarker);
	}
