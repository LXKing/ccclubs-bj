...
<body  scroll="no">
	<div id="navigator">
		<div id="querypanel" class="state-header">
			<select class="combox" style="width:100px;" action="${basePath}${proname}/object/outlets_tree.do?parent={param}&host=${get:host()}" id="outlets" name="outlets">
	 			<option selected value="">所有网点</option>
	 		</select>
	 		<input type="input" style="width:80px;height:22px;float:left;margin:3px;" class="input" name="keyword" id="keyword" value="车辆搜索..."/>
					...
					<s:iterator value="#request.cars" id="car">
						...
						<tr class="carer" id="${car.cscNumber}" tree="o${car.cscOutlets}">
							<td class="node" title="${car.$cscOutlets.csoName$}">
								...
								<em class='ico b'></em>
								<em class='ico c'></em>
								<a class='link car${car.cscId}' onclick="gaodemap.goMarker('${car.cscNumber}')" href="javascript:void(0);" title="${car.cscStatus==2?car.cscRemark:"在线运营"}"  style="${car.cscStatus==2?"text-decoration:line-through;":""}">${car.cscNumber}</a>
	...
	<div id="mapframe">
		<iframe  id="gaodemap" src="map_gaode.do" name="gaodemap" frameborder="0" width="100%" height="100%"></iframe>
	</div>	
</body>
