...
<lz:DefaultCtrl>{
	...
	queryables:"csosId,csosArea,csosHost,csosOutlets,csosUser,csosAddTime,csosFinishTime,csosWorkTime,csosStatus",
	...
	queryables:"csosId,csosArea,csosHost,csosOutlets,csosUser,csosProperty,csosAddTime,csosFinishTime,csosWorkTime,csosStatus",
...
$(function(){
	...
	$("#outsideStatistics_table").hide().show();
	fast.find("a").click(function(){
		if($(this).attr("start")=="" || $(this).attr("end")==""){
			$("#csosFinishTimeEnd").val("");
			$("#csosAddTimeStart").val("");
		}else{
			var start = new Date(Number($(this).attr("start")));
			var end = new Date(Number($(this).attr("end")));
			$("#csosFinishTimeEnd").val($.formatDate("yyyy-MM-dd HH:mm:ss",end));
			$("#csosAddTimeStart").val($.formatDate("yyyy-MM-dd HH:mm:ss",start));
		}
		$.toast("时间设置成功");
	});
...
<div class="title">
	...
	<a class="linker export" href="outsideStatistics_stats.do?export=true">
		<img align="absmiddle" src="${basePath}admin/images/icons/20088256427828778018.png"/>
		数据导出
	</a>
	<a href="outsideStatistics.do?${lz:querys("UTF-8","")}" class="linker">
		<img align="absmiddle" src="${basePath}admin/images/icons/20088256421631277806.png"/>
		数据管理
	</a>
	...
	<span class="caption">外勤整备项统计</span>
...
<div class="plate" style="${(hideQuery || (lz:or(window.imize,"maximize"))=="minimize") ? "display:none;":""}">
	<form ref="CsOutsideStatistics" class="query statsForm" id="queryForm" action="outsideStatistics_stats.do" method="post">
		...
		<div class="group condition">
			...
			<ul>
				...
				<li>
					<input ${csosUserGroup==true?"checked='checked'":""} class="groupBy" type="checkbox" name="groupBy" value="csosUser"/>外勤人员
				 	&nbsp;&nbsp;
				</li>
				<li>
					<input ${csosOutletsGroup==true?"checked='checked'":""} class="groupBy" type="checkbox" name="groupBy" value="csosOutlets"/>网点
				 	&nbsp;&nbsp;
				</li>
				
				<li>
				 	<input ${csosCarGroup==true?"checked='checked'":""} class="groupBy" type="checkbox" name="groupBy" value="csosCar"/>
					车辆&nbsp;&nbsp;
				</li>
		...
		<div class="sum condition">
			...
			<ul>
				...
				<li>
				 	<input ${csosOpenDoorCountSum==true?"checked='checked'":""} class="sumBy" type="checkbox" name="sumBy" value="csosOpenDoorCount"/>
					开门次数&nbsp;&nbsp;
				</li>
				<li>
				 	<input ${csosCloseDoorCountSum==true?"checked='checked'":""} class="sumBy" type="checkbox" name="sumBy" value="csosCloseDoorCount"/>
					关门次数&nbsp;&nbsp;
				</li>
		
				<li>
				 	<input ${csosStartCountSum==true?"checked='checked'":""} class="sumBy" type="checkbox" name="sumBy" value="csosStartCount"/>
					启动次数&nbsp;&nbsp;
				</li>
				
				<li>
				 	<input ${csosWhistleCountSum==true?"checked='checked'":""} class="sumBy" type="checkbox" name="sumBy" value="csosWhistleCount"/>
					鸣笛次数&nbsp;&nbsp;
				</li>
				
				<li>
				 	<input ${csosWorkTimeSum==true?"checked='checked'":""} class="sumBy" type="checkbox" name="sumBy" value="csosWorkTime"/>
					作业时长&nbsp;&nbsp;
				</li>
...
$(function(){
	...
	<div class="content">
		...
		stats.structs={
			  ...
			  ,"csosOpenDoorCount":"开门次数"
			  ,"csosCloseDoorCount":"关门次数"
			  ,"csosStartCount":"启动次数"
			  ,"csosWhistleCount":"鸣笛次数"
			  ,"csosWorkTime":"工作时长"
		...
		stats.datas=[ 
			...
			<s:iterator value="#request.list" id="item" status="i">${i.count>1?",":""}{"count":"${item.count}"
			  ...
			  <lz:set name="itemValue">
					 	 ${get:CsCar(item.csosCar).cscNumber}			 	 
		  	 </lz:set>
		  	 
		  	 ,"csosCar":"${lz:js(lz:trim(itemValue))}"
			  
			  
			 ,"csosOpenDoorCount":"${item.csosOpenDoorCount}"
			 ,"csosCloseDoorCount":"${item.csosCloseDoorCount}"
			 ,"csosStartCountCount":"${item.csosStartCount}"
			 ,"csosWhistleCount":"${item.csosWhistleCount}"
			 ,"csosWorkTime":"${item.csosWorkTimeCount}"
			  
			 <lz:set name="itemValue">
					 	 ${get:SrvProperty(item.csosProperty).spName}			 	 
		  	 </lz:set>
		  	 ,"csosProperty":"${lz:js(lz:trim(itemValue))}"
			  
			 <lz:set name="itemValue">
					 	 ${get:SrvUser(item.csosUser).suRealName}			 	 
		  	 </lz:set>
		  	 ,"csosUser":"${lz:js(lz:trim(itemValue))}"
		  	 
			  <lz:set name="itemValue">
						${get:CsOutlets(item.csosOutlets).csoName}	
		  	 </lz:set>
		  	 ,"csosOutlets":"${lz:js(lz:trim(itemValue))}"
