...
<lz:DefaultCtrl>{
	...
	queryables:"cscId,cscHost,cscMember,cscUser,cscSort,cscTitle,cscUpdateTime,cscAddTime,cscStatus",
	${lz:set("注释","当用户选择显示全部字段时，哪些字段可显示在表格中")}
	listables:"cscId,cscHost,cscMember,cscUser,cscSort,cscType,cscTitle,cscDesc,cscUpdateTime,cscAddTime,cscAlarmTime,cscStatus",
	</s:if>
	<s:else>
	${lz:set("注释","当用户选择显示部分字段时，哪些字段可查询")}
	queryables:"cscHost,cscMember,cscUser,cscSort,cscTitle,cscStatus",
	${lz:set("注释","当用户选择显示部分字段时，哪些字段可显示在表格中")}
	listables:"cscHost,cscMember,cscUser,cscSort,cscType,cscTitle,cscStatus",
	</s:else>
}</lz:DefaultCtrl>
${lz:set("注释","***************************************************")}
...
<div class="title">
	...
	<span class="caption">${empty title?"客户关怀管理":title}</span>
</div>
  	</s:if>
...
<div class="plate" style="${((hideQuery==true && window.imize!="maximize")||(hideQuery!=true && window.imize=="minimize")) ? "display:none;":""}">
	<form ref="CsCare" class="query" id="queryForm" action="${basePath}${lz:replace(servletPath,"^/","")}?${lz:querys("UTF-8","page")}" method="post">
		${lz:set("haveQuery",false)}
		
		${lz:set("注释","****导入查询表单各字段****")}
		<%@include file="care.query.jsp"%>
	...
	<div class="content">
	    <div class="toolbar">
	      ...
	      <s:if test="#request.alias==null">
			...
			<button type="button" class="add" onclick="AddCare();">
				添加
			</button>							
			</s:if>
		  </s:if>
		  ...
		  <s:iterator value="#request.items" id="handle" status="i">
			...
			<dl ref="com.ccclubs.model.CsCare" class="display dropdowan" style="margin-left:30px;">
				<dt>
			...
			<dl ref="com.ccclubs.model.CsCare" class="batch dropdowan">
				<dt>
		...
		${lz:set("注释","*****************数据列表开始*****************")}
		<table id="care_table" ref="CsCare" class="table" width="100%" border="0" cellspacing="1" cellpadding="0">
		<thead class="state-header">
		  ...
		  ${lz:set("注释","****数据列表列头开始****")}
			 ...
			 <td <s:if test="#request.defines['cscMember']>0">colspan="${defines["cscMember"]}" ${all?"width":"iwidth"}="${defines["cscMember"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=120":""} </s:else> tdid="3" ref="cscMember"   title="关怀会员">
			 ...
			 <s:if test="#request.defines==null && #request.CTRL.l.cscUser || #request.defines['cscUser']!=null">
			 <td <s:if test="#request.defines['cscUser']>0">colspan="${defines["cscUser"]}" ${all?"width":"iwidth"}="${defines["cscUser"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=120":""} </s:else> tdid="4" ref="cscUser"   title="执行人">
			 	<a class="${desc=="csc_user" ? "desc" : ""}${asc=="csc_user" ? "asc" : ""}" href="?${desc=="csc_user" ? "asc=csc_user" : ""}${(asc=="csc_user" || desc!="csc_user" )? "desc=csc_user" : ""}&${lz:queryss("UTF-8","desc","asc")}">
			 	执行人
			 	</a>
			 	${lz:set("checkeds[]","cscUser")}
			 </td>
			 </s:if>
				
			 <s:if test="#request.defines==null && #request.CTRL.l.cscSort || #request.defines['cscSort']!=null">
			 <td <s:if test="#request.defines['cscSort']>0">colspan="${defines["cscSort"]}" ${all?"width":"iwidth"}="${defines["cscSort"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=120":""} </s:else> tdid="5" ref="cscSort"   title="关怀类型">
			 	<a class="${desc=="csc_sort" ? "desc" : ""}${asc=="csc_sort" ? "asc" : ""}" href="?${desc=="csc_sort" ? "asc=csc_sort" : ""}${(asc=="csc_sort" || desc!="csc_sort" )? "desc=csc_sort" : ""}&${lz:queryss("UTF-8","desc","asc")}">
			 	关怀类型
			 	</a>
			 	${lz:set("checkeds[]","cscSort")}
			 </td>
			 </s:if>
				
			 <s:if test="#request.defines==null && #request.CTRL.l.cscType || #request.defines['cscType']!=null">
			 <td <s:if test="#request.defines['cscType']>0">colspan="${defines["cscType"]}" ${all?"width":"iwidth"}="${defines["cscType"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=120":""} </s:else> tdid="6" ref="cscType"   title="关怀方式">
			 ...
			 <td <s:if test="#request.defines['cscTitle']>0">colspan="${defines["cscTitle"]}" ${all?"width":"iwidth"}="${defines["cscTitle"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=200":""} </s:else> tdid="7" ref="cscTitle" flagKey  title="关怀主题">
			 ...
			 <td <s:if test="#request.defines['cscDesc']>0">colspan="${defines["cscDesc"]}" ${all?"width":"iwidth"}="${defines["cscDesc"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=200":""} </s:else> tdid="8" ref="cscDesc"   title="关怀描述">
			 ...
			 <td <s:if test="#request.defines['cscUpdateTime']>0">colspan="${defines["cscUpdateTime"]}" ${all?"width":"iwidth"}="${defines["cscUpdateTime"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=140":""} </s:else> tdid="9" ref="cscUpdateTime"   title="修改时间">
			 ...
			 <s:if test="#request.defines==null && #request.CTRL.l.cscAlarmTime || #request.defines['cscAlarmTime']!=null">
			 <td <s:if test="#request.defines['cscAlarmTime']>0">colspan="${defines["cscAlarmTime"]}" ${all?"width":"iwidth"}="${defines["cscAlarmTime"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=140":""} </s:else> tdid="11" ref="cscAlarmTime"   title="提醒时间">
			 	<a class="${desc=="csc_alarm_time" ? "desc" : ""}${asc=="csc_alarm_time" ? "asc" : ""}" href="?${desc=="csc_alarm_time" ? "asc=csc_alarm_time" : ""}${(asc=="csc_alarm_time" || desc!="csc_alarm_time" )? "desc=csc_alarm_time" : ""}&${lz:queryss("UTF-8","desc","asc")}">
			 	提醒时间
			 	</a>
			 	${lz:set("checkeds[]","cscAlarmTime")}
			 </td>
			 </s:if>
				
			 <s:if test="#request.defines==null && #request.CTRL.l.cscStatus || #request.defines['cscStatus']!=null">
			 <td <s:if test="#request.defines['cscStatus']>0">colspan="${defines["cscStatus"]}" ${all?"width":"iwidth"}="${defines["cscStatus"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=120":""} </s:else> tdid="12" ref="cscStatus"   title="状态">
			...
			<td rowspan="2" width="105" class="operation" tdid="14" ref="operation">
				<s:if test="#request.CTRL.canAdd==true">
					<a href="javascript:AddCare()" style="text-decoration:underline">添加</a>
				</s:if>
				<s:else>&nbsp;</s:else>
			</td>
		  </tr>
		 ...
		 <tbody>
		  ...
		  <tr id="${item.cscId}" >
			...
			<s:if test="#request.defines==null && #request.CTRL.l.cscUser || #request.defines['cscUser']!=null">
				<s:if test="#request.defines['cscUser']>0">
					${lz:set("注释","****cscUser关联表的子级字段：如果用户勾选了要显示的话****")}
				  	<s:iterator value="#request.childes" id="fieldName" status="j">
				  		${lz:set("isList",lz:indexOf(fieldName,"[]")>-1)}
				 		<s:if test="#request.isList==true">
				 			${lz:set("sizeList",lz:size(item.cscUser))}
				 			${lz:set("rowspan",rowspan>sizeList?rowspan:sizeList)}
				 		</s:if>
				  		${lz:set("atCscUser",lz:indexOf(fieldName,"cscUser")>-1)}
				  		<s:if test="#request.atCscUser==true">
				 			<td ${isList?"class='onemore'":""}>${lz:left(lz:el(item,fieldName),100)}</td>
				 		</s:if>
				 	</s:iterator>
				</s:if>
				<s:else>
					${lz:set("注释","****cscUser字段的字串格式化输出****")}
					<td ref="cscUser" class="td " relate="${item.cscUser}">
						 
						 	<a <s:if test="#item.cscUser!=null && #item.cscUser!=''"> onclick="window.href('${basePath}${proname}/permissions/user_details.do?key=${item.cscUser}',{ctrl:{editable:false,visible:true}})" href="javascript:void(0);"</s:if>>
						 	${lz:or(item$cscUser[i.count-1],lz:left(item.cscUser$,100))}</a>
					</td>
				</s:else>
			</s:if>
			 
			<s:if test="#request.defines==null && #request.CTRL.l.cscSort || #request.defines['cscSort']!=null">
				<s:if test="#request.defines['cscSort']>0">
					${lz:set("注释","****cscSort关联表的子级字段：如果用户勾选了要显示的话****")}
				  	<s:iterator value="#request.childes" id="fieldName" status="j">
				  		${lz:set("isList",lz:indexOf(fieldName,"[]")>-1)}
				 		<s:if test="#request.isList==true">
				 			${lz:set("sizeList",lz:size(item.cscSort))}
				 			${lz:set("rowspan",rowspan>sizeList?rowspan:sizeList)}
				 		</s:if>
				  		${lz:set("atCscSort",lz:indexOf(fieldName,"cscSort")>-1)}
				  		<s:if test="#request.atCscSort==true">
				 			<td ${isList?"class='onemore'":""}>${lz:left(lz:el(item,fieldName),100)}</td>
				 		</s:if>
				 	</s:iterator>
				</s:if>
				<s:else>
					${lz:set("注释","****cscSort字段的字串格式化输出****")}
					<td ref="cscSort" class="td " relate="${item.cscSort}">
						 
						 	<a <s:if test="#item.cscSort!=null && #item.cscSort!=''"> onclick="window.href('${basePath}${proname}/configurator/property_details.do?key=${item.cscSort}',{ctrl:{editable:false,visible:true}})" href="javascript:void(0);"</s:if>>
						 	${lz:or(item$cscSort[i.count-1],lz:left(item.cscSort$,100))}</a>
					</td>
				</s:else>
			</s:if>
			...
			<s:if test="#request.defines==null && #request.CTRL.l.cscAlarmTime || #request.defines['cscAlarmTime']!=null">
				<s:if test="#request.defines['cscAlarmTime']>0">
					${lz:set("注释","****cscAlarmTime关联表的子级字段：如果用户勾选了要显示的话****")}
				  	<s:iterator value="#request.childes" id="fieldName" status="j">
				  		${lz:set("isList",lz:indexOf(fieldName,"[]")>-1)}
				 		<s:if test="#request.isList==true">
				 			${lz:set("sizeList",lz:size(item.cscAlarmTime))}
				 			${lz:set("rowspan",rowspan>sizeList?rowspan:sizeList)}
				 		</s:if>
				  		${lz:set("atCscAlarmTime",lz:indexOf(fieldName,"cscAlarmTime")>-1)}
				  		<s:if test="#request.atCscAlarmTime==true">
				 			<td ${isList?"class='onemore'":""}>${lz:left(lz:el(item,fieldName),100)}</td>
				 		</s:if>
				 	</s:iterator>
				</s:if>
				<s:else>
					${lz:set("注释","****cscAlarmTime字段的字串格式化输出****")}
					<td ref="cscAlarmTime" class="td ">
						 
						 	${lz:or(item$cscAlarmTime[i.count-1],lz:left(item.cscAlarmTime$,100))}
					</td>
				</s:else>
			</s:if>
		   ...
		   ${lz:set("注释","*****************数据列表数据部分结束*****************")}		  
			...
			<s:if test="#request.alias=='select'">
				<a href="javascript:SelectCare('${item.cscId}')">选择</a>
			</s:if>
			<s:if test="#request.alias==null">
				<s:if test="#request.CTRL.canEdit==true">
				<a href="javascript:EditCare('${item.cscId}')">修改</a></s:if>
				<s:if test="#request.CTRL.canDel==true">
				<a href="javascript:DelCare('${item.cscId}','${item.cscTitle}')">删除</a></s:if>
			</s:if>
			<s:if test="#request.CTRL.canView==true">
			<a href="javascript:DetailsCare('${item.cscId}')">详情</a></s:if>
			</td>
		  </tr>
					...
					${lz:set("注释","****cscUser关联表的子级字段：如果用户勾选了要显示的话****")}
				  	<s:iterator value="#request.childes" id="fieldName" status="j">
				  		...
				  		<s:if test="#request.atCscUser==true">
				  			${lz:set("isList",lz:indexOf(fieldName,"[]")>-1)}
				  			<s:if test="#request.isList==true">
				  				<lz:set name="arrFieldName">cscUser[${k.count}]${lz:split(fieldName,"\\[\\]")[1]}</lz:set>
				 				<td class="onemore">${lz:left(lz:el(item,arrFieldName),100)}</td>
				 			</s:if>
				 			<s:else>
								<s:if test="#k.count==1"><td class="blank" rowspan="${rowspan-1}">&nbsp;</td></s:if>
					...
					${lz:set("注释","****cscSort关联表的子级字段：如果用户勾选了要显示的话****")}
				  	<s:iterator value="#request.childes" id="fieldName" status="j">
				  		...
				  		<s:if test="#request.atCscSort==true">
				  			${lz:set("isList",lz:indexOf(fieldName,"[]")>-1)}
				  			<s:if test="#request.isList==true">
				  				<lz:set name="arrFieldName">cscSort[${k.count}]${lz:split(fieldName,"\\[\\]")[1]}</lz:set>
				 				<td class="onemore">${lz:left(lz:el(item,arrFieldName),100)}</td>
				 			</s:if>
				 			<s:else>
								<s:if test="#k.count==1"><td class="blank" rowspan="${rowspan-1}">&nbsp;</td></s:if>
					...
					${lz:set("注释","****cscAlarmTime关联表的子级字段：如果用户勾选了要显示的话****")}
				  	<s:iterator value="#request.childes" id="fieldName" status="j">
				  		...
				  		<s:if test="#request.atCscAlarmTime==true">
				  			${lz:set("isList",lz:indexOf(fieldName,"[]")>-1)}
				  			<s:if test="#request.isList==true">
				  				<lz:set name="arrFieldName">cscAlarmTime[${k.count}]${lz:split(fieldName,"\\[\\]")[1]}</lz:set>
				 				<td class="onemore">${lz:left(lz:el(item,arrFieldName),100)}</td>
				 			</s:if>
				 			<s:else>
								<s:if test="#k.count==1"><td class="blank" rowspan="${rowspan-1}">&nbsp;</td></s:if>
...
<s:if test="#request.CTRL.canAdd==true">
	...
	* 添加客户关怀
	**/
	function AddCare(parent){		
		var url = "${basePath}${namespace}care_edit.do?edittype=save";
		var params = {entrypoint:"${entrypoint}",parent:(parent?parent:""),ctrl:${ctrl==null?"{title:'添加客户关怀'}":lz:json(ctrl)}};
		href(url,params);
	}
</s:if>
<s:if test="#request.alias=='select'">
...
<s:if test="#request.CTRL.canEdit==true">
	...
	**/
	function EditCare(id){
		var url = "${basePath}${namespace}care_edit.do?edittype=update";
		var params = {entrypoint:"${entrypoint}",id:id,ctrl:${ctrl==null?"{title:'修改客户关怀'}":lz:json(ctrl)}};
		href(url,params);
	}
	/*
	* 修改所选
	...
	function UpdateSel(flag)
	{
		...
		if(checker.length==0){
			...
			$.alert("确定要对当前查询的${objects.count}条记录进行批量修改吗?",function(){
				var url = "${basePath}${namespace}care_edit.do";
				var params = {entrypoint:"${entrypoint}",method:flag,PARAMS:${lz:json(PARAMS)},ctrl:{title:"批量更新客户关怀",visible:false,editable:false,fields:{}}};
			...
			var url = "${basePath}${namespace}care_edit.do";
			var params = {entrypoint:"${entrypoint}",method:flag,ids:ids,ctrl:{title:"批量更新客户关怀",visible:false,editable:false,fields:{}}};
			for(var o in fieldNames[flag])
				params.ctrl["fields"][o]={visible:true,editable:true};
			href(url,params);
		}
	...
	**/
	...
	{
		var url = "${basePath}${namespace}care_edit.do";
		var params = {entrypoint:"${entrypoint}",id:id,method:flag,ctrl:{title:"更新客户关怀",visible:false,editable:false,fields:{}}};
		for(var o in fieldNames[flag])
			params.ctrl["fields"][o]={visible:true,editable:true};
		href(url,params);
	}
	...
	$(function(){
		...
		$(".table tbody td.td").dblclick(function(){
			var url = "${basePath}${namespace}care_edit.do";
			var params = {entrypoint:"${entrypoint}",id:$(this).parents("tr:eq(0)").attr("id"),method:"any",ctrl:{title:"更新客户关怀",visible:false,editable:false,fields:{}}};
...
<s:if test="#request.CTRL.canDel==true">
	...
	* 删除客户关怀
	**/
	function DelCare(id,flag)
	{
		var dialog=top.$.dialog({
				html:"<span style='white-space:nowrap;'>确定要删除客户关怀["+flag+"]吗？</span>",
				title:"删除客户关怀",
				modal: true,			
				onOk:function(){
					dialog.closeDialog();
					var url = "${basePath}${namespace}care_del.do";
					var params = {entrypoint:"${entrypoint}",id:id};
					$.submit(url,params);
				}
		 });	
	...
	*/
	function DelSel(){
		...
		var dialog=top.$.dialog({
				html:"<span style='white-space:nowrap;'>确定要删除所选客户关怀吗？</span>",
				title:"批量删除客户关怀",
				modal: true,			
				onOk:function(){
					var ids = "";
	...
	* 显示客户关怀详情
	**/
	function DetailsCare(id)
	{
		var url = "${basePath}${namespace}care_details.do";
		var params = {entrypoint:"${entrypoint}",id:id,ctrl:{title:"客户关怀详情",editable:false,visible:true}};
		href(url,params);
	}
	/*
	* 显示所选记录详情
