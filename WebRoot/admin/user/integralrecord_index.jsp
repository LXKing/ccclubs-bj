...
<lz:DefaultCtrl>{
	...
	queryables:"csrId,csrHost,csrMember,csrType,csrAmount,csrRemark,csrAddTime,csrOrder,csrEditor",
	${lz:set("注释","当用户选择显示全部字段时，哪些字段可显示在表格中")}
	listables:"csrId,csrHost,csrMember,csrType,csrAmount,csrRemain,csrRemark,csrAddTime,csrOrder,csrRelate,csrEditor",
	</s:if>
	<s:else>
	${lz:set("注释","当用户选择显示部分字段时，哪些字段可查询")}
	queryables:"csrId,csrHost,csrMember,csrType,csrAmount,csrRemark,csrAddTime",
	${lz:set("注释","当用户选择显示部分字段时，哪些字段可显示在表格中")}
	listables:"csrId,csrHost,csrMember,csrType,csrAmount,csrRemain,csrRemark,csrAddTime",
	</s:else>
}</lz:DefaultCtrl>
${lz:set("注释","***************************************************")}
...
<div class="title">
	...
	<span class="caption">${empty title?"积分记录管理":title}</span>
</div>
  	</s:if>
...
<div class="plate" style="${((hideQuery==true && window.imize!="maximize")||(hideQuery!=true && window.imize=="minimize")) ? "display:none;":""}">
	<form ref="CsIntegralRecord" class="query" id="queryForm" action="${basePath}${lz:replace(servletPath,"^/","")}?${lz:querys("UTF-8","page")}" method="post">
		${lz:set("haveQuery",false)}
		
		${lz:set("注释","****导入查询表单各字段****")}
		<%@include file="integralrecord.query.jsp"%>
	...
	<div class="content">
	    <div class="toolbar">
	      ...
	      <s:if test="#request.alias==null">
			...
			<button type="button" class="add" onclick="AddIntegralrecord();">
				添加
			</button>							
			</s:if>
		  </s:if>
		  ...
		  <s:iterator value="#request.items" id="handle" status="i">
			...
			<dl ref="com.ccclubs.model.CsIntegralRecord" class="display dropdowan" style="margin-left:30px;">
				<dt>
			...
			<dl ref="com.ccclubs.model.CsIntegralRecord" class="batch dropdowan">
				<dt>
		...
		${lz:set("注释","*****************数据列表开始*****************")}
		<table id="integralrecord_table" ref="CsIntegralRecord" class="table" width="100%" border="0" cellspacing="1" cellpadding="0">
		<thead class="state-header">
		  ...
		  ${lz:set("注释","****数据列表列头开始****")}
			 ...
			 <td <s:if test="#request.defines['csrMember']>0">colspan="${defines["csrMember"]}" ${all?"width":"iwidth"}="${defines["csrMember"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=120":""} </s:else> tdid="3" ref="csrMember"   title="消费会员">
			 ...
			 <td <s:if test="#request.defines['csrType']>0">colspan="${defines["csrType"]}" ${all?"width":"iwidth"}="${defines["csrType"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=200":""} </s:else> tdid="4" ref="csrType"   title="积分类型">
			 ...
			 <td <s:if test="#request.defines['csrAmount']>0">colspan="${defines["csrAmount"]}" ${all?"width":"iwidth"}="${defines["csrAmount"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=120":""} </s:else> tdid="5" ref="csrAmount"   title="变化积分">
			 ...
			 <td <s:if test="#request.defines['csrRemain']>0">colspan="${defines["csrRemain"]}" ${all?"width":"iwidth"}="${defines["csrRemain"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=120":""} </s:else> tdid="6" ref="csrRemain"   title="剩余积分">
			 ...
			 <td <s:if test="#request.defines['csrRemark']>0">colspan="${defines["csrRemark"]}" ${all?"width":"iwidth"}="${defines["csrRemark"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=200":""} </s:else> tdid="7" ref="csrRemark"   title="描述">
			 ...
			 <td <s:if test="#request.defines['csrAddTime']>0">colspan="${defines["csrAddTime"]}" ${all?"width":"iwidth"}="${defines["csrAddTime"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=140":""} </s:else> tdid="8" ref="csrAddTime"   title="发生时间">
			 ...
			 <td <s:if test="#request.defines['csrOrder']>0">colspan="${defines["csrOrder"]}" ${all?"width":"iwidth"}="${defines["csrOrder"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=120":""} </s:else> tdid="9" ref="csrOrder"   title="关联订单">
			 ...
			 <td <s:if test="#request.defines['csrRelate']>0">colspan="${defines["csrRelate"]}" ${all?"width":"iwidth"}="${defines["csrRelate"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=120":""} </s:else> tdid="10" ref="csrRelate"   title="关联编号">
			 ...
			 <td <s:if test="#request.defines['csrEditor']>0">colspan="${defines["csrEditor"]}" ${all?"width":"iwidth"}="${defines["csrEditor"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=120":""} </s:else> tdid="11" ref="csrEditor"   title="操作人">
			...
			<td rowspan="2" width="105" class="operation" tdid="13" ref="operation">
				<s:if test="#request.CTRL.canAdd==true">
					<a href="javascript:AddIntegralrecord()" style="text-decoration:underline">添加</a>
				</s:if>
				<s:else>&nbsp;</s:else>
			</td>
		  </tr>
		  ...
		  <tr id="${item.csrId}" >
		   ...
		   ${lz:set("注释","*****************数据列表数据部分结束*****************")}		  
			...
			<s:if test="#request.alias=='select'">
				<a href="javascript:SelectIntegralrecord('${item.csrId}')">选择</a>
			</s:if>
			<s:if test="#request.alias==null">
				<s:if test="#request.CTRL.canEdit==true">
				<a href="javascript:EditIntegralrecord('${item.csrId}')">修改</a></s:if>
				<s:if test="#request.CTRL.canDel==true">
				<a href="javascript:DelIntegralrecord('${item.csrId}','${item.csrId}')">删除</a></s:if>
			</s:if>
			<s:if test="#request.CTRL.canView==true">
			<a href="javascript:DetailsIntegralrecord('${item.csrId}')">详情</a></s:if>
			</td>
		  </tr>
...
<s:if test="#request.CTRL.canAdd==true">
	...
	* 添加积分记录
	**/
	function AddIntegralrecord(parent){		
		var url = "${basePath}${namespace}integralrecord_edit.do?edittype=save";
		var params = {entrypoint:"${entrypoint}",parent:(parent?parent:""),ctrl:${ctrl==null?"{title:'添加积分记录'}":lz:json(ctrl)}};
		href(url,params);
	}
</s:if>
<s:if test="#request.alias=='select'">
	...
	* 选择积分记录
	**/
	function SelectIntegralrecord(id){		
...
<s:if test="#request.CTRL.canEdit==true">
	...
	* 修改积分记录
	**/
	function EditIntegralrecord(id){
		var url = "${basePath}${namespace}integralrecord_edit.do?edittype=update";
		var params = {entrypoint:"${entrypoint}",id:id,ctrl:${ctrl==null?"{title:'修改积分记录'}":lz:json(ctrl)}};
		href(url,params);
	}
	/*
	* 修改所选
	*/
	function EditSel(){
		...
		EditIntegralrecord(checker.val());
	}
	...
	function UpdateSel(flag)
	{
		...
		if(checker.length==0){
			...
			$.alert("确定要对当前查询的${objects.count}条记录进行批量修改吗?",function(){
				var url = "${basePath}${namespace}integralrecord_edit.do";
				var params = {entrypoint:"${entrypoint}",method:flag,PARAMS:${lz:json(PARAMS)},ctrl:{title:"批量更新积分记录",visible:false,editable:false,fields:{}}};
			...
			var url = "${basePath}${namespace}integralrecord_edit.do";
			var params = {entrypoint:"${entrypoint}",method:flag,ids:ids,ctrl:{title:"批量更新积分记录",visible:false,editable:false,fields:{}}};
			for(var o in fieldNames[flag])
				params.ctrl["fields"][o]={visible:true,editable:true};
			href(url,params);
		}
	...
	**/
	...
	{
		var url = "${basePath}${namespace}integralrecord_edit.do";
		var params = {entrypoint:"${entrypoint}",id:id,method:flag,ctrl:{title:"更新积分记录",visible:false,editable:false,fields:{}}};
		for(var o in fieldNames[flag])
			params.ctrl["fields"][o]={visible:true,editable:true};
		href(url,params);
	}
	...
	$(function(){
		...
		$(".table tbody td.td").dblclick(function(){
			var url = "${basePath}${namespace}integralrecord_edit.do";
			var params = {entrypoint:"${entrypoint}",id:$(this).parents("tr:eq(0)").attr("id"),method:"any",ctrl:{title:"更新积分记录",visible:false,editable:false,fields:{}}};
...
<s:if test="#request.CTRL.canDel==true">
	...
	* 删除积分记录
	**/
	function DelIntegralrecord(id,flag)
	{
		var dialog=top.$.dialog({
				html:"<span style='white-space:nowrap;'>确定要删除积分记录["+flag+"]吗？</span>",
				title:"删除积分记录",
				modal: true,			
				onOk:function(){
					dialog.closeDialog();
					var url = "${basePath}${namespace}integralrecord_del.do";
					var params = {entrypoint:"${entrypoint}",id:id};
					$.submit(url,params);
				}
		 });	
	...
	*/
	function DelSel(){
		...
		var dialog=top.$.dialog({
				html:"<span style='white-space:nowrap;'>确定要删除所选积分记录吗？</span>",
				title:"批量删除积分记录",
				modal: true,			
				onOk:function(){
					var ids = "";
	...
	* 显示积分记录详情
	**/
	function DetailsIntegralrecord(id)
	{
		var url = "${basePath}${namespace}integralrecord_details.do";
		var params = {entrypoint:"${entrypoint}",id:id,ctrl:{title:"积分记录详情",editable:false,visible:true}};
		href(url,params);
	}
	/*
	* 显示所选记录详情
	*/
	function DetailsSel(){
		...
		DetailsIntegralrecord(checker.val());
	}
	/*
	* 数据导出
	*/
