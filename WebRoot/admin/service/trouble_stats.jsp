...
<div class="title">
	...
	<a class="linker export" href="trouble_stats.do?export=true">
		<img align="absmiddle" src="${basePath}admin/images/icons/20088256427828778018.png"/>
		数据导出
	</a>
	<a href="trouble.do?${lz:querys("UTF-8","")}" class="linker">
		<img align="absmiddle" src="${basePath}admin/images/icons/20088256421631277806.png"/>
		数据管理
	</a>
	...
	<span class="caption">车辆事故统计</span>
...
<div class="plate" style="${(hideQuery || (lz:or(window.imize,"maximize"))=="minimize") ? "display:none;":""}">
	<form ref="CsTrouble" class="query statsForm" id="queryForm" action="trouble_stats.do" method="post">
		...
		<div class="group condition">
			...
			<ul>
				<li>
					<input ${cstOrderGroup==true?"checked='checked'":""} class="groupBy" type="checkbox" name="groupBy" value="cstOrder"/>事故订单
				 	&nbsp;&nbsp;
				</li>
				<li>
					<input ${cstMemberGroup==true?"checked='checked'":""} class="groupBy" type="checkbox" name="groupBy" value="cstMember"/>事故会员
				 	&nbsp;&nbsp;
				</li>
				<li>
					<input ${cstCarGroup==true?"checked='checked'":""} class="groupBy" type="checkbox" name="groupBy" value="cstCar"/>事故车辆
				 	&nbsp;&nbsp;
				</li>
				<li>
					<input ${cstTypeGroup==true?"checked='checked'":""} class="groupBy" type="checkbox" name="groupBy" value="cstType"/>事故类型
				 	&nbsp;&nbsp;
				</li>
				<li>
					<input ${cstAccepterGroup==true?"checked='checked'":""} class="groupBy" type="checkbox" name="groupBy" value="cstAccepter"/>受理人员
				 	&nbsp;&nbsp;
				</li>
				<li>
					<input ${cstCaserGroup==true?"checked='checked'":""} class="groupBy" type="checkbox" name="groupBy" value="cstCaser"/>结案人员
				 	&nbsp;&nbsp;
				</li>
				<li>
					<input ${cstStatusGroup==true?"checked='checked'":""} class="groupBy" type="checkbox" name="groupBy" value="cstStatus"/>处理状态
				 	&nbsp;&nbsp;
				</li>
				<div class="clear"></div>
			</ul>
		...
		<div class="sum condition">
			...
			<ul>
				...
				<li>
				 	<input ${cstInvoiceSum==true?"checked='checked'":""} class="sumBy" type="checkbox" name="sumBy" value="cstInvoice"/>
					开票金额&nbsp;&nbsp;
				</li>
				<li>
				 	<input ${cstInsureFeeSum==true?"checked='checked'":""} class="sumBy" type="checkbox" name="sumBy" value="cstInsureFee"/>
					保险定损&nbsp;&nbsp;
				</li>
				<li>
				 	<input ${cstMoneySum==true?"checked='checked'":""} class="sumBy" type="checkbox" name="sumBy" value="cstMoney"/>
					协商扣款&nbsp;&nbsp;
				</li>
				<li>
				 	<input ${cstThirdLossSum==true?"checked='checked'":""} class="sumBy" type="checkbox" name="sumBy" value="cstThirdLoss"/>
					三者损失&nbsp;&nbsp;
				</li>
				<li>
				 	<input ${cstTrailFeeSum==true?"checked='checked'":""} class="sumBy" type="checkbox" name="sumBy" value="cstTrailFee"/>
					拖车费用&nbsp;&nbsp;
				</li>
				<li>
				 	<input ${cstCompensateSum==true?"checked='checked'":""} class="sumBy" type="checkbox" name="sumBy" value="cstCompensate"/>
					赔付到账&nbsp;&nbsp;
				</li>
				<li>
				 	<input ${cstRepairPaySum==true?"checked='checked'":""} class="sumBy" type="checkbox" name="sumBy" value="cstRepairPay"/>
					维修费用&nbsp;&nbsp;
				</li>
				<li>
				 	<input ${cstAgioSum==true?"checked='checked'":""} class="sumBy" type="checkbox" name="sumBy" value="cstAgio"/>
					合计差价&nbsp;&nbsp;
				</li>
	...
	<div class="content">
		...
		stats.datas=[ 
			  ...
			  <lz:set name="itemValue">
					 	 ${get:CsOrder(item.cstOrder).csoId}			 	 
		  	 </lz:set>
		  	 ,"cstOrder":"${lz:js(lz:trim(itemValue))}"
			  <lz:set name="itemValue">
					 	 ${get:CsMember(item.cstMember).csmName}			 	 
		  	 </lz:set>
		  	 ,"cstMember":"${lz:js(lz:trim(itemValue))}"
			  <lz:set name="itemValue">
					 	 ${get:CsCar(item.cstCar).cscNumber}			 	 
		  	 </lz:set>
		  	 ,"cstCar":"${lz:js(lz:trim(itemValue))}"
			  <lz:set name="itemValue">
						 ${item.cstType==0?"简易事故":""}
						 ${item.cstType==1?"单方事故":""}
						 ${item.cstType==2?"我方全责":""}
		  	 ...
		  	 ,"cstType":"${lz:js(lz:trim(itemValue))}"
			  <lz:set name="itemValue">
					 	 ${get:SrvUser(item.cstAccepter).suRealName}			 	 
		  	 </lz:set>
		  	 ,"cstAccepter":"${lz:js(lz:trim(itemValue))}"
			  <lz:set name="itemValue">
					 	 ${get:SrvUser(item.cstCaser).suRealName}			 	 
		  	 </lz:set>
		  	 ,"cstCaser":"${lz:js(lz:trim(itemValue))}"
			  <lz:set name="itemValue">
						 ${item.cstStatus==0?"已录入待定损":""}
						 ${item.cstStatus==1?"已定损待协商":""}
						 ${item.cstStatus==2?"已协商待维修":""}
						 ${item.cstStatus==3?"正在维修":""}
						 ${item.cstStatus==4?"已维修待扣款":""}
						 ${item.cstStatus==5?"已扣款待结案":""}
						 ${item.cstStatus==6?"已结案":""}
		  	 </lz:set>
		  	 ,"cstStatus":"${lz:js(lz:trim(itemValue))}"
		  	 ,"cstInvoice":"${lz:digit(item.cstInvoice,2)}"
		  	 ,"cstInsureFee":"${lz:digit(item.cstInsureFee,2)}"
		  	 ,"cstMoney":"${lz:digit(item.cstMoney,2)}"
		  	 ,"cstThirdLoss":"${lz:digit(item.cstThirdLoss,2)}"
		  	 ,"cstTrailFee":"${lz:digit(item.cstTrailFee,2)}"
		  	 ,"cstCompensate":"${lz:digit(item.cstCompensate,2)}"
		  	 ,"cstRepairPay":"${lz:digit(item.cstRepairPay,2)}"
		  	 ,"cstAgio":"${lz:digit(item.cstAgio,2)}"
			  <s:iterator value="#request.sumBy" id="sumName" status="i">
			  ${lz:set("array",lz:split(sumName,":"))}${lz:set("arrayCount",lz:size(array))}
