...
<div class="title">
	...
	<a class="linker export" href="violat_stats.do?export=true">
		<img align="absmiddle" src="${basePath}admin/images/icons/20088256427828778018.png"/>
		数据导出
	</a>
	<a href="violat.do?${lz:querys("UTF-8","")}" class="linker">
		<img align="absmiddle" src="${basePath}admin/images/icons/20088256421631277806.png"/>
		数据管理
	</a>
	...
	<span class="caption">车辆违章统计</span>
...
<div class="plate" style="${(hideQuery || (lz:or(window.imize,"maximize"))=="minimize") ? "display:none;":""}">
	<form ref="CsViolat" class="query statsForm" id="queryForm" action="violat_stats.do" method="post">
		...
		<div class="group condition">
			...
			<ul>
				<li>
					<input ${csvOrderGroup==true?"checked='checked'":""} class="groupBy" type="checkbox" name="groupBy" value="csvOrder"/>违章订单
				 	&nbsp;&nbsp;
				</li>
				<li>
					<input ${csvMemberGroup==true?"checked='checked'":""} class="groupBy" type="checkbox" name="groupBy" value="csvMember"/>违章会员
				 	&nbsp;&nbsp;
				</li>
				<li>
					<input ${csvCarGroup==true?"checked='checked'":""} class="groupBy" type="checkbox" name="groupBy" value="csvCar"/>违章车辆
				 	&nbsp;&nbsp;
				</li>
				<li>
					<input ${csvBranchGroup==true?"checked='checked'":""} class="groupBy" type="checkbox" name="groupBy" value="csvBranch"/>处理部门
				 	&nbsp;&nbsp;
				</li>
				<li>
					<input ${csvAccepterGroup==true?"checked='checked'":""} class="groupBy" type="checkbox" name="groupBy" value="csvAccepter"/>受理人员
				 	&nbsp;&nbsp;
				</li>
				<li>
					<input ${csvAdderGroup==true?"checked='checked'":""} class="groupBy" type="checkbox" name="groupBy" value="csvAdder"/>添加人员
				 	&nbsp;&nbsp;
				</li>
				<li>
					<input ${csvStatusGroup==true?"checked='checked'":""} class="groupBy" type="checkbox" name="groupBy" value="csvStatus"/>业务状态
				 	&nbsp;&nbsp;
				</li>
				<div class="clear"></div>
			</ul>
		...
		<div class="sum condition">
			...
			<ul>
				...
				<li>
				 	<input ${csvMoneySum==true?"checked='checked'":""} class="sumBy" type="checkbox" name="sumBy" value="csvMoney"/>
					违章罚款&nbsp;&nbsp;
				</li>
				<li>
				 	<input ${csvScoreSum==true?"checked='checked'":""} class="sumBy" type="checkbox" name="sumBy" value="csvScore"/>
					违章扣分&nbsp;&nbsp;
				</li>
	...
	<div class="content">
		...
		stats.datas=[ 
			  ...
			  <lz:set name="itemValue">
					 	 ${get:CsOrder(item.csvOrder).csoId}			 	 
		  	 </lz:set>
		  	 ,"csvOrder":"${lz:js(lz:trim(itemValue))}"
			  <lz:set name="itemValue">
					 	 ${get:CsMember(item.csvMember).csmName}			 	 
		  	 </lz:set>
		  	 ,"csvMember":"${lz:js(lz:trim(itemValue))}"
			  <lz:set name="itemValue">
					 	 ${get:CsCar(item.csvCar).cscNumber}			 	 
		  	 </lz:set>
		  	 ,"csvCar":"${lz:js(lz:trim(itemValue))}"
			  <lz:set name="itemValue">
						 ${item.csvBranch==0?"交警":""}
						 ${item.csvBranch==1?"城管":""}
						 ${item.csvBranch==2?"铁路":""}
		  	 ...
		  	 ,"csvBranch":"${lz:js(lz:trim(itemValue))}"
			  <lz:set name="itemValue">
					 	 ${get:SrvUser(item.csvAccepter).suRealName}			 	 
		  	 </lz:set>
		  	 ,"csvAccepter":"${lz:js(lz:trim(itemValue))}"
			  <lz:set name="itemValue">
					 	 ${get:SrvUser(item.csvAdder).suRealName}			 	 
		  	 </lz:set>
		  	 ,"csvAdder":"${lz:js(lz:trim(itemValue))}"
			  <lz:set name="itemValue">
						 ${item.csvStatus==0?"未交款，未处理":""}
						 ${item.csvStatus==1?"未交款，已处理":""}
						 ${item.csvStatus==2?"已交款，已处理":""}
						 ${item.csvStatus==3?"等待查询":""}
		  	 </lz:set>
		  	 ,"csvStatus":"${lz:js(lz:trim(itemValue))}"
		  	 ,"csvMoney":"${lz:digit(item.csvMoney,2)}"
		  	 ,"csvScore":"${lz:digit(item.csvScore,2)}"
			  <s:iterator value="#request.sumBy" id="sumName" status="i">
			  ${lz:set("array",lz:split(sumName,":"))}${lz:set("arrayCount",lz:size(array))}
