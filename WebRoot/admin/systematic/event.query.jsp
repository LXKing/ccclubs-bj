...
<%@ taglib prefix="get" uri="/get-tags" %>
				...
				<dl group="${relateObject==null?"cseContent":relateObject}" id="form-dl-cseContent">
					...
					<dd>
						...
						<input class="YesNot" type="hidden" name="${NAME}YesNot" id="${NAME}YesNot" value="${data.cseContentYesNot}"/>
			 			<input type="text" class="input" size="16"  maxlength="256" name="${NAME}" id="${NAME}"  value="${data.cseContent}"/>
		 	 		
					</dd>
				</dl>
			</s:if>
			${lz:set("注释","*****************事件内容字段的查询代码结束*****************")}
			...
			${after$cseContent}
			 ...
			 ${lz:set("NAME",relateObject==null? "cseType" : (lz:join(relateObject,"$cseType")))}
			 ${lz:set("cname",lz:join(relateObject,".cseType"))}
			 ${lz:set("data",relateObject==null?objects:objects[lz:ForMat(relateObject)])}
			 <s:if test="#request.defines==null && #request.CTRL.q.cseType || #request.defines['cseType']!=null || #request.children[#request.cname]!=null">
			 	${lz:set("haveQuery",true)}
				<dl group="${relateObject==null?"cseType":relateObject}" id="form-dl-cseType">
					<dt>事件类型：</dt>
					<dd>
					
						<a href="javascript:void(0);" title="不包含" class="checkbox ${data.cseTypeYesNot=="not"?"checked-not":""}${data.cseTypeYesNot=="yes"?"checked-yes":""}" for="${NAME}YesNot"></a>
						<input class="YesNot" type="hidden" name="${NAME}YesNot" id="${NAME}YesNot" value="${data.cseTypeYesNot}"/>
			 			<select class="combox"  action="${basePath}${proname}/systematic/eventtype_query.do?size=-1" id="${NAME}" name="${NAME}">
				 				<option selected value="${data.cseType}">
				 					${get:CsEventType(data.cseType).csetName}
				 				</option>
				 		</select>
			...
			<s:if test="#request.relateObject==null && #request.defines['cseType']>0">
				${lz:set("注释","****当事件类型字段有关联对象，并且用户勾选了显示其字段的话，把相关的查询字段也显示出来****")}
			  	${lz:set("relateObject","cseType")}
			  	<input type="hidden" name="cseType$on" id="CsEventType$on" value="true"/>
			...
			${after$cseType}
			 ...
			 ${lz:set("注释","*****************关联ID字段的查询输入框代码*****************")}
			 ${lz:set("NAME",relateObject==null? "cseDataId" : (lz:join(relateObject,"$cseDataId")))}
			 ${lz:set("cname",lz:join(relateObject,".cseDataId"))}
			 ${lz:set("data",relateObject==null?objects:objects[lz:ForMat(relateObject)])}
			 <s:if test="#request.defines==null && #request.CTRL.q.cseDataId || #request.defines['cseDataId']!=null || #request.children[#request.cname]!=null">
			 	${lz:set("haveQuery",true)}
				<dl group="${relateObject==null?"cseDataId":relateObject}" id="form-dl-cseDataId">
					<dt>关联ID：</dt>
					<dd>
					
						<a href="javascript:void(0);" title="不包含" class="checkbox ${data.cseDataIdYesNot=="not"?"checked-not":""}${data.cseDataIdYesNot=="yes"?"checked-yes":""}" for="${NAME}YesNot"></a>
						<input class="YesNot" type="hidden" name="${NAME}YesNot" id="${NAME}YesNot" value="${data.cseDataIdYesNot}"/>
			 		<input title="请输入动态查询" class="combox" generic="cseDataIdGeneric" size="16" type="text" id="${NAME}" name="${NAME}" text="${get:generic(data.cseDataId)}" value="${data.cseDataId}"/>
