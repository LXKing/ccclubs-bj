...
<lz:DefaultCtrl>{
	...
	queryables:"cspId,cspName,cspStatus",
	${lz:set("注释","当用户选择显示全部字段时，哪些字段可显示在表格中")}
	listables:"cspId,cspName,cspFlag,cspUpdateTime,cspAddTime,cspStatus",
	</s:if>
	<s:else>
	${lz:set("注释","当用户选择显示部分字段时，哪些字段可查询")}
	queryables:"cspId,cspName,cspStatus",
	${lz:set("注释","当用户选择显示部分字段时，哪些字段可显示在表格中")}
	listables:"cspId,cspName,cspFlag,cspAddTime,cspStatus",
	</s:else>
}</lz:DefaultCtrl>
${lz:set("注释","***************************************************")}
...
<div class="title">
	...
	<span class="caption">${empty title?"产品管理":title}</span>
</div>
  	</s:if>
	...
	<div class="content">
	    <div class="toolbar">
		  ...
		  <s:iterator value="#request.items" id="handle" status="i">
			...
			<dl ref="com.ccclubs.model.CsProduct" class="display dropdowan" style="margin-left:30px;">
				<dt>
			...
			<dl ref="com.ccclubs.model.CsProduct" class="batch dropdowan">
				<dt>
				...
				<dd>
					<table class="shadow" border="0" cellspacing="0" cellpadding="0">
					<tbody><tr>  <td class="l"></td>  <td class="c">
					<s:if test="#request.defines==null && #request.CTRL.l.cspGoods || #request.defines['cspGoods']!=null">
						<a href="javascript:void(0);" onclick="UpdateSel('goods')">商品列表</a>
					</s:if>
		...
		${lz:set("注释","*****************数据列表开始*****************")}
		<table id="product_table" ref="CsProduct" class="table" width="100%" border="0" cellspacing="1" cellpadding="0">
		<thead class="state-header">
		  ...
		  ${lz:set("注释","****数据列表列头开始****")}
			 ...
			 <td <s:if test="#request.defines['cspName']>0">colspan="${defines["cspName"]}" ${all?"width":"iwidth"}="${defines["cspName"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=200":""} </s:else> tdid="2" ref="cspName" flagKey  title="名称">
			 ...
			 <td <s:if test="#request.defines['cspFlag']>0">colspan="${defines["cspFlag"]}" ${all?"width":"iwidth"}="${defines["cspFlag"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=200":""} </s:else> tdid="3" ref="cspFlag"   title="产品标识">
			 ...
			 <td <s:if test="#request.defines['cspUpdateTime']>0">colspan="${defines["cspUpdateTime"]}" ${all?"width":"iwidth"}="${defines["cspUpdateTime"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=140":""} </s:else> tdid="4" ref="cspUpdateTime"   title="修改时间">
			 ...
			 <td <s:if test="#request.defines['cspAddTime']>0">colspan="${defines["cspAddTime"]}" ${all?"width":"iwidth"}="${defines["cspAddTime"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=140":""} </s:else> tdid="5" ref="cspAddTime"   title="添加时间">
			 ...
			 <s:if test="#request.defines==null && #request.CTRL.l.cspStatus || #request.defines['cspStatus']!=null">
			 <td <s:if test="#request.defines['cspStatus']>0">colspan="${defines["cspStatus"]}" ${all?"width":"iwidth"}="${defines["cspStatus"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=120":""} </s:else> tdid="6" ref="cspStatus"   title="状态">
			...
			<td rowspan="2" width="105" class="operation" tdid="8" ref="operation">
				<s:if test="#request.CTRL.canAdd==true">
					<a href="javascript:AddProduct()" style="text-decoration:underline">添加</a>
				</s:if>
				<s:else>&nbsp;</s:else>
			</td>
		  </tr>
		  ...
		  <tr id="${item.cspId}" >
		   ...
		   ${lz:set("注释","*****************数据列表数据部分结束*****************")}		  
			...
			<s:if test="#request.alias=='select'">
				<a href="javascript:SelectProduct('${item.cspId}')">选择</a>
			</s:if>
			<s:if test="#request.alias==null">
				<s:if test="#request.CTRL.canEdit==true">
				<a href="javascript:EditProduct('${item.cspId}')">修改</a></s:if>
				<s:if test="#request.CTRL.canDel==true">
				<a href="javascript:DelProduct('${item.cspId}','${item.cspName}')">删除</a></s:if>
			</s:if>
			<s:if test="#request.CTRL.canView==true">
			<a href="javascript:DetailsProduct('${item.cspId}')">详情</a></s:if>
			</td>
		  </tr>
...
<s:if test="#request.CTRL.canAdd==true">
	...
	**/
	function AddProduct(parent){		
		var url = "${basePath}${namespace}product_edit.do?edittype=save";
		var params = {entrypoint:"${entrypoint}",parent:(parent?parent:""),ctrl:${ctrl==null?"{title:'添加产品'}":lz:json(ctrl)}};
		href(url,params);
	}
</s:if>
<s:if test="#request.alias=='select'">
...
<s:if test="#request.CTRL.canEdit==true">
	...
	**/
	function EditProduct(id){
		var url = "${basePath}${namespace}product_edit.do?edittype=update";
		var params = {entrypoint:"${entrypoint}",id:id,ctrl:${ctrl==null?"{title:'修改产品'}":lz:json(ctrl)}};
		href(url,params);
	}
	/*
	* 修改所选
	...
	function UpdateSel(flag)
	{
		...
		if(checker.length==0){
			...
			$.alert("确定要对当前查询的${objects.count}条记录进行批量修改吗?",function(){
				var url = "${basePath}${namespace}product_edit.do";
				var params = {entrypoint:"${entrypoint}",method:flag,PARAMS:${lz:json(PARAMS)},ctrl:{title:"批量更新产品",visible:false,editable:false,fields:{}}};
			...
			var url = "${basePath}${namespace}product_edit.do";
			var params = {entrypoint:"${entrypoint}",method:flag,ids:ids,ctrl:{title:"批量更新产品",visible:false,editable:false,fields:{}}};
			for(var o in fieldNames[flag])
				params.ctrl["fields"][o]={visible:true,editable:true};
			href(url,params);
		}
	...
	**/
	...
	{
		var url = "${basePath}${namespace}product_edit.do";
		var params = {entrypoint:"${entrypoint}",id:id,method:flag,ctrl:{title:"更新产品",visible:false,editable:false,fields:{}}};
		for(var o in fieldNames[flag])
			params.ctrl["fields"][o]={visible:true,editable:true};
		href(url,params);
	}
	...
	fieldNames["goods"]={};
	fieldNames["status"]={};
	fieldNames["goods"]["cspGoods"]=true;
	...
	$(function(){
		...
		$(".table tbody td.td").dblclick(function(){
			var url = "${basePath}${namespace}product_edit.do";
			var params = {entrypoint:"${entrypoint}",id:$(this).parents("tr:eq(0)").attr("id"),method:"any",ctrl:{title:"更新产品",visible:false,editable:false,fields:{}}};
...
<s:if test="#request.CTRL.canDel==true">
	...
	* 删除产品
	**/
	function DelProduct(id,flag)
	{
		var dialog=top.$.dialog({
				html:"<span style='white-space:nowrap;'>确定要删除产品["+flag+"]吗？</span>",
				title:"删除产品",
				modal: true,			
				onOk:function(){
					dialog.closeDialog();
					var url = "${basePath}${namespace}product_del.do";
					var params = {entrypoint:"${entrypoint}",id:id};
					$.submit(url,params);
				}
		 });	
	...
	*/
	function DelSel(){
		...
		var dialog=top.$.dialog({
				html:"<span style='white-space:nowrap;'>确定要删除所选产品吗？</span>",
				title:"批量删除产品",
				modal: true,			
				onOk:function(){
					var ids = "";
	...
	**/
	...
	{
		var url = "${basePath}${namespace}product_details.do";
		var params = {entrypoint:"${entrypoint}",id:id,ctrl:{title:"产品详情",editable:false,visible:true}};
		href(url,params);
	}
	/*
	* 显示所选记录详情
