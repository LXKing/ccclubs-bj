...
<lz:DefaultCtrl>{
	...
	queryables:"cswmId,cswmHost,cswmMember,cswmContent,cswmAddTime,cswmMask,cswmStatus",
	${lz:set("注释","当用户选择显示全部字段时，哪些字段可显示在表格中")}
	listables:"cswmId,cswmHost,cswmPlatform,cswmOpenid,cswmMember,cswmMsgType,cswmContentType,cswmContent,cswmAddTime,cswmMask,cswmStatus",
	</s:if>
	<s:else>
	${lz:set("注释","当用户选择显示部分字段时，哪些字段可查询")}
	queryables:"cswmHost,cswmMember,cswmContent,cswmAddTime,cswmStatus",
	${lz:set("注释","当用户选择显示部分字段时，哪些字段可显示在表格中")}
	listables:"cswmHost,cswmPlatform,cswmOpenid,cswmMember,cswmMsgType,cswmContentType,cswmContent,cswmAddTime,cswmStatus",
	</s:else>
}</lz:DefaultCtrl>
${lz:set("注释","***************************************************")}
...
<div class="title">
	...
	<span class="caption">${empty title?"微信上行管理":title}</span>
</div>
  	</s:if>
...
<div class="plate" style="${((hideQuery==true && window.imize!="maximize")||(hideQuery!=true && window.imize=="minimize")) ? "display:none;":""}">
	<form ref="CsWxMo" class="query" id="queryForm" action="${basePath}${lz:replace(servletPath,"^/","")}?${lz:querys("UTF-8","page")}" method="post">
		${lz:set("haveQuery",false)}
		
		${lz:set("注释","****导入查询表单各字段****")}
		<%@include file="wxmo.query.jsp"%>
	...
	<div class="content">
	    <div class="toolbar">
	      ...
	      <s:if test="#request.alias==null">
			...
			<button type="button" class="add" onclick="AddWxmo();">
				添加
			</button>							
			</s:if>
		  </s:if>
		  ...
		  <s:iterator value="#request.items" id="handle" status="i">
			...
			<dl ref="com.ccclubs.model.CsWxMo" class="display dropdowan" style="margin-left:30px;">
				<dt>
			...
			<dl ref="com.ccclubs.model.CsWxMo" class="batch dropdowan">
				<dt>
		...
		${lz:set("注释","*****************数据列表开始*****************")}
		<table id="wxmo_table" ref="CsWxMo" class="table" width="100%" border="0" cellspacing="1" cellpadding="0">
		<thead class="state-header">
		  ...
		  ${lz:set("注释","****数据列表列头开始****")}
			 ...
			 <s:if test="#request.defines==null && #request.CTRL.l.cswmMsgType || #request.defines['cswmMsgType']!=null">
			 <td <s:if test="#request.defines['cswmMsgType']>0">colspan="${defines["cswmMsgType"]}" ${all?"width":"iwidth"}="${defines["cswmMsgType"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=120":""} </s:else> tdid="6" ref="cswmMsgType"   title="消息类型">
			 	<a class="${desc=="cswm_msg_type" ? "desc" : ""}${asc=="cswm_msg_type" ? "asc" : ""}" href="?${desc=="cswm_msg_type" ? "asc=cswm_msg_type" : ""}${(asc=="cswm_msg_type" || desc!="cswm_msg_type" )? "desc=cswm_msg_type" : ""}&${lz:queryss("UTF-8","desc","asc")}">
			 	消息类型
			 	</a>
			 	${lz:set("checkeds[]","cswmMsgType")}
			 </td>
			 </s:if>
			 ...
			 <td <s:if test="#request.defines['cswmContent']>0">colspan="${defines["cswmContent"]}" ${all?"width":"iwidth"}="${defines["cswmContent"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=200":""} </s:else> tdid="8" ref="cswmContent"   title="消息内容">
			 ...
			 <td <s:if test="#request.defines['cswmAddTime']>0">colspan="${defines["cswmAddTime"]}" ${all?"width":"iwidth"}="${defines["cswmAddTime"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=140":""} </s:else> tdid="9" ref="cswmAddTime"   title="添加时间">
			 ...
			 <td <s:if test="#request.defines['cswmMask']>0">colspan="${defines["cswmMask"]}" ${all?"width":"iwidth"}="${defines["cswmMask"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=120":""} </s:else> tdid="10" ref="cswmMask"   title="程序掩码">
			 ...
			 <s:if test="#request.defines==null && #request.CTRL.l.cswmStatus || #request.defines['cswmStatus']!=null">
			 <td <s:if test="#request.defines['cswmStatus']>0">colspan="${defines["cswmStatus"]}" ${all?"width":"iwidth"}="${defines["cswmStatus"]*100}" </s:if><s:else>rowspan="2"  ${all?"width=120":""} </s:else> tdid="11" ref="cswmStatus"   title="状态">
			...
			<td rowspan="2" width="105" class="operation" tdid="13" ref="operation">
				<s:if test="#request.CTRL.canAdd==true">
					<a href="javascript:AddWxmo()" style="text-decoration:underline">添加</a>
				</s:if>
				<s:else>&nbsp;</s:else>
			</td>
		  </tr>
		 ...
		 <tbody>
		  ...
		  <tr id="${item.cswmId}" >
			...
			<s:if test="#request.defines==null && #request.CTRL.l.cswmMsgType || #request.defines['cswmMsgType']!=null">
				<s:if test="#request.defines['cswmMsgType']>0">
					${lz:set("注释","****cswmMsgType关联表的子级字段：如果用户勾选了要显示的话****")}
				  	<s:iterator value="#request.childes" id="fieldName" status="j">
				  		${lz:set("isList",lz:indexOf(fieldName,"[]")>-1)}
				 		<s:if test="#request.isList==true">
				 			${lz:set("sizeList",lz:size(item.cswmMsgType))}
				 			${lz:set("rowspan",rowspan>sizeList?rowspan:sizeList)}
				 		</s:if>
				  		${lz:set("atCswmMsgType",lz:indexOf(fieldName,"cswmMsgType")>-1)}
				  		<s:if test="#request.atCswmMsgType==true">
				 			<td ${isList?"class='onemore'":""}>${lz:left(lz:el(item,fieldName),100)}</td>
				 		</s:if>
				 	</s:iterator>
				</s:if>
				<s:else>
					${lz:set("注释","****cswmMsgType字段的字串格式化输出****")}
					<td ref="cswmMsgType" class="td ">
						 
						 	${lz:or(item$cswmMsgType[i.count-1],lz:left(item.cswmMsgType$,100))}
					</td>
				</s:else>
			</s:if>
		   ...
		   ${lz:set("注释","*****************数据列表数据部分结束*****************")}		  
			...
			<s:if test="#request.alias=='select'">
				<a href="javascript:SelectWxmo('${item.cswmId}')">选择</a>
			</s:if>
			<s:if test="#request.alias==null">
				<s:if test="#request.CTRL.canEdit==true">
				<a href="javascript:EditWxmo('${item.cswmId}')">修改</a></s:if>
				<s:if test="#request.CTRL.canDel==true">
				<a href="javascript:DelWxmo('${item.cswmId}','${item.cswmId}')">删除</a></s:if>
			</s:if>
			<s:if test="#request.CTRL.canView==true">
			<a href="javascript:DetailsWxmo('${item.cswmId}')">详情</a></s:if>
			</td>
		  </tr>
					...
					${lz:set("注释","****cswmMsgType关联表的子级字段：如果用户勾选了要显示的话****")}
				  	<s:iterator value="#request.childes" id="fieldName" status="j">
				  		...
				  		<s:if test="#request.atCswmMsgType==true">
				  			${lz:set("isList",lz:indexOf(fieldName,"[]")>-1)}
				  			<s:if test="#request.isList==true">
				  				<lz:set name="arrFieldName">cswmMsgType[${k.count}]${lz:split(fieldName,"\\[\\]")[1]}</lz:set>
				 				<td class="onemore">${lz:left(lz:el(item,arrFieldName),100)}</td>
				 			</s:if>
				 			<s:else>
								<s:if test="#k.count==1"><td class="blank" rowspan="${rowspan-1}">&nbsp;</td></s:if>
...
<s:if test="#request.CTRL.canAdd==true">
	...
	* 添加微信上行
	**/
	function AddWxmo(parent){		
		var url = "${basePath}${namespace}wxmo_edit.do?edittype=save";
		var params = {entrypoint:"${entrypoint}",parent:(parent?parent:""),ctrl:${ctrl==null?"{title:'添加微信上行'}":lz:json(ctrl)}};
		href(url,params);
	}
</s:if>
<s:if test="#request.alias=='select'">
...
<s:if test="#request.CTRL.canEdit==true">
	...
	* 修改微信上行
	**/
	function EditWxmo(id){
		var url = "${basePath}${namespace}wxmo_edit.do?edittype=update";
		var params = {entrypoint:"${entrypoint}",id:id,ctrl:${ctrl==null?"{title:'修改微信上行'}":lz:json(ctrl)}};
		href(url,params);
	}
	/*
	* 修改所选
	...
	function UpdateSel(flag)
	{
		...
		if(checker.length==0){
			...
			$.alert("确定要对当前查询的${objects.count}条记录进行批量修改吗?",function(){
				var url = "${basePath}${namespace}wxmo_edit.do";
				var params = {entrypoint:"${entrypoint}",method:flag,PARAMS:${lz:json(PARAMS)},ctrl:{title:"批量更新微信上行",visible:false,editable:false,fields:{}}};
			...
			var url = "${basePath}${namespace}wxmo_edit.do";
			var params = {entrypoint:"${entrypoint}",method:flag,ids:ids,ctrl:{title:"批量更新微信上行",visible:false,editable:false,fields:{}}};
			for(var o in fieldNames[flag])
				params.ctrl["fields"][o]={visible:true,editable:true};
			href(url,params);
		}
	...
	**/
	...
	{
		var url = "${basePath}${namespace}wxmo_edit.do";
		var params = {entrypoint:"${entrypoint}",id:id,method:flag,ctrl:{title:"更新微信上行",visible:false,editable:false,fields:{}}};
		for(var o in fieldNames[flag])
			params.ctrl["fields"][o]={visible:true,editable:true};
		href(url,params);
	}
	...
	$(function(){
		...
		$(".table tbody td.td").dblclick(function(){
			var url = "${basePath}${namespace}wxmo_edit.do";
			var params = {entrypoint:"${entrypoint}",id:$(this).parents("tr:eq(0)").attr("id"),method:"any",ctrl:{title:"更新微信上行",visible:false,editable:false,fields:{}}};
...
<s:if test="#request.CTRL.canDel==true">
	...
	* 删除微信上行
	**/
	function DelWxmo(id,flag)
	{
		var dialog=top.$.dialog({
				html:"<span style='white-space:nowrap;'>确定要删除微信上行["+flag+"]吗？</span>",
				title:"删除微信上行",
				modal: true,			
				onOk:function(){
					dialog.closeDialog();
					var url = "${basePath}${namespace}wxmo_del.do";
					var params = {entrypoint:"${entrypoint}",id:id};
					$.submit(url,params);
				}
		 });	
	...
	*/
	function DelSel(){
		...
		var dialog=top.$.dialog({
				html:"<span style='white-space:nowrap;'>确定要删除所选微信上行吗？</span>",
				title:"批量删除微信上行",
				modal: true,			
				onOk:function(){
					var ids = "";
	...
	* 显示微信上行详情
	**/
	function DetailsWxmo(id)
	{
		var url = "${basePath}${namespace}wxmo_details.do";
		var params = {entrypoint:"${entrypoint}",id:id,ctrl:{title:"微信上行详情",editable:false,visible:true}};
		href(url,params);
	}
	/*
	* 显示所选记录详情
