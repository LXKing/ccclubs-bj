...
public class CommonUtilService implements ICommonUtilService {
     ...
     * @param params 参数列表
     */
    public Boolean sendTemplateSms(final Long host, final String flag, final String mobile,
            final String defaultTemplate, final SMSType smsType, final Map<String, String> params) {

        new Thread() {
            public void run() {
                CsSmsTemplate csSmsTemplate =
                        CsSmsTemplate.where().csstFlag(flag).csstHost(host).get();
                if (csSmsTemplate == null)
                    csSmsTemplate = CsSmsTemplate.where().csstFlag(flag).csstHost(0l).get();
                if (csSmsTemplate == null) {
                    csSmsTemplate = csSmsTemplateService.saveCsSmsTemplate(new CsSmsTemplate()
                            .csstFlag(flag).setNotNull(0l, "未命名模版", defaultTemplate,
                                    (long) smsType.ordinal(), new Date(), new Date(), (short) 1));
                }
                String csstContent = csSmsTemplate.getCsstContent();
                for (Map.Entry<String, String> entry : params.entrySet()) {
                    csstContent = csstContent.replace("{" + entry.getKey() + "}",
                            entry.getValue() == null ? "" : entry.getValue());
                }
                csSmsMtService.saveCsSmsMt(new CsSmsMt().cssmHost(host == null ? 0l : host)
                        .cssmAddTime(new Date()).cssmUpdateTime(new Date()).cssmContent(csstContent)
                        .cssmType(csSmsTemplate.getCsstType().shortValue())
                        .cssmEditor(LoginHelper.getLoginId()).cssmMobile(mobile)
                        .cssmStatus((short) 1));
            }
        }.start();
     ...
     * @param content 短信内容
     */
    public void sendSms(final Long host, final String mobile, final String content,
            final SMSType smsType) {

        new Thread() {
            public void run() {
                csSmsMtService.saveCsSmsMt(
                        new CsSmsMt().cssmHost(host == null ? 0l : host).cssmAddTime(new Date())
                                .cssmContent(content).cssmType((short) (smsType.ordinal() + 1))
                                .cssmEditor(LoginHelper.getLoginId()).cssmMobile(mobile)
                                .cssmStatus((short) 1));
            }
        }.start();
        ...
        new Thread() {
            public void run() {
                ...
                if(StringUtils.isNotEmpty(append)) {
                    csstContent = csstContent.concat(append);
                }
