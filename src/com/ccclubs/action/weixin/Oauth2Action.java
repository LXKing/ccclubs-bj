...
public class Oauth2Action {
	...
	public String execute() {
		try {
			...
			String baseURL = $.config("weixin.domain");
	...
	public String weixin() {
		try {
			...
			if ($.empty(redirect)) {
				return $.Redirect($.config("weixin.domain") + "weixin/index.html?flag=" + flag);
			} else {
				// fix bug 早期是 index
				if (redirect.equals("index")) {
					...
					if (redirect.indexOf("?") > -1) {
						return $.Redirect($.config("weixin.domain") + redirect + "&flag=" + flag);
					} else {
						return $.Redirect($.config("weixin.domain") + redirect + "?flag=" + flag);
					}
				}
	...
	public String alipay() {
		...
		try {
			...
			if ($.empty(redirect)) {
				return $.Redirect($.config("weixin.domain") + "weixin/index.html");
			} else {
				if (redirect.equals("index")) {
					return $.Redirect($.config("weixin.domain") + "weixin/index.html");
				} else {
					if (redirect.indexOf("?") > -1) {
						return $.Redirect($.config("weixin.domain") + redirect);
					} else {
						return $.Redirect($.config("weixin.domain") + redirect);
					}
				}
			...
			e.printStackTrace();
			Logger.getRootLogger().error(e.getMessage(),e);
			return $.Redirect($.config("weixin.domain") + "weixin/403.html");
		}
	}
	...
	public String zhimaRedirect() {
		...
		if (redirect.equals("account_credit")) {// 我的账户
			return $.Redirect($.config("weixin.domain") + "weixin/" + redirect + ".html?state=" + $.getString("state", ""));
		}
		return $.Redirect($.config("weixin.domain") + "weixin/index.html");
	}
	...
	public String chuxing() {
		...
		if (!csMember.getCsmVDrive().toString().equals("1")) {
			return $.Redirect($.config("weixin.domain") + "weixin/account_setname.html");
		}
		// 是否授权过
		...
		if (orderlist != null && !orderlist.isEmpty()) {
			if (orderlist.size() == 1) {
				// 开关门页面
				return $.Redirect($.config("weixin.domain") + "weixin/account_remote.html?order=" + orderlist.get(0).getCsoId());
				// return $.Redirect($.GetRequest("basePath") +
				// "weixin/account_orderdetail.html?from=" + from + "&order=" +
				// orderlist.get(0).getCsoId());
			} else if (orderlist.size() > 1) {
				// 订单列表页面
				return $.Redirect($.config("weixin.domain") + "weixin/account_order.html");
			} else {
				return $.Redirect($.config("weixin.domain") + "weixin/reserve.html");
			}
		} else {
			return $.Redirect($.config("weixin.domain") + "weixin/reserve.html");
		}
