...
public class WeixinHelper {
	 ...
	 * @return
	 */
	public static short switchRegPlatform(String openid) {
		String from = getOpenId();
		if ($.empty(from)) {
			// $.SetTips("非微信入口，请使用微信链接");
			return -1;
		...
		try {
			...
			if (!isCorrectOpenId()) {
				...
				if (strAgent.toLowerCase().contains("micromessenger")) {
					...
					response.sendRedirect(fetchURL + java.net.URLEncoder.encode(baseURL + "weixin/oauth2_weixin.html?redirect=" + strDirect, "UTF-8") + "&state=STATE#wechat_redirect");
					...
					String authUrl = "https://openauth.alipay.com/oauth2/publicAppAuthorize.htm?app_id=" + AlipayServiceEnvConstants.APP_ID + "&auth_skip=false" + "&scope=auth_userinfo&redirect_uri="
							+ URLEncoder.encode($.GetRequest("basePath") + "weixin/oauth2_alipay.html?redirect=" + strDirect, "UTF-8");
					response.sendRedirect(authUrl);
				}
			} else {
				// 是否已经绑定到车纷享会员，跳登录页面
				response.sendRedirect($.GetRequest("basePath") + "weixin/login.html?redirect=" + strDirect);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * @param memberId
	 */
	public static void remoteController(long carId, String conType, long srvUserId,Long caseProperty) {
		ICsCarService carService = $.GetSpringBean("csCarService");
		CsCar csCar = carService.getCsCarById(carId);
		CsRemote csRemote = new CsRemote();
		csRemote.setCsrNumber(csCar.getCscNumber());
		csRemote.setCsrCar(csCar.getCscId());
		csRemote.setCsrEditor(srvUserId);
		csRemote.setCsrAddTime(new Date());
		csRemote.setCsrState((short) 0);
		csRemote.setCsrStatus((short) 0);
		csRemote.setCsrType(new Short(conType));
		csRemote.setCsrHost(csCar.getCscHost());
		csRemote.setCsrCase(caseProperty);
		csRemote.setCsrRemark("运维端, srvUserId:[" + srvUserId + "]执行远程操作");
	...
	public static String getAlipayZhimaToken(CsMember csMember) {
		...
		try {
			...
			if (response.getErrorCode() != null) {
				...
				if (response.getErrorCode().equals("40003")) {// 未征得用户授权
					// 重新授权
				...
				System.out.println("查询成功：授权令牌=" + token); // zhimaBcd0b9b6733b24740b37ec56907f4cX26
				// 更新信用卡
