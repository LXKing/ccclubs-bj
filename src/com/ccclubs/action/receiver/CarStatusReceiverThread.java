...
import com.ccclubs.util.mq.entity.OrderAmount;
 ...
 * 
 */
public class CarStatusReceiverThread extends Thread {
	...
	private final static int			CAR_STATUS_LENGTH_II			= 69;										// 汽油车
	// //
	// 7个CAN包数据
	private final static int			CAR_STATUS_LENGTH_III			= 181;										// 7个CAN包数据
	private final static int			CAR_STATUS_LENGTH_IV			= 72;										// 电动车
...
//			// 北京
//			// 远程操作的回复
//			mqttClient.subscribe("ser/car/bjh/rt_0/+/pri_0");
//			// 订单的回复
//			mqttClient.subscribe("ser/car/bjh/rt_0/+/pri_4");
	 ...
	 * @param carStatus
	 */
	public void correctionOrderStatus(ICarStatus carStatus) {
		try {
			if (carStatus.getOrderId() == 0) {
				return;
			}

			CsOrder order = csOrderService.getCsOrderById(carStatus.getOrderId());

			if (order != null) {
				int status = order.getCsoStatus();
				...
				// 众泰车机，需要发送订单消费信息 add at 2015-09-14 16:07
				if (order.getCsoDeviceVesion() == 2) {
					// 拆分订单
					OrderInfo orderinfo = commonOrderService.splitOrderDetails(order.getCsoPayMember(), order.getCsoCar(), order.getCsoStartTime(), order.getCsoFinishTime(), new Date(), order.getCsoFreehour(), carStatus.getCarElectric() * 1.0, carStatus.getCarFuel() * 1.0, order.getCsoFeeType(),
							order.getCsoInsureType(), order.getCsoLongPrice(), order.getCsoId());

					OrderAmount orderAmount = new OrderAmount(order.getCsoCarNumber().toUpperCase(), 0l, orderinfo.getPrice());
					// 发送初始化命令
					mqttClientPublish.publish(getTopic(carStatus.getCarNum().toUpperCase()), orderAmount.getBytes(), 0, false);

					writeLog(CarStatusReceiver.class.getResource("/").toURI().getPath() + "RemoteTemp/" + DateHelper.formatDate(new Date(), "yyyy-MM-dd") + ".txt", carStatus.getCarNum() + "  " + carStatus.getOrderId() + " 众泰车机 ，当前订单消费总金额：" + orderinfo.getPrice());
