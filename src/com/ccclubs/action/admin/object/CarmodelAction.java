...
public class CarmodelAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsCarModel.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsCarModel.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsCarModel.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsCarModel.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsCarModel.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsCarModel.C.get($.forMat($.getString("desc")));//降序字段
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/object/carmodel.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csCarModelService.getCsCarModelPage(page, size, finalParams);
					}
				}, CsCarModel.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csCarModelService.getCsCarModelPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csCarModelToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csCarModel这个对象为什么不用$.setRequest,因为csCarModel有get,set方法，由struts处理的
				csCarModel=csCarModelService.getCsCarModelById(id);
			}
			if(csCarModel==null)
				csCarModel = new CsCarModel();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csCarModel);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsCarModel object,CsCarModel old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsCarModel.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csCarModel.getCscmId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsCarModelToken = $.getString("csCarModelToken");
			final String sessionCsCarModelToken = $.getSession("csCarModelToken");
			if($.equals(requestCsCarModelToken,sessionCsCarModelToken)){
				$.removeSession("csCarModelToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csCarModelService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csCarModel.getCscmName()==null?"":csCarModel.getCscmName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csCarModel.setCscmName(String.valueOf(sItem));
							
								//设置默认值
								if(csCarModel.getCscmUpdateTime()==null)
								csCarModel.setCscmUpdateTime(new Date());
								if(csCarModel.getCscmAddTime()==null)
								csCarModel.setCscmAddTime(new Date());
								
								//根据自定义的默认值信息设置默认值
								if(CTRL!=null)
									CTRL.setObjectDefaultValue(csCarModel);
								
								csCarModel=csCarModelService.saveCsCarModel(csCarModel);			
								
								on("save",csCarModel,csCarModel);
							}							
							
							csCarModelService.updateCsCarModel$NotNull(csCarModel);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsCarModel.class,"add","添加了[车型]["+csCarModel.getCscmName()+"]",(Long)$.getSession("ccclubs_login_id"), csCarModel,csCarModel.getCscmId());
							$.SetTips("保存车型成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csCarModelService.updateCsCarModelByConfirm(ActionHelper.getSetValuesFromModel(csCarModel), params);
								LoggerHelper.writeLog(CsCarModel.class, "update", 
											"批量修改了[车型]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csCarModel,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csCarModel.setCscmId(Long.valueOf(sid));
									CsCarModel oldCsCarModel = CsCarModel.get(Long.valueOf(sid));
									csCarModelService.updateCsCarModel$NotNull(csCarModel);
									LoggerHelper.writeLog(CsCarModel.class, "update", 
											"修改了[车型]["+oldCsCarModel.getCscmName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsCarModel, csCarModel,false)
											,Long.valueOf(sid));
									on($.getString("method"),csCarModel,oldCsCarModel);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									CsCarModel oldCsCarModel = CsCarModel.get(csCarModel.getCscmId());
									csCarModelService.updateCsCarModel$NotNull(csCarModel);
									LoggerHelper.writeLog(CsCarModel.class, "update", 
											"修改了[车型]["+oldCsCarModel.getCscmName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsCarModel, csCarModel,false)
											,csCarModel.getCscmId());
									on($.getString("method"),csCarModel,oldCsCarModel);									
								}else{
									CsCarModel oldCsCarModel = csCarModelService.getCsCarModelById(csCarModel.getCscmId());
									//设置默认值
									csCarModel.setCscmUpdateTime(oldCsCarModel.getCscmUpdateTime());	
									csCarModel.setCscmAddTime(oldCsCarModel.getCscmAddTime());	
									
									//设置填充值
									
									//从oldCsCarModel同步csCarModel未set过的值
									csCarModel.mergeSet(oldCsCarModel);
											
									csCarModelService.updateCsCarModel(csCarModel);
									LoggerHelper.writeLog(CsCarModel.class, "update", 
											"修改了[车型]["+oldCsCarModel.getCscmName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsCarModel, csCarModel,true)
											,csCarModel.getCscmId());
									on("update",csCarModel,oldCsCarModel);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csCarModel)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("carmodel_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csCarModel.getCscmId());
				else
		...
		try{
			...
			csCarModelService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsCarModel csCarModel = csCarModelService.getCsCarModelById(id);
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csCarModelService.removeCsCarModelById(id);
							else
							csCarModelService.deleteCsCarModelById(id);			
							//记录操作日志
							if(csCarModel!=null)
								LoggerHelper.writeLog(CsCarModel.class,"del","删除了[车型]["+csCarModel.getCscmName()+"]",(Long)$.getSession("ccclubs_login_id"), csCarModel);
							on("delete",csCarModel,csCarModel);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csCarModel=csCarModelService.getCsCarModelById(id);
			if(key!=null && id==null)
				csCarModel=CsCarModel.Get($.add(CsCarModel.F.cscmId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsCarModel.class);
			...
			List<CsCarModel> list = csCarModelService.getCsCarModelPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsCarModel csCarModel:list){
				...
				map.put("text", $.js(csCarModel.getKeyValue()));
				if(bObject==true)
	...
	public CsCarModel getCsCarModel()
	{
		return csCarModel;
	}

	public void setCsCarModel(CsCarModel csCarModel)
	{
		this.csCarModel = csCarModel;
	}
}