...
public class PowerpileAction
{
	ICsPowerPileService csPowerPileService;
	
	CsPowerPile csPowerPile;
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsPowerPile.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsPowerPile.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsPowerPile.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsPowerPile.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsPowerPile.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsPowerPile.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","cspp_id");//那么，默认以主键降序，传给Dao层
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/object/powerpile.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csPowerPileService.getCsPowerPilePage(page, size, finalParams);
					}
				}, CsPowerPile.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csPowerPileService.getCsPowerPilePage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csPowerPileToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csPowerPile这个对象为什么不用$.setRequest,因为csPowerPile有get,set方法，由struts处理的
				csPowerPile=csPowerPileService.getCsPowerPileById(id);
				//确认域标识
				if(SystemHelper.testHost(csPowerPile.getCsppHost()).equals("0")){
					return $.SendHtml("<script>alert('对不起，你没有权限编辑该数据');</script>", "UTF-8");
				}
			}
			if(csPowerPile==null)
				csPowerPile = new CsPowerPile();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csPowerPile);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsPowerPile object,CsPowerPile old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsPowerPile.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csPowerPile.getCsppId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsPowerPileToken = $.getString("csPowerPileToken");
			final String sessionCsPowerPileToken = $.getSession("csPowerPileToken");
			if($.equals(requestCsPowerPileToken,sessionCsPowerPileToken)){
				$.removeSession("csPowerPileToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csPowerPileService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csPowerPile.getCsppNo()==null?"":csPowerPile.getCsppNo());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csPowerPile.setCsppNo(String.valueOf(sItem));
							
								//设置默认值
								if(csPowerPile.getCsppUpdateTime()==null)
									csPowerPile.setCsppUpdateTime(new Date());
								if(csPowerPile.getCsppAddTime()==null)
									csPowerPile.setCsppAddTime(new Date());
								...
								csPowerPile=csPowerPileService.saveCsPowerPile(csPowerPile);			
								
								on("save",csPowerPile,csPowerPile);
							}							
							
							csPowerPileService.updateCsPowerPile$NotNull(csPowerPile);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsPowerPile.class,"add","添加了[充电桩]["+csPowerPile.getCsppNo()+"]",(Long)$.getSession("ccclubs_login_id"), csPowerPile,csPowerPile.getCsppId());
							$.SetTips("保存充电桩成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csPowerPileService.updateCsPowerPileByConfirm(ActionHelper.getSetValuesFromModel(csPowerPile), params);
								LoggerHelper.writeLog(CsPowerPile.class, "update", 
											"批量修改了[充电桩]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csPowerPile,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csPowerPile.setCsppId(Long.valueOf(sid));
									CsPowerPile oldCsPowerPile = CsPowerPile.get(Long.valueOf(sid));
									csPowerPileService.updateCsPowerPile$NotNull(csPowerPile);
									LoggerHelper.writeLog(CsPowerPile.class, "update", 
											"修改了[充电桩]["+oldCsPowerPile.getCsppNo()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsPowerPile, csPowerPile,false)
											,Long.valueOf(sid));
									on($.getString("method"),csPowerPile,oldCsPowerPile);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsPowerPile oldCsPowerPile = CsPowerPile.get(csPowerPile.getCsppId());
									csPowerPileService.updateCsPowerPile$NotNull(csPowerPile);
									LoggerHelper.writeLog(CsPowerPile.class, "update", 
											"修改了[充电桩]["+oldCsPowerPile.getCsppNo()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsPowerPile, csPowerPile,false)
											,csPowerPile.getCsppId());
									on($.getString("method"),csPowerPile,oldCsPowerPile);									
								}else{
									CsPowerPile oldCsPowerPile = csPowerPileService.getCsPowerPileById(csPowerPile.getCsppId());
									//设置默认值
									csPowerPile.setCsppUpdateTime(oldCsPowerPile.getCsppUpdateTime());	
									csPowerPile.setCsppAddTime(oldCsPowerPile.getCsppAddTime());	
									...
									//从oldCsPowerPile同步csPowerPile未set过的值
									csPowerPile.mergeSet(oldCsPowerPile);
											
									csPowerPileService.updateCsPowerPile(csPowerPile);
									LoggerHelper.writeLog(CsPowerPile.class, "update", 
											"修改了[充电桩]["+oldCsPowerPile.getCsppNo()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsPowerPile, csPowerPile,true)
											,csPowerPile.getCsppId());
									on("update",csPowerPile,oldCsPowerPile);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csPowerPile)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("powerpile_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csPowerPile.getCsppId());
				else
		...
		try{
			...
			csPowerPileService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsPowerPile csPowerPile = csPowerPileService.getCsPowerPileById(id);
							//确认域标识
							if(SystemHelper.testHost(csPowerPile.getCsppHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csPowerPileService.removeCsPowerPileById(id);
							else
							csPowerPileService.deleteCsPowerPileById(id);			
							//记录操作日志
							if(csPowerPile!=null)
								LoggerHelper.writeLog(CsPowerPile.class,"del","删除了[充电桩]["+csPowerPile.getCsppNo()+"]",(Long)$.getSession("ccclubs_login_id"), csPowerPile);
							on("delete",csPowerPile,csPowerPile);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csPowerPile=csPowerPileService.getCsPowerPileById(id);
			if(key!=null && id==null)
				csPowerPile=CsPowerPile.Get($.add(CsPowerPile.F.csppId,key));
			//确认域标识
			if(SystemHelper.testHost(csPowerPile.getCsppHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsPowerPile.class);
			//强制限制域
			...
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				params.put("desc","cspp_id");//那么，默认以主键降序，传给Dao层
								
			String strTerm=$.getString("value");//智能搜索时的参数，一般都是主键后面那个字段
			if(!$.empty(strTerm)){
				if(strTerm.toLowerCase().startsWith("id:"))//如果查询条件以id:开头，则按ID查询
					params.put("csppId", strTerm.toLowerCase().replaceFirst("id:", ""));					
				else//按标识查询，模糊查询请带%
				{
					String strDefinex = "";
					strDefinex+=" or cspp_no like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsPowerPile> list = csPowerPileService.getCsPowerPilePage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsPowerPile csPowerPile:list){
				Map map = new HashMap();
				map.put("value",csPowerPile.getCsppId().toString());
				map.put("text", $.js(csPowerPile.getKeyValue()));
				if(bObject==true)
					map.put("object",csPowerPile);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public CsPowerPile getCsPowerPile()
	{
		return csPowerPile;
	}

	public void setCsPowerPile(CsPowerPile csPowerPile)
	{
		this.csPowerPile = csPowerPile;
	}
}