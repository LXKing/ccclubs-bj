...
public class CaronAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsCarOn.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsCarOn.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsCarOn.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsCarOn.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsCarOn.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsCarOn.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","csco_id");//那么，默认以主键降序，传给Dao层
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/object/car/caron.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csCarOnService.getCsCarOnPage(page, size, finalParams);
					}
				}, CsCarOn.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csCarOnService.getCsCarOnPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csCarOnToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csCarOn这个对象为什么不用$.setRequest,因为csCarOn有get,set方法，由struts处理的
				csCarOn=csCarOnService.getCsCarOnById(id);
				//确认域标识
				if(SystemHelper.testHost(csCarOn.getCscoHost()).equals("0")){
			...
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csCarOn);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsCarOn object,CsCarOn old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsCarOn.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csCarOn.getCscoId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsCarOnToken = $.getString("csCarOnToken");
			final String sessionCsCarOnToken = $.getSession("csCarOnToken");
			if($.equals(requestCsCarOnToken,sessionCsCarOnToken)){
				$.removeSession("csCarOnToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csCarOnService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csCarOn.getCscoCar()==null?"":csCarOn.getCscoCar());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csCarOn.setCscoCar(Long.valueOf(sItem));
							
								//设置默认值
								if(csCarOn.getCscoOnTime()==null)
									csCarOn.setCscoOnTime(new Date());
								if(csCarOn.getCscoOffTime()==null)
								...
								csCarOn=csCarOnService.saveCsCarOn(csCarOn);			
								
								on("save",csCarOn,csCarOn);
							}							
							
							csCarOnService.updateCsCarOn$NotNull(csCarOn);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsCarOn.class,"add","添加了[车辆上线记录]["+csCarOn.getCscoCar()+"]",(Long)$.getSession("ccclubs_login_id"), csCarOn,csCarOn.getCscoId());
							$.SetTips("保存车辆上线记录成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csCarOnService.updateCsCarOnByConfirm(ActionHelper.getSetValuesFromModel(csCarOn), params);
								LoggerHelper.writeLog(CsCarOn.class, "update", 
											"批量修改了[车辆上线记录]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csCarOn,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csCarOn.setCscoId(Long.valueOf(sid));
									CsCarOn oldCsCarOn = CsCarOn.get(Long.valueOf(sid));
									csCarOnService.updateCsCarOn$NotNull(csCarOn);
									LoggerHelper.writeLog(CsCarOn.class, "update", 
											"修改了[车辆上线记录]["+oldCsCarOn.getCscoCar()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsCarOn, csCarOn,false)
											,Long.valueOf(sid));
									on($.getString("method"),csCarOn,oldCsCarOn);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsCarOn oldCsCarOn = CsCarOn.get(csCarOn.getCscoId());
									csCarOnService.updateCsCarOn$NotNull(csCarOn);
									LoggerHelper.writeLog(CsCarOn.class, "update", 
											"修改了[车辆上线记录]["+oldCsCarOn.getCscoCar()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsCarOn, csCarOn,false)
									...
									CsCarOn oldCsCarOn = csCarOnService.getCsCarOnById(csCarOn.getCscoId());
									//设置默认值
									csCarOn.setCscoOnTime(oldCsCarOn.getCscoOnTime());	
									csCarOn.setCscoOffTime(oldCsCarOn.getCscoOffTime());	
									...
									//从oldCsCarOn同步csCarOn未set过的值
									csCarOn.mergeSet(oldCsCarOn);
											
									csCarOnService.updateCsCarOn(csCarOn);
									LoggerHelper.writeLog(CsCarOn.class, "update", 
											"修改了[车辆上线记录]["+oldCsCarOn.getCscoCar()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsCarOn, csCarOn,true)
											,csCarOn.getCscoId());
									on("update",csCarOn,oldCsCarOn);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csCarOn)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("caron_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csCarOn.getCscoId());
				else
		...
		try{
			...
			csCarOnService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsCarOn csCarOn = csCarOnService.getCsCarOnById(id);
							//确认域标识
							if(SystemHelper.testHost(csCarOn.getCscoHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csCarOnService.removeCsCarOnById(id);
							else
							csCarOnService.deleteCsCarOnById(id);			
							//记录操作日志
							if(csCarOn!=null)
								LoggerHelper.writeLog(CsCarOn.class,"del","删除了[车辆上线记录]["+csCarOn.getCscoCar()+"]",(Long)$.getSession("ccclubs_login_id"), csCarOn);
							on("delete",csCarOn,csCarOn);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csCarOn=csCarOnService.getCsCarOnById(id);
			if(key!=null && id==null)
				csCarOn=CsCarOn.Get($.add(CsCarOn.F.cscoId,key));
			//确认域标识
			if(SystemHelper.testHost(csCarOn.getCscoHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsCarOn.class);
			//强制限制域
			...
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				params.put("desc","csco_id");//那么，默认以主键降序，传给Dao层
								
			String strTerm=$.getString("value");//智能搜索时的参数，一般都是主键后面那个字段
			if(!$.empty(strTerm)){
				if(strTerm.toLowerCase().startsWith("id:"))//如果查询条件以id:开头，则按ID查询
					params.put("cscoId", strTerm.toLowerCase().replaceFirst("id:", ""));					
				else//按标识查询，模糊查询请带%
				{
					String strDefinex = "";
					strDefinex+=" or csco_car like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsCarOn> list = csCarOnService.getCsCarOnPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsCarOn csCarOn:list){
				Map map = new HashMap();
				map.put("value",csCarOn.getCscoId().toString());
				map.put("text", $.js(csCarOn.getKeyValue()));
				if(bObject==true)
					map.put("object",csCarOn);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public CsCarOn getCsCarOn()
	{
		return csCarOn;
	}

	public void setCsCarOn(CsCarOn csCarOn)
	{
		this.csCarOn = csCarOn;
	}
}