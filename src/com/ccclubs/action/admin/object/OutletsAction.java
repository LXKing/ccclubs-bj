...
public class OutletsAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsOutlets.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsOutlets.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsOutlets.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsOutlets.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsOutlets.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsOutlets.C.get($.forMat($.getString("desc")));//降序字段
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/object/outlets.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csOutletsService.getCsOutletsPage(page, size, finalParams);
					}
				}, CsOutlets.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csOutletsService.getCsOutletsPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csOutletsToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csOutlets这个对象为什么不用$.setRequest,因为csOutlets有get,set方法，由struts处理的
				csOutlets=csOutletsService.getCsOutletsById(id);
				String profile = csOutlets.getCsoProfile();
				if(profile!=null){
					if(profile.indexOf("descript")!=-1){
						JSONObject json = JSONObject.fromObject(profile);
						$.setRequest("descript", json.get("descript"));
						$.setRequest("range", $.or(json.get("range"), 0));
					}else{
						$.setRequest("descript", profile);
						$.setRequest("range", 0);
					}
				}
				
				//确认域标识
				if(SystemHelper.testHost(csOutlets.getCsoHost()).equals("0")){
					return $.SendHtml("<script>alert('对不起，你没有权限编辑该数据');</script>", "UTF-8");
				}
			}
			if(csOutlets==null)
				csOutlets = new CsOutlets();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csOutlets);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsOutlets object,CsOutlets old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsOutlets.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csOutlets.getCsoId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsOutletsToken = $.getString("csOutletsToken");
			final String sessionCsOutletsToken = $.getSession("csOutletsToken");
			final String range = $.getRequest("range");
			final String descript = $.getRequest("descript");
			String json = $.json($.add("range", range).add("descript", descript));
			csOutlets.setCsoProfile(json);
			if($.equals(requestCsOutletsToken,sessionCsOutletsToken)){
				$.removeSession("csOutletsToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csOutletsService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						/************LAZY3Q_BEFOR_SAVE************/
						/************LAZY3Q_BEFOR_SAVE************/
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csOutlets.getCsoName()==null?"":csOutlets.getCsoName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csOutlets.setCsoName(String.valueOf(sItem));
							
								//设置默认值
								if(csOutlets.getCsoCanA()==null)
									csOutlets.setCsoCanA(false);
								if(csOutlets.getCsoCanB()==null)
									csOutlets.setCsoCanB(false);
								if(csOutlets.getCsoCanReg()==null)
									csOutlets.setCsoCanReg(true);
								if(csOutlets.getCsoCanOrder()==null)
									csOutlets.setCsoCanOrder(true);
								if(csOutlets.getCsoPriority()==null)
									csOutlets.setCsoPriority(0);
								if(csOutlets.getCsoVoteS()==null)
									csOutlets.setCsoVoteS(0);
								if(csOutlets.getCsoUpdateTime()==null)
									csOutlets.setCsoUpdateTime(new Date());
								if(csOutlets.getCsoAddTime()==null)
								csOutlets.setCsoAddTime(new Date());
								...
								csOutlets=csOutletsService.saveCsOutlets(csOutlets);			
								
								on("save",csOutlets,csOutlets);
							}							
							
							csOutletsService.updateCsOutlets$NotNull(csOutlets);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsOutlets.class,"add","添加了[网点]["+csOutlets.getCsoName()+"]",(Long)$.getSession("ccclubs_login_id"), csOutlets,csOutlets.getCsoId());
							$.SetTips("保存网点成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csOutletsService.updateCsOutletsByConfirm(ActionHelper.getSetValuesFromModel(csOutlets), params);
								LoggerHelper.writeLog(CsOutlets.class, "update", 
											"批量修改了[网点]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csOutlets,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csOutlets.setCsoId(Long.valueOf(sid));
									CsOutlets oldCsOutlets = CsOutlets.get(Long.valueOf(sid));
									csOutletsService.updateCsOutlets$NotNull(csOutlets);
									LoggerHelper.writeLog(CsOutlets.class, "update", 
											"修改了[网点]["+oldCsOutlets.getCsoName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsOutlets, csOutlets,false)
											,Long.valueOf(sid));
									on($.getString("method"),csOutlets,oldCsOutlets);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsOutlets oldCsOutlets = CsOutlets.get(csOutlets.getCsoId());
									csOutletsService.updateCsOutlets$NotNull(csOutlets);
									LoggerHelper.writeLog(CsOutlets.class, "update", 
											"修改了[网点]["+oldCsOutlets.getCsoName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsOutlets, csOutlets,false)
									...
									CsOutlets oldCsOutlets = csOutletsService.getCsOutletsById(csOutlets.getCsoId());
									//设置默认值
									csOutlets.setCsoUpdateTime(oldCsOutlets.getCsoUpdateTime());	
									csOutlets.setCsoAddTime(oldCsOutlets.getCsoAddTime());	
									...
									//从oldCsOutlets同步csOutlets未set过的值
									csOutlets.mergeSet(oldCsOutlets);
											
									csOutletsService.updateCsOutlets(csOutlets);
									LoggerHelper.writeLog(CsOutlets.class, "update", 
											"修改了[网点]["+oldCsOutlets.getCsoName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsOutlets, csOutlets,true)
											,csOutlets.getCsoId());
									on("update",csOutlets,oldCsOutlets);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csOutlets)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("outlets_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csOutlets.getCsoId());
				else
		...
		try{
			...
			csOutletsService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsOutlets csOutlets = csOutletsService.getCsOutletsById(id);
							//确认域标识
							if(SystemHelper.testHost(csOutlets.getCsoHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csOutletsService.removeCsOutletsById(id);
							else
							csOutletsService.deleteCsOutletsById(id);			
							//记录操作日志
							if(csOutlets!=null)
								LoggerHelper.writeLog(CsOutlets.class,"del","删除了[网点]["+csOutlets.getCsoName()+"]",(Long)$.getSession("ccclubs_login_id"), csOutlets);
							on("delete",csOutlets,csOutlets);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csOutlets=csOutletsService.getCsOutletsById(id);
			if(key!=null && id==null)
				csOutlets=CsOutlets.Get($.add(CsOutlets.F.csoId,key));
			//确认域标识
			if(SystemHelper.testHost(csOutlets.getCsoHost()).equals("0")){
	...
	public String tree(){
		try{
			...
			if(parent.longValue()==0){
				LzMap params = $.add(CsArea.F.csaHost,SystemHelper.testHost($.getLong("host")));
				...
				List<CsOutlets> list = csOutletsService.getCsOutletsList($.add("csoArea", parent)
						.add("csoStatus", 1)
						.add("csoUsertype", $.getInteger("csoUsertype")), -1);
				List<Map> result = new java.util.ArrayList();
				for(CsOutlets csOutlets:list){
					Map map = new HashMap();
	...
	public String ab(){
		try{
			...
			com.ccclubs.model.CsCar csCar = com.ccclubs.model.CsCar.get(car);
			csOutlets = csCar.get$cscOutlets();
			if(!csOutlets.getCsoCanA()){//如果不支持借出，直接返回当前网点
				List<Map> result = new java.util.ArrayList();
				Map map = new HashMap();
				map.put("value", $.zerofill(csOutlets.getCsoId().toString(), 6));
				map.put("text", $.js(""+csOutlets.getCsoName()));
			...
			if(parent.longValue()==0){
				//获取支持B还的区域,把当前车辆所在的区域也加进来
				List<CsArea> list = CsArea.where().add(CsArea.F.csaHost,SystemHelper.testHost($.getLong("host"))).add("definex", "(csa_id="+csCar.getCscOutlets()+" or csa_id in (select cso_area from cs_outlets where cso_can_b=1 and cso_car_s < cso_position_s group by cso_area))").list(-1);
				String strAjax="";
				for(CsArea o:list){
					if(!strAjax.equals(""))
						strAjax+=",";
				...
				List<CsOutlets> list = CsOutlets.where().csoArea(parent).csoCanB(true).add("definex", "(cso_car_s < cso_position_s  or cso_id="+csCar.getCscOutlets()+")").list(-1);
				List<Map> result = new java.util.ArrayList();
				for(CsOutlets csOutlets:list){
					Map map = new HashMap();
					map.put("value", $.zerofill(csOutlets.getCsoId().toString(), 6));
	...
	public CsOutlets getCsOutlets()
	{
		return csOutlets;
	}

	public void setCsOutlets(CsOutlets csOutlets)
	{
		this.csOutlets = csOutlets;
	}
}