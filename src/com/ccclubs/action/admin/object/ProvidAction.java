...
public class ProvidAction
{
	...
	{	
		try{
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				$.export(1000, 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
	...
	{		
		try{
			...
			if(id!=null){
				...
				//确认域标识
				if(SystemHelper.testHost(csProvid.getCspHost()).equals("0")){
	...
	public void on(String type,CsProvid object,CsProvid old){
		try{
			...
			SystemHelper.executeActionScript(CsProvid.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && csProvid.getCspId()==null);
	...
	public String del()
	{
		try{
			...
			SystemHelper.getSrvLimit("admin/object/provid.do");
					/************LAZY3Q_CAN_DEL************/
					/************LAZY3Q_CAN_DEL************/
	...
	public String query(){
		try{
			...
			List<CsProvid> list = csProvidService.getCsProvidList(params, $.getInteger("size",10));
