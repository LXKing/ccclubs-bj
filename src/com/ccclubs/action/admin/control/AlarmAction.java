...
{
	ICsAlarmService csAlarmService;
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsAlarm.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsAlarm.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsAlarm.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsAlarm.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsAlarm.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsAlarm.C.get($.forMat($.getString("desc")));//降序字段
			...
			//反强制限制域
			if(objects.get("csaOrder")!=null && objects.get("csaOrderNot")==null){
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/control/alarm.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csAlarmService.getCsAlarmPage(page, size, finalParams);
					}
				}, CsAlarm.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csAlarmService.getCsAlarmPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csAlarmToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csAlarm这个对象为什么不用$.setRequest,因为csAlarm有get,set方法，由struts处理的
				csAlarm=csAlarmService.getCsAlarmById(id);
				//确认域标识
				if(SystemHelper.testHost(csAlarm.getCsaHost()).equals("0")){
			...
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csAlarm);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsAlarm object,CsAlarm old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsAlarm.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csAlarm.getCsaId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsAlarmToken = $.getString("csAlarmToken");
			final String sessionCsAlarmToken = $.getSession("csAlarmToken");
			if($.equals(requestCsAlarmToken,sessionCsAlarmToken)){
				$.removeSession("csAlarmToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csAlarmService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csAlarm.getCsaInfo()==null?"":csAlarm.getCsaInfo());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csAlarm.setCsaInfo(String.valueOf(sItem));
								...
								csAlarm=csAlarmService.saveCsAlarm(csAlarm);			
								
								on("save",csAlarm,csAlarm);
							}							
							
							csAlarmService.updateCsAlarm$NotNull(csAlarm);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsAlarm.class,"add","添加了[车辆警报]["+csAlarm.getCsaInfo()+"]",(Long)$.getSession("ccclubs_login_id"), csAlarm,csAlarm.getCsaId());
							$.SetTips("保存车辆警报成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csAlarmService.updateCsAlarmByConfirm(ActionHelper.getSetValuesFromModel(csAlarm), params);
								LoggerHelper.writeLog(CsAlarm.class, "update", 
											"批量修改了[车辆警报]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csAlarm,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csAlarm.setCsaId(Long.valueOf(sid));
									CsAlarm oldCsAlarm = CsAlarm.get(Long.valueOf(sid));
									csAlarmService.updateCsAlarm$NotNull(csAlarm);
									LoggerHelper.writeLog(CsAlarm.class, "update", 
											"修改了[车辆警报]["+oldCsAlarm.getCsaInfo()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsAlarm, csAlarm,false)
											,Long.valueOf(sid));
									on($.getString("method"),csAlarm,oldCsAlarm);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									CsAlarm oldCsAlarm = CsAlarm.get(csAlarm.getCsaId());
									csAlarmService.updateCsAlarm$NotNull(csAlarm);
									LoggerHelper.writeLog(CsAlarm.class, "update", 
											"修改了[车辆警报]["+oldCsAlarm.getCsaInfo()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsAlarm, csAlarm,false)
									...
									CsAlarm oldCsAlarm = csAlarmService.getCsAlarmById(csAlarm.getCsaId());
									//设置默认值
									...
									//从oldCsAlarm同步csAlarm未set过的值
									csAlarm.mergeSet(oldCsAlarm);
											
									csAlarmService.updateCsAlarm(csAlarm);
									LoggerHelper.writeLog(CsAlarm.class, "update", 
											"修改了[车辆警报]["+oldCsAlarm.getCsaInfo()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsAlarm, csAlarm,true)
											,csAlarm.getCsaId());
									on("update",csAlarm,oldCsAlarm);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csAlarm)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("alarm_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csAlarm.getCsaId());
				else
		...
		try{
			...
			csAlarmService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsAlarm csAlarm = csAlarmService.getCsAlarmById(id);
							//确认域标识
							if(SystemHelper.testHost(csAlarm.getCsaHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							csAlarmService.deleteCsAlarmById(id);			
							//记录操作日志
							if(csAlarm!=null)
								LoggerHelper.writeLog(CsAlarm.class,"del","删除了[车辆警报]["+csAlarm.getCsaInfo()+"]",(Long)$.getSession("ccclubs_login_id"), csAlarm);
							on("delete",csAlarm,csAlarm);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csAlarm=csAlarmService.getCsAlarmById(id);
			if(key!=null && id==null)
				csAlarm=CsAlarm.Get($.add(CsAlarm.F.csaId,key));
			//确认域标识
			if(SystemHelper.testHost(csAlarm.getCsaHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsAlarm.class);
			//强制限制域
			params.put("HOSTS",SystemHelper.testHost(null));
			
			//反强制限制域
			if(params.get("csaOrder")!=null && params.get("csaOrderNot")==null){
				params.remove("HOSTS");
			}
			...
			if(!$.empty(strTerm)){
				...
				{
					String strDefinex = "";
					strDefinex+=" or csa_info like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsAlarm> list = csAlarmService.getCsAlarmPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsAlarm csAlarm:list){
				Map map = new HashMap();
				map.put("value",csAlarm.getCsaId().toString());
				map.put("text", $.js(csAlarm.getKeyValue()));
				if(bObject==true)
					map.put("object",csAlarm);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public ICsAlarmService getCsAlarmService()
	{
		return csAlarmService;
	}

	public void setCsAlarmService(ICsAlarmService csAlarmService)
	{
		this.csAlarmService = csAlarmService;
	}

	public CsAlarm getCsAlarm()
	{
		return csAlarm;
	}

	public void setCsAlarm(CsAlarm csAlarm)
	{
		this.csAlarm = csAlarm;
	}
}