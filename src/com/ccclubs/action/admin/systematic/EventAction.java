...
{
	ICsEventService csEventService;
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsEvent.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsEvent.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsEvent.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsEvent.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsEvent.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsEvent.C.get($.forMat($.getString("desc")));//降序字段
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/systematic/event.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csEventService.getCsEventPage(page, size, finalParams);
					}
				}, CsEvent.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csEventService.getCsEventPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csEventToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csEvent这个对象为什么不用$.setRequest,因为csEvent有get,set方法，由struts处理的
				csEvent=csEventService.getCsEventById(id);
			}
			if(csEvent==null)
				csEvent = new CsEvent();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csEvent);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsEvent object,CsEvent old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsEvent.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csEvent.getCseId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsEventToken = $.getString("csEventToken");
			final String sessionCsEventToken = $.getSession("csEventToken");
			if($.equals(requestCsEventToken,sessionCsEventToken)){
				$.removeSession("csEventToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csEventService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csEvent.getCseContent()==null?"":csEvent.getCseContent());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csEvent.setCseContent(String.valueOf(sItem));
							
								//设置默认值
								if(csEvent.getCseUpdateTime()==null)
								csEvent.setCseUpdateTime(new Date());
								if(csEvent.getCseAddTime()==null)
								csEvent.setCseAddTime(new Date());
								
								//根据自定义的默认值信息设置默认值
								if(CTRL!=null)
									CTRL.setObjectDefaultValue(csEvent);
								
								csEvent=csEventService.saveCsEvent(csEvent);			
								
								on("save",csEvent,csEvent);
							}							
							
							csEventService.updateCsEvent$NotNull(csEvent);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsEvent.class,"add","添加了[系统事件]["+csEvent.getCseContent()+"]",(Long)$.getSession("ccclubs_login_id"), csEvent,csEvent.getCseId());
							$.SetTips("保存系统事件成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csEventService.updateCsEventByConfirm(ActionHelper.getSetValuesFromModel(csEvent), params);
								LoggerHelper.writeLog(CsEvent.class, "update", 
											"批量修改了[系统事件]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csEvent,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csEvent.setCseId(Long.valueOf(sid));
									CsEvent oldCsEvent = CsEvent.get(Long.valueOf(sid));
									csEventService.updateCsEvent$NotNull(csEvent);
									LoggerHelper.writeLog(CsEvent.class, "update", 
											"修改了[系统事件]["+oldCsEvent.getCseContent()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsEvent, csEvent,false)
											,Long.valueOf(sid));
									on($.getString("method"),csEvent,oldCsEvent);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									CsEvent oldCsEvent = CsEvent.get(csEvent.getCseId());
									csEventService.updateCsEvent$NotNull(csEvent);
									LoggerHelper.writeLog(CsEvent.class, "update", 
											"修改了[系统事件]["+oldCsEvent.getCseContent()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsEvent, csEvent,false)
									...
									CsEvent oldCsEvent = csEventService.getCsEventById(csEvent.getCseId());
									//设置默认值
									csEvent.setCseUpdateTime(oldCsEvent.getCseUpdateTime());	
									csEvent.setCseAddTime(oldCsEvent.getCseAddTime());	
									
									//设置填充值
									
									//从oldCsEvent同步csEvent未set过的值
									csEvent.mergeSet(oldCsEvent);
											
									csEventService.updateCsEvent(csEvent);
									LoggerHelper.writeLog(CsEvent.class, "update", 
											"修改了[系统事件]["+oldCsEvent.getCseContent()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsEvent, csEvent,true)
											,csEvent.getCseId());
									on("update",csEvent,oldCsEvent);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csEvent)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("event_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csEvent.getCseId());
				else
		...
		try{
			...
			csEventService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsEvent csEvent = csEventService.getCsEventById(id);
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csEventService.removeCsEventById(id);
							else
							csEventService.deleteCsEventById(id);			
							//记录操作日志
							if(csEvent!=null)
								LoggerHelper.writeLog(CsEvent.class,"del","删除了[系统事件]["+csEvent.getCseContent()+"]",(Long)$.getSession("ccclubs_login_id"), csEvent);
							on("delete",csEvent,csEvent);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csEvent=csEventService.getCsEventById(id);
			if(key!=null && id==null)
				csEvent=CsEvent.Get($.add(CsEvent.F.cseId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsEvent.class);
			...
			List<CsEvent> list = csEventService.getCsEventPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsEvent csEvent:list){
				Map map = new HashMap();
				map.put("value",csEvent.getCseId().toString());
				map.put("text", $.js(csEvent.getKeyValue()));
				if(bObject==true)
					map.put("object",csEvent);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public ICsEventService getCsEventService()
	{
		return csEventService;
	}

	public void setCsEventService(ICsEventService csEventService)
	{
		this.csEventService = csEventService;
	}
