...
public class SmsmtAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsSmsMt.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsSmsMt.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsSmsMt.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsSmsMt.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsSmsMt.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsSmsMt.C.get($.forMat($.getString("desc")));//降序字段
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/systematic/smsmt.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csSmsMtService.getCsSmsMtPage(page, size, finalParams);
					}
				}, CsSmsMt.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csSmsMtService.getCsSmsMtPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csSmsMtToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csSmsMt这个对象为什么不用$.setRequest,因为csSmsMt有get,set方法，由struts处理的
				csSmsMt=csSmsMtService.getCsSmsMtById(id);
				//确认域标识
				if(SystemHelper.testHost(csSmsMt.getCssmHost()).equals("0")){
			...
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csSmsMt);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsSmsMt object,CsSmsMt old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsSmsMt.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csSmsMt.getCssmId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsSmsMtToken = $.getString("csSmsMtToken");
			final String sessionCsSmsMtToken = $.getSession("csSmsMtToken");
			if($.equals(requestCsSmsMtToken,sessionCsSmsMtToken)){
				$.removeSession("csSmsMtToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csSmsMtService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csSmsMt.getCssmMobile()==null?"":csSmsMt.getCssmMobile());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csSmsMt.setCssmMobile(String.valueOf(sItem));
							
								//设置默认值
								if(csSmsMt.getCssmEditor()==null)
									csSmsMt.setCssmEditor(LoginHelper.getLoginId());
								if(csSmsMt.getCssmUpdateTime()==null)
									csSmsMt.setCssmUpdateTime(new Date());
								if(csSmsMt.getCssmAddTime()==null)
									csSmsMt.setCssmAddTime(new Date());
								if(csSmsMt.getCssmStatus()==null)
									csSmsMt.setCssmStatus((short)1);
								...
								csSmsMt=csSmsMtService.saveCsSmsMt(csSmsMt);			
								
								on("save",csSmsMt,csSmsMt);
							}							
							
							csSmsMtService.updateCsSmsMt$NotNull(csSmsMt);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsSmsMt.class,"add","添加了[短信发送]["+csSmsMt.getCssmMobile()+"]",(Long)$.getSession("ccclubs_login_id"), csSmsMt,csSmsMt.getCssmId());
							$.SetTips("保存短信发送成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csSmsMtService.updateCsSmsMtByConfirm(ActionHelper.getSetValuesFromModel(csSmsMt), params);
								LoggerHelper.writeLog(CsSmsMt.class, "update", 
											"批量修改了[短信发送]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csSmsMt,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csSmsMt.setCssmId(Long.valueOf(sid));
									CsSmsMt oldCsSmsMt = CsSmsMt.get(Long.valueOf(sid));
									csSmsMtService.updateCsSmsMt$NotNull(csSmsMt);
									LoggerHelper.writeLog(CsSmsMt.class, "update", 
											"修改了[短信发送]["+oldCsSmsMt.getCssmMobile()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsSmsMt, csSmsMt,false)
											,Long.valueOf(sid));
									on($.getString("method"),csSmsMt,oldCsSmsMt);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsSmsMt oldCsSmsMt = CsSmsMt.get(csSmsMt.getCssmId());
									csSmsMtService.updateCsSmsMt$NotNull(csSmsMt);
									LoggerHelper.writeLog(CsSmsMt.class, "update", 
											"修改了[短信发送]["+oldCsSmsMt.getCssmMobile()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsSmsMt, csSmsMt,false)
									...
									CsSmsMt oldCsSmsMt = csSmsMtService.getCsSmsMtById(csSmsMt.getCssmId());
									//设置默认值
									csSmsMt.setCssmEditor(oldCsSmsMt.getCssmEditor());	
									csSmsMt.setCssmChannel(oldCsSmsMt.getCssmChannel());	
									csSmsMt.setCssmUpdateTime(oldCsSmsMt.getCssmUpdateTime());	
									csSmsMt.setCssmAddTime(oldCsSmsMt.getCssmAddTime());	
									csSmsMt.setCssmStatus(oldCsSmsMt.getCssmStatus());	
									...
									//从oldCsSmsMt同步csSmsMt未set过的值
									csSmsMt.mergeSet(oldCsSmsMt);
											
									csSmsMtService.updateCsSmsMt(csSmsMt);
									LoggerHelper.writeLog(CsSmsMt.class, "update", 
											"修改了[短信发送]["+oldCsSmsMt.getCssmMobile()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsSmsMt, csSmsMt,true)
											,csSmsMt.getCssmId());
									on("update",csSmsMt,oldCsSmsMt);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csSmsMt)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("smsmt_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csSmsMt.getCssmId());
				else
		...
		try{
			...
			csSmsMtService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsSmsMt csSmsMt = csSmsMtService.getCsSmsMtById(id);
							//确认域标识
							if(SystemHelper.testHost(csSmsMt.getCssmHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csSmsMtService.removeCsSmsMtById(id);
							else
							csSmsMtService.deleteCsSmsMtById(id);			
							//记录操作日志
							if(csSmsMt!=null)
								LoggerHelper.writeLog(CsSmsMt.class,"del","删除了[短信发送]["+csSmsMt.getCssmMobile()+"]",(Long)$.getSession("ccclubs_login_id"), csSmsMt);
							on("delete",csSmsMt,csSmsMt);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csSmsMt=csSmsMtService.getCsSmsMtById(id);
			if(key!=null && id==null)
				csSmsMt=CsSmsMt.Get($.add(CsSmsMt.F.cssmId,key));
			//确认域标识
			if(SystemHelper.testHost(csSmsMt.getCssmHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsSmsMt.class);
			//强制限制域
			...
			if(!$.empty(strTerm)){
				...
				{
					String strDefinex = "";
					strDefinex+=" or cssm_mobile like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsSmsMt> list = csSmsMtService.getCsSmsMtPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsSmsMt csSmsMt:list){
				Map map = new HashMap();
				map.put("value",csSmsMt.getCssmId().toString());
				map.put("text", $.js(csSmsMt.getKeyValue()));
				if(bObject==true)
					map.put("object",csSmsMt);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public String remain(){
		try{	
			return $.SendAjax($.add("message",
				com.ccclubs.action.thread.SmsThread.remain()
			), "UTF-8");
		}catch(Exception ex){
			ex.printStackTrace();
	...
	public CsSmsMt getCsSmsMt()
	{
		return csSmsMt;
	}

	public void setCsSmsMt(CsSmsMt csSmsMt)
	{
		this.csSmsMt = csSmsMt;
	}
}