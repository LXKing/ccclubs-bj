...
{
	ISrvOauthService srvOauthService;
	...
	{	
		try{
			...
			$.setRequest("modelClass",SrvOauth.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(SrvOauth.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(SrvOauth.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(SrvOauth.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=SrvOauth.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=SrvOauth.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","so_id");//那么，默认以主键降序，传给Dao层
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/systematic/oauth.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return srvOauthService.getSrvOauthPage(page, size, finalParams);
					}
				}, SrvOauth.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = srvOauthService.getSrvOauthPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("srvOauthToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，srvOauth这个对象为什么不用$.setRequest,因为srvOauth有get,set方法，由struts处理的
				srvOauth=srvOauthService.getSrvOauthById(id);
			}
			if(srvOauth==null)
				srvOauth = new SrvOauth();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(srvOauth);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,SrvOauth object,SrvOauth old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(SrvOauth.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && srvOauth.getSoId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestSrvOauthToken = $.getString("srvOauthToken");
			final String sessionSrvOauthToken = $.getSession("srvOauthToken");
			if($.equals(requestSrvOauthToken,sessionSrvOauthToken)){
				$.removeSession("srvOauthToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				srvOauthService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(srvOauth.getSoToken()==null?"":srvOauth.getSoToken());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								srvOauth.setSoToken(String.valueOf(sItem));
							
								//设置默认值
								if(srvOauth.getSoUpdateTime()==null)
								...
								srvOauth=srvOauthService.saveSrvOauth(srvOauth);			
								
								on("save",srvOauth,srvOauth);
							}							
							
							srvOauthService.updateSrvOauth$NotNull(srvOauth);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(SrvOauth.class,"add","添加了[用户授权]["+srvOauth.getSoToken()+"]",(Long)$.getSession("ccclubs_login_id"), srvOauth,srvOauth.getSoId());
							$.SetTips("保存用户授权成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								srvOauthService.updateSrvOauthByConfirm(ActionHelper.getSetValuesFromModel(srvOauth), params);
								LoggerHelper.writeLog(SrvOauth.class, "update", 
											"批量修改了[用户授权]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,srvOauth,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									srvOauth.setSoId(Long.valueOf(sid));
									SrvOauth oldSrvOauth = SrvOauth.get(Long.valueOf(sid));
									srvOauthService.updateSrvOauth$NotNull(srvOauth);
									LoggerHelper.writeLog(SrvOauth.class, "update", 
											"修改了[用户授权]["+oldSrvOauth.getSoToken()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldSrvOauth, srvOauth,false)
											,Long.valueOf(sid));
									on($.getString("method"),srvOauth,oldSrvOauth);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									SrvOauth oldSrvOauth = SrvOauth.get(srvOauth.getSoId());
									srvOauthService.updateSrvOauth$NotNull(srvOauth);
									LoggerHelper.writeLog(SrvOauth.class, "update", 
											"修改了[用户授权]["+oldSrvOauth.getSoToken()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldSrvOauth, srvOauth,false)
									...
									SrvOauth oldSrvOauth = srvOauthService.getSrvOauthById(srvOauth.getSoId());
									//设置默认值
									srvOauth.setSoUpdateTime(oldSrvOauth.getSoUpdateTime());	
									
									//设置填充值
									if(srvOauth.getSoUpdateTime()==null)
										srvOauth.setSoUpdateTime(new Date());
									
									//从oldSrvOauth同步srvOauth未set过的值
									srvOauth.mergeSet(oldSrvOauth);
											
									srvOauthService.updateSrvOauth(srvOauth);
									LoggerHelper.writeLog(SrvOauth.class, "update", 
											"修改了[用户授权]["+oldSrvOauth.getSoToken()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldSrvOauth, srvOauth,true)
											,srvOauth.getSoId());
									on("update",srvOauth,oldSrvOauth);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(srvOauth)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("oauth_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+srvOauth.getSoId());
				else
		...
		try{
			...
			srvOauthService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							SrvOauth srvOauth = srvOauthService.getSrvOauthById(id);
							srvOauthService.deleteSrvOauthById(id);			
							//记录操作日志
							if(srvOauth!=null)
								LoggerHelper.writeLog(SrvOauth.class,"del","删除了[用户授权]["+srvOauth.getSoToken()+"]",(Long)$.getSession("ccclubs_login_id"), srvOauth);
							on("delete",srvOauth,srvOauth);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				srvOauth=srvOauthService.getSrvOauthById(id);
			if(key!=null && id==null)
				srvOauth=SrvOauth.Get($.add(SrvOauth.F.soId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(SrvOauth.class);
			...
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				params.put("desc","so_id");//那么，默认以主键降序，传给Dao层
			...
			List<SrvOauth> list = srvOauthService.getSrvOauthPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(SrvOauth srvOauth:list){
				Map map = new HashMap();
				map.put("value",srvOauth.getSoId().toString());
				map.put("text", $.js(srvOauth.getKeyValue()));
				if(bObject==true)
	...
	public ISrvOauthService getSrvOauthService()
	{
		return srvOauthService;
	}

	public void setSrvOauthService(ISrvOauthService srvOauthService)
	{
		this.srvOauthService = srvOauthService;
	}

	public SrvOauth getSrvOauth()
	{
		return srvOauth;
	}

	public void setSrvOauth(SrvOauth srvOauth)
	{
		this.srvOauth = srvOauth;
	}
}