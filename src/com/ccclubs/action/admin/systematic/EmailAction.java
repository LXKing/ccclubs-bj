...
{
	ICsEmailService csEmailService;
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsEmail.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsEmail.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsEmail.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsEmail.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsEmail.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsEmail.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","cse_id");//那么，默认以主键降序，传给Dao层
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/systematic/email.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csEmailService.getCsEmailPage(page, size, finalParams);
					}
				}, CsEmail.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csEmailService.getCsEmailPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csEmailToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csEmail这个对象为什么不用$.setRequest,因为csEmail有get,set方法，由struts处理的
				csEmail=csEmailService.getCsEmailById(id);
			}
			if(csEmail==null)
				csEmail = new CsEmail();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csEmail);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsEmail object,CsEmail old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsEmail.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csEmail.getCseId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsEmailToken = $.getString("csEmailToken");
			final String sessionCsEmailToken = $.getSession("csEmailToken");
			if($.equals(requestCsEmailToken,sessionCsEmailToken)){
				$.removeSession("csEmailToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csEmailService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csEmail.getCseId()==null?"":csEmail.getCseId());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csEmail.setCseId(Long.valueOf(sItem));
							
								//设置默认值
								if(csEmail.getCseSendTime()==null)
									csEmail.setCseSendTime(new Date());
								if(csEmail.getCseAddTime()==null)
									csEmail.setCseAddTime(new Date());
								
								//根据自定义的默认值信息设置默认值
								if(CTRL!=null)
									CTRL.setObjectDefaultValue(csEmail);
								
								csEmail=csEmailService.saveCsEmail(csEmail);			
								
								on("save",csEmail,csEmail);
							}							
							
							csEmailService.updateCsEmail$NotNull(csEmail);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsEmail.class,"add","添加了[邮件内容]["+csEmail.getCseId()+"]",(Long)$.getSession("ccclubs_login_id"), csEmail,csEmail.getCseId());
							$.SetTips("保存邮件内容成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csEmailService.updateCsEmailByConfirm(ActionHelper.getSetValuesFromModel(csEmail), params);
								LoggerHelper.writeLog(CsEmail.class, "update", 
											"批量修改了[邮件内容]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csEmail,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csEmail.setCseId(Long.valueOf(sid));
									CsEmail oldCsEmail = CsEmail.get(Long.valueOf(sid));
									csEmailService.updateCsEmail$NotNull(csEmail);
									LoggerHelper.writeLog(CsEmail.class, "update", 
											"修改了[邮件内容]["+oldCsEmail.getCseId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsEmail, csEmail,false)
											,Long.valueOf(sid));
									on($.getString("method"),csEmail,oldCsEmail);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsEmail oldCsEmail = CsEmail.get(csEmail.getCseId());
									csEmailService.updateCsEmail$NotNull(csEmail);
									LoggerHelper.writeLog(CsEmail.class, "update", 
											"修改了[邮件内容]["+oldCsEmail.getCseId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsEmail, csEmail,false)
									...
									CsEmail oldCsEmail = csEmailService.getCsEmailById(csEmail.getCseId());
									//设置默认值
									csEmail.setCseSendTime(oldCsEmail.getCseSendTime());	
									csEmail.setCseAddTime(oldCsEmail.getCseAddTime());	
									
									//设置填充值
									if(csEmail.getCseSendTime()==null)
										csEmail.setCseSendTime(new Date());
									
									//从oldCsEmail同步csEmail未set过的值
									csEmail.mergeSet(oldCsEmail);
											
									csEmailService.updateCsEmail(csEmail);
									LoggerHelper.writeLog(CsEmail.class, "update", 
											"修改了[邮件内容]["+oldCsEmail.getCseId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsEmail, csEmail,true)
											,csEmail.getCseId());
									on("update",csEmail,oldCsEmail);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csEmail)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("email_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csEmail.getCseId());
				else
		...
		try{
			...
			csEmailService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsEmail csEmail = csEmailService.getCsEmailById(id);
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csEmailService.removeCsEmailById(id);
							else
							csEmailService.deleteCsEmailById(id);			
							//记录操作日志
							if(csEmail!=null)
								LoggerHelper.writeLog(CsEmail.class,"del","删除了[邮件内容]["+csEmail.getCseId()+"]",(Long)$.getSession("ccclubs_login_id"), csEmail);
							on("delete",csEmail,csEmail);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csEmail=csEmailService.getCsEmailById(id);
			if(key!=null && id==null)
				csEmail=CsEmail.Get($.add(CsEmail.F.cseId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsEmail.class);
			...
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				params.put("desc","cse_id");//那么，默认以主键降序，传给Dao层
								
			String strTerm=$.getString("value");//智能搜索时的参数，一般都是主键后面那个字段
			if(!$.empty(strTerm)){
				if(strTerm.toLowerCase().startsWith("id:"))//如果查询条件以id:开头，则按ID查询
					params.put("cseId", strTerm.toLowerCase().replaceFirst("id:", ""));					
				else//按标识查询，模糊查询请带%
				{
					String strDefinex = "";
					strDefinex+=" or cse_id like '"+strTerm.replaceAll("'", "''")+"%'";
					strDefinex+=" or cse_to like '"+strTerm.replaceAll("'", "''")+"%'";
					strDefinex+=" or cse_content like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsEmail> list = csEmailService.getCsEmailPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsEmail csEmail:list){
				Map map = new HashMap();
				map.put("value",csEmail.getCseId().toString());
				map.put("text", $.js(csEmail.getKeyValue()));
				if(bObject==true)
					map.put("object",csEmail);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public ICsEmailService getCsEmailService()
	{
		return csEmailService;
	}

	public void setCsEmailService(ICsEmailService csEmailService)
	{
		this.csEmailService = csEmailService;
	}

	public CsEmail getCsEmail()
	{
		return csEmail;
	}

	public void setCsEmail(CsEmail csEmail)
	{
