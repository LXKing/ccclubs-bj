...
{
	ISrvApiLogService srvApiLogService;
	...
	{	
		try{
			...
			$.setRequest("modelClass",SrvApiLog.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(SrvApiLog.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(SrvApiLog.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(SrvApiLog.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=SrvApiLog.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=SrvApiLog.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","sal_id");//那么，默认以主键降序，传给Dao层
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/systematic/apilog.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return srvApiLogService.getSrvApiLogPage(page, size, finalParams);
					}
				}, SrvApiLog.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = srvApiLogService.getSrvApiLogPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("srvApiLogToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，srvApiLog这个对象为什么不用$.setRequest,因为srvApiLog有get,set方法，由struts处理的
				srvApiLog=srvApiLogService.getSrvApiLogById(id);
			}
			if(srvApiLog==null)
				srvApiLog = new SrvApiLog();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(srvApiLog);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,SrvApiLog object,SrvApiLog old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(SrvApiLog.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && srvApiLog.getSalId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestSrvApiLogToken = $.getString("srvApiLogToken");
			final String sessionSrvApiLogToken = $.getSession("srvApiLogToken");
			if($.equals(requestSrvApiLogToken,sessionSrvApiLogToken)){
				$.removeSession("srvApiLogToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				srvApiLogService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(srvApiLog.getSalId()==null?"":srvApiLog.getSalId());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								srvApiLog.setSalId(Long.valueOf(sItem));
							
								//设置默认值
								if(srvApiLog.getSalInvokeTime()==null)
									srvApiLog.setSalInvokeTime(new Date());
								
								//根据自定义的默认值信息设置默认值
								if(CTRL!=null)
									CTRL.setObjectDefaultValue(srvApiLog);
								
								srvApiLog=srvApiLogService.saveSrvApiLog(srvApiLog);			
								
								on("save",srvApiLog,srvApiLog);
							}							
							
							srvApiLogService.updateSrvApiLog$NotNull(srvApiLog);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(SrvApiLog.class,"add","添加了[API日志]["+srvApiLog.getSalId()+"]",(Long)$.getSession("ccclubs_login_id"), srvApiLog,srvApiLog.getSalId());
							$.SetTips("保存API日志成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								srvApiLogService.updateSrvApiLogByConfirm(ActionHelper.getSetValuesFromModel(srvApiLog), params);
								LoggerHelper.writeLog(SrvApiLog.class, "update", 
											"批量修改了[API日志]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,srvApiLog,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									srvApiLog.setSalId(Long.valueOf(sid));
									SrvApiLog oldSrvApiLog = SrvApiLog.get(Long.valueOf(sid));
									srvApiLogService.updateSrvApiLog$NotNull(srvApiLog);
									LoggerHelper.writeLog(SrvApiLog.class, "update", 
											"修改了[API日志]["+oldSrvApiLog.getSalId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldSrvApiLog, srvApiLog,false)
											,Long.valueOf(sid));
									on($.getString("method"),srvApiLog,oldSrvApiLog);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									SrvApiLog oldSrvApiLog = SrvApiLog.get(srvApiLog.getSalId());
									srvApiLogService.updateSrvApiLog$NotNull(srvApiLog);
									LoggerHelper.writeLog(SrvApiLog.class, "update", 
											"修改了[API日志]["+oldSrvApiLog.getSalId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldSrvApiLog, srvApiLog,false)
									...
									SrvApiLog oldSrvApiLog = srvApiLogService.getSrvApiLogById(srvApiLog.getSalId());
									//设置默认值
									srvApiLog.setSalInvokeTime(oldSrvApiLog.getSalInvokeTime());	
									
									//设置填充值
									
									//从oldSrvApiLog同步srvApiLog未set过的值
									srvApiLog.mergeSet(oldSrvApiLog);
											
									srvApiLogService.updateSrvApiLog(srvApiLog);
									LoggerHelper.writeLog(SrvApiLog.class, "update", 
											"修改了[API日志]["+oldSrvApiLog.getSalId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldSrvApiLog, srvApiLog,true)
											,srvApiLog.getSalId());
									on("update",srvApiLog,oldSrvApiLog);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(srvApiLog)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("apilog_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+srvApiLog.getSalId());
				else
		...
		try{
			...
			srvApiLogService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							SrvApiLog srvApiLog = srvApiLogService.getSrvApiLogById(id);
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								srvApiLogService.removeSrvApiLogById(id);
							else
							srvApiLogService.deleteSrvApiLogById(id);			
							//记录操作日志
							if(srvApiLog!=null)
								LoggerHelper.writeLog(SrvApiLog.class,"del","删除了[API日志]["+srvApiLog.getSalId()+"]",(Long)$.getSession("ccclubs_login_id"), srvApiLog);
							on("delete",srvApiLog,srvApiLog);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				srvApiLog=srvApiLogService.getSrvApiLogById(id);
			if(key!=null && id==null)
				srvApiLog=SrvApiLog.Get($.add(SrvApiLog.F.salId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(SrvApiLog.class);
			...
			List<SrvApiLog> list = srvApiLogService.getSrvApiLogPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(SrvApiLog srvApiLog:list){
				Map map = new HashMap();
				map.put("value",srvApiLog.getSalId().toString());
				map.put("text", $.js(srvApiLog.getKeyValue()));
				if(bObject==true)
	...
	public ISrvApiLogService getSrvApiLogService()
	{
		return srvApiLogService;
	}

	public void setSrvApiLogService(ISrvApiLogService srvApiLogService)
	{
		this.srvApiLogService = srvApiLogService;
	}

	public SrvApiLog getSrvApiLog()
	{
		return srvApiLog;
	}

	public void setSrvApiLog(SrvApiLog srvApiLog)
	{
		this.srvApiLog = srvApiLog;
	}
}