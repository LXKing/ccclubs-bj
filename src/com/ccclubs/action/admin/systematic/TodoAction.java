...
public class TodoAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsTodo.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsTodo.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsTodo.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsTodo.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsTodo.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsTodo.C.get($.forMat($.getString("desc")));//降序字段
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/systematic/todo.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csTodoService.getCsTodoPage(page, size, finalParams);
					}
				}, CsTodo.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csTodoService.getCsTodoPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csTodoToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csTodo这个对象为什么不用$.setRequest,因为csTodo有get,set方法，由struts处理的
				csTodo=csTodoService.getCsTodoById(id);
				//确认所有人
			}
			if(csTodo==null)
				csTodo = new CsTodo();
			...
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csTodo);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsTodo object,CsTodo old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsTodo.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csTodo.getCstId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsTodoToken = $.getString("csTodoToken");
			final String sessionCsTodoToken = $.getSession("csTodoToken");
			if($.equals(requestCsTodoToken,sessionCsTodoToken)){
				$.removeSession("csTodoToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csTodoService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csTodo.getCstTitle()==null?"":csTodo.getCstTitle());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csTodo.setCstTitle(String.valueOf(sItem));
							
								//设置默认值
								if(csTodo.getCstUser()==null)
									csTodo.setCstUser(LoginHelper.getLoginId());
								if(csTodo.getCstUpdateTime()==null)
									csTodo.setCstUpdateTime(new Date());
								if(csTodo.getCstAddTime()==null)
									csTodo.setCstAddTime(new Date());
								if(csTodo.getCstStatus()==null)
									csTodo.setCstStatus((short)0);
								
								//根据自定义的默认值信息设置默认值
								if(CTRL!=null)
									CTRL.setObjectDefaultValue(csTodo);
								
								csTodo=csTodoService.saveCsTodo(csTodo);			
								
								on("save",csTodo,csTodo);
							}							
							
							csTodoService.updateCsTodo$NotNull(csTodo);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsTodo.class,"add","添加了[待办事项]["+csTodo.getCstTitle()+"]",(Long)$.getSession("ccclubs_login_id"), csTodo,csTodo.getCstId());
							$.SetTips("保存待办事项成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csTodoService.updateCsTodoByConfirm(ActionHelper.getSetValuesFromModel(csTodo), params);
								LoggerHelper.writeLog(CsTodo.class, "update", 
											"批量修改了[待办事项]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csTodo,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csTodo.setCstId(Long.valueOf(sid));
									CsTodo oldCsTodo = CsTodo.get(Long.valueOf(sid));
									csTodoService.updateCsTodo$NotNull(csTodo);
									LoggerHelper.writeLog(CsTodo.class, "update", 
											"修改了[待办事项]["+oldCsTodo.getCstTitle()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsTodo, csTodo,false)
											,Long.valueOf(sid));
									on($.getString("method"),csTodo,oldCsTodo);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsTodo oldCsTodo = CsTodo.get(csTodo.getCstId());
									csTodoService.updateCsTodo$NotNull(csTodo);
									LoggerHelper.writeLog(CsTodo.class, "update", 
											"修改了[待办事项]["+oldCsTodo.getCstTitle()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsTodo, csTodo,false)
									...
									CsTodo oldCsTodo = csTodoService.getCsTodoById(csTodo.getCstId());
									//设置默认值
									csTodo.setCstUser(oldCsTodo.getCstUser());	
									csTodo.setCstUpdateTime(oldCsTodo.getCstUpdateTime());	
									csTodo.setCstAddTime(oldCsTodo.getCstAddTime());	
									
									//设置填充值
									if(csTodo.getCstAddTime()==null)
										csTodo.setCstAddTime(new Date());
									
									//从oldCsTodo同步csTodo未set过的值
									csTodo.mergeSet(oldCsTodo);
											
									csTodoService.updateCsTodo(csTodo);
									LoggerHelper.writeLog(CsTodo.class, "update", 
											"修改了[待办事项]["+oldCsTodo.getCstTitle()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsTodo, csTodo,true)
											,csTodo.getCstId());
									on("update",csTodo,oldCsTodo);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csTodo)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("todo_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csTodo.getCstId());
				else
		...
		try{
			...
			csTodoService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsTodo csTodo = csTodoService.getCsTodoById(id);
							//确认所有人
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csTodoService.removeCsTodoById(id);
							else
							csTodoService.deleteCsTodoById(id);			
							//记录操作日志
							if(csTodo!=null)
								LoggerHelper.writeLog(CsTodo.class,"del","删除了[待办事项]["+csTodo.getCstTitle()+"]",(Long)$.getSession("ccclubs_login_id"), csTodo);
							on("delete",csTodo,csTodo);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csTodo=csTodoService.getCsTodoById(id);
			if(key!=null && id==null)
				csTodo=CsTodo.Get($.add(CsTodo.F.cstId,key));
			//确认所有人
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsTodo.class);
			...
			if(!$.empty(strTerm)){
				...
				{
					String strDefinex = "";
					strDefinex+=" or cst_title like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsTodo> list = csTodoService.getCsTodoPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsTodo csTodo:list){
				Map map = new HashMap();
				map.put("value",csTodo.getCstId().toString());
				map.put("text", $.js(csTodo.getKeyValue()));
				if(bObject==true)
					map.put("object",csTodo);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public ICsTodoService getCsTodoService()
	{
		return csTodoService;
	}

	public void setCsTodoService(ICsTodoService csTodoService)
	{
		this.csTodoService = csTodoService;
	}

	public CsTodo getCsTodo()
	{
		return csTodo;
	}

	public void setCsTodo(CsTodo csTodo)
	{
		this.csTodo = csTodo;
	}
}