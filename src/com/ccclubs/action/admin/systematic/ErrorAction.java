...
{
	ICsErrorService csErrorService;
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsError.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsError.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsError.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsError.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsError.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsError.C.get($.forMat($.getString("desc")));//降序字段
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/systematic/error.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csErrorService.getCsErrorPage(page, size, finalParams);
					}
				}, CsError.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csErrorService.getCsErrorPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csErrorToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csError这个对象为什么不用$.setRequest,因为csError有get,set方法，由struts处理的
				csError=csErrorService.getCsErrorById(id);
			}
			if(csError==null)
				csError = new CsError();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csError);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsError object,CsError old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsError.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csError.getCseId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsErrorToken = $.getString("csErrorToken");
			final String sessionCsErrorToken = $.getSession("csErrorToken");
			if($.equals(requestCsErrorToken,sessionCsErrorToken)){
				$.removeSession("csErrorToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csErrorService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csError.getCseTitle()==null?"":csError.getCseTitle());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csError.setCseTitle(String.valueOf(sItem));
							
								//设置默认值
								if(csError.getCseUpdateTime()==null)
									csError.setCseUpdateTime(new Date());
								if(csError.getCseAddTime()==null)
									csError.setCseAddTime(new Date());
								
								//根据自定义的默认值信息设置默认值
								if(CTRL!=null)
									CTRL.setObjectDefaultValue(csError);
								
								csError=csErrorService.saveCsError(csError);			
								
								on("save",csError,csError);
							}							
							
							csErrorService.updateCsError$NotNull(csError);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsError.class,"add","添加了[错误信息]["+csError.getCseTitle()+"]",(Long)$.getSession("ccclubs_login_id"), csError,csError.getCseId());
							$.SetTips("保存错误信息成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csErrorService.updateCsErrorByConfirm(ActionHelper.getSetValuesFromModel(csError), params);
								LoggerHelper.writeLog(CsError.class, "update", 
											"批量修改了[错误信息]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csError,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csError.setCseId(Long.valueOf(sid));
									CsError oldCsError = CsError.get(Long.valueOf(sid));
									csErrorService.updateCsError$NotNull(csError);
									LoggerHelper.writeLog(CsError.class, "update", 
											"修改了[错误信息]["+oldCsError.getCseTitle()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsError, csError,false)
											,Long.valueOf(sid));
									on($.getString("method"),csError,oldCsError);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsError oldCsError = CsError.get(csError.getCseId());
									csErrorService.updateCsError$NotNull(csError);
									LoggerHelper.writeLog(CsError.class, "update", 
											"修改了[错误信息]["+oldCsError.getCseTitle()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsError, csError,false)
									...
									CsError oldCsError = csErrorService.getCsErrorById(csError.getCseId());
									//设置默认值
									csError.setCseUpdateTime(oldCsError.getCseUpdateTime());	
									csError.setCseAddTime(oldCsError.getCseAddTime());	
									
									//设置填充值
									if(csError.getCseAddTime()==null)
										csError.setCseAddTime(new Date());
									
									//从oldCsError同步csError未set过的值
									csError.mergeSet(oldCsError);
											
									csErrorService.updateCsError(csError);
									LoggerHelper.writeLog(CsError.class, "update", 
											"修改了[错误信息]["+oldCsError.getCseTitle()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsError, csError,true)
											,csError.getCseId());
									on("update",csError,oldCsError);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csError)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("error_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csError.getCseId());
				else
		...
		try{
			...
			csErrorService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsError csError = csErrorService.getCsErrorById(id);
							csErrorService.deleteCsErrorById(id);			
							//记录操作日志
							if(csError!=null)
								LoggerHelper.writeLog(CsError.class,"del","删除了[错误信息]["+csError.getCseTitle()+"]",(Long)$.getSession("ccclubs_login_id"), csError);
							on("delete",csError,csError);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csError=csErrorService.getCsErrorById(id);
			if(key!=null && id==null)
				csError=CsError.Get($.add(CsError.F.cseId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsError.class);
			...
			if(!$.empty(strTerm)){
				...
				{
					String strDefinex = "";
					strDefinex+=" or cse_title like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsError> list = csErrorService.getCsErrorPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsError csError:list){
				Map map = new HashMap();
				map.put("value",csError.getCseId().toString());
				map.put("text", $.js(csError.getKeyValue()));
				if(bObject==true)
					map.put("object",csError);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public ICsErrorService getCsErrorService()
	{
		return csErrorService;
	}

	public void setCsErrorService(ICsErrorService csErrorService)
	{
		this.csErrorService = csErrorService;
	}

	public CsError getCsError()
	{
		return csError;
	}

	public void setCsError(CsError csError)
	{
