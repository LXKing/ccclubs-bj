...
public class BugAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsBug.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsBug.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsBug.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsBug.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsBug.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsBug.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","csb_id");//那么，默认以主键降序，传给Dao层
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/systematic/bug.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csBugService.getCsBugPage(page, size, finalParams);
					}
				}, CsBug.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csBugService.getCsBugPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csBugToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csBug这个对象为什么不用$.setRequest,因为csBug有get,set方法，由struts处理的
				csBug=csBugService.getCsBugById(id);
			}
			if(csBug==null)
				csBug = new CsBug();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csBug);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsBug object,CsBug old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsBug.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csBug.getCsbId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsBugToken = $.getString("csBugToken");
			final String sessionCsBugToken = $.getSession("csBugToken");
			if($.equals(requestCsBugToken,sessionCsBugToken)){
				$.removeSession("csBugToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csBugService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csBug.getCsbTitle()==null?"":csBug.getCsbTitle());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csBug.setCsbTitle(String.valueOf(sItem));
							
								//设置默认值
								if(csBug.getCsbLevel()==null)
									csBug.setCsbLevel((short)0);
								if(csBug.getCsbTester()==null)
									csBug.setCsbTester(LoginHelper.getLoginId());
								if(csBug.getCsbUpdateTime()==null)
									csBug.setCsbUpdateTime(new Date());
								if(csBug.getCsbAddTime()==null)
									csBug.setCsbAddTime(new Date());
								if(csBug.getCsbStatus()==null)
									csBug.setCsbStatus((short)0);
								
								//根据自定义的默认值信息设置默认值
								if(CTRL!=null)
									CTRL.setObjectDefaultValue(csBug);
								
								csBug=csBugService.saveCsBug(csBug);			
								
								on("save",csBug,csBug);
							}							
							
							csBugService.updateCsBug$NotNull(csBug);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsBug.class,"add","添加了[系统BUG]["+csBug.getCsbTitle()+"]",(Long)$.getSession("ccclubs_login_id"), csBug,csBug.getCsbId());
							$.SetTips("保存系统BUG成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csBugService.updateCsBugByConfirm(ActionHelper.getSetValuesFromModel(csBug), params);
								LoggerHelper.writeLog(CsBug.class, "update", 
											"批量修改了[系统BUG]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csBug,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csBug.setCsbId(Long.valueOf(sid));
									CsBug oldCsBug = CsBug.get(Long.valueOf(sid));
									csBugService.updateCsBug$NotNull(csBug);
									LoggerHelper.writeLog(CsBug.class, "update", 
											"修改了[系统BUG]["+oldCsBug.getCsbTitle()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsBug, csBug,false)
											,Long.valueOf(sid));
									on($.getString("method"),csBug,oldCsBug);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsBug oldCsBug = CsBug.get(csBug.getCsbId());
									csBugService.updateCsBug$NotNull(csBug);
									LoggerHelper.writeLog(CsBug.class, "update", 
											"修改了[系统BUG]["+oldCsBug.getCsbTitle()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsBug, csBug,false)
									...
									CsBug oldCsBug = csBugService.getCsBugById(csBug.getCsbId());
									//设置默认值
									csBug.setCsbLevel(oldCsBug.getCsbLevel());	
									csBug.setCsbTester(oldCsBug.getCsbTester());	
									csBug.setCsbTecher(oldCsBug.getCsbTecher());	
									csBug.setCsbUpdateTime(oldCsBug.getCsbUpdateTime());	
									csBug.setCsbAddTime(oldCsBug.getCsbAddTime());	
									csBug.setCsbStatus(oldCsBug.getCsbStatus());	
									
									//设置填充值
									if(csBug.getCsbUpdateTime()==null)
										csBug.setCsbUpdateTime(new Date());
									
									//从oldCsBug同步csBug未set过的值
									csBug.mergeSet(oldCsBug);
											
									csBugService.updateCsBug(csBug);
									LoggerHelper.writeLog(CsBug.class, "update", 
											"修改了[系统BUG]["+oldCsBug.getCsbTitle()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsBug, csBug,true)
											,csBug.getCsbId());
									on("update",csBug,oldCsBug);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csBug)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("bug_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csBug.getCsbId());
				else
		...
		try{
			...
			csBugService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsBug csBug = csBugService.getCsBugById(id);
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csBugService.removeCsBugById(id);
							else
							csBugService.deleteCsBugById(id);			
							//记录操作日志
							if(csBug!=null)
								LoggerHelper.writeLog(CsBug.class,"del","删除了[系统BUG]["+csBug.getCsbTitle()+"]",(Long)$.getSession("ccclubs_login_id"), csBug);
							on("delete",csBug,csBug);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csBug=csBugService.getCsBugById(id);
			if(key!=null && id==null)
				csBug=CsBug.Get($.add(CsBug.F.csbId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsBug.class);
			...
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				params.put("desc","csb_id");//那么，默认以主键降序，传给Dao层
								
			String strTerm=$.getString("value");//智能搜索时的参数，一般都是主键后面那个字段
			if(!$.empty(strTerm)){
				if(strTerm.toLowerCase().startsWith("id:"))//如果查询条件以id:开头，则按ID查询
					params.put("csbId", strTerm.toLowerCase().replaceFirst("id:", ""));					
				else//按标识查询，模糊查询请带%
				{
					String strDefinex = "";
					strDefinex+=" or csb_title like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsBug> list = csBugService.getCsBugPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsBug csBug:list){
				Map map = new HashMap();
				map.put("value",csBug.getCsbId().toString());
				map.put("text", $.js(csBug.getKeyValue()));
				if(bObject==true)
					map.put("object",csBug);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public String dispose(){
		try{
			...
			csBugService.updateCsBug$NotNull(csBug.csbUpdateTime(new Date()));
	...
	public ICsBugService getCsBugService()
	{
		return csBugService;
	}

	public void setCsBugService(ICsBugService csBugService)
	{
		this.csBugService = csBugService;
	}

	public CsBug getCsBug()
	{
		return csBug;
	}

	public void setCsBug(CsBug csBug)
	{
		this.csBug = csBug;
	}
}