...
public class SrvlockAction
{
	ISrvLockService srvLockService;
	...
	{	
		try{
			...
			$.setRequest("modelClass",SrvLock.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(SrvLock.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(SrvLock.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(SrvLock.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=SrvLock.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=SrvLock.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","sl_id");//那么，默认以主键降序，传给Dao层
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/systematic/srvlock.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return srvLockService.getSrvLockPage(page, size, finalParams);
					}
				}, SrvLock.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = srvLockService.getSrvLockPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("srvLockToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，srvLock这个对象为什么不用$.setRequest,因为srvLock有get,set方法，由struts处理的
				srvLock=srvLockService.getSrvLockById(id);
			}
			if(srvLock==null)
				srvLock = new SrvLock();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(srvLock);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,SrvLock object,SrvLock old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(SrvLock.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && srvLock.getSlId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestSrvLockToken = $.getString("srvLockToken");
			final String sessionSrvLockToken = $.getSession("srvLockToken");
			if($.equals(requestSrvLockToken,sessionSrvLockToken)){
				$.removeSession("srvLockToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				srvLockService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(srvLock.getSlProfile()==null?"":srvLock.getSlProfile());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								srvLock.setSlProfile(String.valueOf(sItem));
								...
								srvLock=srvLockService.saveSrvLock(srvLock);			
								
								on("save",srvLock,srvLock);
							}							
							
							srvLockService.updateSrvLock$NotNull(srvLock);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(SrvLock.class,"add","添加了[事务锁]["+srvLock.getSlProfile()+"]",(Long)$.getSession("ccclubs_login_id"), srvLock,srvLock.getSlId());
							$.SetTips("保存事务锁成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								srvLockService.updateSrvLockByConfirm(ActionHelper.getSetValuesFromModel(srvLock), params);
								LoggerHelper.writeLog(SrvLock.class, "update", 
											"批量修改了[事务锁]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,srvLock,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									srvLock.setSlId(Long.valueOf(sid));
									SrvLock oldSrvLock = SrvLock.get(Long.valueOf(sid));
									srvLockService.updateSrvLock$NotNull(srvLock);
									LoggerHelper.writeLog(SrvLock.class, "update", 
											"修改了[事务锁]["+oldSrvLock.getSlProfile()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldSrvLock, srvLock,false)
											,Long.valueOf(sid));
									on($.getString("method"),srvLock,oldSrvLock);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									SrvLock oldSrvLock = SrvLock.get(srvLock.getSlId());
									srvLockService.updateSrvLock$NotNull(srvLock);
									LoggerHelper.writeLog(SrvLock.class, "update", 
											"修改了[事务锁]["+oldSrvLock.getSlProfile()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldSrvLock, srvLock,false)
									...
									SrvLock oldSrvLock = srvLockService.getSrvLockById(srvLock.getSlId());
									//设置默认值
									
									//设置填充值
									
									//从oldSrvLock同步srvLock未set过的值
									srvLock.mergeSet(oldSrvLock);
											
									srvLockService.updateSrvLock(srvLock);
									LoggerHelper.writeLog(SrvLock.class, "update", 
											"修改了[事务锁]["+oldSrvLock.getSlProfile()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldSrvLock, srvLock,true)
											,srvLock.getSlId());
									on("update",srvLock,oldSrvLock);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(srvLock)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("srvlock_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+srvLock.getSlId());
				else
		...
		try{
			...
			srvLockService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							SrvLock srvLock = srvLockService.getSrvLockById(id);
							srvLockService.deleteSrvLockById(id);			
							//记录操作日志
							if(srvLock!=null)
								LoggerHelper.writeLog(SrvLock.class,"del","删除了[事务锁]["+srvLock.getSlProfile()+"]",(Long)$.getSession("ccclubs_login_id"), srvLock);
							on("delete",srvLock,srvLock);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				srvLock=srvLockService.getSrvLockById(id);
			if(key!=null && id==null)
				srvLock=SrvLock.Get($.add(SrvLock.F.slId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(SrvLock.class);
			...
			if(!$.empty(strTerm)){
				...
				{
					String strDefinex = "";
					strDefinex+=" or sl_profile like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<SrvLock> list = srvLockService.getSrvLockPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(SrvLock srvLock:list){
				Map map = new HashMap();
				map.put("value",srvLock.getSlId().toString());
				map.put("text", $.js(srvLock.getKeyValue()));
				if(bObject==true)
					map.put("object",srvLock);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public ISrvLockService getSrvLockService()
	{
		return srvLockService;
	}

	public void setSrvLockService(ISrvLockService srvLockService)
	{
		this.srvLockService = srvLockService;
	}

	public SrvLock getSrvLock()
	{
		return srvLock;
	}

	public void setSrvLock(SrvLock srvLock)
	{
		this.srvLock = srvLock;
	}
}