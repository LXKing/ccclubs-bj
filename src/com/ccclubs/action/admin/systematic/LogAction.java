...
public class LogAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsLog.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsLog.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsLog.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsLog.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsLog.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsLog.C.get($.forMat($.getString("desc")));//降序字段
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/systematic/log.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csLogService.getCsLogPage(page, size, finalParams);
					}
				}, CsLog.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csLogService.getCsLogPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csLogToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csLog这个对象为什么不用$.setRequest,因为csLog有get,set方法，由struts处理的
				csLog=csLogService.getCsLogById(id);
				//确认域标识
				if(SystemHelper.testHost(csLog.getCslHost()).equals("0")){
			...
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csLog);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsLog object,CsLog old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsLog.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csLog.getCslId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsLogToken = $.getString("csLogToken");
			final String sessionCsLogToken = $.getSession("csLogToken");
			if($.equals(requestCsLogToken,sessionCsLogToken)){
				$.removeSession("csLogToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csLogService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csLog.getCslName()==null?"":csLog.getCslName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csLog.setCslName(String.valueOf(sItem));
							
								//设置默认值
								if(csLog.getCslUser()==null)
									csLog.setCslUser(LoginHelper.getLoginId());
								if(csLog.getCslAddTime()==null)
								csLog.setCslAddTime(new Date());
								...
								csLog=csLogService.saveCsLog(csLog);			
								
								on("save",csLog,csLog);
							}							
							
							csLogService.updateCsLog$NotNull(csLog);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsLog.class,"add","添加了[系统日志]["+csLog.getCslName()+"]",(Long)$.getSession("ccclubs_login_id"), csLog,csLog.getCslId());
							$.SetTips("保存系统日志成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csLogService.updateCsLogByConfirm(ActionHelper.getSetValuesFromModel(csLog), params);
								LoggerHelper.writeLog(CsLog.class, "update", 
											"批量修改了[系统日志]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csLog,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csLog.setCslId(Long.valueOf(sid));
									CsLog oldCsLog = CsLog.get(Long.valueOf(sid));
									csLogService.updateCsLog$NotNull(csLog);
									LoggerHelper.writeLog(CsLog.class, "update", 
											"修改了[系统日志]["+oldCsLog.getCslName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsLog, csLog,false)
											,Long.valueOf(sid));
									on($.getString("method"),csLog,oldCsLog);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsLog oldCsLog = CsLog.get(csLog.getCslId());
									csLogService.updateCsLog$NotNull(csLog);
									LoggerHelper.writeLog(CsLog.class, "update", 
											"修改了[系统日志]["+oldCsLog.getCslName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsLog, csLog,false)
									...
									CsLog oldCsLog = csLogService.getCsLogById(csLog.getCslId());
									//设置默认值
									csLog.setCslAddTime(oldCsLog.getCslAddTime());	
									...
									//从oldCsLog同步csLog未set过的值
									csLog.mergeSet(oldCsLog);
											
									csLogService.updateCsLog(csLog);
									LoggerHelper.writeLog(CsLog.class, "update", 
											"修改了[系统日志]["+oldCsLog.getCslName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsLog, csLog,true)
											,csLog.getCslId());
									on("update",csLog,oldCsLog);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csLog)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("log_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csLog.getCslId());
				else
		...
		try{
			...
			csLogService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsLog csLog = csLogService.getCsLogById(id);
							//确认域标识
							if(SystemHelper.testHost(csLog.getCslHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							csLogService.deleteCsLogById(id);			
							//记录操作日志
							if(csLog!=null)
								LoggerHelper.writeLog(CsLog.class,"del","删除了[系统日志]["+csLog.getCslName()+"]",(Long)$.getSession("ccclubs_login_id"), csLog);
							on("delete",csLog,csLog);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csLog=csLogService.getCsLogById(id);
			if(key!=null && id==null)
				csLog=CsLog.Get($.add(CsLog.F.cslId,key));
			//确认域标识
			if(SystemHelper.testHost(csLog.getCslHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsLog.class);
			//强制限制域
			...
			List<CsLog> list = csLogService.getCsLogPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsLog csLog:list){
				Map map = new HashMap();
				map.put("value",csLog.getCslId().toString());
				map.put("text", $.js(csLog.getKeyValue()));
				if(bObject==true)
					map.put("object",csLog);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public ICsLogService getCsLogService()
	{
		return csLogService;
	}

	public void setCsLogService(ICsLogService csLogService)
	{
		this.csLogService = csLogService;
	}
