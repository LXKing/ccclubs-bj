...
public class ChjsignAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsChjSign.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsChjSign.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsChjSign.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsChjSign.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsChjSign.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsChjSign.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","cscs_id");//那么，默认以主键降序，传给Dao层
			...
			//反强制限制域
			if(objects.get("cscsMember")!=null && objects.get("cscsMemberNot")==null){
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/operate/chjsign.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csChjSignService.getCsChjSignPage(page, size, finalParams);
					}
				}, CsChjSign.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csChjSignService.getCsChjSignPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csChjSignToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csChjSign这个对象为什么不用$.setRequest,因为csChjSign有get,set方法，由struts处理的
				csChjSign=csChjSignService.getCsChjSignById(id);
				//确认域标识
				if(SystemHelper.testHost(csChjSign.getCscsHost()).equals("0")){
			...
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csChjSign);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsChjSign object,CsChjSign old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsChjSign.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csChjSign.getCscsId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsChjSignToken = $.getString("csChjSignToken");
			final String sessionCsChjSignToken = $.getSession("csChjSignToken");
			if($.equals(requestCsChjSignToken,sessionCsChjSignToken)){
				$.removeSession("csChjSignToken");//移除令牌				
				...
				csChjSignService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csChjSign.getCscsName()==null?"":csChjSign.getCscsName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csChjSign.setCscsName(String.valueOf(sItem));
							
								//设置默认值
								if(csChjSign.getCscsUpdateTime()==null)
									csChjSign.setCscsUpdateTime(new Date());
								if(csChjSign.getCscsAddTime()==null)
								...
								csChjSign=csChjSignService.saveCsChjSign(csChjSign);			
								
								on("save",csChjSign,csChjSign);
							}							
							
							csChjSignService.updateCsChjSign$NotNull(csChjSign);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsChjSign.class,"add","添加了[春节套餐报名]["+csChjSign.getCscsName()+"]",(Long)$.getSession("ccclubs_login_id"), csChjSign,csChjSign.getCscsId());
							$.SetTips("保存春节套餐报名成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csChjSignService.updateCsChjSignByConfirm(ActionHelper.getSetValuesFromModel(csChjSign), params);
								LoggerHelper.writeLog(CsChjSign.class, "update", 
											"批量修改了[春节套餐报名]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csChjSign,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csChjSign.setCscsId(Long.valueOf(sid));
									CsChjSign oldCsChjSign = CsChjSign.get(Long.valueOf(sid));
									csChjSignService.updateCsChjSign$NotNull(csChjSign);
									LoggerHelper.writeLog(CsChjSign.class, "update", 
											"修改了[春节套餐报名]["+oldCsChjSign.getCscsName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsChjSign, csChjSign,false)
											,Long.valueOf(sid));
									on($.getString("method"),csChjSign,oldCsChjSign);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsChjSign oldCsChjSign = CsChjSign.get(csChjSign.getCscsId());
									csChjSignService.updateCsChjSign$NotNull(csChjSign);
									LoggerHelper.writeLog(CsChjSign.class, "update", 
											"修改了[春节套餐报名]["+oldCsChjSign.getCscsName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsChjSign, csChjSign,false)
									...
									CsChjSign oldCsChjSign = csChjSignService.getCsChjSignById(csChjSign.getCscsId());
									//设置默认值
									csChjSign.setCscsUpdateTime(oldCsChjSign.getCscsUpdateTime());	
									csChjSign.setCscsAddTime(oldCsChjSign.getCscsAddTime());	
									...
									//从oldCsChjSign同步csChjSign未set过的值
									csChjSign.mergeSet(oldCsChjSign);
											
									csChjSignService.updateCsChjSign(csChjSign);
									LoggerHelper.writeLog(CsChjSign.class, "update", 
											"修改了[春节套餐报名]["+oldCsChjSign.getCscsName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsChjSign, csChjSign,true)
											,csChjSign.getCscsId());
									on("update",csChjSign,oldCsChjSign);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csChjSign)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("chjsign_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csChjSign.getCscsId());
				else
		...
		try{
			...
			csChjSignService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsChjSign csChjSign = csChjSignService.getCsChjSignById(id);
							//确认域标识
							if(SystemHelper.testHost(csChjSign.getCscsHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csChjSignService.removeCsChjSignById(id);
							else
							csChjSignService.deleteCsChjSignById(id);			
							//记录操作日志
							if(csChjSign!=null)
								LoggerHelper.writeLog(CsChjSign.class,"del","删除了[春节套餐报名]["+csChjSign.getCscsName()+"]",(Long)$.getSession("ccclubs_login_id"), csChjSign);
							on("delete",csChjSign,csChjSign);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csChjSign=csChjSignService.getCsChjSignById(id);
			if(key!=null && id==null)
				csChjSign=CsChjSign.Get($.add(CsChjSign.F.cscsId,key));
			//确认域标识
			if(SystemHelper.testHost(csChjSign.getCscsHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsChjSign.class);
			//强制限制域
			params.put("HOSTS",SystemHelper.testHost(null));
			
			//反强制限制域
			if(params.get("cscsMember")!=null && params.get("cscsMemberNot")==null){
				params.remove("HOSTS");
			}
			...
			if(!$.empty(strTerm)){
				...
				{
					String strDefinex = "";
					strDefinex+=" or cscs_name like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsChjSign> list = csChjSignService.getCsChjSignPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsChjSign csChjSign:list){
				Map map = new HashMap();
				map.put("value",csChjSign.getCscsId().toString());
				map.put("text", $.js(csChjSign.getKeyValue()));
				if(bObject==true)
					map.put("object",csChjSign);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public CsChjSign getCsChjSign()
	{
		return csChjSign;
	}

	public void setCsChjSign(CsChjSign csChjSign)
	{
		this.csChjSign = csChjSign;
	}
}