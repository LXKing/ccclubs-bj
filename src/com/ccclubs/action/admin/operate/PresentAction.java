...
{
	ICsPresentService csPresentService;
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsPresent.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsPresent.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsPresent.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsPresent.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsPresent.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsPresent.C.get($.forMat($.getString("desc")));//降序字段
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/operate/present.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csPresentService.getCsPresentPage(page, size, finalParams);
					}
				}, CsPresent.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csPresentService.getCsPresentPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csPresentToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csPresent这个对象为什么不用$.setRequest,因为csPresent有get,set方法，由struts处理的
				csPresent=csPresentService.getCsPresentById(id);
				//确认域标识
				if(SystemHelper.testHost(csPresent.getCspHost()).equals("0")){
					return $.SendHtml("<script>alert('对不起，你没有权限编辑该数据');</script>", "UTF-8");
				}
			}
			if(csPresent==null)
				csPresent = new CsPresent();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csPresent);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsPresent object,CsPresent old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsPresent.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csPresent.getCspId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsPresentToken = $.getString("csPresentToken");
			final String sessionCsPresentToken = $.getSession("csPresentToken");
			if($.equals(requestCsPresentToken,sessionCsPresentToken)){
				$.removeSession("csPresentToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csPresentService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csPresent.getCspName()==null?"":csPresent.getCspName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csPresent.setCspName(String.valueOf(sItem));
							
								//设置默认值
								if(csPresent.getCspUpdateTime()==null)
									csPresent.setCspUpdateTime(new Date());
								if(csPresent.getCspAddTime()==null)
									csPresent.setCspAddTime(new Date());
								...
								csPresent=csPresentService.saveCsPresent(csPresent);			
								
								on("save",csPresent,csPresent);
							}							
							
							csPresentService.updateCsPresent$NotNull(csPresent);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsPresent.class,"add","添加了[礼品赠品]["+csPresent.getCspName()+"]",(Long)$.getSession("ccclubs_login_id"), csPresent,csPresent.getCspId());
							$.SetTips("保存礼品赠品成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csPresentService.updateCsPresentByConfirm(ActionHelper.getSetValuesFromModel(csPresent), params);
								LoggerHelper.writeLog(CsPresent.class, "update", 
											"批量修改了[礼品赠品]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csPresent,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csPresent.setCspId(Long.valueOf(sid));
									CsPresent oldCsPresent = CsPresent.get(Long.valueOf(sid));
									csPresentService.updateCsPresent$NotNull(csPresent);
									LoggerHelper.writeLog(CsPresent.class, "update", 
											"修改了[礼品赠品]["+oldCsPresent.getCspName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsPresent, csPresent,false)
											,Long.valueOf(sid));
									on($.getString("method"),csPresent,oldCsPresent);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsPresent oldCsPresent = CsPresent.get(csPresent.getCspId());
									csPresentService.updateCsPresent$NotNull(csPresent);
									LoggerHelper.writeLog(CsPresent.class, "update", 
											"修改了[礼品赠品]["+oldCsPresent.getCspName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsPresent, csPresent,false)
									...
									CsPresent oldCsPresent = csPresentService.getCsPresentById(csPresent.getCspId());
									//设置默认值
									csPresent.setCspUpdateTime(oldCsPresent.getCspUpdateTime());	
									csPresent.setCspAddTime(oldCsPresent.getCspAddTime());	
									...
									//从oldCsPresent同步csPresent未set过的值
									csPresent.mergeSet(oldCsPresent);
											
									csPresentService.updateCsPresent(csPresent);
									LoggerHelper.writeLog(CsPresent.class, "update", 
											"修改了[礼品赠品]["+oldCsPresent.getCspName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsPresent, csPresent,true)
											,csPresent.getCspId());
									on("update",csPresent,oldCsPresent);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csPresent)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("present_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csPresent.getCspId());
				else
		...
		try{
			...
			csPresentService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsPresent csPresent = csPresentService.getCsPresentById(id);
							//确认域标识
							if(SystemHelper.testHost(csPresent.getCspHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csPresentService.removeCsPresentById(id);
							else
							csPresentService.deleteCsPresentById(id);			
							//记录操作日志
							if(csPresent!=null)
								LoggerHelper.writeLog(CsPresent.class,"del","删除了[礼品赠品]["+csPresent.getCspName()+"]",(Long)$.getSession("ccclubs_login_id"), csPresent);
							on("delete",csPresent,csPresent);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csPresent=csPresentService.getCsPresentById(id);
			if(key!=null && id==null)
				csPresent=CsPresent.Get($.add(CsPresent.F.cspId,key));
			//确认域标识
			if(SystemHelper.testHost(csPresent.getCspHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsPresent.class);
			//强制限制域
			...
			List<CsPresent> list = csPresentService.getCsPresentPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsPresent csPresent:list){
				Map map = new HashMap();
				map.put("value",csPresent.getCspId().toString());
				map.put("text", $.js(csPresent.getKeyValue()));
				if(bObject==true)
					map.put("object",csPresent);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public ICsPresentService getCsPresentService()
	{
		return csPresentService;
	}

	public void setCsPresentService(ICsPresentService csPresentService)
	{
		this.csPresentService = csPresentService;
	}

	public CsPresent getCsPresent()
	{
		return csPresent;
	}

	public void setCsPresent(CsPresent csPresent)
	{
		this.csPresent = csPresent;
	}
}