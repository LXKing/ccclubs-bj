...
public class MembershipAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsMemberShip.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsMemberShip.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsMemberShip.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsMemberShip.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsMemberShip.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsMemberShip.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","csms_id");//那么，默认以主键降序，传给Dao层
			...
			//反强制限制域
			if(objects.get("csmsPayer")!=null && objects.get("csmsPayerNot")==null){
				objects.remove("HOSTS");
			}
			if(objects.get("csmsTargeter")!=null && objects.get("csmsTargeterNot")==null){
				objects.remove("HOSTS");
			}
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/user/membership.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csMemberShipService.getCsMemberShipPage(page, size, finalParams);
					}
				}, CsMemberShip.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csMemberShipService.getCsMemberShipPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csMemberShipToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csMemberShip这个对象为什么不用$.setRequest,因为csMemberShip有get,set方法，由struts处理的
				csMemberShip=csMemberShipService.getCsMemberShipById(id);
				//确认域标识
				if(SystemHelper.testHost(csMemberShip.getCsmsHost()).equals("0")){
					return $.SendHtml("<script>alert('对不起，你没有权限编辑该数据');</script>", "UTF-8");
				}
			}
			if(csMemberShip==null)
				csMemberShip = new CsMemberShip();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csMemberShip);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsMemberShip object,CsMemberShip old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsMemberShip.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csMemberShip.getCsmsId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsMemberShipToken = $.getString("csMemberShipToken");
			final String sessionCsMemberShipToken = $.getSession("csMemberShipToken");
			if($.equals(requestCsMemberShipToken,sessionCsMemberShipToken)){
				$.removeSession("csMemberShipToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csMemberShipService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csMemberShip.getCsmsId()==null?"":csMemberShip.getCsmsId());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csMemberShip.setCsmsId(Long.valueOf(sItem));
							
								//设置默认值
								if(csMemberShip.getCsmsAddTime()==null)
								...
								csMemberShip=csMemberShipService.saveCsMemberShip(csMemberShip);			
								
								on("save",csMemberShip,csMemberShip);
							}							
							
							csMemberShipService.updateCsMemberShip$NotNull(csMemberShip);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsMemberShip.class,"add","添加了[会员关系]["+csMemberShip.getCsmsId()+"]",(Long)$.getSession("ccclubs_login_id"), csMemberShip,csMemberShip.getCsmsId());
							$.SetTips("保存会员关系成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csMemberShipService.updateCsMemberShipByConfirm(ActionHelper.getSetValuesFromModel(csMemberShip), params);
								LoggerHelper.writeLog(CsMemberShip.class, "update", 
											"批量修改了[会员关系]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csMemberShip,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csMemberShip.setCsmsId(Long.valueOf(sid));
									CsMemberShip oldCsMemberShip = CsMemberShip.get(Long.valueOf(sid));
									csMemberShipService.updateCsMemberShip$NotNull(csMemberShip);
									LoggerHelper.writeLog(CsMemberShip.class, "update", 
											"修改了[会员关系]["+oldCsMemberShip.getCsmsId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsMemberShip, csMemberShip,false)
											,Long.valueOf(sid));
									on($.getString("method"),csMemberShip,oldCsMemberShip);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									CsMemberShip oldCsMemberShip = CsMemberShip.get(csMemberShip.getCsmsId());
									csMemberShipService.updateCsMemberShip$NotNull(csMemberShip);
									LoggerHelper.writeLog(CsMemberShip.class, "update", 
											"修改了[会员关系]["+oldCsMemberShip.getCsmsId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsMemberShip, csMemberShip,false)
											,csMemberShip.getCsmsId());
									on($.getString("method"),csMemberShip,oldCsMemberShip);									
								}else{
									CsMemberShip oldCsMemberShip = csMemberShipService.getCsMemberShipById(csMemberShip.getCsmsId());
									//设置默认值
									csMemberShip.setCsmsAddTime(oldCsMemberShip.getCsmsAddTime());	
									...
									//从oldCsMemberShip同步csMemberShip未set过的值
									csMemberShip.mergeSet(oldCsMemberShip);
											
									csMemberShipService.updateCsMemberShip(csMemberShip);
									LoggerHelper.writeLog(CsMemberShip.class, "update", 
											"修改了[会员关系]["+oldCsMemberShip.getCsmsId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsMemberShip, csMemberShip,true)
											,csMemberShip.getCsmsId());
									on("update",csMemberShip,oldCsMemberShip);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csMemberShip)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("membership_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csMemberShip.getCsmsId());
				else
		...
		try{
			...
			csMemberShipService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsMemberShip csMemberShip = csMemberShipService.getCsMemberShipById(id);
							//确认域标识
							if(SystemHelper.testHost(csMemberShip.getCsmsHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csMemberShipService.removeCsMemberShipById(id);
							else
							csMemberShipService.deleteCsMemberShipById(id);			
							//记录操作日志
							if(csMemberShip!=null)
								LoggerHelper.writeLog(CsMemberShip.class,"del","删除了[会员关系]["+csMemberShip.getCsmsId()+"]",(Long)$.getSession("ccclubs_login_id"), csMemberShip);
							on("delete",csMemberShip,csMemberShip);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csMemberShip=csMemberShipService.getCsMemberShipById(id);
			if(key!=null && id==null)
				csMemberShip=CsMemberShip.Get($.add(CsMemberShip.F.csmsId,key));
			//确认域标识
			if(SystemHelper.testHost(csMemberShip.getCsmsHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsMemberShip.class);
			//强制限制域
			params.put("HOSTS",SystemHelper.testHost(null));
			
			//反强制限制域
			if(params.get("csmsPayer")!=null && params.get("csmsPayerNot")==null){
				params.remove("HOSTS");
			}
			if(params.get("csmsTargeter")!=null && params.get("csmsTargeterNot")==null){
				params.remove("HOSTS");
			}
			...
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				params.put("desc","csms_id");//那么，默认以主键降序，传给Dao层
								
			String strTerm=$.getString("value");//智能搜索时的参数，一般都是主键后面那个字段
			if(!$.empty(strTerm)){
				if(strTerm.toLowerCase().startsWith("id:"))//如果查询条件以id:开头，则按ID查询
					params.put("csmsId", strTerm.toLowerCase().replaceFirst("id:", ""));					
				else//按标识查询，模糊查询请带%
				{
					String strDefinex = "";
					strDefinex+=" or csms_id like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsMemberShip> list = csMemberShipService.getCsMemberShipPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsMemberShip csMemberShip:list){
				Map map = new HashMap();
				map.put("value",csMemberShip.getCsmsId().toString());
				map.put("text", $.js(csMemberShip.getKeyValue()));
				if(bObject==true)
					map.put("object",csMemberShip);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
