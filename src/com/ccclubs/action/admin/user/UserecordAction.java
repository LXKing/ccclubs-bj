...
public class UserecordAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsUseRecord.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsUseRecord.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsUseRecord.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsUseRecord.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsUseRecord.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsUseRecord.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","csur_id");//那么，默认以主键降序，传给Dao层
			...
			//反强制限制域
			if(objects.get("csurMember")!=null && objects.get("csurMemberNot")==null){
				objects.remove("HOSTS");
			}
			if(objects.get("csurOrder")!=null && objects.get("csurOrderNot")==null){
				objects.remove("HOSTS");
			}
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/user/userecord.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csUseRecordService.getCsUseRecordPage(page, size, finalParams);
					}
				}, CsUseRecord.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csUseRecordService.getCsUseRecordPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csUseRecordToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csUseRecord这个对象为什么不用$.setRequest,因为csUseRecord有get,set方法，由struts处理的
				csUseRecord=csUseRecordService.getCsUseRecordById(id);
				//确认域标识
				if(SystemHelper.testHost(csUseRecord.getCsurHost()).equals("0")){
			...
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csUseRecord);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsUseRecord object,CsUseRecord old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsUseRecord.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csUseRecord.getCsurId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsUseRecordToken = $.getString("csUseRecordToken");
			final String sessionCsUseRecordToken = $.getSession("csUseRecordToken");
			if($.equals(requestCsUseRecordToken,sessionCsUseRecordToken)){
				$.removeSession("csUseRecordToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csUseRecordService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csUseRecord.getCsurProfile()==null?"":csUseRecord.getCsurProfile());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csUseRecord.setCsurProfile(String.valueOf(sItem));
							
								//设置默认值
								if(csUseRecord.getCsurAddTime()==null)
									csUseRecord.setCsurAddTime(new Date());
								...
								csUseRecord=csUseRecordService.saveCsUseRecord(csUseRecord);			
								
								on("save",csUseRecord,csUseRecord);
							}							
							
							csUseRecordService.updateCsUseRecord$NotNull(csUseRecord);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsUseRecord.class,"add","添加了[惠免记录]["+csUseRecord.getCsurProfile()+"]",(Long)$.getSession("ccclubs_login_id"), csUseRecord,csUseRecord.getCsurId());
							$.SetTips("保存惠免记录成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csUseRecordService.updateCsUseRecordByConfirm(ActionHelper.getSetValuesFromModel(csUseRecord), params);
								LoggerHelper.writeLog(CsUseRecord.class, "update", 
											"批量修改了[惠免记录]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csUseRecord,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csUseRecord.setCsurId(Long.valueOf(sid));
									CsUseRecord oldCsUseRecord = CsUseRecord.get(Long.valueOf(sid));
									csUseRecordService.updateCsUseRecord$NotNull(csUseRecord);
									LoggerHelper.writeLog(CsUseRecord.class, "update", 
											"修改了[惠免记录]["+oldCsUseRecord.getCsurProfile()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsUseRecord, csUseRecord,false)
											,Long.valueOf(sid));
									on($.getString("method"),csUseRecord,oldCsUseRecord);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									CsUseRecord oldCsUseRecord = CsUseRecord.get(csUseRecord.getCsurId());
									csUseRecordService.updateCsUseRecord$NotNull(csUseRecord);
									LoggerHelper.writeLog(CsUseRecord.class, "update", 
											"修改了[惠免记录]["+oldCsUseRecord.getCsurProfile()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsUseRecord, csUseRecord,false)
											,csUseRecord.getCsurId());
									on($.getString("method"),csUseRecord,oldCsUseRecord);									
								}else{
									CsUseRecord oldCsUseRecord = csUseRecordService.getCsUseRecordById(csUseRecord.getCsurId());
									//设置默认值
									csUseRecord.setCsurAddTime(oldCsUseRecord.getCsurAddTime());	
									...
									//从oldCsUseRecord同步csUseRecord未set过的值
									csUseRecord.mergeSet(oldCsUseRecord);
											
									csUseRecordService.updateCsUseRecord(csUseRecord);
									LoggerHelper.writeLog(CsUseRecord.class, "update", 
											"修改了[惠免记录]["+oldCsUseRecord.getCsurProfile()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsUseRecord, csUseRecord,true)
											,csUseRecord.getCsurId());
									on("update",csUseRecord,oldCsUseRecord);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csUseRecord)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("userecord_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csUseRecord.getCsurId());
				else
		...
		try{
			...
			csUseRecordService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsUseRecord csUseRecord = csUseRecordService.getCsUseRecordById(id);
							//确认域标识
							if(SystemHelper.testHost(csUseRecord.getCsurHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							csUseRecordService.deleteCsUseRecordById(id);			
							//记录操作日志
							if(csUseRecord!=null)
								LoggerHelper.writeLog(CsUseRecord.class,"del","删除了[惠免记录]["+csUseRecord.getCsurProfile()+"]",(Long)$.getSession("ccclubs_login_id"), csUseRecord);
							on("delete",csUseRecord,csUseRecord);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csUseRecord=csUseRecordService.getCsUseRecordById(id);
			if(key!=null && id==null)
				csUseRecord=CsUseRecord.Get($.add(CsUseRecord.F.csurId,key));
			//确认域标识
			if(SystemHelper.testHost(csUseRecord.getCsurHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsUseRecord.class);
			//强制限制域
			params.put("HOSTS",SystemHelper.testHost(null));
			
			//反强制限制域
			if(params.get("csurMember")!=null && params.get("csurMemberNot")==null){
				params.remove("HOSTS");
			}
			if(params.get("csurOrder")!=null && params.get("csurOrderNot")==null){
				params.remove("HOSTS");
			}
			...
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				params.put("desc","csur_id");//那么，默认以主键降序，传给Dao层
			...
			if(!$.empty(strTerm)){
				...
				{
					String strDefinex = "";
					strDefinex+=" or csur_profile like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsUseRecord> list = csUseRecordService.getCsUseRecordPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsUseRecord csUseRecord:list){
				Map map = new HashMap();
				map.put("value",csUseRecord.getCsurId().toString());
				map.put("text", $.js(csUseRecord.getKeyValue()));
				if(bObject==true)
