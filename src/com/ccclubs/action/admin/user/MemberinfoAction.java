...
public class MemberinfoAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsMemberInfo.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsMemberInfo.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsMemberInfo.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsMemberInfo.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsMemberInfo.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsMemberInfo.C.get($.forMat($.getString("desc")));//降序字段
			...
			//反强制限制域
			if(objects.get("csmiMemberId")!=null && objects.get("csmiMemberIdNot")==null){
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/user/memberinfo.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csMemberInfoService.getCsMemberInfoPage(page, size, finalParams);
					}
				}, CsMemberInfo.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csMemberInfoService.getCsMemberInfoPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csMemberInfoToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csMemberInfo这个对象为什么不用$.setRequest,因为csMemberInfo有get,set方法，由struts处理的
				csMemberInfo=csMemberInfoService.getCsMemberInfoById(id);
				//确认域标识
				if(SystemHelper.testHost(csMemberInfo.getCsmiHost()).equals("0")){
			...
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csMemberInfo);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsMemberInfo object,CsMemberInfo old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsMemberInfo.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csMemberInfo.getCsmiId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsMemberInfoToken = $.getString("csMemberInfoToken");
			final String sessionCsMemberInfoToken = $.getSession("csMemberInfoToken");
			if($.equals(requestCsMemberInfoToken,sessionCsMemberInfoToken)){
				$.removeSession("csMemberInfoToken");//移除令牌				
				...
				csMemberInfoService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csMemberInfo.getCsmiName()==null?"":csMemberInfo.getCsmiName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csMemberInfo.setCsmiName(String.valueOf(sItem));
							
								//设置默认值
								if(csMemberInfo.getCsmiUpdateTime()==null)
								csMemberInfo.setCsmiUpdateTime(new Date());
								if(csMemberInfo.getCsmiAddTime()==null)
								...
								on("save",csMemberInfo,csMemberInfo);
							}							
							...
							//记录操作日志
							LoggerHelper.writeLog(CsMemberInfo.class,"add","添加了[会员信息]["+csMemberInfo.getCsmiName()+"]",(Long)$.getSession("ccclubs_login_id"), csMemberInfo,csMemberInfo.getCsmiId());
							$.SetTips("保存会员信息成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csMemberInfoService.updateCsMemberInfoByConfirm(ActionHelper.getSetValuesFromModel(csMemberInfo), params);
								LoggerHelper.writeLog(CsMemberInfo.class, "update", 
											"批量修改了[会员信息]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csMemberInfo,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csMemberInfo.setCsmiId(Long.valueOf(sid));
									CsMemberInfo oldCsMemberInfo = CsMemberInfo.get(Long.valueOf(sid));
									csMemberInfoService.updateCsMemberInfo$NotNull(csMemberInfo);
									LoggerHelper.writeLog(CsMemberInfo.class, "update", 
											"修改了[会员信息]["+oldCsMemberInfo.getCsmiName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsMemberInfo, csMemberInfo,false)
											,Long.valueOf(sid));
									on($.getString("method"),csMemberInfo,oldCsMemberInfo);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									CsMemberInfo oldCsMemberInfo = CsMemberInfo.get(csMemberInfo.getCsmiId());
									csMemberInfoService.updateCsMemberInfo$NotNull(csMemberInfo);
									LoggerHelper.writeLog(CsMemberInfo.class, "update", 
											"修改了[会员信息]["+oldCsMemberInfo.getCsmiName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsMemberInfo, csMemberInfo,false)
											,csMemberInfo.getCsmiId());
									on($.getString("method"),csMemberInfo,oldCsMemberInfo);									
								}else{
									CsMemberInfo oldCsMemberInfo = csMemberInfoService.getCsMemberInfoById(csMemberInfo.getCsmiId());
									//设置默认值
									csMemberInfo.setCsmiUpdateTime(oldCsMemberInfo.getCsmiUpdateTime());	
									csMemberInfo.setCsmiAddTime(oldCsMemberInfo.getCsmiAddTime());	
									...
									//从oldCsMemberInfo同步csMemberInfo未set过的值
									csMemberInfo.mergeSet(oldCsMemberInfo);
											
									csMemberInfoService.updateCsMemberInfo(csMemberInfo);
									LoggerHelper.writeLog(CsMemberInfo.class, "update", 
											"修改了[会员信息]["+oldCsMemberInfo.getCsmiName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsMemberInfo, csMemberInfo,true)
											,csMemberInfo.getCsmiId());
									on("update",csMemberInfo,oldCsMemberInfo);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csMemberInfo)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("memberinfo_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csMemberInfo.getCsmiId());
				else
		...
		try{
			...
			csMemberInfoService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsMemberInfo csMemberInfo = csMemberInfoService.getCsMemberInfoById(id);
							//确认域标识
							if(SystemHelper.testHost(csMemberInfo.getCsmiHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csMemberInfoService.removeCsMemberInfoById(id);
							else
							csMemberInfoService.deleteCsMemberInfoById(id);			
							//记录操作日志
							if(csMemberInfo!=null)
								LoggerHelper.writeLog(CsMemberInfo.class,"del","删除了[会员信息]["+csMemberInfo.getCsmiName()+"]",(Long)$.getSession("ccclubs_login_id"), csMemberInfo);
							on("delete",csMemberInfo,csMemberInfo);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csMemberInfo=csMemberInfoService.getCsMemberInfoById(id);
			if(key!=null && id==null)
				csMemberInfo=CsMemberInfo.Get($.add(CsMemberInfo.F.csmiId,key));
			//确认域标识
			if(SystemHelper.testHost(csMemberInfo.getCsmiHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsMemberInfo.class);
			//强制限制域
			params.put("HOSTS",SystemHelper.testHost(null));
			
			//反强制限制域
			if(params.get("csmiMemberId")!=null && params.get("csmiMemberIdNot")==null){
				params.remove("HOSTS");
			}
			...
			List<CsMemberInfo> list = csMemberInfoService.getCsMemberInfoPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsMemberInfo csMemberInfo:list){
				Map map = new HashMap();
				map.put("value",csMemberInfo.getCsmiId().toString());
				map.put("text", $.js(csMemberInfo.getKeyValue()));
				if(bObject==true)
					map.put("object",csMemberInfo);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
