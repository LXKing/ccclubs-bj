...
public class MemberAction
{
	...
	{	
		try{
			...
			if(!$.empty(alias) && (alias.startsWith("dyna")||alias.startsWith("public"))){
				...
				if(CTRL==null){
					LzMap fields = $
					.add(CsMember.F.csmId, $.add("listable", true))
					.add(CsMember.F.csmHost, $.add("listable", true).add("queryable", true))
					.add(CsMember.F.csmMobile, $.add("listable", true).add("queryable", true))
					...
					.add(CsMember.F.csmVisitFlag, $.add("listable", true).add("queryable", true))
					.add(CsMember.F.csmRemark, $.add("listable", true));
					for(String showKey:array)
						fields.add(showKey, $.add("listable", true));
					LzMap map = $.add("listable", false).add("queryable", false).add("fields",fields);
			...
			//调用Service获取分页
			Page page = csMemberService.getCsMemberPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{	
		try{
			...
			if($.getBoolean("export",false)==true){//如果传入了一个数据导出的参数
				...
				response.setHeader("Content-Disposition", "attachment;filename="+java.net.URLEncoder.encode("会员帐号统计导出","UTF-8")+".xls");
 			...
 			if(groupNames!=null){
	 			for(String groupName:groupNames){
		 			...
		 			if($.empty(timeType))
		 				...
		 				groups.put(CsMember.C.get(groupName),"DATE_FORMAT({key},'"+timeType.split(",")[0]+"')");
	...
	{		
		try{
			...
			if(csMember==null)
				...
				CsUnitPerson unitPerson = CsUnitPerson.getCsUnitPerson($.add(CsUnitPerson.F.csupMember, csMember.getCsmId()));
				if(unitPerson != null){
					...
					CsMemberShip memberShip = CsMemberShip.getCsMemberShip($.add(CsMemberShip.F.csmsTargeter, id));
					if(memberShip != null){
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csMember.getCsmId()==null);
			...
			if($.equals(requestCsMemberToken,sessionCsMemberToken)){
				...
				csMemberService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(!bIsAdd){
							...
							if(ids!=null && !ids.trim().equals("")){//批量更新
								...
								for(String sid:array){
									...
									checkedmap.put(member.getCsmId(),member.getCsmVDrive()!=null &&  member.getCsmVDrive().shortValue()==1);
								}
							}else{
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csMember.getCsmName()==null?"":csMember.getCsmName());
							...
							//记录操作日志
							LoggerHelper.writeLog(CsMember.class,"add","添加了[会员帐号]["+csMember.getCsmName()+"]",(Long)$.getSession("ccclubs_login_id"), csMember,csMember.getCsmId());
							$.SetTips("保存会员帐号成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csMemberService.updateCsMemberByConfirm(ActionHelper.getSetValuesFromModel(csMember), params);
								LoggerHelper.writeLog(CsMember.class, "update", 
											"批量修改了[会员帐号]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csMember,false)
											,null);
								...
								for(String sid:array){
									...
									LoggerHelper.writeLog(CsMember.class, "update", 
											...
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsMember, csMember,false)
											,Long.valueOf(sid));
								...
								if($.empty($.getString("edittype"))){
									...
									CsMember oldCsMember = csMemberService.getCsMemberById(csMember.getCsmId());
									//设置默认值
									...
									LoggerHelper.writeLog(CsMember.class, "update", 
											"修改了[会员帐号]["+oldCsMember.getCsmName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsMember, csMember,true)
											,csMember.getCsmId());
						...
						for(Long key:keySet){
							...
							if(checked==false){
								...
								if(member.getCsmVDrive()!=null && member.getCsmVDrive().shortValue()==1){
									com.ccclubs.service.common.ICommonMoneyService commonMoneyService = $.getBean("commonMoneyService");
									com.ccclubs.service.admin.ICsRecordService csRecordService = $.getBean("csRecordService");
									if(csRecordService.getCsRecord($.add("csrType", IntegralType.注册送积分.name()).add("csrMember", key))==null)
										commonMoneyService.updateIntegralByRule(key, 1d, IntegralType.注册送积分, "资料审核通过得积分", null);
								}
							}
						}						
						/**************************LAZY3Q_CODE_SAVE****************************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csMember)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("member_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csMember.getCsmId());
				else
				...
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							...
							if(ConfigHelper.GetBoolean("logic_delete")==true){
								csMember.setCsmMobile(csMember.getCsmMobile$()+"-"+csMember.getCsmId$());
								csMemberService.updateCsMember(csMember);
							...
							//记录操作日志
							if(csMember!=null)
								LoggerHelper.writeLog(CsMember.class,"del","删除了[会员帐号]["+csMember.getCsmName()+"]",(Long)$.getSession("ccclubs_login_id"), csMember);
	...
	public String details()
	{
		try{
			...
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
			List<Map> map =  dao.executeQuery("select sum(cso_pay_coupon) as cso_pay_coupon ,sum(cso_pay_coin) as cso_pay_coin from cs_order where cso_status in (3,4) and cso_pay_member = "+csMember.getCsmId());
			...
			String recordSql = "SELECT " +
					...
					" csr_member =  "+csMember.getCsmId()+"  " ; 
			List<Map> record = dao.executeQuery(recordSql);
			
			Double order_pay_coupon = map.get(0).get("cso_pay_coupon") != null ? ((Double)map.get(0).get("cso_pay_coupon")) :0 ;
			Double artif_pay_coupon = artif_record.get(0).get("backConsuMeCouponCount") != null ? ((Double)artif_record.get(0).get("backConsuMeCouponCount")): 0;
			Double record_pay_coupon = record.get(0).get("backConsuMeCouponCount") != null ? ((Double)record.get(0).get("backConsuMeCouponCount")) : 0;

			$.SetRequest("pay_coin",map.get(0).get("cso_pay_coin") != null ? ((Double)map.get(0).get("cso_pay_coin")):0);
	...
	public String query(){
		try{
			...
			List<CsMember> list = csMemberService.getCsMemberPage(0,$.getInteger("size",10),params).getResult();
	...
	public String recharger(){
		try{
			...
			if($.empty(ids))
				ids = $.getString("id");
			List<CsMember> members = csMemberService.getCsMemberList($.add(CsMember.F.csmHost, LoginHelper.getLogin().getSuHost()).add("definex", "csm_id in ("+ids+")"), -1);
			...
			String message = csMemberService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(String sid:ids){
						...
						SYSTEM.IntegralType recordType = way.equals("充积分")?SYSTEM.IntegralType.加积分:SYSTEM.IntegralType.减积分;
						
						ICommonMoneyService commonMoneyService=$.GetSpringBean("commonMoneyService");
						
						commonMoneyService.updateIntegral(id, amount, recordType, $.getString("remark"), null);
					}
					return (T)(way+"成功");
				}
			});			
	...
	public String account(){
		try{
			...
			return $.SendAjax(
					$.add("money",canUseMoney).add("freehour", canFreeHour).add("coin", canCoin).add("integral", member.getCsmIntegral()),
				"UTF-8");	
		}catch(Exception e){
			e.printStackTrace();
			Logger.getRootLogger().error(e.getMessage(),e);
	...
	public String reset(){
		try{			
			...
			if(!com.ccclubs.helper.SystemHelper.isPassword(password))
				return $.SendAjax($.add("success",false).add("message", "密码格式错误，必须是6到12位字母或数字"),"UTF-8");	
			csMemberService.updateCsMemberByConfirm(
					$.add("csmPassword", $.md5(password)), 
					$.add("csmId", id).add("csmHost", LoginHelper.getLogin().getSuHost()).add("confirm", 1)
				);
			CsMember oldCsMember = CsMember.get(id);
			LoggerHelper.writeLog(CsMember.class, "update", 
					"重置会员密码[会员帐号]["+oldCsMember.getCsmMobile()+"]",
					LoginHelper.getLoginId(),"重置会员密码"
					,oldCsMember.getCsmId());
			...
			return CsMember.execute(new Function(){
				...
				public <T> T execute(Object... arg0) {
					...
					if(id==null || bPass==null)
						return (T) $.SendAjax($.add("success",false).add("message", "参数错误"),"UTF-8");	
					CsMember oldCsMember = CsMember.get(id);
					if(bPass){
						csMember = new CsMember().csmId(id).csmVDrive((short)1).csmUpdateTime(new Date()).csmVReal((short)1);
						csMember.update();
						com.ccclubs.service.common.ICommonMoneyService commonMoneyService = $.getBean("commonMoneyService");
						com.ccclubs.service.admin.ICsRecordService csRecordService = $.getBean("csRecordService");
						if(csRecordService.getCsRecord($.add("csrType", IntegralType.注册送积分.name()).add("csrMember", id))==null)
							commonMoneyService.updateIntegralByRule(id, 1d, IntegralType.注册送积分, "资料审核通过得积分", null);
					}else{
						csMember = new CsMember().csmId(id).csmVDrive((short)3).csmVReal((short)3);
						csMember.update();
					}
					LoggerHelper.writeLog(CsMember.class, "update", 
							"快速审核会员资料[会员帐号]["+oldCsMember.getCsmMobile()+"]",
							LoginHelper.getLoginId(),LoggerHelper.getUpdateDescription(oldCsMember, csMember,false)
							,csMember.getCsmId());
				...
				public <T> T execute(Object... arg0) {
					...
					if(id!=null){
						...
						if(csMember.getCsmVDrive().shortValue()==1 && csMember.getCsmVReal().shortValue()==1){
							com.ccclubs.service.common.ICommonMoneyService commonMoneyService = $.getBean("commonMoneyService");
							com.ccclubs.service.admin.ICsRecordService csRecordService = $.getBean("csRecordService");
							if(csRecordService.getCsRecord($.add("csrType", IntegralType.注册送积分.name()).add("csrMember", csMember.getCsmId()))==null)
								commonMoneyService.updateIntegralByRule(csMember.getCsmId(), 1d, IntegralType.注册送积分, "资料审核通过得积分", null);
						}
						csMember.update();
						String message = $.getString("message");
						if(!$.empty(message)){
							com.ccclubs.helper.UtilHelper.sendSms(dbMember.getCsmHost(), dbMember.getCsmMobile(), message, com.ccclubs.service.common.ICommonUtilService.SMSType.提示类短信);
						}
						CsMember oldCsMember = CsMember.get(id);
						LoggerHelper.writeLog(CsMember.class, "update", 
								"审核了[会员帐号]["+csMember.getKeyValue()+"]",
								LoginHelper.getLoginId(),
								LoggerHelper.getUpdateDescription(oldCsMember, csMember,false)+" - "+message
								,csMember.getCsmId());
						$.setRequest("submit","ok");
					}
					
					return (T) CsMember.where().add("asc","rand()").add("HOSTS", SystemHelper.testHost(null)).csmVDrive((short)2).csmVReal((short)2).get();
				}
			});
		...
		try{
			CsMember.execute(new Function(){
				...
				public <T> T execute(Object... arg0) {
					...
					if(csMember.getCsmVDrive().shortValue()==1 && csMember.getCsmVReal().shortValue()==1){
						com.ccclubs.service.common.ICommonMoneyService commonMoneyService = $.getBean("commonMoneyService");
						if(com.ccclubs.model.CsIntegralRecord.where().csrMember(csMember.getCsmId()).csrType(IntegralType.注册送积分.name()).get()==null)
							commonMoneyService.updateIntegralByRule(csMember.getCsmId(), 1d, IntegralType.注册送积分, "资料审核通过得积分", null);
					}//审核通过发送短信
					...
					//只有审核通过才发送短信
					if(csMember.getCsmVReal() ==1 && csMember.getCsmVDrive() == 1 && !$.empty(oldCsMember.getCsmMobile()) && from == 5){
						CsUnitPerson csUnitPerson = CsUnitPerson.getCsUnitPerson($.add(CsUnitPerson.F.csupMember, oldCsMember.getCsmId()));
						if(csUnitPerson != null){
							UtilHelper.sendTemplateSMS(csUnitPerson.getCsupHost(),"REGIST_CODE",oldCsMember.getCsmMobile$(),"欢迎使用北京出行，你的系统登录名为{mobile}，密码为{code}。"
									,SMSType.通知类短信,$.add("mobile", oldCsMember.getCsmMobile$()).add("code", password));
						}
					}
					
					String payMember = $.getString("payMember");
					if(!$.empty(payMember)){
						CsMemberShip.where().csmsTargeter(csMember.getCsmId()).set().csmsPayer(payMember).update();
					}
					Long unitInfo = $.getLong("unitInfo");
					Long unitGroup = $.getLong("unitGroup");
					
					if(unitInfo != null && unitGroup != null){
						CsUnitPerson.where().csupMember(csMember.getCsmId()).set().csupInfo(unitInfo).csupGroup(unitGroup).update();
					}
				
					
					
					LoggerHelper.writeLog(CsMember.class, "update", 
							"审核了[会员帐号]["+oldCsMember.getCsmMobile()+"]"+($.empty(payMember)?"":"修改支付会员"),
							LoginHelper.getLoginId(),
							LoggerHelper.getUpdateDescription(oldCsMember, csMember,false)+" - "+remark
							,csMember.getCsmId());
					$.SetTips("审核表单提交成功！");
					return null;
				}
			...
			String message = csMemberService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
			        ...
			        for(int i=1;i<sheet.getRows();i++){
			        	...
			        	if(dIntegral!=null)
			        		commonMoneyService.updateIntegral(memberId, dIntegral, dIntegral>0?IntegralType.加积分:IntegralType.减积分, sRemark, orderId);
			        }
