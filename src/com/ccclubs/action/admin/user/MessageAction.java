...
public class MessageAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsMessage.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsMessage.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsMessage.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsMessage.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsMessage.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsMessage.C.get($.forMat($.getString("desc")));//降序字段
			...
			//反强制限制域
			if(objects.get("csmReceiver")!=null && objects.get("csmReceiverNot")==null){
				objects.remove("HOSTS");
			}
			if(objects.get("csmSender")!=null && objects.get("csmSenderNot")==null){
				objects.remove("HOSTS");
			}
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/user/message.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csMessageService.getCsMessagePage(page, size, finalParams);
					}
				}, CsMessage.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csMessageService.getCsMessagePage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csMessageToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csMessage这个对象为什么不用$.setRequest,因为csMessage有get,set方法，由struts处理的
				csMessage=csMessageService.getCsMessageById(id);
			}
			if(csMessage==null)
				csMessage = new CsMessage();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csMessage);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsMessage object,CsMessage old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsMessage.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csMessage.getCsmId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsMessageToken = $.getString("csMessageToken");
			final String sessionCsMessageToken = $.getSession("csMessageToken");
			if($.equals(requestCsMessageToken,sessionCsMessageToken)){
				$.removeSession("csMessageToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csMessageService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csMessage.getCsmTitle()==null?"":csMessage.getCsmTitle());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csMessage.setCsmTitle(String.valueOf(sItem));
							
								//设置默认值
								if(csMessage.getCsmRcvTime()==null)
								csMessage.setCsmRcvTime(new Date());
								if(csMessage.getCsmSndTime()==null)
								csMessage.setCsmSndTime(new Date());
								
								//根据自定义的默认值信息设置默认值
								if(CTRL!=null)
									CTRL.setObjectDefaultValue(csMessage);
								
								csMessage=csMessageService.saveCsMessage(csMessage);			
								
								on("save",csMessage,csMessage);
							}							
							
							csMessageService.updateCsMessage$NotNull(csMessage);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsMessage.class,"add","添加了[会员消息]["+csMessage.getCsmTitle()+"]",(Long)$.getSession("ccclubs_login_id"), csMessage,csMessage.getCsmId());
							$.SetTips("保存会员消息成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csMessageService.updateCsMessageByConfirm(ActionHelper.getSetValuesFromModel(csMessage), params);
								LoggerHelper.writeLog(CsMessage.class, "update", 
											"批量修改了[会员消息]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csMessage,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csMessage.setCsmId(Long.valueOf(sid));
									CsMessage oldCsMessage = CsMessage.get(Long.valueOf(sid));
									csMessageService.updateCsMessage$NotNull(csMessage);
									LoggerHelper.writeLog(CsMessage.class, "update", 
											"修改了[会员消息]["+oldCsMessage.getCsmTitle()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsMessage, csMessage,false)
											,Long.valueOf(sid));
									on($.getString("method"),csMessage,oldCsMessage);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									CsMessage oldCsMessage = CsMessage.get(csMessage.getCsmId());
									csMessageService.updateCsMessage$NotNull(csMessage);
									LoggerHelper.writeLog(CsMessage.class, "update", 
											"修改了[会员消息]["+oldCsMessage.getCsmTitle()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsMessage, csMessage,false)
									...
									CsMessage oldCsMessage = csMessageService.getCsMessageById(csMessage.getCsmId());
									//设置默认值
									csMessage.setCsmRcvTime(oldCsMessage.getCsmRcvTime());	
									csMessage.setCsmSndTime(oldCsMessage.getCsmSndTime());	
									
									//设置填充值
									
									//从oldCsMessage同步csMessage未set过的值
									csMessage.mergeSet(oldCsMessage);
											
									csMessageService.updateCsMessage(csMessage);
									LoggerHelper.writeLog(CsMessage.class, "update", 
											"修改了[会员消息]["+oldCsMessage.getCsmTitle()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsMessage, csMessage,true)
											,csMessage.getCsmId());
									on("update",csMessage,oldCsMessage);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csMessage)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("message_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csMessage.getCsmId());
				else
		...
		try{
			...
			csMessageService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsMessage csMessage = csMessageService.getCsMessageById(id);
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csMessageService.removeCsMessageById(id);
							else
							csMessageService.deleteCsMessageById(id);			
							//记录操作日志
							if(csMessage!=null)
								LoggerHelper.writeLog(CsMessage.class,"del","删除了[会员消息]["+csMessage.getCsmTitle()+"]",(Long)$.getSession("ccclubs_login_id"), csMessage);
							on("delete",csMessage,csMessage);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csMessage=csMessageService.getCsMessageById(id);
			if(key!=null && id==null)
				csMessage=CsMessage.Get($.add(CsMessage.F.csmId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsMessage.class);
			
			//反强制限制域
			if(params.get("csmReceiver")!=null && params.get("csmReceiverNot")==null){
				params.remove("HOSTS");
			}
			if(params.get("csmSender")!=null && params.get("csmSenderNot")==null){
				params.remove("HOSTS");
			}
			...
			if(!$.empty(strTerm)){
				...
				{
					String strDefinex = "";
					strDefinex+=" or csm_title like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsMessage> list = csMessageService.getCsMessagePage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsMessage csMessage:list){
				Map map = new HashMap();
				map.put("value",csMessage.getCsmId().toString());
				map.put("text", $.js(csMessage.getKeyValue()));
				if(bObject==true)
					map.put("object",csMessage);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
