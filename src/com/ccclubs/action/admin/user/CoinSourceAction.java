...
public class CoinSourceAction {
	private CsCoinSource csCoinSource;
	
	private ICsCoinSourceService csCoinSourceService;
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsCoinSource.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsCoinSource.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsCoinSource.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsCoinSource.class));
			
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsCoinSource.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsCoinSource.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc",CsCoinSource.C.cscsId);//那么，默认以主键降序，传给Dao层
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/user/coinsource.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csCoinSourceService.getCsCoinSourcePage(page, size, finalParams);
					}
				}, CsCoinSource.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page =	csCoinSourceService.getCsCoinSourcePage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csCoinSourceToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csCoin这个对象为什么不用$.setRequest,因为csCoin有get,set方法，由struts处理的
				csCoinSource=csCoinSourceService.getCsCoinSourceById(id);
				//确认域标识
			}
			if(csCoinSource==null)
				csCoinSource = new CsCoinSource();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csCoinSource);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsCoinSource object,CsCoinSource old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsCoinSource.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csCoinSource.getCscsId()==null);
			...
			if($.equals(requestCsCoinToken,sessionCsCoinToken)){
				...
				csCoinSourceService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csCoinSource.getCscsId()==null?"":csCoinSource.getCscsId());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csCoinSource.setCscsId(Long.valueOf(sItem));
							
								//设置默认值
								if(csCoinSource.getCscsUpdateTime()==null)
									csCoinSource.setCscsUpdateTime(new Date());
								if(csCoinSource.getCscsAddTime()==null)
									csCoinSource.setCscsAddTime(new Date());
								if(csCoinSource.getCscsStatus()==null)
									csCoinSource.setCscsStatus((short)1);
								
								//根据自定义的默认值信息设置默认值
								if(CTRL!=null)
									CTRL.setObjectDefaultValue(csCoinSource);
								
								csCoinSource=csCoinSourceService.saveCsCoinSource(csCoinSource);			
								
								on("save",csCoinSource,csCoinSource);
							}							
							
							csCoinSourceService.updateCsCoinSource$NotNull(csCoinSource);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsCoinSource.class,"add","添加了[红包]["+csCoinSource.getCscsId()+"]",(Long)$.getSession("ccclubs_login_id"), csCoinSource,csCoinSource.getCscsId());
							$.SetTips("保存红包成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csCoinSourceService.updateCsCoinSourceByConfirm(ActionHelper.getSetValuesFromModel(csCoinSource), params);
								LoggerHelper.writeLog(CsCoinSource.class, "update", 
											"批量修改了[红包]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csCoinSource,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csCoinSource.setCscsId(Long.valueOf(sid));
									CsCoinSource oldCsCoin = csCoinSource.get(Long.valueOf(sid));
									csCoinSourceService.updateCsCoinSource$NotNull(csCoinSource);
									LoggerHelper.writeLog(CsCoinSource.class, "update", 
											"修改了[红包]["+oldCsCoin.getCscsId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsCoin, csCoinSource,false)
											,Long.valueOf(sid));
									on($.getString("method"),csCoinSource,oldCsCoin);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsCoinSource oldCsCoin = csCoinSource.get(csCoinSource.getCscsId());
									csCoinSourceService.updateCsCoinSource$NotNull(csCoinSource);
									LoggerHelper.writeLog(CsCoinSource.class, "update", 
											"修改了[红包]["+oldCsCoin.getCscsId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsCoin, csCoinSource,false)
											,csCoinSource.getCscsId());
									on($.getString("method"),csCoinSource,oldCsCoin);									
								}else{
									CsCoinSource oldCsCoin = csCoinSourceService.getCsCoinSourceById(csCoinSource.getCscsId());
									//设置默认值
									csCoinSource.setCscsUpdateTime(oldCsCoin.getCscsUpdateTime());	
									csCoinSource.setCscsAddTime(oldCsCoin.getCscsAddTime());	
									csCoinSource.setCscsStatus(oldCsCoin.getCscsStatus());	
									...
									csCoinSource.mergeSet(oldCsCoin);
											
									csCoinSourceService.updateCsCoinSource(csCoinSource);
									LoggerHelper.writeLog(CsCoinSource.class, "update", 
											"修改了[红包]["+oldCsCoin.getCscsId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsCoin, csCoinSource,true)
											,csCoinSource.getCscsId());
									on("update",csCoinSource,oldCsCoin);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csCoinSource)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("coin_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csCoinSource.getCscsId());
				else
		...
		try{
			...
			csCoinSourceService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsCoinSource csCoinSource = csCoinSourceService.getCsCoinSourceById(id);
							//确认域标识
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csCoinSourceService.removeCsCoinSourceById(id);
							else
							csCoinSourceService.deleteCsCoinSourceById(id);			
							//记录操作日志
							if(csCoinSource!=null)
								LoggerHelper.writeLog(CsCoinSource.class,"del","删除了[红包]["+csCoinSource.getCscsId()+"]",(Long)$.getSession("ccclubs_login_id"), csCoinSource);
							on("delete",csCoinSource,csCoinSource);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csCoinSource=csCoinSourceService.getCsCoinSourceById(id);
			if(key!=null && id==null)
				csCoinSource=csCoinSource.Get($.add(CsCoinSource.F.cscsId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsCoinSource.class);
			//强制限制域
			...
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				params.put("desc","cscs_id");//那么，默认以主键降序，传给Dao层
								
			String strTerm=$.getString("value");//智能搜索时的参数，一般都是主键后面那个字段
			if(!$.empty(strTerm)){
				if(strTerm.toLowerCase().startsWith("id:"))//如果查询条件以id:开头，则按ID查询
					params.put("cscsId", strTerm.toLowerCase().replaceFirst("id:", ""));					
				else//按标识查询，模糊查询请带%
				{
					String strDefinex = "";
					strDefinex+=" or cscs_id like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsCoinSource> list = csCoinSourceService.getCsCoinSourcePage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsCoinSource csCoinSource:list){
				Map map = new HashMap();
				map.put("value",csCoinSource.getCscsId().toString());
				map.put("text", $.js(csCoinSource.getKeyValue()));
				if(bObject==true)
					map.put("object",csCoinSource);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public CsCoinSource getCsCoinSource() {
		return csCoinSource;
	}

	public void setCsCoinSource(CsCoinSource csCoinSource) {
		this.csCoinSource = csCoinSource;
	}

	public ICsCoinSourceService getCsCoinSourceService() {
		return csCoinSourceService;
	}

	public void setCsCoinSourceService(ICsCoinSourceService csCoinSourceService) {
		this.csCoinSourceService = csCoinSourceService;
	}
