...
public class TypeAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsUserType.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsUserType.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsUserType.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsUserType.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsUserType.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsUserType.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","csut_id");//那么，默认以主键降序，传给Dao层
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/user/type.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csUserTypeService.getCsUserTypePage(page, size, finalParams);
					}
				}, CsUserType.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csUserTypeService.getCsUserTypePage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csUserTypeToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csUserType这个对象为什么不用$.setRequest,因为csUserType有get,set方法，由struts处理的
				csUserType=csUserTypeService.getCsUserTypeById(id);
			}
			if(csUserType==null)
				csUserType = new CsUserType();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csUserType);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsUserType object,CsUserType old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsUserType.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csUserType.getCsutId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsUserTypeToken = $.getString("csUserTypeToken");
			final String sessionCsUserTypeToken = $.getSession("csUserTypeToken");
			if($.equals(requestCsUserTypeToken,sessionCsUserTypeToken)){
				$.removeSession("csUserTypeToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csUserTypeService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csUserType.getCsutName()==null?"":csUserType.getCsutName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csUserType.setCsutName(String.valueOf(sItem));
								...
								csUserType=csUserTypeService.saveCsUserType(csUserType);			
								
								on("save",csUserType,csUserType);
							}							
							
							csUserTypeService.updateCsUserType$NotNull(csUserType);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsUserType.class,"add","添加了[计费方式]["+csUserType.getCsutName()+"]",(Long)$.getSession("ccclubs_login_id"), csUserType,csUserType.getCsutId());
							$.SetTips("保存计费方式成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csUserTypeService.updateCsUserTypeByConfirm(ActionHelper.getSetValuesFromModel(csUserType), params);
								LoggerHelper.writeLog(CsUserType.class, "update", 
											"批量修改了[计费方式]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csUserType,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csUserType.setCsutId(Long.valueOf(sid));
									CsUserType oldCsUserType = CsUserType.get(Long.valueOf(sid));
									csUserTypeService.updateCsUserType$NotNull(csUserType);
									LoggerHelper.writeLog(CsUserType.class, "update", 
											"修改了[计费方式]["+oldCsUserType.getCsutName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsUserType, csUserType,false)
											,Long.valueOf(sid));
									on($.getString("method"),csUserType,oldCsUserType);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									CsUserType oldCsUserType = CsUserType.get(csUserType.getCsutId());
									csUserTypeService.updateCsUserType$NotNull(csUserType);
									LoggerHelper.writeLog(CsUserType.class, "update", 
											"修改了[计费方式]["+oldCsUserType.getCsutName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsUserType, csUserType,false)
									...
									CsUserType oldCsUserType = csUserTypeService.getCsUserTypeById(csUserType.getCsutId());
									//设置默认值
									csUserType.setCsutFlag(oldCsUserType.getCsutFlag());	
									
									//设置填充值
									
									//从oldCsUserType同步csUserType未set过的值
									csUserType.mergeSet(oldCsUserType);
											
									csUserTypeService.updateCsUserType(csUserType);
									LoggerHelper.writeLog(CsUserType.class, "update", 
											"修改了[计费方式]["+oldCsUserType.getCsutName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsUserType, csUserType,true)
											,csUserType.getCsutId());
									on("update",csUserType,oldCsUserType);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csUserType)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("type_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csUserType.getCsutId());
				else
		...
		try{
			...
			csUserTypeService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsUserType csUserType = csUserTypeService.getCsUserTypeById(id);
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csUserTypeService.removeCsUserTypeById(id);
							else
							csUserTypeService.deleteCsUserTypeById(id);			
							//记录操作日志
							if(csUserType!=null)
								LoggerHelper.writeLog(CsUserType.class,"del","删除了[计费方式]["+csUserType.getCsutName()+"]",(Long)$.getSession("ccclubs_login_id"), csUserType);
							on("delete",csUserType,csUserType);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csUserType=csUserTypeService.getCsUserTypeById(id);
			if(key!=null && id==null)
				csUserType=CsUserType.Get($.add(CsUserType.F.csutId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsUserType.class);
			...
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				params.put("desc","csut_id");//那么，默认以主键降序，传给Dao层
								
			String strTerm=$.getString("value");//智能搜索时的参数，一般都是主键后面那个字段
			if(!$.empty(strTerm)){
				if(strTerm.toLowerCase().startsWith("id:"))//如果查询条件以id:开头，则按ID查询
					params.put("csutId", strTerm.toLowerCase().replaceFirst("id:", ""));					
				else//按标识查询，模糊查询请带%
				{
					String strDefinex = "";
					strDefinex+=" or csut_name like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsUserType> list = csUserTypeService.getCsUserTypePage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsUserType csUserType:list){
				Map map = new HashMap();
				map.put("value",csUserType.getCsutId().toString());
				map.put("text", $.js(csUserType.getKeyValue()));
				if(bObject==true)
					map.put("object",csUserType);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public CsUserType getCsUserType()
	{
		return csUserType;
	}

	public void setCsUserType(CsUserType csUserType)
	{
