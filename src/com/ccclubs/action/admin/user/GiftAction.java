...
public class GiftAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsGift.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsGift.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsGift.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsGift.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsGift.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsGift.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","csg_id");//那么，默认以主键降序，传给Dao层
			...
			//反强制限制域
			if(objects.get("csgMember")!=null && objects.get("csgMemberNot")==null){
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/user/gift.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csGiftService.getCsGiftPage(page, size, finalParams);
					}
				}, CsGift.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csGiftService.getCsGiftPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csGiftToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csGift这个对象为什么不用$.setRequest,因为csGift有get,set方法，由struts处理的
				csGift=csGiftService.getCsGiftById(id);
				//确认域标识
				if(SystemHelper.testHost(csGift.getCsgHost()).equals("0")){
			...
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csGift);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsGift object,CsGift old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsGift.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csGift.getCsgId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsGiftToken = $.getString("csGiftToken");
			final String sessionCsGiftToken = $.getSession("csGiftToken");
			if($.equals(requestCsGiftToken,sessionCsGiftToken)){
				$.removeSession("csGiftToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csGiftService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csGift.getCsgName()==null?"":csGift.getCsgName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csGift.setCsgName(String.valueOf(sItem));
							
								//设置默认值
								if(csGift.getCsgUpdateTime()==null)
									csGift.setCsgUpdateTime(new Date());
								if(csGift.getCsgAddTime()==null)
								...
								csGift=csGiftService.saveCsGift(csGift);			
								
								on("save",csGift,csGift);
							}							
							
							csGiftService.updateCsGift$NotNull(csGift);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsGift.class,"add","添加了[优惠内容]["+csGift.getCsgName()+"]",(Long)$.getSession("ccclubs_login_id"), csGift,csGift.getCsgId());
							$.SetTips("保存优惠内容成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csGiftService.updateCsGiftByConfirm(ActionHelper.getSetValuesFromModel(csGift), params);
								LoggerHelper.writeLog(CsGift.class, "update", 
											"批量修改了[优惠内容]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csGift,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csGift.setCsgId(Long.valueOf(sid));
									CsGift oldCsGift = CsGift.get(Long.valueOf(sid));
									csGiftService.updateCsGift$NotNull(csGift);
									LoggerHelper.writeLog(CsGift.class, "update", 
											"修改了[优惠内容]["+oldCsGift.getCsgName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsGift, csGift,false)
											,Long.valueOf(sid));
									on($.getString("method"),csGift,oldCsGift);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsGift oldCsGift = CsGift.get(csGift.getCsgId());
									csGiftService.updateCsGift$NotNull(csGift);
									LoggerHelper.writeLog(CsGift.class, "update", 
											"修改了[优惠内容]["+oldCsGift.getCsgName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsGift, csGift,false)
									...
									CsGift oldCsGift = csGiftService.getCsGiftById(csGift.getCsgId());
									//设置默认值
									csGift.setCsgMember(oldCsGift.getCsgMember());	
									csGift.setCsgSysPack(oldCsGift.getCsgSysPack());	
									csGift.setCsgUserPack(oldCsGift.getCsgUserPack());	
									csGift.setCsgGoods(oldCsGift.getCsgGoods());	
									csGift.setCsgOutlets(oldCsGift.getCsgOutlets());	
									csGift.setCsgType(oldCsGift.getCsgType());	
									csGift.setCsgScript(oldCsGift.getCsgScript());	
									csGift.setCsgUpdateTime(oldCsGift.getCsgUpdateTime());	
									csGift.setCsgAddTime(oldCsGift.getCsgAddTime());	
									...
									//从oldCsGift同步csGift未set过的值
									csGift.mergeSet(oldCsGift);
											
									csGiftService.updateCsGift(csGift);
									LoggerHelper.writeLog(CsGift.class, "update", 
											"修改了[优惠内容]["+oldCsGift.getCsgName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsGift, csGift,true)
											,csGift.getCsgId());
									on("update",csGift,oldCsGift);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csGift)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("gift_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csGift.getCsgId());
				else
		...
		try{
			...
			csGiftService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsGift csGift = csGiftService.getCsGiftById(id);
							//确认域标识
							if(SystemHelper.testHost(csGift.getCsgHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csGiftService.removeCsGiftById(id);
							else
							csGiftService.deleteCsGiftById(id);			
							//记录操作日志
							if(csGift!=null)
								LoggerHelper.writeLog(CsGift.class,"del","删除了[优惠内容]["+csGift.getCsgName()+"]",(Long)$.getSession("ccclubs_login_id"), csGift);
							on("delete",csGift,csGift);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csGift=csGiftService.getCsGiftById(id);
			if(key!=null && id==null)
				csGift=CsGift.Get($.add(CsGift.F.csgId,key));
			//确认域标识
			if(SystemHelper.testHost(csGift.getCsgHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsGift.class);
			//强制限制域
			params.put("HOSTS",SystemHelper.testHost(null));
			
			//反强制限制域
			if(params.get("csgMember")!=null && params.get("csgMemberNot")==null){
				params.remove("HOSTS");
			}
			...
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				params.put("desc","csg_id");//那么，默认以主键降序，传给Dao层
			...
			if(!$.empty(strTerm)){
				...
				{
					String strDefinex = "";
					strDefinex+=" or csg_name like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsGift> list = csGiftService.getCsGiftPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsGift csGift:list){
				Map map = new HashMap();
				map.put("value",csGift.getCsgId().toString());
				map.put("text", $.js(csGift.getKeyValue()));
				if(bObject==true)
