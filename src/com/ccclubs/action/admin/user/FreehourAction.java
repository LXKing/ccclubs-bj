...
public class FreehourAction
{
	ICsFreeHourService csFreeHourService;
	
	CsFreeHour csFreeHour;
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsFreeHour.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsFreeHour.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsFreeHour.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsFreeHour.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsFreeHour.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsFreeHour.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","csfh_id");//那么，默认以主键降序，传给Dao层
			...
			//反强制限制域
			if(objects.get("csfhMember")!=null && objects.get("csfhMemberNot")==null){
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/user/freehour.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csFreeHourService.getCsFreeHourPage(page, size, finalParams);
					}
				}, CsFreeHour.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csFreeHourService.getCsFreeHourPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csFreeHourToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csFreeHour这个对象为什么不用$.setRequest,因为csFreeHour有get,set方法，由struts处理的
				csFreeHour=csFreeHourService.getCsFreeHourById(id);
				//确认域标识
				if(SystemHelper.testHost(csFreeHour.getCsfhHost()).equals("0")){
					return $.SendHtml("<script>alert('对不起，你没有权限编辑该数据');</script>", "UTF-8");
				}
			}
			if(csFreeHour==null)
				csFreeHour = new CsFreeHour();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csFreeHour);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsFreeHour object,CsFreeHour old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsFreeHour.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csFreeHour.getCsfhId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsFreeHourToken = $.getString("csFreeHourToken");
			final String sessionCsFreeHourToken = $.getSession("csFreeHourToken");
			if($.equals(requestCsFreeHourToken,sessionCsFreeHourToken)){
				$.removeSession("csFreeHourToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csFreeHourService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csFreeHour.getCsfhId()==null?"":csFreeHour.getCsfhId());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csFreeHour.setCsfhId(Long.valueOf(sItem));
							
								//设置默认值
								if(csFreeHour.getCsfhRemain()==null)
									csFreeHour.setCsfhRemain(csFreeHour.getCsfhCount());
								if(csFreeHour.getCsfhEnd()==null)
									csFreeHour.setCsfhEnd(com.ccclubs.helper.SystemHelper.getValidUntil(csFreeHour.getCsfhValidity()));
								if(csFreeHour.getCsfhEditor()==null)
									csFreeHour.setCsfhEditor(LoginHelper.getLoginId());
								if(csFreeHour.getCsfhUpdateTime()==null)
									csFreeHour.setCsfhUpdateTime(new Date());
								if(csFreeHour.getCsfhAddTime()==null)
									csFreeHour.setCsfhAddTime(new Date());
								if(csFreeHour.getCsfhStatus()==null)
								...
								csFreeHour=csFreeHourService.saveCsFreeHour(csFreeHour);			
								
								on("save",csFreeHour,csFreeHour);
							}							
							
							csFreeHourService.updateCsFreeHour$NotNull(csFreeHour);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsFreeHour.class,"add","添加了[免费小时]["+csFreeHour.getCsfhId()+"]",(Long)$.getSession("ccclubs_login_id"), csFreeHour,csFreeHour.getCsfhId());
							$.SetTips("保存免费小时成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csFreeHourService.updateCsFreeHourByConfirm(ActionHelper.getSetValuesFromModel(csFreeHour), params);
								LoggerHelper.writeLog(CsFreeHour.class, "update", 
											"批量修改了[免费小时]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csFreeHour,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csFreeHour.setCsfhId(Long.valueOf(sid));
									CsFreeHour oldCsFreeHour = CsFreeHour.get(Long.valueOf(sid));
									csFreeHourService.updateCsFreeHour$NotNull(csFreeHour);
									LoggerHelper.writeLog(CsFreeHour.class, "update", 
											"修改了[免费小时]["+oldCsFreeHour.getCsfhId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsFreeHour, csFreeHour,false)
											,Long.valueOf(sid));
									on($.getString("method"),csFreeHour,oldCsFreeHour);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsFreeHour oldCsFreeHour = CsFreeHour.get(csFreeHour.getCsfhId());
									csFreeHourService.updateCsFreeHour$NotNull(csFreeHour);
									LoggerHelper.writeLog(CsFreeHour.class, "update", 
											"修改了[免费小时]["+oldCsFreeHour.getCsfhId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsFreeHour, csFreeHour,false)
											,csFreeHour.getCsfhId());
									on($.getString("method"),csFreeHour,oldCsFreeHour);									
								}else{
									CsFreeHour oldCsFreeHour = csFreeHourService.getCsFreeHourById(csFreeHour.getCsfhId());
									//设置默认值
									csFreeHour.setCsfhCount(oldCsFreeHour.getCsfhCount());	
									csFreeHour.setCsfhValidity(oldCsFreeHour.getCsfhValidity());	
									csFreeHour.setCsfhEditor(oldCsFreeHour.getCsfhEditor());	
									csFreeHour.setCsfhUpdateTime(oldCsFreeHour.getCsfhUpdateTime());	
									csFreeHour.setCsfhAddTime(oldCsFreeHour.getCsfhAddTime());	
									...
									//从oldCsFreeHour同步csFreeHour未set过的值
									csFreeHour.mergeSet(oldCsFreeHour);
											
									csFreeHourService.updateCsFreeHour(csFreeHour);
									LoggerHelper.writeLog(CsFreeHour.class, "update", 
											"修改了[免费小时]["+oldCsFreeHour.getCsfhId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsFreeHour, csFreeHour,true)
											,csFreeHour.getCsfhId());
									on("update",csFreeHour,oldCsFreeHour);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csFreeHour)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("freehour_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csFreeHour.getCsfhId());
				else
		...
		try{
			...
			csFreeHourService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsFreeHour csFreeHour = csFreeHourService.getCsFreeHourById(id);
							//确认域标识
							if(SystemHelper.testHost(csFreeHour.getCsfhHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csFreeHourService.removeCsFreeHourById(id);
							else
							csFreeHourService.deleteCsFreeHourById(id);			
							//记录操作日志
							if(csFreeHour!=null)
								LoggerHelper.writeLog(CsFreeHour.class,"del","删除了[免费小时]["+csFreeHour.getCsfhId()+"]",(Long)$.getSession("ccclubs_login_id"), csFreeHour);
							on("delete",csFreeHour,csFreeHour);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csFreeHour=csFreeHourService.getCsFreeHourById(id);
			if(key!=null && id==null)
				csFreeHour=CsFreeHour.Get($.add(CsFreeHour.F.csfhId,key));
			//确认域标识
			if(SystemHelper.testHost(csFreeHour.getCsfhHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsFreeHour.class);
			//强制限制域
			params.put("HOSTS",SystemHelper.testHost(null));
			
			//反强制限制域
			if(params.get("csfhMember")!=null && params.get("csfhMemberNot")==null){
				params.remove("HOSTS");
			}
			...
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				params.put("desc","csfh_id");//那么，默认以主键降序，传给Dao层
								
			String strTerm=$.getString("value");//智能搜索时的参数，一般都是主键后面那个字段
			if(!$.empty(strTerm)){
				if(strTerm.toLowerCase().startsWith("id:"))//如果查询条件以id:开头，则按ID查询
					params.put("csfhId", strTerm.toLowerCase().replaceFirst("id:", ""));					
				else//按标识查询，模糊查询请带%
				{
					String strDefinex = "";
					strDefinex+=" or csfh_id like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsFreeHour> list = csFreeHourService.getCsFreeHourPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsFreeHour csFreeHour:list){
				Map map = new HashMap();
				map.put("value",csFreeHour.getCsfhId().toString());
				map.put("text", $.js(csFreeHour.getKeyValue()));
				if(bObject==true)
					map.put("object",csFreeHour);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public String batch(){
		try{		
			String message = csFreeHourService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					//设置默认值
					if(csFreeHour.getCsfhRemain()==null)
						csFreeHour.setCsfhRemain(csFreeHour.getCsfhCount());
					...
					if(csFreeHour.getCsfhStatus()==null)
						csFreeHour.setCsfhStatus((short)1);
					csFreeHour.setCsfhHost(SystemHelper.getAdminHost().getShId());					
					Workbook book=null;
					try {
			        ...
			        for(int i=1;i<sheet.getRows();i++){
			        	...
			        	String mobile = sheet.getCell(1,i).getContents();
			        	if(memberId==null){
			        		CsMember member = CsMember.getCsMember($.add(CsMember.F.csmMobile, mobile));
			        		if(member!=null)
			        			memberId = member.getCsmId();
			        	}
			        	if(memberId==null)
			        		continue;
			        	csFreeHour.setCsfhMember(memberId);						
						csFreeHour.save();
			        }			        
			        book.close();
					return (T)"批量充值成功";
	...
	public CsFreeHour getCsFreeHour()
	{
		return csFreeHour;
	}

	public void setCsFreeHour(CsFreeHour csFreeHour)
	{
		this.csFreeHour = csFreeHour;
	}
}