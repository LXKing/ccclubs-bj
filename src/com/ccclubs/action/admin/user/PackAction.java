...
public class PackAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsUserPack.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsUserPack.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsUserPack.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsUserPack.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsUserPack.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsUserPack.C.get($.forMat($.getString("desc")));//降序字段
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/user/pack.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csUserPackService.getCsUserPackPage(page, size, finalParams);
					}
				}, CsUserPack.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csUserPackService.getCsUserPackPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csUserPackToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csUserPack这个对象为什么不用$.setRequest,因为csUserPack有get,set方法，由struts处理的
				csUserPack=csUserPackService.getCsUserPackById(id);
				//确认域标识
				if(SystemHelper.testHost(csUserPack.getCsupHost()).equals("0")){
			...
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csUserPack);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsUserPack object,CsUserPack old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsUserPack.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csUserPack.getCsupId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsUserPackToken = $.getString("csUserPackToken");
			final String sessionCsUserPackToken = $.getSession("csUserPackToken");
			if($.equals(requestCsUserPackToken,sessionCsUserPackToken)){
				$.removeSession("csUserPackToken");//移除令牌				
				...
				csUserPackService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csUserPack.getCsupId()==null?"":csUserPack.getCsupId());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csUserPack.setCsupId(Long.valueOf(sItem));
							
								//设置默认值
								if(csUserPack.getCsupUntilTime()==null)
									csUserPack.setCsupUntilTime(new Date());
								if(csUserPack.getCsupUpdateTime()==null)
									csUserPack.setCsupUpdateTime(new Date());
								if(csUserPack.getCsupAddTime()==null)
									csUserPack.setCsupAddTime(new Date());
								if(csUserPack.getCsupState()==null)
									csUserPack.setCsupState((short)0);
								if(csUserPack.getCsupStatus()==null)
									csUserPack.setCsupStatus((short)1);
								...
								on("save",csUserPack,csUserPack);
							}							
							
							csUserPackService.updateCsUserPack$NotNull(csUserPack);						
						/*****************LAZY3Q_CODE_ADD*******************/
						//更新套餐的城市为会员的城市
						new CsUserPack().csupId(csUserPack.getCsupId()).csupHost(csUserPack.get$csupMember().getCsmHost()).update();
						//添加会员套餐时，自动添加会员套餐优惠项，并且设置套餐结束时间
						CsPack systemPack = csUserPack.get$csupPack();
						Calendar calendar = Calendar.getInstance();						
						switch(systemPack.getCspValidity()){
						case 0://不限
							calendar.add(Calendar.YEAR, 100);//100年以后
							break;
						case 1://一月
						...
						case 3://半年
							...
							break;
						case 4://一年
							calendar.add(Calendar.YEAR, 1);//100年以后
							break;
						case 5://天数  
							calendar.add(Calendar.DATE, systemPack.getCspDay());//100年以后
							break;						
						}
						Date untilTime = calendar.getTime();
						csUserPackService.updateCsUserPackByConfirm(
								$.add("csupUntilTime", untilTime)
								, $.add("csupId", csUserPack.getCsupId()).add("confirm", 1)
							);
						
						//扣除会员相应余额
						ICommonMoneyService commonMoneyService = $.GetSpringBean("commonMoneyService");
						commonMoneyService.updateMoney(
								csUserPack.getCsupMember(),
								ICommonMoneyService.MoneyType.Money, 
								-(Math.abs(systemPack.getCspPrice())), 
								SYSTEM.RecordType.购买套餐, 
								"", 
								null,
								csUserPack.getCsupId(),
								CsUserPack.class
							);
						
						ICsPackInfoService csPackInfoService=$.GetSpringBean("csPackInfoService");
						ICsGiftService csGiftService=$.GetSpringBean("csGiftService");
						List<CsPackInfo> infos = csPackInfoService.getCsPackInfoList(
								$.add("cspiPack", csUserPack.getCsupPack())
								.add("asc", CsPackInfo.C.cspiId)
							, -1);
						for(CsPackInfo csPackInfo:infos){
							CsGift csGift=new CsGift();
							csGift.setCsgAddTime(new Date());
							csGift.setCsgHost(csUserPack.getCsupHost());
							csGift.setCsgName(csPackInfo.getCspiName());
							csGift.setCsgGoods(csPackInfo.getCspiGoods());
							csGift.setCsgMember(csUserPack.getCsupMember());
							csGift.setCsgModel(csPackInfo.getCspiModel());
							csGift.setCsgOutlets(csPackInfo.getCspiOutlets());
							csGift.setCsgSysPack(csUserPack.getCsupPack());
							csGift.setCsgType(csPackInfo.getCspiType());
							csGift.setCsgScript(csPackInfo.getCspiScript());
							csGift.setCsgUntilTime(untilTime);
							csGift.setCsgUpdateTime(new Date());
							csGift.setCsgUserPack(csUserPack.getCsupId());
							switch(csPackInfo.getCspiType()){
							case 0:
								csGift.setCsgCount(csPackInfo.getCspiValue());
								csGift.setCsgRemain(csPackInfo.getCspiValue());
								break;
							case 1:
								csGift.setCsgPrice(csPackInfo.getCspiValue());
								break;
							case 2:
								csGift.setCsgRebate(csPackInfo.getCspiValue());
								break;
							}
							csGift.setCsgStatus((short)1);
							
							if($.empty(csGift.getCsgName())){
								csGift.setCsgName(csPackInfo.get$cspiGoods().getCsgName()+"优惠");
							}
						...
						/*****************LAZY3Q_CODE_ADD*******************/
							...
							//记录操作日志
							LoggerHelper.writeLog(CsUserPack.class,"add","添加了[用户套餐]["+csUserPack.getCsupId()+"]",(Long)$.getSession("ccclubs_login_id"), csUserPack,csUserPack.getCsupId());
							$.SetTips("保存用户套餐成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csUserPackService.updateCsUserPackByConfirm(ActionHelper.getSetValuesFromModel(csUserPack), params);
								LoggerHelper.writeLog(CsUserPack.class, "update", 
											"批量修改了[用户套餐]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csUserPack,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csUserPack.setCsupId(Long.valueOf(sid));
									CsUserPack oldCsUserPack = CsUserPack.get(Long.valueOf(sid));
									csUserPackService.updateCsUserPack$NotNull(csUserPack);
									LoggerHelper.writeLog(CsUserPack.class, "update", 
											"修改了[用户套餐]["+oldCsUserPack.getCsupId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsUserPack, csUserPack,false)
											,Long.valueOf(sid));
									on($.getString("method"),csUserPack,oldCsUserPack);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsUserPack oldCsUserPack = CsUserPack.get(csUserPack.getCsupId());
									csUserPackService.updateCsUserPack$NotNull(csUserPack);
									LoggerHelper.writeLog(CsUserPack.class, "update", 
											"修改了[用户套餐]["+oldCsUserPack.getCsupId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsUserPack, csUserPack,false)
									...
									CsUserPack oldCsUserPack = csUserPackService.getCsUserPackById(csUserPack.getCsupId());
									//设置默认值
									csUserPack.setCsupUpdateTime(oldCsUserPack.getCsupUpdateTime());	
									csUserPack.setCsupAddTime(oldCsUserPack.getCsupAddTime());	
									csUserPack.setCsupState(oldCsUserPack.getCsupState());	
									...
									//从oldCsUserPack同步csUserPack未set过的值
									csUserPack.mergeSet(oldCsUserPack);
											
									csUserPackService.updateCsUserPack(csUserPack);
									LoggerHelper.writeLog(CsUserPack.class, "update", 
											"修改了[用户套餐]["+oldCsUserPack.getCsupId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsUserPack, csUserPack,true)
											,csUserPack.getCsupId());
									on("update",csUserPack,oldCsUserPack);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csUserPack)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("pack_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csUserPack.getCsupId());
				else
		...
		try{
			...
			csUserPackService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsUserPack csUserPack = csUserPackService.getCsUserPackById(id);
							//确认域标识
							if(SystemHelper.testHost(csUserPack.getCsupHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csUserPackService.removeCsUserPackById(id);
							else
							csUserPackService.deleteCsUserPackById(id);			
							//记录操作日志
							if(csUserPack!=null)
								LoggerHelper.writeLog(CsUserPack.class,"del","删除了[用户套餐]["+csUserPack.getCsupId()+"]",(Long)$.getSession("ccclubs_login_id"), csUserPack);
							on("delete",csUserPack,csUserPack);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csUserPack=csUserPackService.getCsUserPackById(id);
			if(key!=null && id==null)
				csUserPack=CsUserPack.Get($.add(CsUserPack.F.csupId,key));
			//确认域标识
			if(SystemHelper.testHost(csUserPack.getCsupHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsUserPack.class);
			//强制限制域
			...
			List<CsUserPack> list = csUserPackService.getCsUserPackPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsUserPack csUserPack:list){
				Map map = new HashMap();
				map.put("value",csUserPack.getCsupId().toString());
				map.put("text", $.js(csUserPack.getKeyValue()));
				if(bObject==true)
