...
public class UserexpAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",SrvUserExp.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(SrvUserExp.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(SrvUserExp.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(SrvUserExp.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=SrvUserExp.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=SrvUserExp.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","sue_id");//那么，默认以主键降序，传给Dao层
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/permissions/userexp.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return srvUserExpService.getSrvUserExpPage(page, size, finalParams);
					}
				}, SrvUserExp.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = srvUserExpService.getSrvUserExpPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("srvUserExpToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，srvUserExp这个对象为什么不用$.setRequest,因为srvUserExp有get,set方法，由struts处理的
				srvUserExp=srvUserExpService.getSrvUserExpById(id);
			}
			if(srvUserExp==null)
				srvUserExp = new SrvUserExp();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(srvUserExp);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,SrvUserExp object,SrvUserExp old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(SrvUserExp.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && srvUserExp.getSueId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestSrvUserExpToken = $.getString("srvUserExpToken");
			final String sessionSrvUserExpToken = $.getSession("srvUserExpToken");
			if($.equals(requestSrvUserExpToken,sessionSrvUserExpToken)){
				$.removeSession("srvUserExpToken");//移除令牌				
				...
				srvUserExpService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(srvUserExp.getSueId()==null?"":srvUserExp.getSueId());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								srvUserExp.setSueId(Long.valueOf(sItem));
								...
								srvUserExp=srvUserExpService.saveSrvUserExp(srvUserExp);			
								
								on("save",srvUserExp,srvUserExp);
							}							
							
							srvUserExpService.updateSrvUserExp$NotNull(srvUserExp);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(SrvUserExp.class,"add","添加了[用户扩展信息]["+srvUserExp.getSueId()+"]",(Long)$.getSession("ccclubs_login_id"), srvUserExp,srvUserExp.getSueId());
							$.SetTips("保存用户扩展信息成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								srvUserExpService.updateSrvUserExpByConfirm(ActionHelper.getSetValuesFromModel(srvUserExp), params);
								LoggerHelper.writeLog(SrvUserExp.class, "update", 
											"批量修改了[用户扩展信息]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,srvUserExp,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									srvUserExp.setSueId(Long.valueOf(sid));
									SrvUserExp oldSrvUserExp = SrvUserExp.get(Long.valueOf(sid));
									srvUserExpService.updateSrvUserExp$NotNull(srvUserExp);
									LoggerHelper.writeLog(SrvUserExp.class, "update", 
											"修改了[用户扩展信息]["+oldSrvUserExp.getSueId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldSrvUserExp, srvUserExp,false)
											,Long.valueOf(sid));
									on($.getString("method"),srvUserExp,oldSrvUserExp);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									SrvUserExp oldSrvUserExp = SrvUserExp.get(srvUserExp.getSueId());
									srvUserExpService.updateSrvUserExp$NotNull(srvUserExp);
									LoggerHelper.writeLog(SrvUserExp.class, "update", 
											"修改了[用户扩展信息]["+oldSrvUserExp.getSueId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldSrvUserExp, srvUserExp,false)
									...
									SrvUserExp oldSrvUserExp = srvUserExpService.getSrvUserExpById(srvUserExp.getSueId());
									//设置默认值
									srvUserExp.setSueUser(oldSrvUserExp.getSueUser());	
									
									//设置填充值
									
									//从oldSrvUserExp同步srvUserExp未set过的值
									srvUserExp.mergeSet(oldSrvUserExp);
											
									srvUserExpService.updateSrvUserExp(srvUserExp);
									LoggerHelper.writeLog(SrvUserExp.class, "update", 
											"修改了[用户扩展信息]["+oldSrvUserExp.getSueId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldSrvUserExp, srvUserExp,true)
											,srvUserExp.getSueId());
									on("update",srvUserExp,oldSrvUserExp);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(srvUserExp)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("userexp_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+srvUserExp.getSueId());
				else
		...
		try{
			...
			srvUserExpService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							SrvUserExp srvUserExp = srvUserExpService.getSrvUserExpById(id);
							srvUserExpService.deleteSrvUserExpById(id);			
							//记录操作日志
							if(srvUserExp!=null)
								LoggerHelper.writeLog(SrvUserExp.class,"del","删除了[用户扩展信息]["+srvUserExp.getSueId()+"]",(Long)$.getSession("ccclubs_login_id"), srvUserExp);
							on("delete",srvUserExp,srvUserExp);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				srvUserExp=srvUserExpService.getSrvUserExpById(id);
			if(key!=null && id==null)
				srvUserExp=SrvUserExp.Get($.add(SrvUserExp.F.sueId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(SrvUserExp.class);
			...
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				params.put("desc","sue_id");//那么，默认以主键降序，传给Dao层
								
			String strTerm=$.getString("value");//智能搜索时的参数，一般都是主键后面那个字段
			if(!$.empty(strTerm)){
				if(strTerm.toLowerCase().startsWith("id:"))//如果查询条件以id:开头，则按ID查询
					params.put("sueId", strTerm.toLowerCase().replaceFirst("id:", ""));					
				else//按标识查询，模糊查询请带%
				{
					String strDefinex = "";
					strDefinex+=" or sue_id like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<SrvUserExp> list = srvUserExpService.getSrvUserExpPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(SrvUserExp srvUserExp:list){
				Map map = new HashMap();
				map.put("value",srvUserExp.getSueId().toString());
				map.put("text", $.js(srvUserExp.getKeyValue()));
				if(bObject==true)
					map.put("object",srvUserExp);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
