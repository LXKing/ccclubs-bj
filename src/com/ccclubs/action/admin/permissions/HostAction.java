...
public class HostAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",SrvHost.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(SrvHost.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(SrvHost.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(SrvHost.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=SrvHost.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=SrvHost.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","sh_id");//那么，默认以主键降序，传给Dao层
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/permissions/host.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return srvHostService.getSrvHostPage(page, size, finalParams);
					}
				}, SrvHost.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = srvHostService.getSrvHostPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("srvHostToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，srvHost这个对象为什么不用$.setRequest,因为srvHost有get,set方法，由struts处理的
				srvHost=srvHostService.getSrvHostById(id);
			}
			if(srvHost==null)
				srvHost = new SrvHost();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(srvHost);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,SrvHost object,SrvHost old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(SrvHost.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && srvHost.getShId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestSrvHostToken = $.getString("srvHostToken");
			final String sessionSrvHostToken = $.getSession("srvHostToken");
			if($.equals(requestSrvHostToken,sessionSrvHostToken)){
				$.removeSession("srvHostToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				srvHostService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(srvHost.getShName()==null?"":srvHost.getShName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								srvHost.setShName(String.valueOf(sItem));
								...
								srvHost=srvHostService.saveSrvHost(srvHost);			
								
								on("save",srvHost,srvHost);
							}							
							
							srvHostService.updateSrvHost$NotNull(srvHost);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(SrvHost.class,"add","添加了[运营城市]["+srvHost.getShName()+"]",(Long)$.getSession("ccclubs_login_id"), srvHost,srvHost.getShId());
							$.SetTips("保存运营城市成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								srvHostService.updateSrvHostByConfirm(ActionHelper.getSetValuesFromModel(srvHost), params);
								LoggerHelper.writeLog(SrvHost.class, "update", 
											"批量修改了[运营城市]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,srvHost,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									srvHost.setShId(Long.valueOf(sid));
									SrvHost oldSrvHost = SrvHost.get(Long.valueOf(sid));
									srvHostService.updateSrvHost$NotNull(srvHost);
									LoggerHelper.writeLog(SrvHost.class, "update", 
											"修改了[运营城市]["+oldSrvHost.getShName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldSrvHost, srvHost,false)
											,Long.valueOf(sid));
									on($.getString("method"),srvHost,oldSrvHost);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									SrvHost oldSrvHost = SrvHost.get(srvHost.getShId());
									srvHostService.updateSrvHost$NotNull(srvHost);
									LoggerHelper.writeLog(SrvHost.class, "update", 
											"修改了[运营城市]["+oldSrvHost.getShName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldSrvHost, srvHost,false)
									...
									SrvHost oldSrvHost = srvHostService.getSrvHostById(srvHost.getShId());
									//设置默认值
									
									//设置填充值
									
									//从oldSrvHost同步srvHost未set过的值
									srvHost.mergeSet(oldSrvHost);
											
									srvHostService.updateSrvHost(srvHost);
									LoggerHelper.writeLog(SrvHost.class, "update", 
											"修改了[运营城市]["+oldSrvHost.getShName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldSrvHost, srvHost,true)
											,srvHost.getShId());
									on("update",srvHost,oldSrvHost);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(srvHost)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("host_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+srvHost.getShId());
				else
		...
		try{
			...
			srvHostService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							SrvHost srvHost = srvHostService.getSrvHostById(id);
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								srvHostService.removeSrvHostById(id);
							else
							srvHostService.deleteSrvHostById(id);			
							//记录操作日志
							if(srvHost!=null)
								LoggerHelper.writeLog(SrvHost.class,"del","删除了[运营城市]["+srvHost.getShName()+"]",(Long)$.getSession("ccclubs_login_id"), srvHost);
							on("delete",srvHost,srvHost);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				srvHost=srvHostService.getSrvHostById(id);
			if(key!=null && id==null)
				srvHost=SrvHost.Get($.add(SrvHost.F.shId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(SrvHost.class);
			...
			if(!$.empty(strTerm)){
				...
				else//按标识查询，模糊查询请带%
				{
					...
					strDefinex+=" or sh_oper like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<SrvHost> list = srvHostService.getSrvHostPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(SrvHost srvHost:list){
				Map map = new HashMap();
				map.put("value",srvHost.getShId().toString());
				map.put("text", $.js(srvHost.getKeyValue()));
				if(bObject==true)
	...
	public SrvHost getSrvHost()
	{
		return srvHost;
	}

	public void setSrvHost(SrvHost srvHost)
	{
