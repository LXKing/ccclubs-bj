...
{
	ICsRepairService csRepairService;
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsRepair.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsRepair.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsRepair.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsRepair.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsRepair.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsRepair.C.get($.forMat($.getString("desc")));//降序字段
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/service/repair.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csRepairService.getCsRepairPage(page, size, finalParams);
					}
				}, CsRepair.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csRepairService.getCsRepairPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csRepairToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csRepair这个对象为什么不用$.setRequest,因为csRepair有get,set方法，由struts处理的
				csRepair=csRepairService.getCsRepairById(id);
				//确认域标识
				if(SystemHelper.testHost(csRepair.getCsrHost()).equals("0")){
			...
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csRepair);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsRepair object,CsRepair old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsRepair.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csRepair.getCsrId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsRepairToken = $.getString("csRepairToken");
			final String sessionCsRepairToken = $.getSession("csRepairToken");
			if($.equals(requestCsRepairToken,sessionCsRepairToken)){
				$.removeSession("csRepairToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csRepairService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csRepair.getCsrProfile()==null?"":csRepair.getCsrProfile());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csRepair.setCsrProfile(String.valueOf(sItem));
							
								//设置默认值
								if(csRepair.getCsrEditor()==null)
								csRepair.setCsrEditor((Long) $.getSession("ccclubs_login_id"));
								if(csRepair.getCsrAddTime()==null)
								csRepair.setCsrAddTime(new Date());
								...
								on("save",csRepair,csRepair);
							}							
							
							csRepairService.updateCsRepair$NotNull(csRepair);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsRepair.class,"add","添加了[车辆维修]["+csRepair.getCsrProfile()+"]",(Long)$.getSession("ccclubs_login_id"), csRepair,csRepair.getCsrId());
							$.SetTips("保存车辆维修成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csRepairService.updateCsRepairByConfirm(ActionHelper.getSetValuesFromModel(csRepair), params);
								LoggerHelper.writeLog(CsRepair.class, "update", 
											"批量修改了[车辆维修]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csRepair,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csRepair.setCsrId(Long.valueOf(sid));
									CsRepair oldCsRepair = CsRepair.get(Long.valueOf(sid));
									csRepairService.updateCsRepair$NotNull(csRepair);
									LoggerHelper.writeLog(CsRepair.class, "update", 
											"修改了[车辆维修]["+oldCsRepair.getCsrProfile()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsRepair, csRepair,false)
											,Long.valueOf(sid));
									on($.getString("method"),csRepair,oldCsRepair);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									CsRepair oldCsRepair = CsRepair.get(csRepair.getCsrId());
									csRepairService.updateCsRepair$NotNull(csRepair);
									LoggerHelper.writeLog(CsRepair.class, "update", 
											"修改了[车辆维修]["+oldCsRepair.getCsrProfile()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsRepair, csRepair,false)
									...
									CsRepair oldCsRepair = csRepairService.getCsRepairById(csRepair.getCsrId());
									//设置默认值
									csRepair.setCsrEditor(oldCsRepair.getCsrEditor());	
									csRepair.setCsrAddTime(oldCsRepair.getCsrAddTime());	
									...
									//从oldCsRepair同步csRepair未set过的值
									csRepair.mergeSet(oldCsRepair);
											
									csRepairService.updateCsRepair(csRepair);
									LoggerHelper.writeLog(CsRepair.class, "update", 
											"修改了[车辆维修]["+oldCsRepair.getCsrProfile()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsRepair, csRepair,true)
											,csRepair.getCsrId());
									on("update",csRepair,oldCsRepair);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
					...
					/*************LAZY3Q_CODE_SAVE***************/
					//同步维修费用
					if(csRepair.getCsrMoney()!=null && $.empty($.getString("ids")) && csRepair.getCsrId()!=null){
						ICsTroubleService csTroubleService=$.getBean("csTroubleService");
						csTroubleService.updateCsTroubleByConfirm(
								$.add("cstRepairPay", csRepair.getCsrMoney()), 
								$.add("cstRepair", csRepair.getCsrId()).add("confirm", 1)
						);
					}
					/*************LAZY3Q_CODE_SAVE***************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csRepair)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("repair_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csRepair.getCsrId());
				else
		...
		try{
			...
			csRepairService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsRepair csRepair = csRepairService.getCsRepairById(id);
							//确认域标识
							if(SystemHelper.testHost(csRepair.getCsrHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csRepairService.removeCsRepairById(id);
							else
							csRepairService.deleteCsRepairById(id);			
							//记录操作日志
							if(csRepair!=null)
								LoggerHelper.writeLog(CsRepair.class,"del","删除了[车辆维修]["+csRepair.getCsrProfile()+"]",(Long)$.getSession("ccclubs_login_id"), csRepair);
							on("delete",csRepair,csRepair);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csRepair=csRepairService.getCsRepairById(id);
			if(key!=null && id==null)
				csRepair=CsRepair.Get($.add(CsRepair.F.csrId,key));
			//确认域标识
			if(SystemHelper.testHost(csRepair.getCsrHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsRepair.class);
			//强制限制域
			...
			if(!$.empty(strTerm)){
				...
				{
					String strDefinex = "";
					strDefinex+=" or csr_profile like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsRepair> list = csRepairService.getCsRepairPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsRepair csRepair:list){
				Map map = new HashMap();
				map.put("value",csRepair.getCsrId().toString());
				map.put("text", $.js(csRepair.getKeyValue()));
				if(bObject==true)
					map.put("object",csRepair);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public ICsRepairService getCsRepairService()
	{
		return csRepairService;
	}

	public void setCsRepairService(ICsRepairService csRepairService)
	{
		this.csRepairService = csRepairService;
	}

	public CsRepair getCsRepair()
	{
		return csRepair;
	}

	public void setCsRepair(CsRepair csRepair)
	{
		this.csRepair = csRepair;
	}
}