...
public class RemoteStatisticsAction {
	...
	{	
		try{
			...
			$.setRequest("modelClass",SrvUser.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(SrvUser.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(SrvUser.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(SrvUser.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=SrvUser.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=SrvUser.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","su_id");//那么，默认以主键降序，传给Dao层
			...
			final Page remotePage = csAnalysisService.getReMoteDataPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				}, RemoteData.class, $.getArray("keys"));
