...
public class BillAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsUnitBill.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsUnitBill.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsUnitBill.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsUnitBill.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsUnitBill.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsUnitBill.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","csub_id");//那么，默认以主键降序，传给Dao层
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/unit/bill.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csUnitBillService.getCsUnitBillPage(page, size, finalParams);
					}
				}, CsUnitBill.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csUnitBillService.getCsUnitBillPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csUnitBillToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csUnitBill这个对象为什么不用$.setRequest,因为csUnitBill有get,set方法，由struts处理的
				csUnitBill=csUnitBillService.getCsUnitBillById(id);
				//确认域标识
				if(SystemHelper.testHost(csUnitBill.getCsubHost()).equals("0")){
			...
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csUnitBill);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsUnitBill object,CsUnitBill old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsUnitBill.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csUnitBill.getCsubId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsUnitBillToken = $.getString("csUnitBillToken");
			final String sessionCsUnitBillToken = $.getSession("csUnitBillToken");
			if($.equals(requestCsUnitBillToken,sessionCsUnitBillToken)){
				$.removeSession("csUnitBillToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csUnitBillService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csUnitBill.getCsubMonth()==null?"":csUnitBill.getCsubMonth());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csUnitBill.setCsubMonth(String.valueOf(sItem));
							
								//设置默认值
								if(csUnitBill.getCsubMonth()==null)
								csUnitBill.setCsubMonth("");
								if(csUnitBill.getCsubUnit()==null)
								csUnitBill.setCsubUnit((long)0);
								if(csUnitBill.getCsubEditor()==null)
									csUnitBill.setCsubEditor(LoginHelper.getLoginId());
								if(csUnitBill.getCsubUpdateTime()==null)
									csUnitBill.setCsubUpdateTime(new Date());
								if(csUnitBill.getCsubAddTime()==null)
									csUnitBill.setCsubAddTime(new Date());
								if(csUnitBill.getCsubStatus()==null)
								...
								csUnitBill=csUnitBillService.saveCsUnitBill(csUnitBill);			
								
								on("save",csUnitBill,csUnitBill);
							}							
							
							csUnitBillService.updateCsUnitBill$NotNull(csUnitBill);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsUnitBill.class,"add","添加了[企业对账]["+csUnitBill.getCsubMonth()+"]",(Long)$.getSession("ccclubs_login_id"), csUnitBill,csUnitBill.getCsubId());
							$.SetTips("保存企业对账成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csUnitBillService.updateCsUnitBillByConfirm(ActionHelper.getSetValuesFromModel(csUnitBill), params);
								LoggerHelper.writeLog(CsUnitBill.class, "update", 
											"批量修改了[企业对账]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csUnitBill,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csUnitBill.setCsubId(Long.valueOf(sid));
									CsUnitBill oldCsUnitBill = CsUnitBill.get(Long.valueOf(sid));
									csUnitBillService.updateCsUnitBill$NotNull(csUnitBill);
									LoggerHelper.writeLog(CsUnitBill.class, "update", 
											"修改了[企业对账]["+oldCsUnitBill.getCsubMonth()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsUnitBill, csUnitBill,false)
											,Long.valueOf(sid));
									on($.getString("method"),csUnitBill,oldCsUnitBill);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsUnitBill oldCsUnitBill = CsUnitBill.get(csUnitBill.getCsubId());
									csUnitBillService.updateCsUnitBill$NotNull(csUnitBill);
									LoggerHelper.writeLog(CsUnitBill.class, "update", 
											"修改了[企业对账]["+oldCsUnitBill.getCsubMonth()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsUnitBill, csUnitBill,false)
											,csUnitBill.getCsubId());
									on($.getString("method"),csUnitBill,oldCsUnitBill);									
								}else{
									CsUnitBill oldCsUnitBill = csUnitBillService.getCsUnitBillById(csUnitBill.getCsubId());
									//设置默认值
									csUnitBill.setCsubMonth(oldCsUnitBill.getCsubMonth());	
									csUnitBill.setCsubUnit(oldCsUnitBill.getCsubUnit());	
									csUnitBill.setCsubEditor(oldCsUnitBill.getCsubEditor());	
									csUnitBill.setCsubUpdateTime(oldCsUnitBill.getCsubUpdateTime());	
									csUnitBill.setCsubAddTime(oldCsUnitBill.getCsubAddTime());	
									csUnitBill.setCsubStatus(oldCsUnitBill.getCsubStatus());	
									...
									//从oldCsUnitBill同步csUnitBill未set过的值
									csUnitBill.mergeSet(oldCsUnitBill);
											
									csUnitBillService.updateCsUnitBill(csUnitBill);
									LoggerHelper.writeLog(CsUnitBill.class, "update", 
											"修改了[企业对账]["+oldCsUnitBill.getCsubMonth()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsUnitBill, csUnitBill,true)
											,csUnitBill.getCsubId());
									on("update",csUnitBill,oldCsUnitBill);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csUnitBill)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("bill_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csUnitBill.getCsubId());
				else
		...
		try{
			...
			csUnitBillService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsUnitBill csUnitBill = csUnitBillService.getCsUnitBillById(id);
							//确认域标识
							if(SystemHelper.testHost(csUnitBill.getCsubHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csUnitBillService.removeCsUnitBillById(id);
							else
							csUnitBillService.deleteCsUnitBillById(id);			
							//记录操作日志
							if(csUnitBill!=null)
								LoggerHelper.writeLog(CsUnitBill.class,"del","删除了[企业对账]["+csUnitBill.getCsubMonth()+"]",(Long)$.getSession("ccclubs_login_id"), csUnitBill);
							on("delete",csUnitBill,csUnitBill);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csUnitBill=csUnitBillService.getCsUnitBillById(id);
			if(key!=null && id==null)
				csUnitBill=CsUnitBill.Get($.add(CsUnitBill.F.csubId,key));
			//确认域标识
			if(SystemHelper.testHost(csUnitBill.getCsubHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsUnitBill.class);
			//强制限制域
			...
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				params.put("desc","csub_id");//那么，默认以主键降序，传给Dao层
								
			String strTerm=$.getString("value");//智能搜索时的参数，一般都是主键后面那个字段
			if(!$.empty(strTerm)){
				if(strTerm.toLowerCase().startsWith("id:"))//如果查询条件以id:开头，则按ID查询
					params.put("csubId", strTerm.toLowerCase().replaceFirst("id:", ""));					
				else//按标识查询，模糊查询请带%
				{
					String strDefinex = "";
					strDefinex+=" or csub_month like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsUnitBill> list = csUnitBillService.getCsUnitBillPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsUnitBill csUnitBill:list){
				Map map = new HashMap();
				map.put("value",csUnitBill.getCsubId().toString());
				map.put("text", $.js(csUnitBill.getKeyValue()));
				if(bObject==true)
					map.put("object",csUnitBill);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
