...
public class UserAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsUnitUser.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsUnitUser.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsUnitUser.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsUnitUser.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsUnitUser.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsUnitUser.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","csuu_id");//那么，默认以主键降序，传给Dao层
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/unit/user.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csUnitUserService.getCsUnitUserPage(page, size, finalParams);
					}
				}, CsUnitUser.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csUnitUserService.getCsUnitUserPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csUnitUserToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csUnitUser这个对象为什么不用$.setRequest,因为csUnitUser有get,set方法，由struts处理的
				csUnitUser=csUnitUserService.getCsUnitUserById(id);
				//确认域标识
				if(SystemHelper.testHost(csUnitUser.getCsuuHost()).equals("0")){
			...
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csUnitUser);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsUnitUser object,CsUnitUser old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsUnitUser.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csUnitUser.getCsuuId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsUnitUserToken = $.getString("csUnitUserToken");
			final String sessionCsUnitUserToken = $.getSession("csUnitUserToken");
					...
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							...
							for(String sItem:sArray){
								...
								//设置默认值
								if(csUnitUser.getCsuuPassword()==null)
									csUnitUser.setCsuuPassword($.md5("123456"));
							...
							//记录操作日志
							LoggerHelper.writeLog(CsUnitUser.class,"add","添加了[企业车管]["+csUnitUser.getCsuuUsername()+"]",(Long)$.getSession("ccclubs_login_id"), csUnitUser,csUnitUser.getCsuuId());
							$.SetTips("保存企业车管成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csUnitUserService.updateCsUnitUserByConfirm(ActionHelper.getSetValuesFromModel(csUnitUser), params);
								LoggerHelper.writeLog(CsUnitUser.class, "update", 
											"批量修改了[企业车管]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csUnitUser,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csUnitUser.setCsuuId(Long.valueOf(sid));
									CsUnitUser oldCsUnitUser = CsUnitUser.get(Long.valueOf(sid));
									csUnitUserService.updateCsUnitUser$NotNull(csUnitUser);
									LoggerHelper.writeLog(CsUnitUser.class, "update", 
											"修改了[企业车管]["+oldCsUnitUser.getCsuuUsername()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsUnitUser, csUnitUser,false)
											,Long.valueOf(sid));
									on($.getString("method"),csUnitUser,oldCsUnitUser);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									CsUnitUser oldCsUnitUser = CsUnitUser.get(csUnitUser.getCsuuId());
									csUnitUserService.updateCsUnitUser$NotNull(csUnitUser);
									LoggerHelper.writeLog(CsUnitUser.class, "update", 
											"修改了[企业车管]["+oldCsUnitUser.getCsuuUsername()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsUnitUser, csUnitUser,false)
											,csUnitUser.getCsuuId());
									on($.getString("method"),csUnitUser,oldCsUnitUser);									
								}else{
									...
									//从oldCsUnitUser同步csUnitUser未set过的值
									csUnitUser.mergeSet(oldCsUnitUser);
											
									csUnitUserService.updateCsUnitUser(csUnitUser);
									LoggerHelper.writeLog(CsUnitUser.class, "update", 
											"修改了[企业车管]["+oldCsUnitUser.getCsuuUsername()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsUnitUser, csUnitUser,true)
											,csUnitUser.getCsuuId());
									on("update",csUnitUser,oldCsUnitUser);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csUnitUser)+")"+"</script>","UTF-8");
			}
			...
			csUnitUserService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							...
							//记录操作日志
							if(csUnitUser!=null)
								LoggerHelper.writeLog(CsUnitUser.class,"del","删除了[企业车管]["+csUnitUser.getCsuuUsername()+"]",(Long)$.getSession("ccclubs_login_id"), csUnitUser);
							on("delete",csUnitUser,csUnitUser);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csUnitUser=csUnitUserService.getCsUnitUserById(id);
			if(key!=null && id==null)
				csUnitUser=CsUnitUser.Get($.add(CsUnitUser.F.csuuId,key));
			//确认域标识
			if(SystemHelper.testHost(csUnitUser.getCsuuHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsUnitUser.class);
			//强制限制域
			...
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				params.put("desc","csuu_id");//那么，默认以主键降序，传给Dao层
			...
			List<CsUnitUser> list = csUnitUserService.getCsUnitUserPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsUnitUser csUnitUser:list){
				Map map = new HashMap();
				map.put("value",csUnitUser.getCsuuId().toString());
				map.put("text", $.js(csUnitUser.getKeyValue()));
				if(bObject==true)
