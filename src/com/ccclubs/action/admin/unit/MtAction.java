...
public class MtAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsUnitMt.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsUnitMt.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsUnitMt.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsUnitMt.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsUnitMt.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsUnitMt.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","csum_id");//那么，默认以主键降序，传给Dao层
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/unit/mt.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csUnitMtService.getCsUnitMtPage(page, size, finalParams);
					}
				}, CsUnitMt.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csUnitMtService.getCsUnitMtPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csUnitMtToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csUnitMt这个对象为什么不用$.setRequest,因为csUnitMt有get,set方法，由struts处理的
				csUnitMt=csUnitMtService.getCsUnitMtById(id);
				//确认域标识
				if(SystemHelper.testHost(csUnitMt.getCsumHost()).equals("0")){
					return $.SendHtml("<script>alert('对不起，你没有权限编辑该数据');</script>", "UTF-8");
				}
			}
			if(csUnitMt==null)
				csUnitMt = new CsUnitMt();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csUnitMt);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsUnitMt object,CsUnitMt old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsUnitMt.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csUnitMt.getCsumId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsUnitMtToken = $.getString("csUnitMtToken");
			final String sessionCsUnitMtToken = $.getSession("csUnitMtToken");
			if($.equals(requestCsUnitMtToken,sessionCsUnitMtToken)){
				$.removeSession("csUnitMtToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csUnitMtService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csUnitMt.getCsumTitle()==null?"":csUnitMt.getCsumTitle());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csUnitMt.setCsumTitle(String.valueOf(sItem));
							
								//设置默认值
								if(csUnitMt.getCsumUpdateTime()==null)
									csUnitMt.setCsumUpdateTime(new Date());
								if(csUnitMt.getCsumAddTime()==null)
								...
								csUnitMt=csUnitMtService.saveCsUnitMt(csUnitMt);			
								
								on("save",csUnitMt,csUnitMt);
							}							
							
							csUnitMtService.updateCsUnitMt$NotNull(csUnitMt);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsUnitMt.class,"add","添加了[推送消息]["+csUnitMt.getCsumTitle()+"]",(Long)$.getSession("ccclubs_login_id"), csUnitMt,csUnitMt.getCsumId());
							$.SetTips("保存推送消息成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csUnitMtService.updateCsUnitMtByConfirm(ActionHelper.getSetValuesFromModel(csUnitMt), params);
								LoggerHelper.writeLog(CsUnitMt.class, "update", 
											"批量修改了[推送消息]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csUnitMt,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csUnitMt.setCsumId(Long.valueOf(sid));
									CsUnitMt oldCsUnitMt = CsUnitMt.get(Long.valueOf(sid));
									csUnitMtService.updateCsUnitMt$NotNull(csUnitMt);
									LoggerHelper.writeLog(CsUnitMt.class, "update", 
											"修改了[推送消息]["+oldCsUnitMt.getCsumTitle()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsUnitMt, csUnitMt,false)
											,Long.valueOf(sid));
									on($.getString("method"),csUnitMt,oldCsUnitMt);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsUnitMt oldCsUnitMt = CsUnitMt.get(csUnitMt.getCsumId());
									csUnitMtService.updateCsUnitMt$NotNull(csUnitMt);
									LoggerHelper.writeLog(CsUnitMt.class, "update", 
											"修改了[推送消息]["+oldCsUnitMt.getCsumTitle()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsUnitMt, csUnitMt,false)
									...
									CsUnitMt oldCsUnitMt = csUnitMtService.getCsUnitMtById(csUnitMt.getCsumId());
									//设置默认值
									csUnitMt.setCsumUpdateTime(oldCsUnitMt.getCsumUpdateTime());	
									csUnitMt.setCsumAddTime(oldCsUnitMt.getCsumAddTime());	
									...
									//从oldCsUnitMt同步csUnitMt未set过的值
									csUnitMt.mergeSet(oldCsUnitMt);
											
									csUnitMtService.updateCsUnitMt(csUnitMt);
									LoggerHelper.writeLog(CsUnitMt.class, "update", 
											"修改了[推送消息]["+oldCsUnitMt.getCsumTitle()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsUnitMt, csUnitMt,true)
											,csUnitMt.getCsumId());
									on("update",csUnitMt,oldCsUnitMt);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csUnitMt)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("mt_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csUnitMt.getCsumId());
				else
				...
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							...
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csUnitMtService.removeCsUnitMtById(id);
							else
							csUnitMtService.deleteCsUnitMtById(id);			
							//记录操作日志
							if(csUnitMt!=null)
								LoggerHelper.writeLog(CsUnitMt.class,"del","删除了[推送消息]["+csUnitMt.getCsumTitle()+"]",(Long)$.getSession("ccclubs_login_id"), csUnitMt);
							on("delete",csUnitMt,csUnitMt);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csUnitMt=csUnitMtService.getCsUnitMtById(id);
			if(key!=null && id==null)
				csUnitMt=CsUnitMt.Get($.add(CsUnitMt.F.csumId,key));
			//确认域标识
			if(SystemHelper.testHost(csUnitMt.getCsumHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsUnitMt.class);
			//强制限制域
			...
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				params.put("desc","csum_id");//那么，默认以主键降序，传给Dao层
								
			String strTerm=$.getString("value");//智能搜索时的参数，一般都是主键后面那个字段
			if(!$.empty(strTerm)){
				if(strTerm.toLowerCase().startsWith("id:"))//如果查询条件以id:开头，则按ID查询
					params.put("csumId", strTerm.toLowerCase().replaceFirst("id:", ""));					
				else//按标识查询，模糊查询请带%
				{
					String strDefinex = "";
					strDefinex+=" or csum_title like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsUnitMt> list = csUnitMtService.getCsUnitMtPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsUnitMt csUnitMt:list){
				Map map = new HashMap();
				map.put("value",csUnitMt.getCsumId().toString());
				map.put("text", $.js(csUnitMt.getKeyValue()));
				if(bObject==true)
					map.put("object",csUnitMt);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public CsUnitMt getCsUnitMt()
	{
		return csUnitMt;
	}

	public void setCsUnitMt(CsUnitMt csUnitMt)
	{
		this.csUnitMt = csUnitMt;
	}
}