...
public class CarResAction {
	ICsCarResService csCarResService;
	CsCarRes csCarRes;
	...
	public String execute() {
		...
		try {
			...
			$.setRequest("modelClass", CsCarRes.class);// 把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsCarRes.class);
			// 根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String, Object> objects = ActionHelper
					.getQueryFormParams(CsCarRes.class);
			$.setRequest("PARAMS",
					ActionHelper.getQueryFormParams(CsCarRes.class));

			// 取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc = CsCarRes.C.get($.forMat($.getString("asc")));// 升序字段
			objects.put("asc", strAsc);// 放入查询条件
			String strDesc = CsCarRes.C.get($.forMat($.getString("desc")));// 降序字段
			...
			//车牌名称模糊查询
			if(objects.get("cscrCarNo") !=null){
				$.setRequest("cscrCarNo", objects.get("cscrCarNo"));
				buffer.append("and  cscr_car_no "+(objects.get("cscrCarNoYesNot")==null ?"":"not")+" like '%"+objects.get("cscrCarNo").toString().replaceAll("'", "''")+"%'");
				objects.remove("cscrCarNo");
			}
			//车牌名称模糊查询
			if(objects.get("cscrLocation") !=null){
				$.setRequest("cscrLocation", objects.get("cscrLocation"));
				buffer.append("and  cscr_location "+(objects.get("cscrLocationYesNot")==null ?"":"not")+" like '%"+objects.get("cscrLocation").toString().replaceAll("'", "''")+"%'");
				objects.remove("cscrLocation");
			}
			...
			if ($.getBoolean("export", false) == true)// 如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/unit/carres.do");
				...
				$.export($.getInteger("size", 1000), 100,
						$.getInteger("page", 0), new Export() {// new
							...
							public Page getPage(Integer page, int size) {// 这时的page和size是Export转化过后的当前页，每页查询数
								return csCarResService.getCsCarResPage(page,
										size, finalParams);
							}
						}, CsCarRes.class, $.getArray("keys"));
			...
			// 调用Service获取分页
			Page page = csCarResService.getCsCarResPage(
	...
	public String edit() {
		...
		try {
			...
			$.setSession("csCarResToken", $.uuid());// 防重复提交Token
			// 当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id = $.getLong("id");
			if (id != null) {
				// 把当前要修改的数据查出来,注意了，CsCarRes这个对象为什么不用$.setRequest,因为CsCarRes有get,set方法，由struts处理的
				csCarRes = csCarResService.getCsCarResById(id);
				// 确认域标识
				if (SystemHelper.testHost(csCarRes.getCscrHost()).equals("0")) {
					return $.SendHtml(
			...
			if (csCarRes == null)
				csCarRes = new CsCarRes();
			// 根据自定义配置ctrl中配置的默认值信息设置默认值
			if (CTRL != null)
				CTRL.setObjectDefaultValue(csCarRes);
			/************ LAZY3Q_CODE_EDIT ************/
			/************ LAZY3Q_CODE_EDIT ************/
	...
	public String save() {
		...
		try {
			...
			final Boolean bIsAdd = ($.empty($.getString("ids"))
					&& $.empty($.getString("PARAMS")) && csCarRes.getCscrId() == null);
			// Token技术建议百度下，就是为了防止重复提交
			final String requestCsCarResToken = $.getString("csCarResToken");
			final String sessionCsCarResToken = $.getSession("csCarResToken");
			if ($.equals(requestCsCarResToken, sessionCsCarResToken)) {
				$.removeSession("csCarResToken");// 移除令牌
				// 多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csCarResService.executeTransaction(new Function() {
					public <T> T execute(Object... arg0) {
						...
						if (bIsAdd) {
							// 批量插入处理
							String vValue = ""
									+ (csCarRes.getCscrCarNo() == null ? ""
											: csCarRes.getCscrCarNo());
							...
							for (String sItem : sArray) {
								if (!$.empty(sItem))
								csCarRes.setCscrCarNo(String.valueOf(sItem));
									// 设置默认值
								if (csCarRes.getCscrAddTime() == null)
										csCarRes.setCscrAddTime(new Date());
								
								if(csCarRes.getCscrUpdateTime() == null)
										csCarRes.setCscrUpdateTime(new Date());
								...
								if (SystemHelper.testHost(
										csCarRes.getCscrHost()).equals("0")) {
									throw new RuntimeException("非法指定域值");
								}

								csCarRes = csCarResService
										.saveCsCarRes(csCarRes);

								on("save", csCarRes, csCarRes);
							}

							csCarResService.updateCsCarRes$NotNull(csCarRes);
							/************ LAZY3Q_CODE_ADD ************/
							/************ LAZY3Q_CODE_ADD ************/

							// 记录操作日志
							LoggerHelper.writeLog(CsCarRes.class, "add",
									"添加了[专车信息][" + csCarRes.getCscrCarNo()
											+ "]",
									(Long) $.getSession("ccclubs_login_id"),
									csCarRes, csCarRes.getCscrId());
							$.SetTips("保存专车信息成功");
						} else {
							...
							if (!$.empty(PARAMS)) {// 根据查询条件批量更新
								...
								csCarRes.setCscrUpdateTime(new Date());
								csCarResService.updateCsCarResByConfirm(
										ActionHelper
												.getSetValuesFromModel(csCarRes),
										params);
								...
								for (String sid : array) {
									csCarRes.setCscrId(Long.valueOf(sid));
									csCarRes.setCscrUpdateTime(new Date());
									CsCarRes oldCsCarRes = CsCarRes.get(Long
											.valueOf(sid));
									csCarResService
											.updateCsCarRes$NotNull(csCarRes);
									LoggerHelper.writeLog(
											CsCarRes.class,
											"update",
											"修改了[专车信息]["
											...
											LoggerHelper.getUpdateDescription(
													oldCsCarRes, csCarRes,
													false), Long.valueOf(sid));
									on($.getString("method"), csCarRes,
											oldCsCarRes);
								}
							} else {
								if ($.empty($.getString("edittype"))) {
									// 设置填充值
									csCarRes.setCscrUpdateTime(new Date());
									CsCarRes oldCsCarRes = CsCarRes
											.get(csCarRes.getCscrId());
									csCarResService
											.updateCsCarRes$NotNull(csCarRes);
									LoggerHelper.writeLog(
											CsCarRes.class,
											"update",
											"修改了[专车信息]["
													+ oldCsCarRes.getCscrCarNo()
													+ "]",
											(Long) $.getSession("ccclubs_login_id"),
											LoggerHelper.getUpdateDescription(
													oldCsCarRes, csCarRes,
													false), csCarRes.getCscrId());
									on($.getString("method"), csCarRes,
											oldCsCarRes);
								} else {
									CsCarRes oldCsCarRes = csCarResService
											.getCsCarResById(csCarRes.getCscrId());
									// 设置默认值

									csCarRes.setCscrUpdateTime(new Date());
									csCarRes.setCscrAddTime(oldCsCarRes
											.getCscrAddTime());
									...
									// 从oldCsCarRes同步CsCarRes未set过的值
									csCarRes.mergeSet(oldCsCarRes);

									csCarResService.updateCsCarRes(csCarRes);
									LoggerHelper.writeLog(
											CsCarRes.class,
											"update",
											"修改了[用车人员]["
											...
											LoggerHelper
													.getUpdateDescription(
															...
															csCarRes, true),
															csCarRes.getCscrId());
									on("update", csCarRes, oldCsCarRes);
								}
							}
							/************ LAZY3Q_CODE_UPDATE ************/
							/************ LAZY3Q_CODE_UPDATE ************/
			...
			if ($.getString("submiter", "").indexOf("保存") > -1) {// 则回到编辑页
				if (bIsAdd)
					return $.Redirect("carres_edit.do?entrypoint="
	 ...
	 * @return
	 */
	public void on(String type, CsCarRes object, CsCarRes old) {
		try {
			// 执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsCarRes.class, type, object, old);
			/************ LAZY3Q_ACTION_ON ************/
			/************ LAZY3Q_ACTION_ON ************/
		...
		try {
			...
			csCarResService.executeTransaction(new Function() {
				public <T> T execute(Object... arg0) {
					...
					for (Long id : ids) {
						CsCarRes csCarRes = csCarResService.getCsCarResById(id);
						// 确认域标识
						if (SystemHelper.testHost(csCarRes.getCscrHost())
								.equals("0")) {
							throw new RuntimeException("当前用户没有权限删除该数据");
						}
						if (ConfigHelper.GetBoolean("logic_delete") == true)
							csCarResService.removeCsCarResById(id);
						else
							csCarResService.deleteCsCarResById(id);
						// 记录操作日志
						if (csCarRes != null)
							LoggerHelper.writeLog(CsDriver.class, "del",
									"删除了[用车人员][" + csCarRes.getCscrCarNo() + "]",
									(Long) $.getSession("ccclubs_login_id"),
									csCarRes);
						on("delete", csCarRes, csCarRes);
					}
					/************ LAZY3Q_CODE_DEL ************/
					/************ LAZY3Q_CODE_DEL ************/
	...
	public String details() {
		...
		try {
			...
			if (id != null)
				csCarRes = csCarResService.getCsCarResById(id);
			if (key != null && id == null)
				csCarRes = CsCarRes.Get($.add(CsCarRes.F.cscrId,
						key));
			// 确认域标识
			if (SystemHelper.testHost(csCarRes.getCscrHost()).equals("0")) {
	 ...
	 * @return
	 */
	public String query_carres(){

		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsCarRes.class);
			//强制限制域
			...
			if(!$.empty(strTerm)){
				...
				else//按标识查询，模糊查询请带%
				{
					...
					strDefinex+=" or cscr_car_no like '"+strTerm.replaceAll("'", "''")+"%'";
					if(params.get("cscrOutlets")!=null){
						int csdOutlets = Integer.parseInt(params.get("cscrOutlets").toString());
						params.put("cscrOutlets",csdOutlets);
					}
					
				}//	
				strDefinex="(2=1 "+strDefinex+" and not EXISTS (select csdc_car from cs_driver_car  where csdc_car=cscr_id) and cscr_status = 1)";
			...
			List<CsCarRes> list = csCarResService.getCsCarResPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsCarRes carres:list){
				Map map = new HashMap();
				map.put("value",carres.getCscrId().toString());
				map.put("text", $.js(carres.getCscrCarNo()));
				if(bObject==true)
					map.put("object",carres);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
		...
		try{
			...
			**/
			...
			for(CsCar csCar:list){
				...
				map.put("text", $.js(csCar.getCscNumber()));
				if(bObject==true)
			...
			for(CsCar csCar:list){
				...
				map.put("value",csCar.getCscId().toString());
				map.put("text", $.js(csCar.getCscNumber()));
				map.put("cscrLocation", csCar.get$cscOutlets()==null ? " ": csCar.get$cscOutlets().getCsoAddress());
				map.put("cscmName", csCar.get$cscModel().getCscmName());
				if(bObject==true)
					map.put("object",csCar);
				result.add(map);
			}
	...
	{	
		...
		try{
			...
			objects.put("definex", "(not EXISTS(select cscr.cscr_car from cs_car_res cscr where cscr.cscr_car = csc_id ))");
			...
			//调用Service获取分页
			Page page = csCarService.getCsCarPage($.getInteger("page", 0), $.getInteger("size",10), objects);
			//把分页对象放入request
			$.setRequest("page", page);	
	...
	public ICsCarResService getCsCarResService() {
		return csCarResService;
	}

	public void setCsCarResService(ICsCarResService csCarResService) {
		this.csCarResService = csCarResService;
	}

	public CsCarRes getCsCarRes() {
		return csCarRes;
	}

	public void setCsCarRes(CsCarRes csCarRes) {
		this.csCarRes = csCarRes;
	}
