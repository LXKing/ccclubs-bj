...
public class AirportAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsUnitAirport.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsUnitAirport.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsUnitAirport.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsUnitAirport.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsUnitAirport.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsUnitAirport.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","csua_id");//那么，默认以主键降序，传给Dao层
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/unit/airport.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csUnitAirportService.getCsUnitAirportPage(page, size, finalParams);
					}
				}, CsUnitAirport.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csUnitAirportService.getCsUnitAirportPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csUnitAirportToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csUnitAirport这个对象为什么不用$.setRequest,因为csUnitAirport有get,set方法，由struts处理的
				csUnitAirport=csUnitAirportService.getCsUnitAirportById(id);
				//确认域标识
				if(SystemHelper.testHost(csUnitAirport.getCsuaHost()).equals("0")){
			...
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csUnitAirport);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsUnitAirport object,CsUnitAirport old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsUnitAirport.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csUnitAirport.getCsuaId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsUnitAirportToken = $.getString("csUnitAirportToken");
			final String sessionCsUnitAirportToken = $.getSession("csUnitAirportToken");
			if($.equals(requestCsUnitAirportToken,sessionCsUnitAirportToken)){
				...
				csUnitAirportService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csUnitAirport.getCsuaId()==null?"":csUnitAirport.getCsuaId());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csUnitAirport.setCsuaId(Long.valueOf(sItem));
								...
								on("save",csUnitAirport,csUnitAirport);
							}							
							...
							//记录操作日志
							LoggerHelper.writeLog(CsUnitAirport.class,"add","添加了[机场服务]["+csUnitAirport.getCsuaId()+"]",(Long)$.getSession("ccclubs_login_id"), csUnitAirport,csUnitAirport.getCsuaId());
							$.SetTips("保存机场服务成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csUnitAirportService.updateCsUnitAirportByConfirm(ActionHelper.getSetValuesFromModel(csUnitAirport), params);
								LoggerHelper.writeLog(CsUnitAirport.class, "update", 
											"批量修改了[机场服务]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csUnitAirport,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csUnitAirport.setCsuaId(Long.valueOf(sid));
									CsUnitAirport oldCsUnitAirport = CsUnitAirport.get(Long.valueOf(sid));
									csUnitAirportService.updateCsUnitAirport$NotNull(csUnitAirport);
									LoggerHelper.writeLog(CsUnitAirport.class, "update", 
											"修改了[机场服务]["+oldCsUnitAirport.getCsuaId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsUnitAirport, csUnitAirport,false)
											,Long.valueOf(sid));
									on($.getString("method"),csUnitAirport,oldCsUnitAirport);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsUnitAirport oldCsUnitAirport = CsUnitAirport.get(csUnitAirport.getCsuaId());
									csUnitAirportService.updateCsUnitAirport$NotNull(csUnitAirport);
									LoggerHelper.writeLog(CsUnitAirport.class, "update", 
											"修改了[机场服务]["+oldCsUnitAirport.getCsuaId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsUnitAirport, csUnitAirport,false)
											,csUnitAirport.getCsuaId());
									on($.getString("method"),csUnitAirport,oldCsUnitAirport);									
								}else{
									...
									//从oldCsUnitAirport同步csUnitAirport未set过的值
									csUnitAirport.mergeSet(oldCsUnitAirport);
											
									csUnitAirportService.updateCsUnitAirport(csUnitAirport);
									LoggerHelper.writeLog(CsUnitAirport.class, "update", 
											"修改了[机场服务]["+oldCsUnitAirport.getCsuaId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsUnitAirport, csUnitAirport,true)
											,csUnitAirport.getCsuaId());
									on("update",csUnitAirport,oldCsUnitAirport);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csUnitAirport)+")"+"</script>","UTF-8");
			}
			...
			csUnitAirportService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							...
							//记录操作日志
							if(csUnitAirport!=null)
								LoggerHelper.writeLog(CsUnitAirport.class,"del","删除了[机场服务]["+csUnitAirport.getCsuaId()+"]",(Long)$.getSession("ccclubs_login_id"), csUnitAirport);
							on("delete",csUnitAirport,csUnitAirport);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csUnitAirport=csUnitAirportService.getCsUnitAirportById(id);
			if(key!=null && id==null)
				csUnitAirport=CsUnitAirport.Get($.add(CsUnitAirport.F.csuaId,key));
			//确认域标识
			if(SystemHelper.testHost(csUnitAirport.getCsuaHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsUnitAirport.class);
			//强制限制域
			...
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				params.put("desc","csua_id");//那么，默认以主键降序，传给Dao层
			...
			List<CsUnitAirport> list = csUnitAirportService.getCsUnitAirportPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsUnitAirport csUnitAirport:list){
				Map map = new HashMap();
				map.put("value",csUnitAirport.getCsuaId().toString());
				map.put("text", $.js(csUnitAirport.getKeyValue()));
				if(bObject==true)
