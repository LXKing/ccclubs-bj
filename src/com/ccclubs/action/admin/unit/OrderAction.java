...
public class OrderAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsUnitOrder.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsUnitOrder.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsUnitOrder.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsUnitOrder.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsUnitOrder.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsUnitOrder.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","csuo_id");//那么，默认以主键降序，传给Dao层
			...
			//反强制限制域
			if(objects.get("csuoOrder")!=null && objects.get("csuoOrderNot")==null){
				objects.remove("HOSTS");
			}
			if(objects.get("csuoUnitMember")!=null && objects.get("csuoUnitMemberNot")==null){
				objects.remove("HOSTS");
			}
			if(objects.get("csuoUnitChild")!=null && objects.get("csuoUnitChildNot")==null){
				objects.remove("HOSTS");
			}
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/unit/order.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csUnitOrderService.getCsUnitOrderPage(page, size, finalParams);
					}
				}, CsUnitOrder.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csUnitOrderService.getCsUnitOrderPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csUnitOrderToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csUnitOrder这个对象为什么不用$.setRequest,因为csUnitOrder有get,set方法，由struts处理的
				csUnitOrder=csUnitOrderService.getCsUnitOrderById(id);
				//确认域标识
				if(SystemHelper.testHost(csUnitOrder.getCsuoHost()).equals("0")){
			...
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csUnitOrder);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsUnitOrder object,CsUnitOrder old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsUnitOrder.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csUnitOrder.getCsuoId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsUnitOrderToken = $.getString("csUnitOrderToken");
			final String sessionCsUnitOrderToken = $.getSession("csUnitOrderToken");
			if($.equals(requestCsUnitOrderToken,sessionCsUnitOrderToken)){
				...
				csUnitOrderService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csUnitOrder.getCsuoOrder()==null?"":csUnitOrder.getCsuoOrder());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csUnitOrder.setCsuoOrder(Long.valueOf(sItem));
							
								//设置默认值
								if(csUnitOrder.getCsuoOrder()==null)
								csUnitOrder.setCsuoOrder((long)0);
								if(csUnitOrder.getCsuoSpecial()==null)
								csUnitOrder.setCsuoSpecial((long)0);
								if(csUnitOrder.getCsuoAddTime()==null)
									csUnitOrder.setCsuoAddTime(new Date());
								if(csUnitOrder.getCsuoUpdateTime()==null)
									csUnitOrder.setCsuoUpdateTime(new Date());
								if(csUnitOrder.getCsuoFrom()==null)
									csUnitOrder.setCsuoFrom("admin");
								if(csUnitOrder.getCsuoState()==null)
									csUnitOrder.setCsuoState((short)0);
								...
								csUnitOrder=csUnitOrderService.saveCsUnitOrder(csUnitOrder);			
								
								on("save",csUnitOrder,csUnitOrder);
							}							
							
							csUnitOrderService.updateCsUnitOrder$NotNull(csUnitOrder);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsUnitOrder.class,"add","添加了[企业订单]["+csUnitOrder.getCsuoOrder()+"]",(Long)$.getSession("ccclubs_login_id"), csUnitOrder,csUnitOrder.getCsuoId());
							$.SetTips("保存企业订单成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csUnitOrderService.updateCsUnitOrderByConfirm(ActionHelper.getSetValuesFromModel(csUnitOrder), params);
								LoggerHelper.writeLog(CsUnitOrder.class, "update", 
											"批量修改了[企业订单]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csUnitOrder,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csUnitOrder.setCsuoId(Long.valueOf(sid));
									CsUnitOrder oldCsUnitOrder = CsUnitOrder.get(Long.valueOf(sid));
									csUnitOrderService.updateCsUnitOrder$NotNull(csUnitOrder);
									LoggerHelper.writeLog(CsUnitOrder.class, "update", 
											"修改了[企业订单]["+oldCsUnitOrder.getCsuoOrder()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsUnitOrder, csUnitOrder,false)
											,Long.valueOf(sid));
									on($.getString("method"),csUnitOrder,oldCsUnitOrder);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsUnitOrder oldCsUnitOrder = CsUnitOrder.get(csUnitOrder.getCsuoId());
									csUnitOrderService.updateCsUnitOrder$NotNull(csUnitOrder);
									LoggerHelper.writeLog(CsUnitOrder.class, "update", 
											"修改了[企业订单]["+oldCsUnitOrder.getCsuoOrder()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsUnitOrder, csUnitOrder,false)
											,csUnitOrder.getCsuoId());
									on($.getString("method"),csUnitOrder,oldCsUnitOrder);									
								}else{
									CsUnitOrder oldCsUnitOrder = csUnitOrderService.getCsUnitOrderById(csUnitOrder.getCsuoId());
									//设置默认值
									csUnitOrder.setCsuoAddTime(oldCsUnitOrder.getCsuoAddTime());	
									csUnitOrder.setCsuoUpdateTime(oldCsUnitOrder.getCsuoUpdateTime());	
									csUnitOrder.setCsuoFrom(oldCsUnitOrder.getCsuoFrom());	
									csUnitOrder.setCsuoCheckTime(oldCsUnitOrder.getCsuoCheckTime());	
									csUnitOrder.setCsuoChecker(oldCsUnitOrder.getCsuoChecker());	
									csUnitOrder.setCsuoState(oldCsUnitOrder.getCsuoState());	
									...
									//从oldCsUnitOrder同步csUnitOrder未set过的值
									csUnitOrder.mergeSet(oldCsUnitOrder);
											
									csUnitOrderService.updateCsUnitOrder(csUnitOrder);
									LoggerHelper.writeLog(CsUnitOrder.class, "update", 
											"修改了[企业订单]["+oldCsUnitOrder.getCsuoOrder()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsUnitOrder, csUnitOrder,true)
											,csUnitOrder.getCsuoId());
									on("update",csUnitOrder,oldCsUnitOrder);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csUnitOrder)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("order_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csUnitOrder.getCsuoId());
				else
		...
		try{
			...
			csUnitOrderService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsUnitOrder csUnitOrder = csUnitOrderService.getCsUnitOrderById(id);
							//确认域标识
							if(SystemHelper.testHost(csUnitOrder.getCsuoHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csUnitOrderService.removeCsUnitOrderById(id);
							else
							csUnitOrderService.deleteCsUnitOrderById(id);			
							//记录操作日志
							if(csUnitOrder!=null)
								LoggerHelper.writeLog(CsUnitOrder.class,"del","删除了[企业订单]["+csUnitOrder.getCsuoOrder()+"]",(Long)$.getSession("ccclubs_login_id"), csUnitOrder);
							on("delete",csUnitOrder,csUnitOrder);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csUnitOrder=csUnitOrderService.getCsUnitOrderById(id);
			if(key!=null && id==null)
				csUnitOrder=CsUnitOrder.Get($.add(CsUnitOrder.F.csuoId,key));
			//确认域标识
			if(SystemHelper.testHost(csUnitOrder.getCsuoHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsUnitOrder.class);
			//强制限制域
			params.put("HOSTS",SystemHelper.testHost(null));
			
			//反强制限制域
			if(params.get("csuoOrder")!=null && params.get("csuoOrderNot")==null){
				params.remove("HOSTS");
			}
			if(params.get("csuoUnitMember")!=null && params.get("csuoUnitMemberNot")==null){
				params.remove("HOSTS");
			}
			if(params.get("csuoUnitChild")!=null && params.get("csuoUnitChildNot")==null){
				params.remove("HOSTS");
			}
			...
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				params.put("desc","csuo_id");//那么，默认以主键降序，传给Dao层
								
			String strTerm=$.getString("value");//智能搜索时的参数，一般都是主键后面那个字段
			if(!$.empty(strTerm)){
				if(strTerm.toLowerCase().startsWith("id:"))//如果查询条件以id:开头，则按ID查询
					params.put("csuoId", strTerm.toLowerCase().replaceFirst("id:", ""));					
				else//按标识查询，模糊查询请带%
				{
					String strDefinex = "";
					strDefinex+=" or csuo_order like '"+strTerm.replaceAll("'", "''")+"%'";
					strDefinex+=" or csuo_special like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsUnitOrder> list = csUnitOrderService.getCsUnitOrderPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsUnitOrder csUnitOrder:list){
				Map map = new HashMap();
				map.put("value",csUnitOrder.getCsuoId().toString());
				map.put("text", $.js(csUnitOrder.getKeyValue()));
				if(bObject==true)
					map.put("object",csUnitOrder);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
			...
			if($.equals(requestCsUnitOrderToken,sessionCsUnitOrderToken)){
				...
				csUnitOrderService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						CsUnitOrder oldCsUnitOrder = CsUnitOrder.get(csUnitOrder.getCsuoId());
						CommonUnitService.getBean().executeReUnitOrder(oldCsUnitOrder.getCsuoUnitInfo(), csUnitOrder.getCsuoId(), csUnitOrder.getCsuoFinishTime());
						csUnitOrder.update();
						$.SetTips("订单续订成功");
						/************LAZY3Q_FORM_CODE_reorder************/
	...
	public String cancel()
	{
		try{
			...
			if($.equals(requestCsUnitOrderToken,sessionCsUnitOrderToken)){
				...
				csUnitOrderService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						/************LAZY3Q_FORM_CODE_cancel************/
						CsUnitOrder oldCsUnitOrder = CsUnitOrder.get(csUnitOrder.getCsuoId());
						CommonUnitService.getBean().executeCancelUnitOrder(oldCsUnitOrder.getCsuoUnitInfo(), csUnitOrder.getCsuoId(),csUnitOrder.getCsuoRemark());
						csUnitOrder.update();
						$.SetTips("订单取消成功");
						/************LAZY3Q_FORM_CODE_cancel************/
