...
public class PropertyAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",SrvProperty.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(SrvProperty.class);
			...
			//调用Service取列表
			List<SrvProperty> list = srvPropertyService.getSrvPropertyList(objects,-1);
			//把取回的list排成树形[ORDER_TREE]或层形[ORDER_COAT]
			list=Tree.OrderList(list,"spId", "spParent", Tree.ORDER_TREE);
			//把列表放入request
	...
	public String tree(){
		try{
			...
			//根据父节点取数据列表
			List<SrvProperty> list = srvPropertyService.getSrvPropertyList($.add("spParent",parent),-1);
	...
	{		
		try{
			...
			$.setSession("srvPropertyToken", $.uuid());//防重复提交Token
			{
			//父节点数据ID
			Long parent=$.getLong("parent");
			if(parent!=null)//如果父节点数据ID不为空，把父对象查出来
				$.setRequest("parentSrvProperty", srvPropertyService.getSrvPropertyById(parent));
			}
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，srvProperty这个对象为什么不用$.setRequest,因为srvProperty有get,set方法，由struts处理的
				srvProperty=srvPropertyService.getSrvPropertyById(id);
				//把当前修改的数据的父级数据也查出来，放到request中
				$.setRequest("parentSrvProperty", srvPropertyService.getSrvPropertyById(srvProperty.getSpParent()));
			}
			if(srvProperty==null)
				srvProperty = new SrvProperty();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(srvProperty);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,SrvProperty object,SrvProperty old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(SrvProperty.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && srvProperty.getSpId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestSrvPropertyToken = $.getString("srvPropertyToken");
			final String sessionSrvPropertyToken = $.getSession("srvPropertyToken");
			if($.equals(requestSrvPropertyToken,sessionSrvPropertyToken)){
				$.removeSession("srvPropertyToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				srvPropertyService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(srvProperty.getSpName()==null?"":srvProperty.getSpName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								srvProperty.setSpName(String.valueOf(sItem));
								...
								srvProperty=srvPropertyService.saveSrvProperty(srvProperty);			
								
								on("save",srvProperty,srvProperty);
							}							
							
							srvPropertyService.updateSrvProperty$NotNull(srvProperty);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(SrvProperty.class,"add","添加了[系统属性]["+srvProperty.getSpName()+"]",(Long)$.getSession("ccclubs_login_id"), srvProperty,srvProperty.getSpId());
							$.SetTips("保存系统属性成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								srvPropertyService.updateSrvPropertyByConfirm(ActionHelper.getSetValuesFromModel(srvProperty), params);
								LoggerHelper.writeLog(SrvProperty.class, "update", 
											"批量修改了[系统属性]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,srvProperty,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									srvProperty.setSpId(Long.valueOf(sid));
									SrvProperty oldSrvProperty = SrvProperty.get(Long.valueOf(sid));
									srvPropertyService.updateSrvProperty$NotNull(srvProperty);
									LoggerHelper.writeLog(SrvProperty.class, "update", 
											"修改了[系统属性]["+oldSrvProperty.getSpName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldSrvProperty, srvProperty,false)
											,Long.valueOf(sid));
									on($.getString("method"),srvProperty,oldSrvProperty);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									SrvProperty oldSrvProperty = SrvProperty.get(srvProperty.getSpId());
									srvPropertyService.updateSrvProperty$NotNull(srvProperty);
									LoggerHelper.writeLog(SrvProperty.class, "update", 
											"修改了[系统属性]["+oldSrvProperty.getSpName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldSrvProperty, srvProperty,false)
									...
									SrvProperty oldSrvProperty = srvPropertyService.getSrvPropertyById(srvProperty.getSpId());
									//设置默认值
									
									//设置填充值
									
									//从oldSrvProperty同步srvProperty未set过的值
									srvProperty.mergeSet(oldSrvProperty);
											
									srvPropertyService.updateSrvProperty(srvProperty);
									LoggerHelper.writeLog(SrvProperty.class, "update", 
											"修改了[系统属性]["+oldSrvProperty.getSpName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldSrvProperty, srvProperty,true)
											,srvProperty.getSpId());
									on("update",srvProperty,oldSrvProperty);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(srvProperty)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("property_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+srvProperty.getSpId());
				else
	...
	public String del()
	{
		try{
			...
			SystemHelper.getSrvLimit("admin/configurator/property.do");
					/************LAZY3Q_CAN_DEL************/
					/************LAZY3Q_CAN_DEL************/
			...
			srvPropertyService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							SrvProperty srvProperty = srvPropertyService.getSrvPropertyById(id);
							srvPropertyService.deleteSrvPropertyById(id);			
							//记录操作日志
							if(srvProperty!=null)
								LoggerHelper.writeLog(SrvProperty.class,"del","删除了[系统属性]["+srvProperty.getSpName()+"]",(Long)$.getSession("ccclubs_login_id"), srvProperty);
							on("delete",srvProperty,srvProperty);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				srvProperty=srvPropertyService.getSrvPropertyById(id);
			if(key!=null && id==null)
				srvProperty=SrvProperty.Get($.add(SrvProperty.F.spId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			...
			Map<String,Object> params = ActionHelper.getQueryFormParams(SrvProperty.class);
			...
			List<SrvProperty> list = srvPropertyService.getSrvPropertyPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(SrvProperty srvProperty:list){
				Map map = new HashMap();
				map.put("value",srvProperty.getSpId().toString());
				map.put("text", $.js(srvProperty.getKeyValue()));
				if(bObject==true)
	...
	public SrvProperty getSrvProperty()
	{
		return srvProperty;
	}

	public void setSrvProperty(SrvProperty srvProperty)
	{
		this.srvProperty = srvProperty;
	}
}