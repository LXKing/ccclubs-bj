...
public class ArgumentAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsArgument.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsArgument.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsArgument.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsArgument.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsArgument.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsArgument.C.get($.forMat($.getString("desc")));//降序字段
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/configurator/argument.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csArgumentService.getCsArgumentPage(page, size, finalParams);
					}
				}, CsArgument.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csArgumentService.getCsArgumentPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csArgumentToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csArgument这个对象为什么不用$.setRequest,因为csArgument有get,set方法，由struts处理的
				csArgument=csArgumentService.getCsArgumentById(id);
			}
			if(csArgument==null)
				csArgument = new CsArgument();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csArgument);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsArgument object,CsArgument old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsArgument.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csArgument.getCsaId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsArgumentToken = $.getString("csArgumentToken");
			final String sessionCsArgumentToken = $.getSession("csArgumentToken");
			if($.equals(requestCsArgumentToken,sessionCsArgumentToken)){
				$.removeSession("csArgumentToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csArgumentService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csArgument.getCsaName()==null?"":csArgument.getCsaName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csArgument.setCsaName(String.valueOf(sItem));
							
								//设置默认值
								if(csArgument.getCsaName()==null)
								csArgument.setCsaName("");
								if(csArgument.getCsaFlag()==null)
								csArgument.setCsaFlag("");
								if(csArgument.getCsaValue()==null)
								csArgument.setCsaValue("");
								
								//根据自定义的默认值信息设置默认值
								if(CTRL!=null)
									CTRL.setObjectDefaultValue(csArgument);
								
								csArgument=csArgumentService.saveCsArgument(csArgument);			
								
								on("save",csArgument,csArgument);
							}							
							
							csArgumentService.updateCsArgument$NotNull(csArgument);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsArgument.class,"add","添加了[系统参数]["+csArgument.getCsaName()+"]",(Long)$.getSession("ccclubs_login_id"), csArgument,csArgument.getCsaId());
							$.SetTips("保存系统参数成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csArgumentService.updateCsArgumentByConfirm(ActionHelper.getSetValuesFromModel(csArgument), params);
								LoggerHelper.writeLog(CsArgument.class, "update", 
											"批量修改了[系统参数]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csArgument,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csArgument.setCsaId(Long.valueOf(sid));
									CsArgument oldCsArgument = CsArgument.get(Long.valueOf(sid));
									csArgumentService.updateCsArgument$NotNull(csArgument);
									LoggerHelper.writeLog(CsArgument.class, "update", 
											"修改了[系统参数]["+oldCsArgument.getCsaName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsArgument, csArgument,false)
											,Long.valueOf(sid));
									on($.getString("method"),csArgument,oldCsArgument);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									CsArgument oldCsArgument = CsArgument.get(csArgument.getCsaId());
									csArgumentService.updateCsArgument$NotNull(csArgument);
									LoggerHelper.writeLog(CsArgument.class, "update", 
											"修改了[系统参数]["+oldCsArgument.getCsaName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsArgument, csArgument,false)
									...
									CsArgument oldCsArgument = csArgumentService.getCsArgumentById(csArgument.getCsaId());
									//设置默认值
									csArgument.setCsaName(oldCsArgument.getCsaName());	
									csArgument.setCsaFlag(oldCsArgument.getCsaFlag());	
									
									//设置填充值
									
									//从oldCsArgument同步csArgument未set过的值
									csArgument.mergeSet(oldCsArgument);
											
									csArgumentService.updateCsArgument(csArgument);
									LoggerHelper.writeLog(CsArgument.class, "update", 
											"修改了[系统参数]["+oldCsArgument.getCsaName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsArgument, csArgument,true)
											,csArgument.getCsaId());
									on("update",csArgument,oldCsArgument);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csArgument)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("argument_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csArgument.getCsaId());
				else
		...
		try{
			...
			csArgumentService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsArgument csArgument = csArgumentService.getCsArgumentById(id);
							csArgumentService.deleteCsArgumentById(id);			
							//记录操作日志
							if(csArgument!=null)
								LoggerHelper.writeLog(CsArgument.class,"del","删除了[系统参数]["+csArgument.getCsaName()+"]",(Long)$.getSession("ccclubs_login_id"), csArgument);
							on("delete",csArgument,csArgument);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csArgument=csArgumentService.getCsArgumentById(id);
			if(key!=null && id==null)
				csArgument=CsArgument.Get($.add(CsArgument.F.csaId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsArgument.class);
			...
			if(!$.empty(strTerm)){
				...
				{
					String strDefinex = "";
					strDefinex+=" or csa_name like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsArgument> list = csArgumentService.getCsArgumentPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsArgument csArgument:list){
				Map map = new HashMap();
				map.put("value",csArgument.getCsaId().toString());
				map.put("text", $.js(csArgument.getKeyValue()));
				if(bObject==true)
					map.put("object",csArgument);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public CsArgument getCsArgument()
	{
		return csArgument;
	}

	public void setCsArgument(CsArgument csArgument)
	{
		this.csArgument = csArgument;
	}
}