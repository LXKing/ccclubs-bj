...
public class AreaAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsArea.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsArea.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsArea.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsArea.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsArea.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsArea.C.get($.forMat($.getString("desc")));//降序字段
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/configurator/area.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csAreaService.getCsAreaPage(page, size, finalParams);
					}
				}, CsArea.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csAreaService.getCsAreaPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csAreaToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csArea这个对象为什么不用$.setRequest,因为csArea有get,set方法，由struts处理的
				csArea=csAreaService.getCsAreaById(id);
				//确认域标识
				if(SystemHelper.testHost(csArea.getCsaHost()).equals("0")){
			...
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csArea);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsArea object,CsArea old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsArea.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csArea.getCsaId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsAreaToken = $.getString("csAreaToken");
			final String sessionCsAreaToken = $.getSession("csAreaToken");
			if($.equals(requestCsAreaToken,sessionCsAreaToken)){
				$.removeSession("csAreaToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csAreaService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csArea.getCsaName()==null?"":csArea.getCsaName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csArea.setCsaName(String.valueOf(sItem));
								...
								csArea=csAreaService.saveCsArea(csArea);			
								
								on("save",csArea,csArea);
							}							
							
							csAreaService.updateCsArea$NotNull(csArea);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsArea.class,"add","添加了[网点区域]["+csArea.getCsaName()+"]",(Long)$.getSession("ccclubs_login_id"), csArea,csArea.getCsaId());
							$.SetTips("保存网点区域成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csAreaService.updateCsAreaByConfirm(ActionHelper.getSetValuesFromModel(csArea), params);
								LoggerHelper.writeLog(CsArea.class, "update", 
											"批量修改了[网点区域]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csArea,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csArea.setCsaId(Long.valueOf(sid));
									CsArea oldCsArea = CsArea.get(Long.valueOf(sid));
									csAreaService.updateCsArea$NotNull(csArea);
									LoggerHelper.writeLog(CsArea.class, "update", 
											"修改了[网点区域]["+oldCsArea.getCsaName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsArea, csArea,false)
											,Long.valueOf(sid));
									on($.getString("method"),csArea,oldCsArea);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									CsArea oldCsArea = CsArea.get(csArea.getCsaId());
									csAreaService.updateCsArea$NotNull(csArea);
									LoggerHelper.writeLog(CsArea.class, "update", 
											"修改了[网点区域]["+oldCsArea.getCsaName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsArea, csArea,false)
									...
									CsArea oldCsArea = csAreaService.getCsAreaById(csArea.getCsaId());
									//设置默认值
									...
									//从oldCsArea同步csArea未set过的值
									csArea.mergeSet(oldCsArea);
											
									csAreaService.updateCsArea(csArea);
									LoggerHelper.writeLog(CsArea.class, "update", 
											"修改了[网点区域]["+oldCsArea.getCsaName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsArea, csArea,true)
											,csArea.getCsaId());
									on("update",csArea,oldCsArea);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csArea)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("area_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csArea.getCsaId());
				else
		...
		try{
			...
			csAreaService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsArea csArea = csAreaService.getCsAreaById(id);
							//确认域标识
							if(SystemHelper.testHost(csArea.getCsaHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csAreaService.removeCsAreaById(id);
							else
							csAreaService.deleteCsAreaById(id);			
							//记录操作日志
							if(csArea!=null)
								LoggerHelper.writeLog(CsArea.class,"del","删除了[网点区域]["+csArea.getCsaName()+"]",(Long)$.getSession("ccclubs_login_id"), csArea);
							on("delete",csArea,csArea);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csArea=csAreaService.getCsAreaById(id);
			if(key!=null && id==null)
				csArea=CsArea.Get($.add(CsArea.F.csaId,key));
			//确认域标识
			if(SystemHelper.testHost(csArea.getCsaHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsArea.class);
			//强制限制域
			...
			List<CsArea> list = csAreaService.getCsAreaPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsArea csArea:list){
				Map map = new HashMap();
				map.put("value",csArea.getCsaId().toString());
				map.put("text", $.js(csArea.getKeyValue()));
				if(bObject==true)
					map.put("object",csArea);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public ICsAreaService getCsAreaService()
	{
		return csAreaService;
	}

	public void setCsAreaService(ICsAreaService csAreaService)
	{
		this.csAreaService = csAreaService;
	}

	public CsArea getCsArea()
	{
		return csArea;
	}

	public void setCsArea(CsArea csArea)
	{
		this.csArea = csArea;
	}
}