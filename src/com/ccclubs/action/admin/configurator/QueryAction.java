...
{
	ISrvQueryService srvQueryService;
	...
	{	
		try{
			...
			$.setRequest("modelClass",SrvQuery.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(SrvQuery.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(SrvQuery.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(SrvQuery.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=SrvQuery.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=SrvQuery.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","sq_id");//那么，默认以主键降序，传给Dao层
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/configurator/query.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return srvQueryService.getSrvQueryPage(page, size, finalParams);
					}
				}, SrvQuery.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = srvQueryService.getSrvQueryPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("srvQueryToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，srvQuery这个对象为什么不用$.setRequest,因为srvQuery有get,set方法，由struts处理的
				srvQuery=srvQueryService.getSrvQueryById(id);
			}
			if(srvQuery==null)
				srvQuery = new SrvQuery();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(srvQuery);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,SrvQuery object,SrvQuery old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(SrvQuery.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && srvQuery.getSqId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestSrvQueryToken = $.getString("srvQueryToken");
			final String sessionSrvQueryToken = $.getSession("srvQueryToken");
			if($.equals(requestSrvQueryToken,sessionSrvQueryToken)){
				$.removeSession("srvQueryToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				srvQueryService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(srvQuery.getSqName()==null?"":srvQuery.getSqName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								srvQuery.setSqName(String.valueOf(sItem));
							
								//设置默认值
								if(srvQuery.getSqEditor()==null)
									srvQuery.setSqEditor(LoginHelper.getLoginId());
								if(srvQuery.getSqUpdateTime()==null)
									srvQuery.setSqUpdateTime(new Date());
								if(srvQuery.getSqAddTime()==null)
									srvQuery.setSqAddTime(new Date());
								if(srvQuery.getSqStatus()==null)
									srvQuery.setSqStatus((short)1);
								
								//根据自定义的默认值信息设置默认值
								if(CTRL!=null)
									CTRL.setObjectDefaultValue(srvQuery);
								
								srvQuery=srvQueryService.saveSrvQuery(srvQuery);			
								
								on("save",srvQuery,srvQuery);
							}							
							
							srvQueryService.updateSrvQuery$NotNull(srvQuery);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(SrvQuery.class,"add","添加了[表单查询]["+srvQuery.getSqName()+"]",(Long)$.getSession("ccclubs_login_id"), srvQuery,srvQuery.getSqId());
							$.SetTips("保存表单查询成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								srvQueryService.updateSrvQueryByConfirm(ActionHelper.getSetValuesFromModel(srvQuery), params);
								LoggerHelper.writeLog(SrvQuery.class, "update", 
											"批量修改了[表单查询]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,srvQuery,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									srvQuery.setSqId(Long.valueOf(sid));
									SrvQuery oldSrvQuery = SrvQuery.get(Long.valueOf(sid));
									srvQueryService.updateSrvQuery$NotNull(srvQuery);
									LoggerHelper.writeLog(SrvQuery.class, "update", 
											"修改了[表单查询]["+oldSrvQuery.getSqName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldSrvQuery, srvQuery,false)
											,Long.valueOf(sid));
									on($.getString("method"),srvQuery,oldSrvQuery);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									SrvQuery oldSrvQuery = SrvQuery.get(srvQuery.getSqId());
									srvQueryService.updateSrvQuery$NotNull(srvQuery);
									LoggerHelper.writeLog(SrvQuery.class, "update", 
											"修改了[表单查询]["+oldSrvQuery.getSqName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldSrvQuery, srvQuery,false)
									...
									SrvQuery oldSrvQuery = srvQueryService.getSrvQueryById(srvQuery.getSqId());
									//设置默认值
									srvQuery.setSqUpdateTime(oldSrvQuery.getSqUpdateTime());	
									srvQuery.setSqAddTime(oldSrvQuery.getSqAddTime());	
									...
									//从oldSrvQuery同步srvQuery未set过的值
									srvQuery.mergeSet(oldSrvQuery);
											
									srvQueryService.updateSrvQuery(srvQuery);
									LoggerHelper.writeLog(SrvQuery.class, "update", 
											"修改了[表单查询]["+oldSrvQuery.getSqName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldSrvQuery, srvQuery,true)
											,srvQuery.getSqId());
									on("update",srvQuery,oldSrvQuery);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(srvQuery)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("query_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+srvQuery.getSqId());
				else
	...
	public String del()
	{
		try{
			...
			SystemHelper.getSrvLimit("admin/configurator/query.do");
			/************LAZY3Q_CAN_DEL************/
			$.setRequest("canDel", true);
			Long pid=$.getLong("id");
			if(pid!=null && SrvQuery.get(pid).getSqEditor().longValue()!=LoginHelper.getLoginId().longValue()){
			...
			srvQueryService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							SrvQuery srvQuery = srvQueryService.getSrvQueryById(id);
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								srvQueryService.removeSrvQueryById(id);
							else
							srvQueryService.deleteSrvQueryById(id);			
							//记录操作日志
							if(srvQuery!=null)
								LoggerHelper.writeLog(SrvQuery.class,"del","删除了[表单查询]["+srvQuery.getSqName()+"]",(Long)$.getSession("ccclubs_login_id"), srvQuery);
							on("delete",srvQuery,srvQuery);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				srvQuery=srvQueryService.getSrvQueryById(id);
			if(key!=null && id==null)
				srvQuery=SrvQuery.Get($.add(SrvQuery.F.sqId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(SrvQuery.class);
			...
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				params.put("desc","sq_id");//那么，默认以主键降序，传给Dao层
			...
			List<SrvQuery> list = srvQueryService.getSrvQueryPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(SrvQuery srvQuery:list){
				Map map = new HashMap();
				map.put("value",srvQuery.getSqId().toString());
				map.put("text", $.js(srvQuery.getKeyValue()));
				if(bObject==true)
	...
	public ISrvQueryService getSrvQueryService()
	{
		return srvQueryService;
	}

	public void setSrvQueryService(ISrvQueryService srvQueryService)
	{
		this.srvQueryService = srvQueryService;
	}

	public SrvQuery getSrvQuery()
	{
		return srvQuery;
	}

	public void setSrvQuery(SrvQuery srvQuery)
	{
