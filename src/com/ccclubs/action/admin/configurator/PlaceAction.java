...
{
	ICsPlaceService csPlaceService;
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsPlace.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsPlace.class);
			...
			//调用Service取列表
			List<CsPlace> list = csPlaceService.getCsPlaceList(objects,-1);
			//把取回的list排成树形[ORDER_TREE]或层形[ORDER_COAT]
			list=Tree.OrderList(list,"cspId", "cspParent", Tree.ORDER_TREE);
			//把列表放入request
	...
	public String tree(){
		try{
			...
			//根据父节点取数据列表
			List<CsPlace> list = csPlaceService.getCsPlaceList($.add("cspParent",parent),-1);
			...
			**/
			...
			for(CsPlace o:list){
				if(!strAjax.equals(""))
					strAjax+=",";
				strAjax+="{";
				strAjax+="\"value\":\""+o.getCspId()+"\"";
				strAjax+=",";
				strAjax+="\"text\":\""+$.js(o.getCspName())+"\"";
				strAjax+=",";
				strAjax+="\"more\":true";
				strAjax+="}";
			}
	...
	{		
		try{
			...
			$.setSession("csPlaceToken", $.uuid());//防重复提交Token
			{
			//父节点数据ID
			Long parent=$.getLong("parent");
			if(parent!=null)//如果父节点数据ID不为空，把父对象查出来
				$.setRequest("parentCsPlace", csPlaceService.getCsPlaceById(parent));
			}
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csPlace这个对象为什么不用$.setRequest,因为csPlace有get,set方法，由struts处理的
				csPlace=csPlaceService.getCsPlaceById(id);
				//把当前修改的数据的父级数据也查出来，放到request中
				$.setRequest("parentCsPlace", csPlaceService.getCsPlaceById(csPlace.getCspParent()));
			}
			if(csPlace==null)
				csPlace = new CsPlace();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csPlace);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsPlace object,CsPlace old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsPlace.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csPlace.getCspId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsPlaceToken = $.getString("csPlaceToken");
			final String sessionCsPlaceToken = $.getSession("csPlaceToken");
			if($.equals(requestCsPlaceToken,sessionCsPlaceToken)){
				$.removeSession("csPlaceToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csPlaceService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csPlace.getCspName()==null?"":csPlace.getCspName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csPlace.setCspName(String.valueOf(sItem));
								...
								csPlace=csPlaceService.saveCsPlace(csPlace);			
								
								on("save",csPlace,csPlace);
							}							
							
							csPlaceService.updateCsPlace$NotNull(csPlace);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsPlace.class,"add","添加了[省市地区]["+csPlace.getCspName()+"]",(Long)$.getSession("ccclubs_login_id"), csPlace,csPlace.getCspId());
							$.SetTips("保存省市地区成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csPlaceService.updateCsPlaceByConfirm(ActionHelper.getSetValuesFromModel(csPlace), params);
								LoggerHelper.writeLog(CsPlace.class, "update", 
											"批量修改了[省市地区]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csPlace,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csPlace.setCspId(Long.valueOf(sid));
									CsPlace oldCsPlace = CsPlace.get(Long.valueOf(sid));
									csPlaceService.updateCsPlace$NotNull(csPlace);
									LoggerHelper.writeLog(CsPlace.class, "update", 
											"修改了[省市地区]["+oldCsPlace.getCspName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsPlace, csPlace,false)
											,Long.valueOf(sid));
									on($.getString("method"),csPlace,oldCsPlace);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									CsPlace oldCsPlace = CsPlace.get(csPlace.getCspId());
									csPlaceService.updateCsPlace$NotNull(csPlace);
									LoggerHelper.writeLog(CsPlace.class, "update", 
											"修改了[省市地区]["+oldCsPlace.getCspName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsPlace, csPlace,false)
									...
									CsPlace oldCsPlace = csPlaceService.getCsPlaceById(csPlace.getCspId());
									//设置默认值
									
									//设置填充值
									
									//从oldCsPlace同步csPlace未set过的值
									csPlace.mergeSet(oldCsPlace);
											
									csPlaceService.updateCsPlace(csPlace);
									LoggerHelper.writeLog(CsPlace.class, "update", 
											"修改了[省市地区]["+oldCsPlace.getCspName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsPlace, csPlace,true)
											,csPlace.getCspId());
									on("update",csPlace,oldCsPlace);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csPlace)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("place_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csPlace.getCspId());
				else
	...
	public String del()
	{
		try{
			...
			SystemHelper.getSrvLimit("admin/configurator/place.do");
					/************LAZY3Q_CAN_DEL************/
					/************LAZY3Q_CAN_DEL************/
			...
			csPlaceService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsPlace csPlace = csPlaceService.getCsPlaceById(id);
							csPlaceService.deleteCsPlaceById(id);			
							//记录操作日志
							if(csPlace!=null)
								LoggerHelper.writeLog(CsPlace.class,"del","删除了[省市地区]["+csPlace.getCspName()+"]",(Long)$.getSession("ccclubs_login_id"), csPlace);
							on("delete",csPlace,csPlace);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csPlace=csPlaceService.getCsPlaceById(id);
			if(key!=null && id==null)
				csPlace=CsPlace.Get($.add(CsPlace.F.cspId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			...
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsPlace.class);
			...
			List<CsPlace> list = csPlaceService.getCsPlacePage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsPlace csPlace:list){
				Map map = new HashMap();
				map.put("value",csPlace.getCspId().toString());
				map.put("text", $.js(csPlace.getKeyValue()));
				if(bObject==true)
					map.put("object",csPlace);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public ICsPlaceService getCsPlaceService()
	{
		return csPlaceService;
	}

	public void setCsPlaceService(ICsPlaceService csPlaceService)
	{
		this.csPlaceService = csPlaceService;
	}

	public CsPlace getCsPlace()
	{
		return csPlace;
	}

	public void setCsPlace(CsPlace csPlace)
	{
		this.csPlace = csPlace;
	}
}