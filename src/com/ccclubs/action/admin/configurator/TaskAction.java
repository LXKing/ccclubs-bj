...
public class TaskAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsTask.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsTask.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsTask.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsTask.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsTask.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsTask.C.get($.forMat($.getString("desc")));//降序字段
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/configurator/task.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csTaskService.getCsTaskPage(page, size, finalParams);
					}
				}, CsTask.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csTaskService.getCsTaskPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csTaskToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csTask这个对象为什么不用$.setRequest,因为csTask有get,set方法，由struts处理的
				csTask=csTaskService.getCsTaskById(id);
			}
			if(csTask==null)
				csTask = new CsTask();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csTask);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsTask object,CsTask old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsTask.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csTask.getCstId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsTaskToken = $.getString("csTaskToken");
			final String sessionCsTaskToken = $.getSession("csTaskToken");
			if($.equals(requestCsTaskToken,sessionCsTaskToken)){
				$.removeSession("csTaskToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csTaskService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csTask.getCstName()==null?"":csTask.getCstName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csTask.setCstName(String.valueOf(sItem));
							
								//设置默认值
								if(csTask.getCstEditTime()==null)
									csTask.setCstEditTime(new Date());
								
								//根据自定义的默认值信息设置默认值
								if(CTRL!=null)
									CTRL.setObjectDefaultValue(csTask);
								
								csTask=csTaskService.saveCsTask(csTask);			
								
								on("save",csTask,csTask);
							}							
							
							csTaskService.updateCsTask$NotNull(csTask);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsTask.class,"add","添加了[数据任务]["+csTask.getCstName()+"]",(Long)$.getSession("ccclubs_login_id"), csTask,csTask.getCstId());
							$.SetTips("保存数据任务成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csTaskService.updateCsTaskByConfirm(ActionHelper.getSetValuesFromModel(csTask), params);
								LoggerHelper.writeLog(CsTask.class, "update", 
											"批量修改了[数据任务]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csTask,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csTask.setCstId(Long.valueOf(sid));
									CsTask oldCsTask = CsTask.get(Long.valueOf(sid));
									csTaskService.updateCsTask$NotNull(csTask);
									LoggerHelper.writeLog(CsTask.class, "update", 
											"修改了[数据任务]["+oldCsTask.getCstName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsTask, csTask,false)
											,Long.valueOf(sid));
									on($.getString("method"),csTask,oldCsTask);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsTask oldCsTask = CsTask.get(csTask.getCstId());
									csTaskService.updateCsTask$NotNull(csTask);
									LoggerHelper.writeLog(CsTask.class, "update", 
											"修改了[数据任务]["+oldCsTask.getCstName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsTask, csTask,false)
									...
									CsTask oldCsTask = csTaskService.getCsTaskById(csTask.getCstId());
									//设置默认值
									csTask.setCstEditTime(oldCsTask.getCstEditTime());	
									
									//设置填充值
									if(csTask.getCstEditTime()==null)
										csTask.setCstEditTime(new Date());
									
									//从oldCsTask同步csTask未set过的值
									csTask.mergeSet(oldCsTask);
											
									csTaskService.updateCsTask(csTask);
									LoggerHelper.writeLog(CsTask.class, "update", 
											"修改了[数据任务]["+oldCsTask.getCstName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsTask, csTask,true)
											,csTask.getCstId());
									on("update",csTask,oldCsTask);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csTask)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("task_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csTask.getCstId());
				else
		...
		try{
			...
			csTaskService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsTask csTask = csTaskService.getCsTaskById(id);
							csTaskService.deleteCsTaskById(id);			
							//记录操作日志
							if(csTask!=null)
								LoggerHelper.writeLog(CsTask.class,"del","删除了[数据任务]["+csTask.getCstName()+"]",(Long)$.getSession("ccclubs_login_id"), csTask);
							on("delete",csTask,csTask);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csTask=csTaskService.getCsTaskById(id);
			if(key!=null && id==null)
				csTask=CsTask.Get($.add(CsTask.F.cstId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsTask.class);
			...
			if(!$.empty(strTerm)){
				...
				{
					String strDefinex = "";
					strDefinex+=" or cst_name like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsTask> list = csTaskService.getCsTaskPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsTask csTask:list){
				Map map = new HashMap();
				map.put("value",csTask.getCstId().toString());
				map.put("text", $.js(csTask.getKeyValue()));
				if(bObject==true)
					map.put("object",csTask);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public ICsTaskService getCsTaskService()
	{
		return csTaskService;
	}

	public void setCsTaskService(ICsTaskService csTaskService)
	{
		this.csTaskService = csTaskService;
	}

	public CsTask getCsTask()
	{
		return csTask;
	}

	public void setCsTask(CsTask csTask)
	{
		this.csTask = csTask;
	}
}