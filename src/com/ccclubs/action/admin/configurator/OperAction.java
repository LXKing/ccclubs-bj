...
public class OperAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsOper.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsOper.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsOper.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsOper.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsOper.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsOper.C.get($.forMat($.getString("desc")));//降序字段
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/configurator/oper.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csOperService.getCsOperPage(page, size, finalParams);
					}
				}, CsOper.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csOperService.getCsOperPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csOperToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csOper这个对象为什么不用$.setRequest,因为csOper有get,set方法，由struts处理的
				csOper=csOperService.getCsOperById(id);
			}
			if(csOper==null)
				csOper = new CsOper();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csOper);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsOper object,CsOper old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsOper.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csOper.getCsoId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsOperToken = $.getString("csOperToken");
			final String sessionCsOperToken = $.getSession("csOperToken");
			if($.equals(requestCsOperToken,sessionCsOperToken)){
				$.removeSession("csOperToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csOperService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csOper.getCsoName()==null?"":csOper.getCsoName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csOper.setCsoName(String.valueOf(sItem));
							
								//设置默认值
								if(csOper.getCsoAdder()==null)
									csOper.setCsoAdder(LoginHelper.getLoginId());
								if(csOper.getCsoUpdateTime()==null)
									csOper.setCsoUpdateTime(new Date());
								if(csOper.getCsoAddTime()==null)
									csOper.setCsoAddTime(new Date());
								
								//根据自定义的默认值信息设置默认值
								if(CTRL!=null)
									CTRL.setObjectDefaultValue(csOper);
								
								csOper=csOperService.saveCsOper(csOper);			
								
								on("save",csOper,csOper);
							}							
							
							csOperService.updateCsOper$NotNull(csOper);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsOper.class,"add","添加了[运营商]["+csOper.getCsoName()+"]",(Long)$.getSession("ccclubs_login_id"), csOper,csOper.getCsoId());
							$.SetTips("保存运营商成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csOperService.updateCsOperByConfirm(ActionHelper.getSetValuesFromModel(csOper), params);
								LoggerHelper.writeLog(CsOper.class, "update", 
											"批量修改了[运营商]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csOper,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csOper.setCsoId(Long.valueOf(sid));
									CsOper oldCsOper = CsOper.get(Long.valueOf(sid));
									csOperService.updateCsOper$NotNull(csOper);
									LoggerHelper.writeLog(CsOper.class, "update", 
											"修改了[运营商]["+oldCsOper.getCsoName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsOper, csOper,false)
											,Long.valueOf(sid));
									on($.getString("method"),csOper,oldCsOper);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsOper oldCsOper = CsOper.get(csOper.getCsoId());
									csOperService.updateCsOper$NotNull(csOper);
									LoggerHelper.writeLog(CsOper.class, "update", 
											"修改了[运营商]["+oldCsOper.getCsoName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsOper, csOper,false)
									...
									CsOper oldCsOper = csOperService.getCsOperById(csOper.getCsoId());
									//设置默认值
									csOper.setCsoAdder(oldCsOper.getCsoAdder());	
									csOper.setCsoRemark(oldCsOper.getCsoRemark());	
									csOper.setCsoData(oldCsOper.getCsoData());	
									csOper.setCsoUpdateTime(oldCsOper.getCsoUpdateTime());	
									csOper.setCsoAddTime(oldCsOper.getCsoAddTime());	
									
									//设置填充值
									if(csOper.getCsoUpdateTime()==null)
										csOper.setCsoUpdateTime(new Date());
									
									//从oldCsOper同步csOper未set过的值
									csOper.mergeSet(oldCsOper);
											
									csOperService.updateCsOper(csOper);
									LoggerHelper.writeLog(CsOper.class, "update", 
											"修改了[运营商]["+oldCsOper.getCsoName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsOper, csOper,true)
											,csOper.getCsoId());
									on("update",csOper,oldCsOper);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csOper)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("oper_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csOper.getCsoId());
				else
		...
		try{
			...
			csOperService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsOper csOper = csOperService.getCsOperById(id);
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csOperService.removeCsOperById(id);
							else
							csOperService.deleteCsOperById(id);			
							//记录操作日志
							if(csOper!=null)
								LoggerHelper.writeLog(CsOper.class,"del","删除了[运营商]["+csOper.getCsoName()+"]",(Long)$.getSession("ccclubs_login_id"), csOper);
							on("delete",csOper,csOper);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csOper=csOperService.getCsOperById(id);
			if(key!=null && id==null)
				csOper=CsOper.Get($.add(CsOper.F.csoId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsOper.class);
			...
			List<CsOper> list = csOperService.getCsOperPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsOper csOper:list){
				Map map = new HashMap();
				map.put("value",csOper.getCsoId().toString());
				map.put("text", $.js(csOper.getKeyValue()));
				if(bObject==true)
					map.put("object",csOper);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public ICsOperService getCsOperService()
	{
		return csOperService;
	}

	public void setCsOperService(ICsOperService csOperService)
	{
		this.csOperService = csOperService;
	}

	public CsOper getCsOper()
	{
		return csOper;
	}

	public void setCsOper(CsOper csOper)
	{
		this.csOper = csOper;
	}
}