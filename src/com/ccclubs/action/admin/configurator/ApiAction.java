...
public class ApiAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",SrvApi.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(SrvApi.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(SrvApi.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(SrvApi.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=SrvApi.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=SrvApi.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","sa_id");//那么，默认以主键降序，传给Dao层
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/configurator/api.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return srvApiService.getSrvApiPage(page, size, finalParams);
					}
				}, SrvApi.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = srvApiService.getSrvApiPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("srvApiToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，srvApi这个对象为什么不用$.setRequest,因为srvApi有get,set方法，由struts处理的
				srvApi=srvApiService.getSrvApiById(id);
			}
			if(srvApi==null)
				srvApi = new SrvApi();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(srvApi);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,SrvApi object,SrvApi old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(SrvApi.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && srvApi.getSaId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestSrvApiToken = $.getString("srvApiToken");
			final String sessionSrvApiToken = $.getSession("srvApiToken");
			if($.equals(requestSrvApiToken,sessionSrvApiToken)){
				$.removeSession("srvApiToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				srvApiService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(srvApi.getSaName()==null?"":srvApi.getSaName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								srvApi.setSaName(String.valueOf(sItem));
							
								//设置默认值
								if(srvApi.getSaEditor()==null)
									srvApi.setSaEditor(LoginHelper.getLoginId());
								if(srvApi.getSaUpdateTime()==null)
									srvApi.setSaUpdateTime(new Date());
								if(srvApi.getSaAddTime()==null)
									srvApi.setSaAddTime(new Date());
								if(srvApi.getSaStatus()==null)
									srvApi.setSaStatus((short)0);
								
								//根据自定义的默认值信息设置默认值
								if(CTRL!=null)
									CTRL.setObjectDefaultValue(srvApi);
								
								srvApi=srvApiService.saveSrvApi(srvApi);			
								
								on("save",srvApi,srvApi);
							}							
							
							srvApiService.updateSrvApi$NotNull(srvApi);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(SrvApi.class,"add","添加了[API接口]["+srvApi.getSaName()+"]",(Long)$.getSession("ccclubs_login_id"), srvApi,srvApi.getSaId());
							$.SetTips("保存API接口成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								srvApiService.updateSrvApiByConfirm(ActionHelper.getSetValuesFromModel(srvApi), params);
								LoggerHelper.writeLog(SrvApi.class, "update", 
											"批量修改了[API接口]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,srvApi,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									srvApi.setSaId(Long.valueOf(sid));
									SrvApi oldSrvApi = SrvApi.get(Long.valueOf(sid));
									srvApiService.updateSrvApi$NotNull(srvApi);
									LoggerHelper.writeLog(SrvApi.class, "update", 
											"修改了[API接口]["+oldSrvApi.getSaName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldSrvApi, srvApi,false)
											,Long.valueOf(sid));
									on($.getString("method"),srvApi,oldSrvApi);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									SrvApi oldSrvApi = SrvApi.get(srvApi.getSaId());
									srvApiService.updateSrvApi$NotNull(srvApi);
									LoggerHelper.writeLog(SrvApi.class, "update", 
											"修改了[API接口]["+oldSrvApi.getSaName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldSrvApi, srvApi,false)
									...
									SrvApi oldSrvApi = srvApiService.getSrvApiById(srvApi.getSaId());
									//设置默认值
									srvApi.setSaEditor(oldSrvApi.getSaEditor());	
									srvApi.setSaUpdateTime(oldSrvApi.getSaUpdateTime());	
									srvApi.setSaAddTime(oldSrvApi.getSaAddTime());	
									srvApi.setSaStatus(oldSrvApi.getSaStatus());	
									...
									//从oldSrvApi同步srvApi未set过的值
									srvApi.mergeSet(oldSrvApi);
											
									srvApiService.updateSrvApi(srvApi);
									LoggerHelper.writeLog(SrvApi.class, "update", 
											"修改了[API接口]["+oldSrvApi.getSaName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldSrvApi, srvApi,true)
											,srvApi.getSaId());
									on("update",srvApi,oldSrvApi);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(srvApi)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("api_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+srvApi.getSaId());
				else
		...
		try{
			...
			srvApiService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							SrvApi srvApi = srvApiService.getSrvApiById(id);
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								srvApiService.removeSrvApiById(id);
							else
							srvApiService.deleteSrvApiById(id);			
							//记录操作日志
							if(srvApi!=null)
								LoggerHelper.writeLog(SrvApi.class,"del","删除了[API接口]["+srvApi.getSaName()+"]",(Long)$.getSession("ccclubs_login_id"), srvApi);
							on("delete",srvApi,srvApi);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				srvApi=srvApiService.getSrvApiById(id);
			if(key!=null && id==null)
				srvApi=SrvApi.Get($.add(SrvApi.F.saId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(SrvApi.class);
			...
			List<SrvApi> list = srvApiService.getSrvApiPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(SrvApi srvApi:list){
				Map map = new HashMap();
				map.put("value",srvApi.getSaId().toString());
				map.put("text", $.js(srvApi.getKeyValue()));
				if(bObject==true)
	...
	public ISrvApiService getSrvApiService()
	{
		return srvApiService;
	}

	public void setSrvApiService(ISrvApiService srvApiService)
	{
		this.srvApiService = srvApiService;
	}

	public SrvApi getSrvApi()
	{
		return srvApi;
	}

	public void setSrvApi(SrvApi srvApi)
	{
		this.srvApi = srvApi;
	}
}