...
public class ProvidAction
{
	ICsProvidService csProvidService;
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsProvid.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsProvid.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsProvid.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsProvid.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsProvid.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsProvid.C.get($.forMat($.getString("desc")));//降序字段
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/configurator/provid.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csProvidService.getCsProvidPage(page, size, finalParams);
					}
				}, CsProvid.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csProvidService.getCsProvidPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csProvidToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csProvid这个对象为什么不用$.setRequest,因为csProvid有get,set方法，由struts处理的
				csProvid=csProvidService.getCsProvidById(id);
			}
			if(csProvid==null)
				csProvid = new CsProvid();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csProvid);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsProvid object,CsProvid old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsProvid.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csProvid.getCspId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsProvidToken = $.getString("csProvidToken");
			final String sessionCsProvidToken = $.getSession("csProvidToken");
			if($.equals(requestCsProvidToken,sessionCsProvidToken)){
				$.removeSession("csProvidToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csProvidService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csProvid.getCspName()==null?"":csProvid.getCspName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csProvid.setCspName(String.valueOf(sItem));
							
								//设置默认值
								if(csProvid.getCspUpdateTime()==null)
								csProvid.setCspUpdateTime(new Date());
								if(csProvid.getCspAddTime()==null)
								csProvid.setCspAddTime(new Date());
								
								//根据自定义的默认值信息设置默认值
								if(CTRL!=null)
									CTRL.setObjectDefaultValue(csProvid);
								
								csProvid=csProvidService.saveCsProvid(csProvid);			
								
								on("save",csProvid,csProvid);
							}							
							
							csProvidService.updateCsProvid$NotNull(csProvid);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsProvid.class,"add","添加了[供应商]["+csProvid.getCspName()+"]",(Long)$.getSession("ccclubs_login_id"), csProvid,csProvid.getCspId());
							$.SetTips("保存供应商成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csProvidService.updateCsProvidByConfirm(ActionHelper.getSetValuesFromModel(csProvid), params);
								LoggerHelper.writeLog(CsProvid.class, "update", 
											"批量修改了[供应商]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csProvid,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csProvid.setCspId(Long.valueOf(sid));
									CsProvid oldCsProvid = CsProvid.get(Long.valueOf(sid));
									csProvidService.updateCsProvid$NotNull(csProvid);
									LoggerHelper.writeLog(CsProvid.class, "update", 
											"修改了[供应商]["+oldCsProvid.getCspName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsProvid, csProvid,false)
											,Long.valueOf(sid));
									on($.getString("method"),csProvid,oldCsProvid);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									CsProvid oldCsProvid = CsProvid.get(csProvid.getCspId());
									csProvidService.updateCsProvid$NotNull(csProvid);
									LoggerHelper.writeLog(CsProvid.class, "update", 
											"修改了[供应商]["+oldCsProvid.getCspName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsProvid, csProvid,false)
									...
									CsProvid oldCsProvid = csProvidService.getCsProvidById(csProvid.getCspId());
									//设置默认值
									csProvid.setCspUpdateTime(oldCsProvid.getCspUpdateTime());	
									csProvid.setCspAddTime(oldCsProvid.getCspAddTime());	
									
									//设置填充值
									
									//从oldCsProvid同步csProvid未set过的值
									csProvid.mergeSet(oldCsProvid);
											
									csProvidService.updateCsProvid(csProvid);
									LoggerHelper.writeLog(CsProvid.class, "update", 
											"修改了[供应商]["+oldCsProvid.getCspName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsProvid, csProvid,true)
											,csProvid.getCspId());
									on("update",csProvid,oldCsProvid);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csProvid)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("provid_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csProvid.getCspId());
				else
		...
		try{
			...
			csProvidService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsProvid csProvid = csProvidService.getCsProvidById(id);
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csProvidService.removeCsProvidById(id);
							else
							csProvidService.deleteCsProvidById(id);			
							//记录操作日志
							if(csProvid!=null)
								LoggerHelper.writeLog(CsProvid.class,"del","删除了[供应商]["+csProvid.getCspName()+"]",(Long)$.getSession("ccclubs_login_id"), csProvid);
							on("delete",csProvid,csProvid);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csProvid=csProvidService.getCsProvidById(id);
			if(key!=null && id==null)
				csProvid=CsProvid.Get($.add(CsProvid.F.cspId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsProvid.class);
			...
			List<CsProvid> list = csProvidService.getCsProvidPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsProvid csProvid:list){
				Map map = new HashMap();
				map.put("value",csProvid.getCspId().toString());
				map.put("text", $.js(csProvid.getKeyValue()));
				if(bObject==true)
					map.put("object",csProvid);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public ICsProvidService getCsProvidService()
	{
		return csProvidService;
	}

	public void setCsProvidService(ICsProvidService csProvidService)
	{
		this.csProvidService = csProvidService;
	}

	public CsProvid getCsProvid()
	{
		return csProvid;
	}

	public void setCsProvid(CsProvid csProvid)
	{
		this.csProvid = csProvid;
	}
}