...
public class WxorderAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsWxOrder.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsWxOrder.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsWxOrder.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsWxOrder.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsWxOrder.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsWxOrder.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","cswo_id");//那么，默认以主键降序，传给Dao层
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/configurator/wx/wxorder.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csWxOrderService.getCsWxOrderPage(page, size, finalParams);
					}
				}, CsWxOrder.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csWxOrderService.getCsWxOrderPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csWxOrderToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csWxOrder这个对象为什么不用$.setRequest,因为csWxOrder有get,set方法，由struts处理的
				csWxOrder=csWxOrderService.getCsWxOrderById(id);
			}
			if(csWxOrder==null)
				csWxOrder = new CsWxOrder();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csWxOrder);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsWxOrder object,CsWxOrder old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsWxOrder.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csWxOrder.getCswoId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsWxOrderToken = $.getString("csWxOrderToken");
			final String sessionCsWxOrderToken = $.getSession("csWxOrderToken");
			if($.equals(requestCsWxOrderToken,sessionCsWxOrderToken)){
				$.removeSession("csWxOrderToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csWxOrderService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csWxOrder.getCswoName()==null?"":csWxOrder.getCswoName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csWxOrder.setCswoName(String.valueOf(sItem));
							
								//设置默认值
								if(csWxOrder.getCswoTime()==null)
									csWxOrder.setCswoTime(new Date());
								
								//根据自定义的默认值信息设置默认值
								if(CTRL!=null)
									CTRL.setObjectDefaultValue(csWxOrder);
								
								csWxOrder=csWxOrderService.saveCsWxOrder(csWxOrder);			
								
								on("save",csWxOrder,csWxOrder);
							}							
							
							csWxOrderService.updateCsWxOrder$NotNull(csWxOrder);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsWxOrder.class,"add","添加了[微信指令]["+csWxOrder.getCswoName()+"]",(Long)$.getSession("ccclubs_login_id"), csWxOrder,csWxOrder.getCswoId());
							$.SetTips("保存微信指令成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csWxOrderService.updateCsWxOrderByConfirm(ActionHelper.getSetValuesFromModel(csWxOrder), params);
								LoggerHelper.writeLog(CsWxOrder.class, "update", 
											"批量修改了[微信指令]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csWxOrder,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csWxOrder.setCswoId(Long.valueOf(sid));
									CsWxOrder oldCsWxOrder = CsWxOrder.get(Long.valueOf(sid));
									csWxOrderService.updateCsWxOrder$NotNull(csWxOrder);
									LoggerHelper.writeLog(CsWxOrder.class, "update", 
											"修改了[微信指令]["+oldCsWxOrder.getCswoName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsWxOrder, csWxOrder,false)
											,Long.valueOf(sid));
									on($.getString("method"),csWxOrder,oldCsWxOrder);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsWxOrder oldCsWxOrder = CsWxOrder.get(csWxOrder.getCswoId());
									csWxOrderService.updateCsWxOrder$NotNull(csWxOrder);
									LoggerHelper.writeLog(CsWxOrder.class, "update", 
											"修改了[微信指令]["+oldCsWxOrder.getCswoName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsWxOrder, csWxOrder,false)
									...
									CsWxOrder oldCsWxOrder = csWxOrderService.getCsWxOrderById(csWxOrder.getCswoId());
									//设置默认值
									csWxOrder.setCswoTime(oldCsWxOrder.getCswoTime());	
									
									//设置填充值
									if(csWxOrder.getCswoTime()==null)
										csWxOrder.setCswoTime(new Date());
									
									//从oldCsWxOrder同步csWxOrder未set过的值
									csWxOrder.mergeSet(oldCsWxOrder);
											
									csWxOrderService.updateCsWxOrder(csWxOrder);
									LoggerHelper.writeLog(CsWxOrder.class, "update", 
											"修改了[微信指令]["+oldCsWxOrder.getCswoName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsWxOrder, csWxOrder,true)
											,csWxOrder.getCswoId());
									on("update",csWxOrder,oldCsWxOrder);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csWxOrder)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("wxorder_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csWxOrder.getCswoId());
				else
		...
		try{
			...
			csWxOrderService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsWxOrder csWxOrder = csWxOrderService.getCsWxOrderById(id);
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csWxOrderService.removeCsWxOrderById(id);
							else
							csWxOrderService.deleteCsWxOrderById(id);			
							//记录操作日志
							if(csWxOrder!=null)
								LoggerHelper.writeLog(CsWxOrder.class,"del","删除了[微信指令]["+csWxOrder.getCswoName()+"]",(Long)$.getSession("ccclubs_login_id"), csWxOrder);
							on("delete",csWxOrder,csWxOrder);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csWxOrder=csWxOrderService.getCsWxOrderById(id);
			if(key!=null && id==null)
				csWxOrder=CsWxOrder.Get($.add(CsWxOrder.F.cswoId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsWxOrder.class);
			...
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				params.put("desc","cswo_id");//那么，默认以主键降序，传给Dao层
								
			String strTerm=$.getString("value");//智能搜索时的参数，一般都是主键后面那个字段
			if(!$.empty(strTerm)){
				if(strTerm.toLowerCase().startsWith("id:"))//如果查询条件以id:开头，则按ID查询
					params.put("cswoId", strTerm.toLowerCase().replaceFirst("id:", ""));					
				else//按标识查询，模糊查询请带%
				{
					String strDefinex = "";
					strDefinex+=" or cswo_name like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsWxOrder> list = csWxOrderService.getCsWxOrderPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsWxOrder csWxOrder:list){
				Map map = new HashMap();
				map.put("value",csWxOrder.getCswoId().toString());
				map.put("text", $.js(csWxOrder.getKeyValue()));
				if(bObject==true)
					map.put("object",csWxOrder);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public CsWxOrder getCsWxOrder()
	{
		return csWxOrder;
	}

	public void setCsWxOrder(CsWxOrder csWxOrder)
	{
		this.csWxOrder = csWxOrder;
	}
}