...
public class WxmoAction
{
	ICsWxMoService csWxMoService;
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsWxMo.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsWxMo.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsWxMo.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsWxMo.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsWxMo.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsWxMo.C.get($.forMat($.getString("desc")));//降序字段
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/configurator/wx/wxmo.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csWxMoService.getCsWxMoPage(page, size, finalParams);
					}
				}, CsWxMo.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csWxMoService.getCsWxMoPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csWxMoToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csWxMo这个对象为什么不用$.setRequest,因为csWxMo有get,set方法，由struts处理的
				csWxMo=csWxMoService.getCsWxMoById(id);
				//确认域标识
				if(SystemHelper.testHost(csWxMo.getCswmHost()).equals("0")){
			...
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csWxMo);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsWxMo object,CsWxMo old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsWxMo.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csWxMo.getCswmId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsWxMoToken = $.getString("csWxMoToken");
			final String sessionCsWxMoToken = $.getSession("csWxMoToken");
			if($.equals(requestCsWxMoToken,sessionCsWxMoToken)){
				$.removeSession("csWxMoToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csWxMoService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csWxMo.getCswmId()==null?"":csWxMo.getCswmId());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csWxMo.setCswmId(Long.valueOf(sItem));
							
								//设置默认值
								if(csWxMo.getCswmAddTime()==null)
								...
								csWxMo=csWxMoService.saveCsWxMo(csWxMo);			
								
								on("save",csWxMo,csWxMo);
							}							
							
							csWxMoService.updateCsWxMo$NotNull(csWxMo);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsWxMo.class,"add","添加了[微信上行]["+csWxMo.getCswmId()+"]",(Long)$.getSession("ccclubs_login_id"), csWxMo,csWxMo.getCswmId());
							$.SetTips("保存微信上行成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csWxMoService.updateCsWxMoByConfirm(ActionHelper.getSetValuesFromModel(csWxMo), params);
								LoggerHelper.writeLog(CsWxMo.class, "update", 
											"批量修改了[微信上行]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csWxMo,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csWxMo.setCswmId(Long.valueOf(sid));
									CsWxMo oldCsWxMo = CsWxMo.get(Long.valueOf(sid));
									csWxMoService.updateCsWxMo$NotNull(csWxMo);
									LoggerHelper.writeLog(CsWxMo.class, "update", 
											"修改了[微信上行]["+oldCsWxMo.getCswmId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsWxMo, csWxMo,false)
											,Long.valueOf(sid));
									on($.getString("method"),csWxMo,oldCsWxMo);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									CsWxMo oldCsWxMo = CsWxMo.get(csWxMo.getCswmId());
									csWxMoService.updateCsWxMo$NotNull(csWxMo);
									LoggerHelper.writeLog(CsWxMo.class, "update", 
											"修改了[微信上行]["+oldCsWxMo.getCswmId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsWxMo, csWxMo,false)
									...
									CsWxMo oldCsWxMo = csWxMoService.getCsWxMoById(csWxMo.getCswmId());
									//设置默认值
									csWxMo.setCswmAddTime(oldCsWxMo.getCswmAddTime());	
									...
									//从oldCsWxMo同步csWxMo未set过的值
									csWxMo.mergeSet(oldCsWxMo);
											
									csWxMoService.updateCsWxMo(csWxMo);
									LoggerHelper.writeLog(CsWxMo.class, "update", 
											"修改了[微信上行]["+oldCsWxMo.getCswmId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsWxMo, csWxMo,true)
											,csWxMo.getCswmId());
									on("update",csWxMo,oldCsWxMo);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csWxMo)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("wxmo_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csWxMo.getCswmId());
				else
		...
		try{
			...
			csWxMoService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsWxMo csWxMo = csWxMoService.getCsWxMoById(id);
							//确认域标识
							if(SystemHelper.testHost(csWxMo.getCswmHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csWxMoService.removeCsWxMoById(id);
							else
							csWxMoService.deleteCsWxMoById(id);			
							//记录操作日志
							if(csWxMo!=null)
								LoggerHelper.writeLog(CsWxMo.class,"del","删除了[微信上行]["+csWxMo.getCswmId()+"]",(Long)$.getSession("ccclubs_login_id"), csWxMo);
							on("delete",csWxMo,csWxMo);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csWxMo=csWxMoService.getCsWxMoById(id);
			if(key!=null && id==null)
				csWxMo=CsWxMo.Get($.add(CsWxMo.F.cswmId,key));
			//确认域标识
			if(SystemHelper.testHost(csWxMo.getCswmHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsWxMo.class);
			//强制限制域
			...
			List<CsWxMo> list = csWxMoService.getCsWxMoPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsWxMo csWxMo:list){
				Map map = new HashMap();
				map.put("value",csWxMo.getCswmId().toString());
				map.put("text", $.js(csWxMo.getKeyValue()));
				if(bObject==true)
					map.put("object",csWxMo);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public ICsWxMoService getCsWxMoService()
	{
		return csWxMoService;
	}

	public void setCsWxMoService(ICsWxMoService csWxMoService)
	{
		this.csWxMoService = csWxMoService;
	}

	public CsWxMo getCsWxMo()
	{
		return csWxMo;
	}

	public void setCsWxMo(CsWxMo csWxMo)
	{
		this.csWxMo = csWxMo;
	}
}