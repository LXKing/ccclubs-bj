...
public class VisitipAction
{
	ICsVisitIpService csVisitIpService;
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsVisitIp.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsVisitIp.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsVisitIp.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsVisitIp.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsVisitIp.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsVisitIp.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","csvi_id");//那么，默认以主键降序，传给Dao层
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/extension/visitip.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csVisitIpService.getCsVisitIpPage(page, size, finalParams);
					}
				}, CsVisitIp.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csVisitIpService.getCsVisitIpPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csVisitIpToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csVisitIp这个对象为什么不用$.setRequest,因为csVisitIp有get,set方法，由struts处理的
				csVisitIp=csVisitIpService.getCsVisitIpById(id);
				//确认域标识
				if(SystemHelper.testHost(csVisitIp.getCsviHost()).equals("0")){
					return $.SendHtml("<script>alert('对不起，你没有权限编辑该数据');</script>", "UTF-8");
				}
			}
			if(csVisitIp==null)
				csVisitIp = new CsVisitIp();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csVisitIp);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsVisitIp object,CsVisitIp old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsVisitIp.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csVisitIp.getCsviId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsVisitIpToken = $.getString("csVisitIpToken");
			final String sessionCsVisitIpToken = $.getSession("csVisitIpToken");
			if($.equals(requestCsVisitIpToken,sessionCsVisitIpToken)){
				$.removeSession("csVisitIpToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csVisitIpService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csVisitIp.getCsviAddress()==null?"":csVisitIp.getCsviAddress());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csVisitIp.setCsviAddress(String.valueOf(sItem));
							
								//设置默认值
								if(csVisitIp.getCsviAddTime()==null)
									csVisitIp.setCsviAddTime(new Date());
								...
								csVisitIp=csVisitIpService.saveCsVisitIp(csVisitIp);			
								
								on("save",csVisitIp,csVisitIp);
							}							
							
							csVisitIpService.updateCsVisitIp$NotNull(csVisitIp);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsVisitIp.class,"add","添加了[独立IP访问]["+csVisitIp.getCsviAddress()+"]",(Long)$.getSession("ccclubs_login_id"), csVisitIp,csVisitIp.getCsviId());
							$.SetTips("保存独立IP访问成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csVisitIpService.updateCsVisitIpByConfirm(ActionHelper.getSetValuesFromModel(csVisitIp), params);
								LoggerHelper.writeLog(CsVisitIp.class, "update", 
											"批量修改了[独立IP访问]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csVisitIp,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csVisitIp.setCsviId(Long.valueOf(sid));
									CsVisitIp oldCsVisitIp = CsVisitIp.get(Long.valueOf(sid));
									csVisitIpService.updateCsVisitIp$NotNull(csVisitIp);
									LoggerHelper.writeLog(CsVisitIp.class, "update", 
											"修改了[独立IP访问]["+oldCsVisitIp.getCsviAddress()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsVisitIp, csVisitIp,false)
											,Long.valueOf(sid));
									on($.getString("method"),csVisitIp,oldCsVisitIp);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									CsVisitIp oldCsVisitIp = CsVisitIp.get(csVisitIp.getCsviId());
									csVisitIpService.updateCsVisitIp$NotNull(csVisitIp);
									LoggerHelper.writeLog(CsVisitIp.class, "update", 
											"修改了[独立IP访问]["+oldCsVisitIp.getCsviAddress()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsVisitIp, csVisitIp,false)
									...
									CsVisitIp oldCsVisitIp = csVisitIpService.getCsVisitIpById(csVisitIp.getCsviId());
									//设置默认值
									csVisitIp.setCsviAddTime(oldCsVisitIp.getCsviAddTime());	
									...
									//从oldCsVisitIp同步csVisitIp未set过的值
									csVisitIp.mergeSet(oldCsVisitIp);
											
									csVisitIpService.updateCsVisitIp(csVisitIp);
									LoggerHelper.writeLog(CsVisitIp.class, "update", 
											"修改了[独立IP访问]["+oldCsVisitIp.getCsviAddress()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsVisitIp, csVisitIp,true)
											,csVisitIp.getCsviId());
									on("update",csVisitIp,oldCsVisitIp);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csVisitIp)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("visitip_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csVisitIp.getCsviId());
				else
		...
		try{
			...
			csVisitIpService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsVisitIp csVisitIp = csVisitIpService.getCsVisitIpById(id);
							//确认域标识
							if(SystemHelper.testHost(csVisitIp.getCsviHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							csVisitIpService.deleteCsVisitIpById(id);			
							//记录操作日志
							if(csVisitIp!=null)
								LoggerHelper.writeLog(CsVisitIp.class,"del","删除了[独立IP访问]["+csVisitIp.getCsviAddress()+"]",(Long)$.getSession("ccclubs_login_id"), csVisitIp);
							on("delete",csVisitIp,csVisitIp);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csVisitIp=csVisitIpService.getCsVisitIpById(id);
			if(key!=null && id==null)
				csVisitIp=CsVisitIp.Get($.add(CsVisitIp.F.csviId,key));
			//确认域标识
			if(SystemHelper.testHost(csVisitIp.getCsviHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsVisitIp.class);
			//强制限制域
			...
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				params.put("desc","csvi_id");//那么，默认以主键降序，传给Dao层
								
			String strTerm=$.getString("value");//智能搜索时的参数，一般都是主键后面那个字段
			if(!$.empty(strTerm)){
				if(strTerm.toLowerCase().startsWith("id:"))//如果查询条件以id:开头，则按ID查询
					params.put("csviId", strTerm.toLowerCase().replaceFirst("id:", ""));					
				else//按标识查询，模糊查询请带%
				{
					String strDefinex = "";
					strDefinex+=" or csvi_address like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsVisitIp> list = csVisitIpService.getCsVisitIpPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsVisitIp csVisitIp:list){
				Map map = new HashMap();
				map.put("value",csVisitIp.getCsviId().toString());
				map.put("text", $.js(csVisitIp.getKeyValue()));
				if(bObject==true)
					map.put("object",csVisitIp);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public ICsVisitIpService getCsVisitIpService()
	{
		return csVisitIpService;
	}

	public void setCsVisitIpService(ICsVisitIpService csVisitIpService)
	{
		this.csVisitIpService = csVisitIpService;
	}

	public CsVisitIp getCsVisitIp()
	{
		return csVisitIp;
	}

	public void setCsVisitIp(CsVisitIp csVisitIp)
	{
		this.csVisitIp = csVisitIp;
	}
}