...
{
	ICsChannelService csChannelService;
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsChannel.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsChannel.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsChannel.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsChannel.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsChannel.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsChannel.C.get($.forMat($.getString("desc")));//降序字段
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/extension/channel.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csChannelService.getCsChannelPage(page, size, finalParams);
					}
				}, CsChannel.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csChannelService.getCsChannelPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csChannelToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csChannel这个对象为什么不用$.setRequest,因为csChannel有get,set方法，由struts处理的
				csChannel=csChannelService.getCsChannelById(id);
				//确认域标识
				if(SystemHelper.testHost(csChannel.getCscHost()).equals("0")){
			...
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csChannel);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsChannel object,CsChannel old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsChannel.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csChannel.getCscId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsChannelToken = $.getString("csChannelToken");
			final String sessionCsChannelToken = $.getSession("csChannelToken");
			if($.equals(requestCsChannelToken,sessionCsChannelToken)){
				$.removeSession("csChannelToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csChannelService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csChannel.getCscName()==null?"":csChannel.getCscName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csChannel.setCscName(String.valueOf(sItem));
							
								//设置默认值
								if(csChannel.getCscUpdateTime()==null)
									csChannel.setCscUpdateTime(new Date());
								if(csChannel.getCscAddTime()==null)
									csChannel.setCscAddTime(new Date());
								...
								csChannel=csChannelService.saveCsChannel(csChannel);			
								
								on("save",csChannel,csChannel);
							}							
							
							csChannelService.updateCsChannel$NotNull(csChannel);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsChannel.class,"add","添加了[访问渠道]["+csChannel.getCscName()+"]",(Long)$.getSession("ccclubs_login_id"), csChannel,csChannel.getCscId());
							$.SetTips("保存访问渠道成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csChannelService.updateCsChannelByConfirm(ActionHelper.getSetValuesFromModel(csChannel), params);
								LoggerHelper.writeLog(CsChannel.class, "update", 
											"批量修改了[访问渠道]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csChannel,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csChannel.setCscId(Long.valueOf(sid));
									CsChannel oldCsChannel = CsChannel.get(Long.valueOf(sid));
									csChannelService.updateCsChannel$NotNull(csChannel);
									LoggerHelper.writeLog(CsChannel.class, "update", 
											"修改了[访问渠道]["+oldCsChannel.getCscName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsChannel, csChannel,false)
											,Long.valueOf(sid));
									on($.getString("method"),csChannel,oldCsChannel);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsChannel oldCsChannel = CsChannel.get(csChannel.getCscId());
									csChannelService.updateCsChannel$NotNull(csChannel);
									LoggerHelper.writeLog(CsChannel.class, "update", 
											"修改了[访问渠道]["+oldCsChannel.getCscName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsChannel, csChannel,false)
									...
									CsChannel oldCsChannel = csChannelService.getCsChannelById(csChannel.getCscId());
									//设置默认值
									csChannel.setCscUpdateTime(oldCsChannel.getCscUpdateTime());	
									csChannel.setCscAddTime(oldCsChannel.getCscAddTime());	
									...
									//从oldCsChannel同步csChannel未set过的值
									csChannel.mergeSet(oldCsChannel);
											
									csChannelService.updateCsChannel(csChannel);
									LoggerHelper.writeLog(CsChannel.class, "update", 
											"修改了[访问渠道]["+oldCsChannel.getCscName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsChannel, csChannel,true)
											,csChannel.getCscId());
									on("update",csChannel,oldCsChannel);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csChannel)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("channel_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csChannel.getCscId());
				else
		...
		try{
			...
			csChannelService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsChannel csChannel = csChannelService.getCsChannelById(id);
							//确认域标识
							if(SystemHelper.testHost(csChannel.getCscHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csChannelService.removeCsChannelById(id);
							else
							csChannelService.deleteCsChannelById(id);			
							//记录操作日志
							if(csChannel!=null)
								LoggerHelper.writeLog(CsChannel.class,"del","删除了[访问渠道]["+csChannel.getCscName()+"]",(Long)$.getSession("ccclubs_login_id"), csChannel);
							on("delete",csChannel,csChannel);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csChannel=csChannelService.getCsChannelById(id);
			if(key!=null && id==null)
				csChannel=CsChannel.Get($.add(CsChannel.F.cscId,key));
			//确认域标识
			if(SystemHelper.testHost(csChannel.getCscHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsChannel.class);
			//强制限制域
			...
			if(!$.empty(strTerm)){
				...
				{
					String strDefinex = "";
					strDefinex+=" or csc_name like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsChannel> list = csChannelService.getCsChannelPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsChannel csChannel:list){
				Map map = new HashMap();
				map.put("value",csChannel.getCscId().toString());
				map.put("text", $.js(csChannel.getKeyValue()));
				if(bObject==true)
					map.put("object",csChannel);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public ICsChannelService getCsChannelService()
	{
		return csChannelService;
	}

	public void setCsChannelService(ICsChannelService csChannelService)
	{
		this.csChannelService = csChannelService;
	}

	public CsChannel getCsChannel()
	{
		return csChannel;
	}

	public void setCsChannel(CsChannel csChannel)
	{
