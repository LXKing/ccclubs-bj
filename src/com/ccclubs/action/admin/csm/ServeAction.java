...
{
	ICsServeService csServeService;
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsServe.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsServe.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsServe.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsServe.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsServe.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsServe.C.get($.forMat($.getString("desc")));//降序字段
			...
			//反强制限制域
			if(objects.get("cssMember")!=null && objects.get("cssMemberNot")==null){
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/csm/serve.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csServeService.getCsServePage(page, size, finalParams);
					}
				}, CsServe.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csServeService.getCsServePage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csServeToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csServe这个对象为什么不用$.setRequest,因为csServe有get,set方法，由struts处理的
				csServe=csServeService.getCsServeById(id);
				//确认域标识
				if(SystemHelper.testHost(csServe.getCssHost()).equals("0")){
			...
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csServe);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsServe object,CsServe old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsServe.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csServe.getCssId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsServeToken = $.getString("csServeToken");
			final String sessionCsServeToken = $.getSession("csServeToken");
			if($.equals(requestCsServeToken,sessionCsServeToken)){
				$.removeSession("csServeToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csServeService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csServe.getCssTitle()==null?"":csServe.getCssTitle());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csServe.setCssTitle(String.valueOf(sItem));
							
								//设置默认值
								if(csServe.getCssUser()==null)
									csServe.setCssUser(LoginHelper.getLoginId());
								if(csServe.getCssUpdateTime()==null)
								csServe.setCssUpdateTime(new Date());
								if(csServe.getCssAddTime()==null)
								csServe.setCssAddTime(new Date());
								...
								csServe=csServeService.saveCsServe(csServe);			
								
								on("save",csServe,csServe);
							}							
							
							csServeService.updateCsServe$NotNull(csServe);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsServe.class,"add","添加了[客户服务]["+csServe.getCssTitle()+"]",(Long)$.getSession("ccclubs_login_id"), csServe,csServe.getCssId());
							$.SetTips("保存客户服务成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csServeService.updateCsServeByConfirm(ActionHelper.getSetValuesFromModel(csServe), params);
								LoggerHelper.writeLog(CsServe.class, "update", 
											"批量修改了[客户服务]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csServe,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csServe.setCssId(Long.valueOf(sid));
									CsServe oldCsServe = CsServe.get(Long.valueOf(sid));
									csServeService.updateCsServe$NotNull(csServe);
									LoggerHelper.writeLog(CsServe.class, "update", 
											"修改了[客户服务]["+oldCsServe.getCssTitle()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsServe, csServe,false)
											,Long.valueOf(sid));
									on($.getString("method"),csServe,oldCsServe);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsServe oldCsServe = CsServe.get(csServe.getCssId());
									csServeService.updateCsServe$NotNull(csServe);
									LoggerHelper.writeLog(CsServe.class, "update", 
											"修改了[客户服务]["+oldCsServe.getCssTitle()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsServe, csServe,false)
									...
									CsServe oldCsServe = csServeService.getCsServeById(csServe.getCssId());
									//设置默认值
									csServe.setCssUser(oldCsServe.getCssUser());	
									csServe.setCssUpdateTime(oldCsServe.getCssUpdateTime());	
									csServe.setCssAddTime(oldCsServe.getCssAddTime());	
									...
									//从oldCsServe同步csServe未set过的值
									csServe.mergeSet(oldCsServe);
											
									csServeService.updateCsServe(csServe);
									LoggerHelper.writeLog(CsServe.class, "update", 
											"修改了[客户服务]["+oldCsServe.getCssTitle()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsServe, csServe,true)
											,csServe.getCssId());
									on("update",csServe,oldCsServe);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csServe)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("serve_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csServe.getCssId());
				else
		...
		try{
			...
			csServeService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsServe csServe = csServeService.getCsServeById(id);
							//确认域标识
							if(SystemHelper.testHost(csServe.getCssHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csServeService.removeCsServeById(id);
							else
							csServeService.deleteCsServeById(id);			
							//记录操作日志
							if(csServe!=null)
								LoggerHelper.writeLog(CsServe.class,"del","删除了[客户服务]["+csServe.getCssTitle()+"]",(Long)$.getSession("ccclubs_login_id"), csServe);
							on("delete",csServe,csServe);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csServe=csServeService.getCsServeById(id);
			if(key!=null && id==null)
				csServe=CsServe.Get($.add(CsServe.F.cssId,key));
			//确认域标识
			if(SystemHelper.testHost(csServe.getCssHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsServe.class);
			//强制限制域
			params.put("HOSTS",SystemHelper.testHost(null));
			
			//反强制限制域
			if(params.get("cssMember")!=null && params.get("cssMemberNot")==null){
				params.remove("HOSTS");
			}
			...
			List<CsServe> list = csServeService.getCsServePage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsServe csServe:list){
				Map map = new HashMap();
				map.put("value",csServe.getCssId().toString());
				map.put("text", $.js(csServe.getKeyValue()));
				if(bObject==true)
					map.put("object",csServe);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public ICsServeService getCsServeService()
	{
		return csServeService;
	}

	public void setCsServeService(ICsServeService csServeService)
	{
		this.csServeService = csServeService;
	}

	public CsServe getCsServe()
	{
		return csServe;
	}

	public void setCsServe(CsServe csServe)
	{
		this.csServe = csServe;
	}
}