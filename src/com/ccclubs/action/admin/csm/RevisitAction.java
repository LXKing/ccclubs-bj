...
{
	ICsRevisitService csRevisitService;
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsRevisit.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsRevisit.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsRevisit.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsRevisit.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsRevisit.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsRevisit.C.get($.forMat($.getString("desc")));//降序字段
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/csm/revisit.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csRevisitService.getCsRevisitPage(page, size, finalParams);
					}
				}, CsRevisit.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csRevisitService.getCsRevisitPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csRevisitToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csRevisit这个对象为什么不用$.setRequest,因为csRevisit有get,set方法，由struts处理的
				csRevisit=csRevisitService.getCsRevisitById(id);
				//确认域标识
				if(SystemHelper.testHost(csRevisit.getCsrHost()).equals("0")){
					return $.SendHtml("<script>alert('对不起，你没有权限编辑该数据');</script>", "UTF-8");
				}
			}
			if(csRevisit==null)
				csRevisit = new CsRevisit();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csRevisit);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsRevisit object,CsRevisit old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsRevisit.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csRevisit.getCsrId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsRevisitToken = $.getString("csRevisitToken");
			final String sessionCsRevisitToken = $.getSession("csRevisitToken");
			if($.equals(requestCsRevisitToken,sessionCsRevisitToken)){
				$.removeSession("csRevisitToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csRevisitService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csRevisit.getCsrId()==null?"":csRevisit.getCsrId());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csRevisit.setCsrId(Long.valueOf(sItem));
							
								//设置默认值
								if(csRevisit.getCsrVisitor()==null)
									csRevisit.setCsrVisitor(LoginHelper.getLoginId());
								if(csRevisit.getCsrUpdateTime()==null)
									csRevisit.setCsrUpdateTime(new Date());
								if(csRevisit.getCsrAddTime()==null)
									csRevisit.setCsrAddTime(new Date());
								...
								csRevisit=csRevisitService.saveCsRevisit(csRevisit);			
								
								on("save",csRevisit,csRevisit);
							}							
							
							csRevisitService.updateCsRevisit$NotNull(csRevisit);						
							/************LAZY3Q_CODE_ADD************/							
							com.ccclubs.model.CsMember csrMember = csRevisit.get$csrMember();
							com.ccclubs.model.CsMember updateMember = new com.ccclubs.model.CsMember().csmId(csrMember.getCsmId());
							String csmVisitFlag = updateMember.getCsmVisitFlag();
							if($.empty(csmVisitFlag))
								csmVisitFlag="#"+csRevisit.getCsrType().toString()+"#";
							else if(!$.idin(csmVisitFlag, csRevisit.getCsrId()))
								csmVisitFlag+=",#"+csRevisit.getCsrType().toString()+"#";
							updateMember.setCsmVisitFlag(csmVisitFlag);
							updateMember.update();					
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsRevisit.class,"add","添加了[用户回访]["+csRevisit.getCsrId()+"]",(Long)$.getSession("ccclubs_login_id"), csRevisit,csRevisit.getCsrId());
							$.SetTips("保存用户回访成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csRevisitService.updateCsRevisitByConfirm(ActionHelper.getSetValuesFromModel(csRevisit), params);
								LoggerHelper.writeLog(CsRevisit.class, "update", 
											"批量修改了[用户回访]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csRevisit,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csRevisit.setCsrId(Long.valueOf(sid));
									CsRevisit oldCsRevisit = CsRevisit.get(Long.valueOf(sid));
									csRevisitService.updateCsRevisit$NotNull(csRevisit);
									LoggerHelper.writeLog(CsRevisit.class, "update", 
											"修改了[用户回访]["+oldCsRevisit.getCsrId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsRevisit, csRevisit,false)
											,Long.valueOf(sid));
									on($.getString("method"),csRevisit,oldCsRevisit);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsRevisit oldCsRevisit = CsRevisit.get(csRevisit.getCsrId());
									csRevisitService.updateCsRevisit$NotNull(csRevisit);
									LoggerHelper.writeLog(CsRevisit.class, "update", 
											"修改了[用户回访]["+oldCsRevisit.getCsrId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsRevisit, csRevisit,false)
									...
									CsRevisit oldCsRevisit = csRevisitService.getCsRevisitById(csRevisit.getCsrId());
									//设置默认值
									csRevisit.setCsrMember(oldCsRevisit.getCsrMember());	
									csRevisit.setCsrVisitor(oldCsRevisit.getCsrVisitor());	
									csRevisit.setCsrType(oldCsRevisit.getCsrType());	
									csRevisit.setCsrUpdateTime(oldCsRevisit.getCsrUpdateTime());	
									csRevisit.setCsrAddTime(oldCsRevisit.getCsrAddTime());	
									...
									//从oldCsRevisit同步csRevisit未set过的值
									csRevisit.mergeSet(oldCsRevisit);
											
									csRevisitService.updateCsRevisit(csRevisit);
									LoggerHelper.writeLog(CsRevisit.class, "update", 
											"修改了[用户回访]["+oldCsRevisit.getCsrId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsRevisit, csRevisit,true)
											,csRevisit.getCsrId());
									on("update",csRevisit,oldCsRevisit);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csRevisit)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("revisit_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csRevisit.getCsrId());
				else
		...
		try{
			...
			csRevisitService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsRevisit csRevisit = csRevisitService.getCsRevisitById(id);
							//确认域标识
							if(SystemHelper.testHost(csRevisit.getCsrHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csRevisitService.removeCsRevisitById(id);
							else
							csRevisitService.deleteCsRevisitById(id);			
							//记录操作日志
							if(csRevisit!=null)
								LoggerHelper.writeLog(CsRevisit.class,"del","删除了[用户回访]["+csRevisit.getCsrId()+"]",(Long)$.getSession("ccclubs_login_id"), csRevisit);
							on("delete",csRevisit,csRevisit);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csRevisit=csRevisitService.getCsRevisitById(id);
			if(key!=null && id==null)
				csRevisit=CsRevisit.Get($.add(CsRevisit.F.csrId,key));
			//确认域标识
			if(SystemHelper.testHost(csRevisit.getCsrHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsRevisit.class);
			//强制限制域
			...
			List<CsRevisit> list = csRevisitService.getCsRevisitPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsRevisit csRevisit:list){
				Map map = new HashMap();
				map.put("value",csRevisit.getCsrId().toString());
				map.put("text", $.js(csRevisit.getKeyValue()));
				if(bObject==true)
					map.put("object",csRevisit);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public ICsRevisitService getCsRevisitService()
	{
		return csRevisitService;
	}

	public void setCsRevisitService(ICsRevisitService csRevisitService)
	{
		this.csRevisitService = csRevisitService;
	}

	public CsRevisit getCsRevisit()
	{
		return csRevisit;
	}

	public void setCsRevisit(CsRevisit csRevisit)
	{
		this.csRevisit = csRevisit;
	}
}