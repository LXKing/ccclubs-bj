package com.ccclubs.action.admin.oa.outside;
...
public class OutsideStatisticsAction {
	
	ICsOutsideStatisticsService	csOutsideStatisticsService;
	CsOutsideStatistics  csOutsideStatistics;
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsOutsideStatistics.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsOutsideStatistics.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsOutsideStatistics.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsOutsideStatistics.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsOutsideStatistics.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsOutsideStatistics.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","csos_id");//那么，默认以主键降序，传给Dao层
...
//			}
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/oa/outside/outsideStatistics.do");//----------------------呆改--------
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csOutsideStatisticsService.getCsOutsideStatisticsPage(page, size, finalParams);
					}
				}, CsOutsideStatistics.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csOutsideStatisticsService.getCsOutsideStatisticsPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csOutsideStatisticsToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csViolat这个对象为什么不用$.setRequest,因为csViolat有get,set方法，由struts处理的
				csOutsideStatistics=csOutsideStatisticsService.getCsOutsideStatisticsById(id);
				//确认域标识
				if(SystemHelper.testHost(csOutsideStatistics.getCsosHost()).equals("0")){
					return $.SendHtml("<script>alert('对不起，你没有权限编辑该数据');</script>", "UTF-8");
				}
			}
			if(csOutsideStatistics==null)
				csOutsideStatistics = new CsOutsideStatistics();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csOutsideStatistics);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	...
	public String del()
	{
		try{
			//获取当前用户的权限，看当前用户有没有删除权限
			SystemHelper.getSrvLimit("admin/oa/outside/outsideStatistics.do");
					/************LAZY3Q_CAN_DEL************/
					/************LAZY3Q_CAN_DEL************/
			...
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//多个删除数据动作合并事务过程
			csOutsideStatisticsService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
						CsOutsideStatistics outsideStatistics = csOutsideStatisticsService.getCsOutsideStatisticsById(id);
							//确认域标识
							if(SystemHelper.testHost(outsideStatistics.getCsosHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csOutsideStatisticsService.removeCsOutsideStatisticsById(id);
							else
								csOutsideStatisticsService.deleteCsOutsideStatisticsById(id);			
							//记录操作日志
							if(outsideStatistics!=null)
								LoggerHelper.writeLog(CsOutsideStatistics.class,"del","删除了[外勤整备项统计]["+outsideStatistics.getCsosId()+"]",(Long)$.getSession("ccclubs_login_id"), outsideStatistics);
							on("delete",outsideStatistics,outsideStatistics);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csOutsideStatistics=csOutsideStatisticsService.getCsOutsideStatisticsById(id);
			if(key!=null && id==null)
				csOutsideStatistics=CsOutsideStatistics.Get($.add(CsOutsideStatistics.F.csosId,key));//_____________________________________________________呆改
			//确认域标识
			if(SystemHelper.testHost(csOutsideStatistics.getCsosHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsOutsideStatistics.class);
			//强制限制域
...
//			}
			...
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				params.put("desc","csos_id");//那么，默认以主键降序，传给Dao层
								
			String strTerm=$.getString("value");//智能搜索时的参数，一般都是主键后面那个字段
			if(!$.empty(strTerm)){
				if(strTerm.toLowerCase().startsWith("id:"))//如果查询条件以id:开头，则按ID查询
					params.put("csosId", strTerm.toLowerCase().replaceFirst("id:", ""));					
				else//按标识查询，模糊查询请带%
				{
					String strDefinex = "";
					strDefinex+=" or csos_id like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsOutsideStatistics> list = csOutsideStatisticsService.getCsOutsideStatisticsPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsOutsideStatistics outsideStatistics:list){
				Map map = new HashMap();
				map.put("value",outsideStatistics.getCsosId().toString());
				map.put("text", $.js(outsideStatistics.getKeyValue()));
				if(bObject==true)
					map.put("object",outsideStatistics);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	 ...
	 * @return
	 */
	public void on(String type,CsOutsideStatistics object,CsOutsideStatistics old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsOutsideStatistics.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	{	
		try{
			...
			if($.getBoolean("export",false)==true){//如果传入了一个数据导出的参数
				...
				response.setHeader("Content-Disposition", "attachment;filename="+java.net.URLEncoder.encode("车辆整备统计导出","UTF-8")+".xls");
...
//			}
			...
			//获取要分组的字段，放入Map中
 			...
 			if(groupNames!=null){
	 			for(String groupName:groupNames){
		 			...
		 			if($.empty(timeType))
		 				groups.put(CsOutsideStatistics.C.get(groupName),"{key}");
		 			else{
		 				groups.put(CsOutsideStatistics.C.get(groupName),"DATE_FORMAT({key},'"+timeType.split(",")[0]+"')");
		 				$.setRequest("haveTimeType",true);
		 				timeTypes.add(groupName);
		 				if(timeType.startsWith("%Y"))
		 					objects.put("asc", CsOutsideStatistics.C.get(groupName));
		 			}
 			...
 			if(sumNames!=null){
	 			for(int i=0;i<sumNames.length;i++){
	 				...
	 				if(sumName.indexOf(":")==-1){
		 				sums.put(CsOutsideStatistics.C.get(sumName),"sum({key})");
			...
			//调用Service获取统计数据
			List list = csOutsideStatisticsService.getCsOutsideStatisticsStats(objects,groups,sums);
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csOutsideStatistics.getCsosId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsOutsideStatisticsToken = $.getString("csOutsideStatisticsToken");
			final String sessionCsOutsideStatisticsToken = $.getSession("csOutsideStatisticsToken");
			if($.equals(requestCsOutsideStatisticsToken,sessionCsOutsideStatisticsToken)){
				$.removeSession("csOutsideStatisticsToken");//移除令牌				
				...
				csOutsideStatisticsService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csOutsideStatistics.getCsosId()==null?"":csOutsideStatistics.getCsosId());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csOutsideStatistics.setCsosId(Long.valueOf(sItem));
							
								//设置默认值
								if(csOutsideStatistics.getCsosAddTime()==null)
									csOutsideStatistics.setCsosAddTime(new Date());
								...
								csOutsideStatistics=csOutsideStatisticsService.saveCsOutsideStatistics(csOutsideStatistics);			
								
								on("save",csOutsideStatistics,csOutsideStatistics);
							}							
							
							csOutsideStatisticsService.updateCsOutsideStatistics$NotNull(csOutsideStatistics);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsOutsideStatistics.class,"add","添加了[巡检作业]["+csOutsideStatistics.getCsosId()+"]",(Long)$.getSession("ccclubs_login_id"), csOutsideStatistics,csOutsideStatistics.getCsosId());
							$.SetTips("保存车辆整备成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csOutsideStatisticsService.updateCsOutsideStatisticsByConfirm(ActionHelper.getSetValuesFromModel(csOutsideStatistics), params);
								LoggerHelper.writeLog(CsOutsideStatistics.class, "update", 
											"批量修改了[车辆整备]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csOutsideStatistics,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csOutsideStatistics.setCsosId(Long.valueOf(sid));
									CsOutsideStatistics oldCsOutsideStatistics = CsOutsideStatistics.get(Long.valueOf(sid));
									csOutsideStatisticsService.updateCsOutsideStatistics$NotNull(csOutsideStatistics);
									LoggerHelper.writeLog(CsOutsideStatistics.class, "update", 
											"修改了[车辆整备]["+oldCsOutsideStatistics.getCsosId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsOutsideStatistics, csOutsideStatistics,false)
											,Long.valueOf(sid));
									on($.getString("method"),csOutsideStatistics,oldCsOutsideStatistics);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									CsOutsideStatistics oldCsOutsideStatistics = CsOutsideStatistics.get(csOutsideStatistics.getCsosId());
									csOutsideStatisticsService.updateCsOutsideStatistics$NotNull(csOutsideStatistics);
									LoggerHelper.writeLog(CsOutsideStatistics.class, "update", 
											"修改了[车辆整备]["+oldCsOutsideStatistics.getCsosId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsOutsideStatistics, csOutsideStatistics,false)
											,csOutsideStatistics.getCsosId());
									on($.getString("method"),csOutsideStatistics,oldCsOutsideStatistics);									
								}else{
									CsOutsideStatistics oldCsOutsideStatistics = csOutsideStatisticsService.getCsOutsideStatisticsById(csOutsideStatistics.getCsosId());
									//设置默认值
									csOutsideStatistics.setCsosAddTime(oldCsOutsideStatistics.getCsosAddTime());	
									...
									//从oldcsOutsideStatistics同步csOutsideStatistics未set过的值
									csOutsideStatistics.mergeSet(oldCsOutsideStatistics);
											
									csOutsideStatisticsService.updateCsOutsideStatistics(csOutsideStatistics);
									LoggerHelper.writeLog(CsOutsideStatistics.class, "update", 
											"修改了[车辆整备]["+oldCsOutsideStatistics.getCsosId()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsOutsideStatistics, csOutsideStatistics,true)
											,csOutsideStatistics.getCsosId());
									on("update",csOutsideStatistics,oldCsOutsideStatistics);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csOutsideStatistics)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("outsideqr_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csOutsideStatistics.getCsosId());
				else
	...
	public CsOutsideStatistics getCsOutsideStatistics() {
		return csOutsideStatistics;
	}
	public void setCsOutsideStatistics(CsOutsideStatistics csOutsideStatistics) {
		this.csOutsideStatistics = csOutsideStatistics;
	}
