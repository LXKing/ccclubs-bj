...
public class ProblemAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",TbProblem.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(TbProblem.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(TbProblem.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(TbProblem.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=TbProblem.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=TbProblem.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","tbp_id");//那么，默认以主键降序，传给Dao层
			...
			//反强制限制域
			if(objects.get("tbpMember")!=null && objects.get("tbpMemberNot")==null){
				objects.remove("HOSTS");
			}
			if(objects.get("tbpOrder")!=null && objects.get("tbpOrderNot")==null){
				objects.remove("HOSTS");
			}
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/oa/cooperate/problem.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return tbProblemService.getTbProblemPage(page, size, finalParams);
					}
				}, TbProblem.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = tbProblemService.getTbProblemPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("tbProblemToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，tbProblem这个对象为什么不用$.setRequest,因为tbProblem有get,set方法，由struts处理的
				tbProblem=tbProblemService.getTbProblemById(id);
				//确认域标识
				if(SystemHelper.testHost(tbProblem.getTbpHost()).equals("0")){
			...
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(tbProblem);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,TbProblem object,TbProblem old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(TbProblem.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			if($.equals("save", type)){
				new com.ccclubs.model.TbSolve(
					object.getTbpHost(),
				 	"有新的故障报修，请及时处理"//处理描述 [非空]
				 	,object.getTbpId()//所属问题 [非空]
				 	,object.getTbpDealer()//处理人 [非空]
				 	,null//流转到
				 	,(short)0
				 ).save();
				com.ccclubs.model.SrvUserExp user = com.ccclubs.model.SrvUserExp.where().sueUser(object.getTbpDealer()).get();
				if(user!=null)
				com.ccclubs.helper.WeixinHelper.sendWxTemplateMessage(
						null,(short)1,user.getSueWxOpenid(), 
						com.ccclubs.helper.WeixinHelper.WxTemplateMsgType.故障报修
						, TbProblem.class, object.getTbpId());
			}
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && tbProblem.getTbpId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestTbProblemToken = $.getString("tbProblemToken");
			final String sessionTbProblemToken = $.getSession("tbProblemToken");
			if($.equals(requestTbProblemToken,sessionTbProblemToken)){
				$.removeSession("tbProblemToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				tbProblemService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(tbProblem.getTbpProfile()==null?"":tbProblem.getTbpProfile());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								tbProblem.setTbpProfile(String.valueOf(sItem));
							
								//设置默认值
								if(tbProblem.getTbpAdder()==null)
									tbProblem.setTbpAdder(LoginHelper.getLoginId());
								if(tbProblem.getTbpEditor()==null)
									tbProblem.setTbpEditor(LoginHelper.getLoginId());
								if(tbProblem.getTbpUpdateTime()==null)
									tbProblem.setTbpUpdateTime(new Date());
								if(tbProblem.getTbpAddTime()==null)
									tbProblem.setTbpAddTime(new Date());
								if(tbProblem.getTbpStatus()==null)
								...
								tbProblem=tbProblemService.saveTbProblem(tbProblem);			
								
								on("save",tbProblem,tbProblem);
							}							
							
							tbProblemService.updateTbProblem$NotNull(tbProblem);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(TbProblem.class,"add","添加了[问题故障报修]["+tbProblem.getTbpProfile()+"]",(Long)$.getSession("ccclubs_login_id"), tbProblem,tbProblem.getTbpId());
							$.SetTips("保存问题故障报修成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								tbProblemService.updateTbProblemByConfirm(ActionHelper.getSetValuesFromModel(tbProblem), params);
								LoggerHelper.writeLog(TbProblem.class, "update", 
											"批量修改了[问题故障报修]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,tbProblem,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									tbProblem.setTbpId(Long.valueOf(sid));
									TbProblem oldTbProblem = TbProblem.get(Long.valueOf(sid));
									tbProblemService.updateTbProblem$NotNull(tbProblem);
									LoggerHelper.writeLog(TbProblem.class, "update", 
											"修改了[问题故障报修]["+oldTbProblem.getTbpProfile()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldTbProblem, tbProblem,false)
											,Long.valueOf(sid));
									on($.getString("method"),tbProblem,oldTbProblem);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									TbProblem oldTbProblem = TbProblem.get(tbProblem.getTbpId());
									tbProblemService.updateTbProblem$NotNull(tbProblem);
									LoggerHelper.writeLog(TbProblem.class, "update", 
											"修改了[问题故障报修]["+oldTbProblem.getTbpProfile()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldTbProblem, tbProblem,false)
									...
									TbProblem oldTbProblem = tbProblemService.getTbProblemById(tbProblem.getTbpId());
									//设置默认值
									tbProblem.setTbpAdder(oldTbProblem.getTbpAdder());	
									tbProblem.setTbpEditor(oldTbProblem.getTbpEditor());	
									tbProblem.setTbpRemark(oldTbProblem.getTbpRemark());	
									tbProblem.setTbpData(oldTbProblem.getTbpData());	
									tbProblem.setTbpUpdateTime(oldTbProblem.getTbpUpdateTime());	
									tbProblem.setTbpAddTime(oldTbProblem.getTbpAddTime());	
									...
									//从oldTbProblem同步tbProblem未set过的值
									tbProblem.mergeSet(oldTbProblem);
											
									tbProblemService.updateTbProblem(tbProblem);
									LoggerHelper.writeLog(TbProblem.class, "update", 
											"修改了[问题故障报修]["+oldTbProblem.getTbpProfile()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldTbProblem, tbProblem,true)
											,tbProblem.getTbpId());
									on("update",tbProblem,oldTbProblem);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(tbProblem)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("problem_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+tbProblem.getTbpId());
				else
		...
		try{
			...
			tbProblemService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							TbProblem tbProblem = tbProblemService.getTbProblemById(id);
							//确认域标识
							if(SystemHelper.testHost(tbProblem.getTbpHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								tbProblemService.removeTbProblemById(id);
							else
							tbProblemService.deleteTbProblemById(id);			
							//记录操作日志
							if(tbProblem!=null)
								LoggerHelper.writeLog(TbProblem.class,"del","删除了[问题故障报修]["+tbProblem.getTbpProfile()+"]",(Long)$.getSession("ccclubs_login_id"), tbProblem);
							on("delete",tbProblem,tbProblem);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				tbProblem=tbProblemService.getTbProblemById(id);
			if(key!=null && id==null)
				tbProblem=TbProblem.Get($.add(TbProblem.F.tbpId,key));
			//确认域标识
			if(SystemHelper.testHost(tbProblem.getTbpHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(TbProblem.class);
			//强制限制域
			params.put("HOSTS",SystemHelper.testHost(null));
			
			//反强制限制域
			if(params.get("tbpMember")!=null && params.get("tbpMemberNot")==null){
				params.remove("HOSTS");
			}
			if(params.get("tbpOrder")!=null && params.get("tbpOrderNot")==null){
				params.remove("HOSTS");
			}
			...
			if(!$.empty(strTerm)){
				...
				{
					String strDefinex = "";
					strDefinex+=" or tbp_profile like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<TbProblem> list = tbProblemService.getTbProblemPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(TbProblem tbProblem:list){
				Map map = new HashMap();
				map.put("value",tbProblem.getTbpId().toString());
				map.put("text", $.js(tbProblem.getKeyValue()));
				if(bObject==true)
					map.put("object",tbProblem);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
