...
{
	ITbDepartmentService tbDepartmentService;
	...
	{	
		try{
			...
			$.setRequest("modelClass",TbDepartment.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(TbDepartment.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(TbDepartment.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(TbDepartment.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=TbDepartment.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=TbDepartment.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","tbd_id");//那么，默认以主键降序，传给Dao层
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/oa/hr/department.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return tbDepartmentService.getTbDepartmentPage(page, size, finalParams);
					}
				}, TbDepartment.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = tbDepartmentService.getTbDepartmentPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("tbDepartmentToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，tbDepartment这个对象为什么不用$.setRequest,因为tbDepartment有get,set方法，由struts处理的
				tbDepartment=tbDepartmentService.getTbDepartmentById(id);
				//确认域标识
				if(SystemHelper.testHost(tbDepartment.getTbdHost()).equals("0")){
			...
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(tbDepartment);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,TbDepartment object,TbDepartment old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(TbDepartment.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && tbDepartment.getTbdId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestTbDepartmentToken = $.getString("tbDepartmentToken");
			final String sessionTbDepartmentToken = $.getSession("tbDepartmentToken");
			if($.equals(requestTbDepartmentToken,sessionTbDepartmentToken)){
				$.removeSession("tbDepartmentToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				tbDepartmentService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(tbDepartment.getTbdName()==null?"":tbDepartment.getTbdName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								tbDepartment.setTbdName(String.valueOf(sItem));
								...
								tbDepartment=tbDepartmentService.saveTbDepartment(tbDepartment);			
								
								on("save",tbDepartment,tbDepartment);
							}							
							
							tbDepartmentService.updateTbDepartment$NotNull(tbDepartment);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(TbDepartment.class,"add","添加了[公司部门]["+tbDepartment.getTbdName()+"]",(Long)$.getSession("ccclubs_login_id"), tbDepartment,tbDepartment.getTbdId());
							$.SetTips("保存公司部门成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								tbDepartmentService.updateTbDepartmentByConfirm(ActionHelper.getSetValuesFromModel(tbDepartment), params);
								LoggerHelper.writeLog(TbDepartment.class, "update", 
											"批量修改了[公司部门]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,tbDepartment,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									tbDepartment.setTbdId(Long.valueOf(sid));
									TbDepartment oldTbDepartment = TbDepartment.get(Long.valueOf(sid));
									tbDepartmentService.updateTbDepartment$NotNull(tbDepartment);
									LoggerHelper.writeLog(TbDepartment.class, "update", 
											"修改了[公司部门]["+oldTbDepartment.getTbdName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldTbDepartment, tbDepartment,false)
											,Long.valueOf(sid));
									on($.getString("method"),tbDepartment,oldTbDepartment);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									TbDepartment oldTbDepartment = TbDepartment.get(tbDepartment.getTbdId());
									tbDepartmentService.updateTbDepartment$NotNull(tbDepartment);
									LoggerHelper.writeLog(TbDepartment.class, "update", 
											"修改了[公司部门]["+oldTbDepartment.getTbdName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldTbDepartment, tbDepartment,false)
											,tbDepartment.getTbdId());
									on($.getString("method"),tbDepartment,oldTbDepartment);									
								}else{
									TbDepartment oldTbDepartment = tbDepartmentService.getTbDepartmentById(tbDepartment.getTbdId());
									//设置默认值
									...
									//从oldTbDepartment同步tbDepartment未set过的值
									tbDepartment.mergeSet(oldTbDepartment);
											
									tbDepartmentService.updateTbDepartment(tbDepartment);
									LoggerHelper.writeLog(TbDepartment.class, "update", 
											"修改了[公司部门]["+oldTbDepartment.getTbdName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldTbDepartment, tbDepartment,true)
											,tbDepartment.getTbdId());
									on("update",tbDepartment,oldTbDepartment);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(tbDepartment)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("department_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+tbDepartment.getTbdId());
				else
		...
		try{
			...
			tbDepartmentService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							TbDepartment tbDepartment = tbDepartmentService.getTbDepartmentById(id);
							//确认域标识
							if(SystemHelper.testHost(tbDepartment.getTbdHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								tbDepartmentService.removeTbDepartmentById(id);
							else
							tbDepartmentService.deleteTbDepartmentById(id);			
							//记录操作日志
							if(tbDepartment!=null)
								LoggerHelper.writeLog(TbDepartment.class,"del","删除了[公司部门]["+tbDepartment.getTbdName()+"]",(Long)$.getSession("ccclubs_login_id"), tbDepartment);
							on("delete",tbDepartment,tbDepartment);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				tbDepartment=tbDepartmentService.getTbDepartmentById(id);
			if(key!=null && id==null)
				tbDepartment=TbDepartment.Get($.add(TbDepartment.F.tbdId,key));
			//确认域标识
			if(SystemHelper.testHost(tbDepartment.getTbdHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(TbDepartment.class);
			//强制限制域
			...
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				params.put("desc","tbd_id");//那么，默认以主键降序，传给Dao层
								
			String strTerm=$.getString("value");//智能搜索时的参数，一般都是主键后面那个字段
			if(!$.empty(strTerm)){
				if(strTerm.toLowerCase().startsWith("id:"))//如果查询条件以id:开头，则按ID查询
					params.put("tbdId", strTerm.toLowerCase().replaceFirst("id:", ""));					
				else//按标识查询，模糊查询请带%
				{
					String strDefinex = "";
					strDefinex+=" or tbd_name like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<TbDepartment> list = tbDepartmentService.getTbDepartmentPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(TbDepartment tbDepartment:list){
				Map map = new HashMap();
				map.put("value",tbDepartment.getTbdId().toString());
				map.put("text", $.js(tbDepartment.getKeyValue()));
				if(bObject==true)
					map.put("object",tbDepartment);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public ITbDepartmentService getTbDepartmentService()
	{
		return tbDepartmentService;
	}

	public void setTbDepartmentService(ITbDepartmentService tbDepartmentService)
	{
