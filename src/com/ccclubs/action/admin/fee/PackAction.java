...
public class PackAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsPack.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsPack.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsPack.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsPack.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsPack.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsPack.C.get($.forMat($.getString("desc")));//降序字段
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/fee/pack.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csPackService.getCsPackPage(page, size, finalParams);
					}
				}, CsPack.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csPackService.getCsPackPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csPackToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csPack这个对象为什么不用$.setRequest,因为csPack有get,set方法，由struts处理的
				csPack=csPackService.getCsPackById(id);
			}
			if(csPack==null)
				csPack = new CsPack();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csPack);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsPack object,CsPack old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsPack.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csPack.getCspId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsPackToken = $.getString("csPackToken");
			final String sessionCsPackToken = $.getSession("csPackToken");
			if($.equals(requestCsPackToken,sessionCsPackToken)){
				$.removeSession("csPackToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csPackService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csPack.getCspName()==null?"":csPack.getCspName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csPack.setCspName(String.valueOf(sItem));
							
								//设置默认值
								if(csPack.getCspReorder()==null)
									csPack.setCspReorder((short)0);
								if(csPack.getCspUpdateTime()==null)
									csPack.setCspUpdateTime(new Date());
								if(csPack.getCspAddTime()==null)
									csPack.setCspAddTime(new Date());
								
								//根据自定义的默认值信息设置默认值
								if(CTRL!=null)
									CTRL.setObjectDefaultValue(csPack);
								
								csPack=csPackService.saveCsPack(csPack);			
								
								on("save",csPack,csPack);
							}							
							
							csPackService.updateCsPack$NotNull(csPack);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsPack.class,"add","添加了[系统套餐]["+csPack.getCspName()+"]",(Long)$.getSession("ccclubs_login_id"), csPack,csPack.getCspId());
							$.SetTips("保存系统套餐成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csPackService.updateCsPackByConfirm(ActionHelper.getSetValuesFromModel(csPack), params);
								LoggerHelper.writeLog(CsPack.class, "update", 
											"批量修改了[系统套餐]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csPack,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csPack.setCspId(Long.valueOf(sid));
									CsPack oldCsPack = CsPack.get(Long.valueOf(sid));
									csPackService.updateCsPack$NotNull(csPack);
									LoggerHelper.writeLog(CsPack.class, "update", 
											"修改了[系统套餐]["+oldCsPack.getCspName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsPack, csPack,false)
											,Long.valueOf(sid));
									on($.getString("method"),csPack,oldCsPack);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsPack oldCsPack = CsPack.get(csPack.getCspId());
									csPackService.updateCsPack$NotNull(csPack);
									LoggerHelper.writeLog(CsPack.class, "update", 
											"修改了[系统套餐]["+oldCsPack.getCspName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsPack, csPack,false)
									...
									CsPack oldCsPack = csPackService.getCsPackById(csPack.getCspId());
									//设置默认值
									csPack.setCspReorder(oldCsPack.getCspReorder());	
									csPack.setCspUpdateTime(oldCsPack.getCspUpdateTime());	
									csPack.setCspAddTime(oldCsPack.getCspAddTime());	
									...
									//从oldCsPack同步csPack未set过的值
									csPack.mergeSet(oldCsPack);
											
									csPackService.updateCsPack(csPack);
									LoggerHelper.writeLog(CsPack.class, "update", 
											"修改了[系统套餐]["+oldCsPack.getCspName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsPack, csPack,true)
											,csPack.getCspId());
									on("update",csPack,oldCsPack);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csPack)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("pack_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csPack.getCspId());
				else
		...
		try{
			...
			csPackService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsPack csPack = csPackService.getCsPackById(id);
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csPackService.removeCsPackById(id);
							else
							csPackService.deleteCsPackById(id);			
							//记录操作日志
							if(csPack!=null)
								LoggerHelper.writeLog(CsPack.class,"del","删除了[系统套餐]["+csPack.getCspName()+"]",(Long)$.getSession("ccclubs_login_id"), csPack);
							on("delete",csPack,csPack);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csPack=csPackService.getCsPackById(id);
			if(key!=null && id==null)
				csPack=CsPack.Get($.add(CsPack.F.cspId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsPack.class);
			...
			List<CsPack> list = csPackService.getCsPackPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsPack csPack:list){
				Map map = new HashMap();
				map.put("value",csPack.getCspId().toString());
				map.put("text", $.js(csPack.getKeyValue()));
				if(bObject==true)
					map.put("object",csPack);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public CsPack getCsPack()
	{
		return csPack;
	}

	public void setCsPack(CsPack csPack)
	{
		this.csPack = csPack;
	}
}