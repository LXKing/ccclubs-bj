...
public class RuleAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsRule.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsRule.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsRule.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsRule.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsRule.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsRule.C.get($.forMat($.getString("desc")));//降序字段
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/fee/rule.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csRuleService.getCsRulePage(page, size, finalParams);
					}
				}, CsRule.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csRuleService.getCsRulePage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csRuleToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csRule这个对象为什么不用$.setRequest,因为csRule有get,set方法，由struts处理的
				csRule=csRuleService.getCsRuleById(id);
			}
			if(csRule==null)
				csRule = new CsRule();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csRule);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsRule object,CsRule old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsRule.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csRule.getCsrId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsRuleToken = $.getString("csRuleToken");
			final String sessionCsRuleToken = $.getSession("csRuleToken");
			if($.equals(requestCsRuleToken,sessionCsRuleToken)){
				$.removeSession("csRuleToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csRuleService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csRule.getCsrName()==null?"":csRule.getCsrName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csRule.setCsrName(String.valueOf(sItem));
								...
								csRule=csRuleService.saveCsRule(csRule);			
								
								on("save",csRule,csRule);
							}							
							
							csRuleService.updateCsRule$NotNull(csRule);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsRule.class,"add","添加了[规则]["+csRule.getCsrName()+"]",(Long)$.getSession("ccclubs_login_id"), csRule,csRule.getCsrId());
							$.SetTips("保存规则成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csRuleService.updateCsRuleByConfirm(ActionHelper.getSetValuesFromModel(csRule), params);
								LoggerHelper.writeLog(CsRule.class, "update", 
											"批量修改了[规则]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csRule,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csRule.setCsrId(Long.valueOf(sid));
									CsRule oldCsRule = CsRule.get(Long.valueOf(sid));
									csRuleService.updateCsRule$NotNull(csRule);
									LoggerHelper.writeLog(CsRule.class, "update", 
											"修改了[规则]["+oldCsRule.getCsrName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsRule, csRule,false)
											,Long.valueOf(sid));
									on($.getString("method"),csRule,oldCsRule);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									CsRule oldCsRule = CsRule.get(csRule.getCsrId());
									csRuleService.updateCsRule$NotNull(csRule);
									LoggerHelper.writeLog(CsRule.class, "update", 
											"修改了[规则]["+oldCsRule.getCsrName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsRule, csRule,false)
									...
									CsRule oldCsRule = csRuleService.getCsRuleById(csRule.getCsrId());
									//设置默认值
									csRule.setCsrName(oldCsRule.getCsrName());	
									csRule.setCsrMeas(oldCsRule.getCsrMeas());	
									
									//设置填充值
									
									//从oldCsRule同步csRule未set过的值
									csRule.mergeSet(oldCsRule);
											
									csRuleService.updateCsRule(csRule);
									LoggerHelper.writeLog(CsRule.class, "update", 
											"修改了[规则]["+oldCsRule.getCsrName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsRule, csRule,true)
											,csRule.getCsrId());
									on("update",csRule,oldCsRule);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csRule)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("rule_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csRule.getCsrId());
				else
		...
		try{
			...
			csRuleService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsRule csRule = csRuleService.getCsRuleById(id);
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csRuleService.removeCsRuleById(id);
							else
							csRuleService.deleteCsRuleById(id);			
							//记录操作日志
							if(csRule!=null)
								LoggerHelper.writeLog(CsRule.class,"del","删除了[规则]["+csRule.getCsrName()+"]",(Long)$.getSession("ccclubs_login_id"), csRule);
							on("delete",csRule,csRule);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csRule=csRuleService.getCsRuleById(id);
			if(key!=null && id==null)
				csRule=CsRule.Get($.add(CsRule.F.csrId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsRule.class);
			...
			if(!$.empty(strTerm)){
				...
				{
					String strDefinex = "";
					strDefinex+=" or csr_name like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsRule> list = csRuleService.getCsRulePage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsRule csRule:list){
				Map map = new HashMap();
				map.put("value",csRule.getCsrId().toString());
				map.put("text", $.js(csRule.getKeyValue()));
				if(bObject==true)
					map.put("object",csRule);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public ICsRuleService getCsRuleService()
	{
		return csRuleService;
	}

	public void setCsRuleService(ICsRuleService csRuleService)
	{
		this.csRuleService = csRuleService;
	}

	public CsRule getCsRule()
	{
		return csRule;
	}

	public void setCsRule(CsRule csRule)
	{
		this.csRule = csRule;
	}
}