...
public class ItemAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsItem.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsItem.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsItem.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsItem.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsItem.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsItem.C.get($.forMat($.getString("desc")));//降序字段
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/fee/item.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csItemService.getCsItemPage(page, size, finalParams);
					}
				}, CsItem.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csItemService.getCsItemPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csItemToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csItem这个对象为什么不用$.setRequest,因为csItem有get,set方法，由struts处理的
				csItem=csItemService.getCsItemById(id);
			}
			if(csItem==null)
				csItem = new CsItem();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csItem);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsItem object,CsItem old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsItem.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csItem.getCsiId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsItemToken = $.getString("csItemToken");
			final String sessionCsItemToken = $.getSession("csItemToken");
			if($.equals(requestCsItemToken,sessionCsItemToken)){
				$.removeSession("csItemToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csItemService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csItem.getCsiTitle()==null?"":csItem.getCsiTitle());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csItem.setCsiTitle(String.valueOf(sItem));
							
								//设置默认值
								if(csItem.getCsiSale()==null)
									csItem.setCsiSale(0);
								if(csItem.getCsiLevel()==null)
									csItem.setCsiLevel(0);
								if(csItem.getCsiUpdateTime()==null)
									csItem.setCsiUpdateTime(new Date());
								if(csItem.getCsiAddTime()==null)
									csItem.setCsiAddTime(new Date());
								
								//根据自定义的默认值信息设置默认值
								if(CTRL!=null)
									CTRL.setObjectDefaultValue(csItem);
								
								csItem=csItemService.saveCsItem(csItem);			
								
								on("save",csItem,csItem);
							}							
							
							csItemService.updateCsItem$NotNull(csItem);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsItem.class,"add","添加了[商品信息]["+csItem.getCsiTitle()+"]",(Long)$.getSession("ccclubs_login_id"), csItem,csItem.getCsiId());
							$.SetTips("保存商品信息成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csItemService.updateCsItemByConfirm(ActionHelper.getSetValuesFromModel(csItem), params);
								LoggerHelper.writeLog(CsItem.class, "update", 
											"批量修改了[商品信息]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csItem,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csItem.setCsiId(Long.valueOf(sid));
									CsItem oldCsItem = CsItem.get(Long.valueOf(sid));
									csItemService.updateCsItem$NotNull(csItem);
									LoggerHelper.writeLog(CsItem.class, "update", 
											"修改了[商品信息]["+oldCsItem.getCsiTitle()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsItem, csItem,false)
											,Long.valueOf(sid));
									on($.getString("method"),csItem,oldCsItem);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsItem oldCsItem = CsItem.get(csItem.getCsiId());
									csItemService.updateCsItem$NotNull(csItem);
									LoggerHelper.writeLog(CsItem.class, "update", 
											"修改了[商品信息]["+oldCsItem.getCsiTitle()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsItem, csItem,false)
									...
									CsItem oldCsItem = csItemService.getCsItemById(csItem.getCsiId());
									//设置默认值
									csItem.setCsiUpdateTime(oldCsItem.getCsiUpdateTime());	
									csItem.setCsiAddTime(oldCsItem.getCsiAddTime());	
									
									//设置填充值
									if(csItem.getCsiUpdateTime()==null)
										csItem.setCsiUpdateTime(new Date());
									
									//从oldCsItem同步csItem未set过的值
									csItem.mergeSet(oldCsItem);
											
									csItemService.updateCsItem(csItem);
									LoggerHelper.writeLog(CsItem.class, "update", 
											"修改了[商品信息]["+oldCsItem.getCsiTitle()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsItem, csItem,true)
											,csItem.getCsiId());
									on("update",csItem,oldCsItem);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csItem)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("item_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csItem.getCsiId());
				else
		...
		try{
			...
			csItemService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsItem csItem = csItemService.getCsItemById(id);
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csItemService.removeCsItemById(id);
							else
							csItemService.deleteCsItemById(id);			
							//记录操作日志
							if(csItem!=null)
								LoggerHelper.writeLog(CsItem.class,"del","删除了[商品信息]["+csItem.getCsiTitle()+"]",(Long)$.getSession("ccclubs_login_id"), csItem);
							on("delete",csItem,csItem);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csItem=csItemService.getCsItemById(id);
			if(key!=null && id==null)
				csItem=CsItem.Get($.add(CsItem.F.csiId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsItem.class);
			...
			List<CsItem> list = csItemService.getCsItemPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsItem csItem:list){
				Map map = new HashMap();
				map.put("value",csItem.getCsiId().toString());
				map.put("text", $.js(csItem.getKeyValue()));
				if(bObject==true)
					map.put("object",csItem);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
