...
public class ProductAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsProduct.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsProduct.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsProduct.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsProduct.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsProduct.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsProduct.C.get($.forMat($.getString("desc")));//降序字段
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/fee/product.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csProductService.getCsProductPage(page, size, finalParams);
					}
				}, CsProduct.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csProductService.getCsProductPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csProductToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csProduct这个对象为什么不用$.setRequest,因为csProduct有get,set方法，由struts处理的
				csProduct=csProductService.getCsProductById(id);
			}
			if(csProduct==null)
				csProduct = new CsProduct();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csProduct);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsProduct object,CsProduct old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsProduct.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csProduct.getCspId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsProductToken = $.getString("csProductToken");
			final String sessionCsProductToken = $.getSession("csProductToken");
			if($.equals(requestCsProductToken,sessionCsProductToken)){
				$.removeSession("csProductToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csProductService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csProduct.getCspName()==null?"":csProduct.getCspName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csProduct.setCspName(String.valueOf(sItem));
							
								//设置默认值
								if(csProduct.getCspUpdateTime()==null)
									csProduct.setCspUpdateTime(new Date());
								if(csProduct.getCspAddTime()==null)
									csProduct.setCspAddTime(new Date());
								
								//根据自定义的默认值信息设置默认值
								if(CTRL!=null)
									CTRL.setObjectDefaultValue(csProduct);
								
								csProduct=csProductService.saveCsProduct(csProduct);			
								
								on("save",csProduct,csProduct);
							}							
							
							csProductService.updateCsProduct$NotNull(csProduct);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsProduct.class,"add","添加了[产品]["+csProduct.getCspName()+"]",(Long)$.getSession("ccclubs_login_id"), csProduct,csProduct.getCspId());
							$.SetTips("保存产品成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csProductService.updateCsProductByConfirm(ActionHelper.getSetValuesFromModel(csProduct), params);
								LoggerHelper.writeLog(CsProduct.class, "update", 
											"批量修改了[产品]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csProduct,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csProduct.setCspId(Long.valueOf(sid));
									CsProduct oldCsProduct = CsProduct.get(Long.valueOf(sid));
									csProductService.updateCsProduct$NotNull(csProduct);
									LoggerHelper.writeLog(CsProduct.class, "update", 
											"修改了[产品]["+oldCsProduct.getCspName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsProduct, csProduct,false)
											,Long.valueOf(sid));
									on($.getString("method"),csProduct,oldCsProduct);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsProduct oldCsProduct = CsProduct.get(csProduct.getCspId());
									csProductService.updateCsProduct$NotNull(csProduct);
									LoggerHelper.writeLog(CsProduct.class, "update", 
											"修改了[产品]["+oldCsProduct.getCspName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsProduct, csProduct,false)
									...
									CsProduct oldCsProduct = csProductService.getCsProductById(csProduct.getCspId());
									//设置默认值
									csProduct.setCspName(oldCsProduct.getCspName());	
									csProduct.setCspUpdateTime(oldCsProduct.getCspUpdateTime());	
									csProduct.setCspAddTime(oldCsProduct.getCspAddTime());	
									
									//设置填充值
									if(csProduct.getCspUpdateTime()==null)
										csProduct.setCspUpdateTime(new Date());
									
									//从oldCsProduct同步csProduct未set过的值
									csProduct.mergeSet(oldCsProduct);
											
									csProductService.updateCsProduct(csProduct);
									LoggerHelper.writeLog(CsProduct.class, "update", 
											"修改了[产品]["+oldCsProduct.getCspName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsProduct, csProduct,true)
											,csProduct.getCspId());
									on("update",csProduct,oldCsProduct);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csProduct)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("product_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csProduct.getCspId());
				else
		...
		try{
			...
			csProductService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsProduct csProduct = csProductService.getCsProductById(id);
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csProductService.removeCsProductById(id);
							else
							csProductService.deleteCsProductById(id);			
							//记录操作日志
							if(csProduct!=null)
								LoggerHelper.writeLog(CsProduct.class,"del","删除了[产品]["+csProduct.getCspName()+"]",(Long)$.getSession("ccclubs_login_id"), csProduct);
							on("delete",csProduct,csProduct);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csProduct=csProductService.getCsProductById(id);
			if(key!=null && id==null)
				csProduct=CsProduct.Get($.add(CsProduct.F.cspId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsProduct.class);
			...
			List<CsProduct> list = csProductService.getCsProductPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsProduct csProduct:list){
				Map map = new HashMap();
				map.put("value",csProduct.getCspId().toString());
				map.put("text", $.js(csProduct.getKeyValue()));
				if(bObject==true)
					map.put("object",csProduct);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
