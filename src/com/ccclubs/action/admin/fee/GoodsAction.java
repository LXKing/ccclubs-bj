...
public class GoodsAction
{
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsGoods.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsGoods.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsGoods.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsGoods.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsGoods.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsGoods.C.get($.forMat($.getString("desc")));//降序字段
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/fee/goods.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csGoodsService.getCsGoodsPage(page, size, finalParams);
					}
				}, CsGoods.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csGoodsService.getCsGoodsPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{		
		try{
			...
			$.setSession("csGoodsToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csGoods这个对象为什么不用$.setRequest,因为csGoods有get,set方法，由struts处理的
				csGoods=csGoodsService.getCsGoodsById(id);
			}
			if(csGoods==null)
				csGoods = new CsGoods();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csGoods);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsGoods object,CsGoods old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsGoods.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csGoods.getCsgId()==null);
			...
			if($.equals(requestCsGoodsToken,sessionCsGoodsToken)){
				...
				csGoodsService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csGoods.getCsgName()==null?"":csGoods.getCsgName());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csGoods.setCsgName(String.valueOf(sItem));
							
								//设置默认值
								if(csGoods.getCsgName()==null)
									csGoods.setCsgName(csGoods.get$csgProduct().getCspName()+csGoods.get$csgRule().getCsrName());
								if(csGoods.getCsgUpdateTime()==null)
									csGoods.setCsgUpdateTime(new Date());
								if(csGoods.getCsgAddTime()==null)
									csGoods.setCsgAddTime(new Date());
								
								//根据自定义的默认值信息设置默认值
								if(CTRL!=null)
									CTRL.setObjectDefaultValue(csGoods);
								
								csGoods=csGoodsService.saveCsGoods(csGoods);			
								
								on("save",csGoods,csGoods);
							}							
							
							csGoodsService.updateCsGoods$NotNull(csGoods);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsGoods.class,"add","添加了[商品]["+csGoods.getCsgName()+"]",(Long)$.getSession("ccclubs_login_id"), csGoods,csGoods.getCsgId());
							$.SetTips("保存商品成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csGoodsService.updateCsGoodsByConfirm(ActionHelper.getSetValuesFromModel(csGoods), params);
								LoggerHelper.writeLog(CsGoods.class, "update", 
											"批量修改了[商品]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csGoods,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csGoods.setCsgId(Long.valueOf(sid));
									CsGoods oldCsGoods = CsGoods.get(Long.valueOf(sid));
									csGoodsService.updateCsGoods$NotNull(csGoods);
									LoggerHelper.writeLog(CsGoods.class, "update", 
											"修改了[商品]["+oldCsGoods.getCsgName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsGoods, csGoods,false)
											,Long.valueOf(sid));
									on($.getString("method"),csGoods,oldCsGoods);
								}
							}else{
								if($.empty($.getString("edittype"))){
									...
									CsGoods oldCsGoods = CsGoods.get(csGoods.getCsgId());
									csGoodsService.updateCsGoods$NotNull(csGoods);
									LoggerHelper.writeLog(CsGoods.class, "update", 
											"修改了[商品]["+oldCsGoods.getCsgName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsGoods, csGoods,false)
									...
									CsGoods oldCsGoods = csGoodsService.getCsGoodsById(csGoods.getCsgId());
									//设置默认值
									csGoods.setCsgUpdateTime(oldCsGoods.getCsgUpdateTime());	
									csGoods.setCsgAddTime(oldCsGoods.getCsgAddTime());	
									...
									//从oldCsGoods同步csGoods未set过的值
									csGoods.mergeSet(oldCsGoods);
											
									csGoodsService.updateCsGoods(csGoods);
									LoggerHelper.writeLog(CsGoods.class, "update", 
											"修改了[商品]["+oldCsGoods.getCsgName()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsGoods, csGoods,true)
											,csGoods.getCsgId());
									on("update",csGoods,oldCsGoods);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csGoods)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("goods_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csGoods.getCsgId());
				else
		...
		try{
			...
			csGoodsService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsGoods csGoods = csGoodsService.getCsGoodsById(id);
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csGoodsService.removeCsGoodsById(id);
							else
							csGoodsService.deleteCsGoodsById(id);			
							//记录操作日志
							if(csGoods!=null)
								LoggerHelper.writeLog(CsGoods.class,"del","删除了[商品]["+csGoods.getCsgName()+"]",(Long)$.getSession("ccclubs_login_id"), csGoods);
							on("delete",csGoods,csGoods);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csGoods=csGoodsService.getCsGoodsById(id);
			if(key!=null && id==null)
				csGoods=CsGoods.Get($.add(CsGoods.F.csgId,key));
			/************LAZY3Q_CODE_DETAIL************/
			/************LAZY3Q_CODE_DETAIL************/
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsGoods.class);
			...
			List<CsGoods> list = csGoodsService.getCsGoodsPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsGoods csGoods:list){
				Map map = new HashMap();
				map.put("value",csGoods.getCsgId().toString());
				map.put("text", $.js(csGoods.getKeyValue()));
				if(bObject==true)
					map.put("object",csGoods);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public CsGoods getCsGoods()
	{
		return csGoods;
	}

	public void setCsGoods(CsGoods csGoods)
	{
		this.csGoods = csGoods;
	}
}