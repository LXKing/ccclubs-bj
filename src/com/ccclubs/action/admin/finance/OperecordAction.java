...
public class OperecordAction
{
	ICsOperRecordService csOperRecordService;
	
	CsOperRecord csOperRecord;
	...
	{	
		try{
			...
			$.setRequest("modelClass",CsOperRecord.class);//把当前Action对应的模型类放入到request中,以作为后面导出、、、、等功能的反射使用
			...
			ActionHelper.doPageViewFields(CsOperRecord.class);
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> objects = ActionHelper.getQueryFormParams(CsOperRecord.class);
			$.setRequest("PARAMS",ActionHelper.getQueryFormParams(CsOperRecord.class));
									
			//取排序参数,放入查询条件中，取不到放入查询条件中也没关系，因为Dao层会判断的
			String strAsc=CsOperRecord.C.get($.forMat($.getString("asc")));//升序字段
			objects.put("asc", strAsc);//放入查询条件
			String strDesc=CsOperRecord.C.get($.forMat($.getString("desc")));//降序字段
			objects.put("desc",strDesc);//放入查询条件
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				objects.put("desc","csor_id");//那么，默认以主键降序，传给Dao层
			...
			//反强制限制域
			if(objects.get("csorMember")!=null && objects.get("csorMemberNot")==null){
				objects.remove("HOSTS");
			}
			if(objects.get("csorOrder")!=null && objects.get("csorOrderNot")==null){
				objects.remove("HOSTS");
			}
			...
			if($.getBoolean("export",false)==true)//如果传入了一个数据导出的参数
			{
				...
				SystemHelper.getSrvLimit("admin/finance/operecord.do");
				...
				$.export($.getInteger("size",1000), 100, $.getInteger("page",0), new Export(){//new Export这个类，他会根据参数分次查询，边查边写，目的只有一个，就是为了避免把大量的数据放在内存中
					...
					public Page getPage(Integer page, int size) {//这时的page和size是Export转化过后的当前页，每页查询数					
						return csOperRecordService.getCsOperRecordPage(page, size, finalParams);
					}
				}, CsOperRecord.class, $.getArray("keys"));
			...
			//调用Service获取分页
			Page page = csOperRecordService.getCsOperRecordPage($.getInteger("page", 0), $.getInteger("size",16), objects);
			//把分页对象放入request
	...
	{	
		try{
			...
			if($.getBoolean("export",false)==true){//如果传入了一个数据导出的参数
				...
				response.setHeader("Content-Disposition", "attachment;filename="+java.net.URLEncoder.encode("收支记录统计导出","UTF-8")+".xls");
			...
			//获取要分组的字段，放入Map中
 			...
 			if(groupNames!=null){
	 			for(String groupName:groupNames){
		 			...
		 			if($.empty(timeType))
		 				groups.put(CsOperRecord.C.get(groupName),"{key}");
		 			else{
		 				groups.put(CsOperRecord.C.get(groupName),"DATE_FORMAT({key},'"+timeType.split(",")[0]+"')");
		 				$.setRequest("haveTimeType",true);
		 				timeTypes.add(groupName);
		 				if(timeType.startsWith("%Y"))
		 					objects.put("asc", CsOperRecord.C.get(groupName));
		 			}
 			...
 			if(sumNames!=null){
	 			for(int i=0;i<sumNames.length;i++){
	 				...
	 				if(sumName.indexOf(":")==-1){
		 				sums.put(CsOperRecord.C.get(sumName),"sum({key})");
			...
			//调用Service获取统计数据
			List list = csOperRecordService.getCsOperRecordStats(objects,groups,sums);
	...
	{		
		try{
			...
			$.setSession("csOperRecordToken", $.uuid());//防重复提交Token
			//当前要编辑的数据ID，有这个ID表示修改数据，没这个ID表示添加数据
			Long id=$.getLong("id");
			if(id!=null){
				//把当前要修改的数据查出来,注意了，csOperRecord这个对象为什么不用$.setRequest,因为csOperRecord有get,set方法，由struts处理的
				csOperRecord=csOperRecordService.getCsOperRecordById(id);
				//确认域标识
				if(SystemHelper.testHost(csOperRecord.getCsorHost()).equals("0")){
					return $.SendHtml("<script>alert('对不起，你没有权限编辑该数据');</script>", "UTF-8");
				}
			}
			if(csOperRecord==null)
				csOperRecord = new CsOperRecord();
			//根据自定义配置ctrl中配置的默认值信息设置默认值
			if(CTRL!=null)
				CTRL.setObjectDefaultValue(csOperRecord);
			/************LAZY3Q_CODE_EDIT************/
			/************LAZY3Q_CODE_EDIT************/
	 ...
	 * @return
	 */
	public void on(String type,CsOperRecord object,CsOperRecord old){
		try{
			//执行后台配置的动态代码脚本，如果有在后台配置的话(*_*)
			ActionHelper.executeActionScript(CsOperRecord.class, type, object, old);
			/************LAZY3Q_ACTION_ON************/
			/************LAZY3Q_ACTION_ON************/
	...
	public String save()
	{
		try{
			final Ctrl CTRL = $.setRequest("CTRL",$.CTRL($.getString("ctrl")));//可自定义配置控制器,定义哪些字段可显示、可编辑、是否可查询等
			//主键为空，表示插入,否则表示更新
			final Boolean bIsAdd = ($.empty($.getString("ids")) && $.empty($.getString("PARAMS")) && csOperRecord.getCsorId()==null);
			//Token技术建议百度下，就是为了防止重复提交
			final String requestCsOperRecordToken = $.getString("csOperRecordToken");
			final String sessionCsOperRecordToken = $.getSession("csOperRecordToken");
			if($.equals(requestCsOperRecordToken,sessionCsOperRecordToken)){
				$.removeSession("csOperRecordToken");//移除令牌				
				//多个写数据动作合并事务过程（当这个过程中发生任务错误时，由spring控制处理数据回滚，把所有修改的，添加的，删除的....统统恢复）
				csOperRecordService.executeTransaction(new Function(){
					public <T> T execute(Object... arg0){
						...
						if(bIsAdd){
							//批量插入处理
							String vValue = ""+(csOperRecord.getCsorTitle()==null?"":csOperRecord.getCsorTitle());
							...
							for(String sItem:sArray){
								if(!$.empty(sItem))
								csOperRecord.setCsorTitle(String.valueOf(sItem));
								...
								csOperRecord=csOperRecordService.saveCsOperRecord(csOperRecord);			
								
								on("save",csOperRecord,csOperRecord);
							}							
							
							csOperRecordService.updateCsOperRecord$NotNull(csOperRecord);						
							/************LAZY3Q_CODE_ADD************/
							/************LAZY3Q_CODE_ADD************/

							
							//记录操作日志
							LoggerHelper.writeLog(CsOperRecord.class,"add","添加了[收支记录]["+csOperRecord.getCsorTitle()+"]",(Long)$.getSession("ccclubs_login_id"), csOperRecord,csOperRecord.getCsorId());
							$.SetTips("保存收支记录成功");
						}else{
							...
							if(!$.empty(PARAMS)){//根据查询条件批量更新
								...
								csOperRecordService.updateCsOperRecordByConfirm(ActionHelper.getSetValuesFromModel(csOperRecord), params);
								LoggerHelper.writeLog(CsOperRecord.class, "update", 
											"批量修改了[收支记录]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(null,csOperRecord,false)
											,null);
							}else if(!$.empty(ids)){//根据ids批量更新
								String[] array = ids.split(",");
								for(String sid:array){
									csOperRecord.setCsorId(Long.valueOf(sid));
									CsOperRecord oldCsOperRecord = CsOperRecord.get(Long.valueOf(sid));
									csOperRecordService.updateCsOperRecord$NotNull(csOperRecord);
									LoggerHelper.writeLog(CsOperRecord.class, "update", 
											"修改了[收支记录]["+oldCsOperRecord.getCsorTitle()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsOperRecord, csOperRecord,false)
											,Long.valueOf(sid));
									on($.getString("method"),csOperRecord,oldCsOperRecord);
								}
							}else{
								if($.empty($.getString("edittype"))){
									//设置填充值
									
									CsOperRecord oldCsOperRecord = CsOperRecord.get(csOperRecord.getCsorId());
									csOperRecordService.updateCsOperRecord$NotNull(csOperRecord);
									LoggerHelper.writeLog(CsOperRecord.class, "update", 
											"修改了[收支记录]["+oldCsOperRecord.getCsorTitle()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsOperRecord, csOperRecord,false)
											,csOperRecord.getCsorId());
									on($.getString("method"),csOperRecord,oldCsOperRecord);									
								}else{
									CsOperRecord oldCsOperRecord = csOperRecordService.getCsOperRecordById(csOperRecord.getCsorId());
									//设置默认值
									...
									//从oldCsOperRecord同步csOperRecord未set过的值
									csOperRecord.mergeSet(oldCsOperRecord);
											
									csOperRecordService.updateCsOperRecord(csOperRecord);
									LoggerHelper.writeLog(CsOperRecord.class, "update", 
											"修改了[收支记录]["+oldCsOperRecord.getCsorTitle()+"]",
											(Long)$.getSession("ccclubs_login_id"),LoggerHelper.getUpdateDescription(oldCsOperRecord, csOperRecord,true)
											,csOperRecord.getCsorId());
									on("update",csOperRecord,oldCsOperRecord);
								}
							}
							/************LAZY3Q_CODE_UPDATE************/
							/************LAZY3Q_CODE_UPDATE************/
			...
			if(!$.empty(complete)){
				$.removeSession("tips");
				return $.SendHtml("<script>"+complete+"("+$.json(csOperRecord)+")"+"</script>","UTF-8");
			}
			...
			if($.getString("submiter","").indexOf("保存")>-1){//则回到编辑页
				if(bIsAdd)
					return $.Redirect("operecord_edit.do?entrypoint="+$.getString("entrypoint")+"&id="+csOperRecord.getCsorId());
				else
		...
		try{
			...
			csOperRecordService.executeTransaction(new Function(){
				public <T> T execute(Object... arg0){
					...
					for(Long id:ids){
							CsOperRecord csOperRecord = csOperRecordService.getCsOperRecordById(id);
							//确认域标识
							if(SystemHelper.testHost(csOperRecord.getCsorHost()).equals("0")){
								throw new RuntimeException("当前用户没有权限删除该数据");
							}
							if(ConfigHelper.GetBoolean("logic_delete")==true)
								csOperRecordService.removeCsOperRecordById(id);
							else
							csOperRecordService.deleteCsOperRecordById(id);			
							//记录操作日志
							if(csOperRecord!=null)
								LoggerHelper.writeLog(CsOperRecord.class,"del","删除了[收支记录]["+csOperRecord.getCsorTitle()+"]",(Long)$.getSession("ccclubs_login_id"), csOperRecord);
							on("delete",csOperRecord,csOperRecord);
					}
					/************LAZY3Q_CODE_DEL************/
					/************LAZY3Q_CODE_DEL************/
	...
	public String details()
	{
		try{
			...
			if(id!=null)
				csOperRecord=csOperRecordService.getCsOperRecordById(id);
			if(key!=null && id==null)
				csOperRecord=CsOperRecord.Get($.add(CsOperRecord.F.csorId,key));
			//确认域标识
			if(SystemHelper.testHost(csOperRecord.getCsorHost()).equals("0")){
	...
	public String query(){
		try{
			//根据当前的类，把表单参数转成Dao查询需要的Map参数格式,ActionHelper.getQueryFormParams这个东东在Lazy3q.jar中
			Map<String,Object> params = ActionHelper.getQueryFormParams(CsOperRecord.class);
			//强制限制域
			params.put("HOSTS",SystemHelper.testHost(null));
			
			//反强制限制域
			if(params.get("csorMember")!=null && params.get("csorMemberNot")==null){
				params.remove("HOSTS");
			}
			if(params.get("csorOrder")!=null && params.get("csorOrderNot")==null){
				params.remove("HOSTS");
			}
			...
			if($.empty(strAsc) && $.empty(strDesc))//如果未传入排序字段
				params.put("desc","csor_id");//那么，默认以主键降序，传给Dao层
								
			String strTerm=$.getString("value");//智能搜索时的参数，一般都是主键后面那个字段
			if(!$.empty(strTerm)){
				if(strTerm.toLowerCase().startsWith("id:"))//如果查询条件以id:开头，则按ID查询
					params.put("csorId", strTerm.toLowerCase().replaceFirst("id:", ""));					
				else//按标识查询，模糊查询请带%
				{
					String strDefinex = "";
					strDefinex+=" or csor_title like '"+strTerm.replaceAll("'", "''")+"%'";
					strDefinex+=" or csor_record_type like '"+strTerm.replaceAll("'", "''")+"%'";
			...
			List<CsOperRecord> list = csOperRecordService.getCsOperRecordPage(0,$.getInteger("size",10),params).getResult();
			...
			**/
			...
			for(CsOperRecord csOperRecord:list){
				Map map = new HashMap();
				map.put("value",csOperRecord.getCsorId().toString());
				map.put("text", $.js(csOperRecord.getKeyValue()));
				if(bObject==true)
					map.put("object",csOperRecord);
				result.add(map);
			}
			/************LAZY3Q_AFTER_QUERY************/
			/************LAZY3Q_AFTER_QUERY************/
	...
	public ICsOperRecordService getCsOperRecordService()
	{
		return csOperRecordService;
	}

	public void setCsOperRecordService(ICsOperRecordService csOperRecordService)
	{
		this.csOperRecordService = csOperRecordService;
	}

	public CsOperRecord getCsOperRecord()
	{
		return csOperRecord;
	}

	public void setCsOperRecord(CsOperRecord csOperRecord)
	{
		this.csOperRecord = csOperRecord;
	}
}