...
public class DefaultAction extends BaseAction{
	...
	public String login() {
		try {
			...
			if (SystemHelper.isEmail(strUsername))// 邮箱方式登录
				user = csMemberService.getCsMember($.Map("csmEmail",
						strUsername));
			else if (SystemHelper.isMobile(strUsername))// 手机方式登录
				user = csMemberService.getCsMember($.Map("csmMobile",
						strUsername));
			else
				// 帐号方式登录
				user = csMemberService.getCsMember($.Map("csmUsername",
						strUsername));
			...
			if (user.getCsmStatus() != 1) {
				return returnError("105", "该帐户已经失效，请联系车纷享");
			}
			...
			CsUnitPerson person = CsUnitPerson.getCsUnitPerson($.add("csupMember", user.getCsmId()));
			if (person == null)
			...
			if (user.getCsmLoginS() == null)
				csMemberService.updateCsMember$NotNull(new CsMember()
						.csmLastIp(request.getRemoteAddr())
						.csmLastTime(new java.util.Date()).csmLoginS(1)
						.csmId(user.getCsmId()));
			else {
				csMemberService.updateCsMember$NotNull(new CsMember()
						.csmLastIp(request.getRemoteAddr())
						.csmLastTime(new java.util.Date())
						.csmLoginS(user.getCsmLoginS() + 1)
						.csmId(user.getCsmId()));
			}
			...
			JsonFormat result = JsonFormat.success().setData($.add("access_token", token).add("id",String.valueOf(user.getCsmId())));
	...
	public String forgetsms() {
		try {
			...
			String flag = "FORGOT_PWD" ;//短信模块标识 0 忘记密码 1 重置密码 2 注册
			...
			Boolean bOk = UtilHelper.sendTemplateSMS(type == 2 ? 1l : csMember.getCsmHost(),flag, mobile,defaultTemplate, SMSType.提示类短信,$.add("code", strCode));
			if (bOk == true) {
	...
	public String forgetvalid() {
		try {
			...
			if (!$.equals(strValidMobCode,(String) SessionMgr.get(csmMobile, FORGET_SMS_CODE)) && type == 0) {
				return returnError("110", "短信校验码输入错误");
			}
			
			if (!$.equals(strValidMobCode,(String) SessionMgr.get(csmMobile, RESET_SMS_CODE)) && type == 1) {
				return returnError("110", "短信校验码输入错误");
			}
			
			if (!$.equals(strValidMobCode,(String) SessionMgr.get(csmMobile, REGISTER_SMS_CODE)) && type == 2) {
				return returnError("110", "短信校验码输入错误");
			}
	...
	public String resetpwd() {
		try {
			...
			// ----------------------------------------------------------验证通过，重置密码
			csMemberService.updateCsMember$NotNull(new CsMember().csmPassword($.md5(rePwd)).csmId(csMember.getCsmId()));
	...
	public String register(){
		try {
			...
			if (!$.equals(strValidMobCode,(String) SessionMgr.get(csmMobile, REGISTER_SMS_CODE)) && from != 2) {
			...
			csMemberService.executeTransaction(new Function() {
				...
				public <T> T execute(Object... arg0) {
					...
					CsMember csMember = getRegisterMember(new CsMember(), csmPassword, from, csmMobile, inviteCode);
					...
					if(from  == 2){
						...
						CsMemberInfo csMemberInfo = getMemberInfo(new CsMemberInfo(), host, realName, csMember, sex);
						...
						CsMemberShip csMemberShip = getMemberShip(new CsMemberShip(), host, -1l, member.getCsmId());
						csMemberShipService.saveCsMemberShip(csMemberShip);
						
						//保存人员信息到cs_unit_person
						
						if(from == 2){
							CsUnitPerson csUnitPerson = getUnitPerson(new CsUnitPerson(), host, csUnitInfo.getCsuiId(), member.getCsmId(), -1l);
							csUnitPersonService.saveCsUnitPerson(csUnitPerson);
							UtilHelper.sendTemplateSMS(csUnitPerson.getCsupHost(),"REGIST_CODE",$.str(csmMobile),"欢迎使用北京出行，你的系统登录名为{mobile}，密码为{code}。"
									,SMSType.通知类短信,$.add("mobile", $.str(csmMobile)).add("code", csmPassword));
						}
	...
	public String putMemberInfo(){
		try {
			...
			final CsMember csMember = csMemberService.getCsMember($.add(CsMember.F.csmMobile, csmMobile));
			if (csMember == null) {
				return returnError("106", "注册失败，重新注册");
			}
			
			CsUnitPerson existUnitPerson = CsUnitPerson.getCsUnitPerson($.add(CsUnitPerson.F.csupMember, csMember.getCsmId()));
			...
			csUnitPersonService.executeTransaction(new Function() {
				...
				public <T> T execute(Object... arg0) {
					...
					//保存会员信息相关信息到cs_member_info
					CsMemberInfo csMemberInfo = CsMemberInfo.getCsMemberInfo($.add(CsMemberInfo.F.csmiMemberId, member.getCsmId()));
					if(csMemberInfo == null){
						csMemberInfo = getMemberInfo(new CsMemberInfo(), host, realName, csMember, sex);
						csMemberInfo = csMemberInfoService.saveCsMemberInfo(csMemberInfo);
					}else{
					...
					//保存会员关系到cs_member_ship表中
					CsMemberShip csMemberShip = CsMemberShip.getCsMemberShip($.add(CsMemberShip.F.csmsTargeter, member.getCsmId()));
					if(csMemberShip == null){
						csMemberShip = getMemberShip(new CsMemberShip(), host, -1l, member.getCsmId());
						csMemberShipService.saveCsMemberShip(csMemberShip);
					}else{
					...
					//保存人员信息到cs_unit_person
					CsUnitPerson csUnitPerson = CsUnitPerson.getCsUnitPerson($.add(CsUnitPerson.F.csupMember,member.getCsmId()));
					if(csUnitPerson == null){
						csUnitPerson = getUnitPerson(new CsUnitPerson(), host, csUnitInfo.getCsuiId(), member.getCsmId(), -1l);
						csUnitPersonService.saveCsUnitPerson(csUnitPerson);
					}else{
						csUnitPerson = getUnitPerson(csUnitPerson,  host, csUnitInfo.getCsuiId(), member.getCsmId(), -1l);
	...
	public String authInfo(){
		try {
			...
			final CsMember member = csMemberService.getCsMember($.add(CsMember.F.csmMobile, csmMobile));
			if (member == null) {
			...
			if(!$.empty(cardNo)){
				Long count = CsCreditCard.getCsCreditCardCount($.add(CsCreditCard.F.csccNo, cardNo).add(CsCreditCard.F.csccType, 1).add("definex", "( cscc_member != "+member.getCsmId()+" )"));
			...
			csMemberInfoService.executeTransaction(new Function() {
				...
				public <T> T execute(Object... arg0) {
					// TODO Auto-generated method stub
					if(!$.empty(certifyImg)){
						CsMemberInfo csMemberInfo = updateMemberInfo(member, certifyImg, certifyNum, driverImage, onCertifyImg);
						csMemberInfoService.updateCsMemberInfo$NotNull(csMemberInfo);
						csMemberService.updateCsMember$NotNull(updateAutoState(member,(short)2,(short)2));
					}
					if(!$.empty(cardNo)){
	...
	public void updateCreditCard(CsMember member,String cardNo,String cardImage){
		CsCreditCard creditCard = CsCreditCard.getCsCreditCard($.add(CsCreditCard.F.csccMember, member.getCsmId()).add(CsCreditCard.F.csccType, 1));
		if(creditCard == null){
	...
	public String getAreasUnit(){
		try {
			...
			for(CsArea area : areaList){
				List<CsUnitInfo> unitInfoList = CsUnitInfo.getCsUnitInfoList($.add(CsUnitInfo.F.csuiArea, area.getCsaId()).add(CsUnitInfo.F.csuiStatus, 1), 1000);
				cldList.add($.add("areaId", area.getCsaId())
						.add("areaName", area.getCsaName$())
	...
	public String getQueryUnit(){
		try {
			...
			List<CsUnitInfo> unitInfoList = CsUnitInfo.getCsUnitInfoList(param, unitInfoCount.intValue());
			return $.SendHtml($.json(JsonFormat.success().setData($.$("list", assembleUnitInfo(unitInfoList)))),CHARSET);
		} catch (Exception e) {
			return returnError(e);
		}
	}
	...
	public String getOutletsInfo() {
		try {
			...
			if (id == null) {
				CsUnitPerson person = CsUnitPerson.getCsUnitPerson($.add(
						"csupMember", member.getCsmId()));
				if (person == null)
			...
			return $.SendHtml(
					$.json(JsonFormat.success().setData($.$("data", outlets))),
					CHARSET);
		} catch (Exception ex) {
			return returnError(ex);
		}
	...
	public String getCarPrices() {
		try {
			...
			Prices prices = SystemHelper.getPrices(car.getCscHost(),
					car.getCscModel(), car.getCscOutlets());
			LzMap data = $.add("hourPrice",
					prices.get("租金.工作日白天小时|租金.白天小时|租金.每小时"));
			...
			return $.SendHtml(
					$.json(JsonFormat.success().setData($.$("map", data))),
					CHARSET);
		} catch (Exception ex) {
			return returnError(ex);
		}
	...
	public String checkOrder() {
		...
		try {
			if (type == 1) { // 单位
				...
				if (unitInfo == null)
					return returnError("106", "用户未绑定企业用户");
				orderInfo = commonOrderService
						.splitOrderDetails(member.getCsmId(),
								baseOrderInfo.getCsoCar(),
								baseOrderInfo.getCsoStart(),
								baseOrderInfo.getCsoFinish(), null, freehour,
								null, null, unitInfo.getCsuiFeeType(),
								insureType, null, null);
			} else { // 个人
				orderInfo = commonOrderService.splitOrderDetails(
						member.getCsmId(), baseOrderInfo.getCsoCar(),
						baseOrderInfo.getCsoStart(),
						baseOrderInfo.getCsoFinish(), null, freehour, null,
						null, null, insureType, null, null);
			}
			if (orderInfo != null) {
			...
			return $.SendHtml($.json(JsonFormat.success().setData(values)),
					CHARSET);
		} catch (Exception e) {
			return returnError(e);
		}
...
//		}
		...
		try {
			...
			if (!mealId.trim().equals("0")) {
				CsOrderCluster orderCluster = commonDisposeService
						.saveMealOrder(member.getCsmId(), member.getCsmId(),
								car.getCscId(), takeOutletsId, retOutletsId,
								takeTime, Long.valueOf(mealId), null,
								insureType, null, freeHours, 0l,
								From.APP, "鹏龙app" + version);
				orderId = orderCluster.getCsocId();
			} else {
				CsOrder csOrder = commonDisposeService.executeSaveOrder(
						payMemberId, member.getCsmId(), car.getCscId(),
						takeOutletsId, retOutletsId, takeTime, retTime, null,
						insureType, null, freeHours, null, 0l, From.APP,
						"鹏龙app" + version);
			...
			return $.SendHtml($.json(JsonFormat.success().setData($.Map("orderId", orderId))), CHARSET);
	...
	public String getPowerpiles() {
		try {
			...
			List<CsPowerPile> data = csPowerPileService.getCsPowerPileList($.add("csppOutlets", outlets.getCsoId()), -1);
	...
	public String getTakeOutlets() {
		try {
			...
			CsUnitPerson person = CsUnitPerson.getCsUnitPerson($.add("csupMember", member.getCsmId()));
			if (person == null)
				return returnError("101", "用户未绑定企业用户");
			CsUnitInfo unitInfo = person.get$csupInfo();
			if (unitInfo == null)
				return returnError("102", "用户未绑定单位");
			List<CsOutlets> outletsList = CsOutlets.getCsOutletsList($.add(CsOutlets.F.csoCanOrder, 1).add("definex","cso_id in ("+$.str(unitInfo.getCsuiOutlets()).replace("#","")+")").add("asc", "cso_priority"),-1);
			if (CollectionUtils.isEmpty(outletsList)) {
				return returnError("103", "用户单位没有可用网点");
			}
			return $.SendHtml($.json(JsonFormat.success().setData($.Map("list", outletsList))), CHARSET);
	...
	public String getRetOutlets() {
		try {
			...
			for (int i = 0; i < canUseOutletsList.size(); i++) {
				ids += (i == 0 ? "" : ",")
						+ canUseOutletsList.get(i).getCsoId();
			}
			...
			if(outlets.getCsoCanA()){
				// 可用车位数大于0
				params.put("definex",
						"(" + CsOutlets.C.csoId + " = " + outlets.getCsoId()
								+ " OR (cso_id in (" + ids + ") AND "
								+ CsOutlets.C.csoPositionS + " - "
								+ CsOutlets.C.csoCarS + " > 0 AND "
								+ CsOutlets.C.csoCanB + " = 1))");
			}else{
				params.put(CsOutlets.F.csoId,takeOutletsId );
			}
			
			
			List<CsOutlets> outletsList = csOutletsService.getCsOutletsList(
					params, -1);
			return $.SendHtml(
					$.json(JsonFormat.success().setData(
							$.Map("list", outletsList))), CHARSET);
		...
		try {
			...
			if (mealId > 0) {
				...
				if (takeTime.after(new Date())) {
					List<CsOrder> orderList = MealHelper.calcMealOrders(
							item.getCsiDepict(), takeTime);
					// ==============================================
					definex.append("(");
					...
					for (int i = 0; i < orderList.size(); i++) {
						...
						definex.append("           ( cso_start_time <= '"
								+ startTime + "' and cso_finish_time > '"
								+ startTime + "')");
						definex.append("        or (cso_start_time > '"
								+ startTime + "' and cso_start_time <'"
								+ finishTime + "' )");
...
//			}
			...
			CsUnitOrder unitOrder = commonUnitService.executeAddUnitOrder(
					person.getCsupInfo(), person.getCsupId(),
					member.getCsmId(), member.getCsmName(),
					member.getCsmMobile(), URLDecoder.decode(profile, "UTF-8"),
					member.get$csmInfo().getCsmiCertifyNum(),
					Long.valueOf(carId), takeTime, retTime, type, null, null,
					null, takeOutletsId, retOutletsId);
			/*
			 * Map<String, Object> price_Estimate = new HashMap<String,
			 * Object>(); Double fee_total_estimate = 0d;
			 * if(unitOrder.get$csuoSpecial().getCsscThird() == 1){
			 * price_Estimate =
			 * DidiApi.getEstimateOrderPrice(unitOrder.get$csuoSpecial());
			 * fee_total_estimate =
			 * Double.parseDouble(price_Estimate.get("price").toString()); }else
			 * if(unitOrder.get$csuoSpecial().getCsscThird() == 2){
			 * price_Estimate =
			 * YidaoApi.getOrderEstimatedPrice(unitOrder.get$csuoSpecial());
			 * fee_total_estimate =
			...
			try {
				...
				JpushClientHelper.sendAlertWithTag(auditPerson.getCsupMember()
						.toString(), "待办提醒", "您有一笔" + ApiBase.RENT_CAR
						+ "订单申请待审批" + formatter.format(new Date()), extra,
						JpushConfig.OFC_APP);

				// 如果是紧急公务，给审核人发送短信
				if (type.intValue() == 2) {
					UtilHelper.sendTemplateSMS(person.getCsupHost(),
							"CALLCAR_AUDIT_NOTICE", auditPerson
									.get$csupMember().getCsmMobile(), "您有一笔"
									+ ApiBase.RENT_CAR
									+ "订单(订单号:{ORDER_ID})申请等待审批，请及时处理。",
							SMSType.通知类短信, $.add("ORDER_ID",
									unitOrder.getCsuoOrder() + ""));
				}
			} catch (Exception e) {
				Logger.getRootLogger().error(e.getMessage(), e);
			}
			return $.SendHtml($.json(JsonFormat.success().setData($.add("unitOrderId", ""))), CHARSET);
		} catch (com.ccclubs.exception.MessageException oe) {
			oe.printStackTrace();
			Logger.getRootLogger().error(oe);
			return returnError("104", oe.getMessage());
	...
	public String getMemberInfo() {
		try {
			...
			CsUnitGroup csUnitGroup = CsUnitGroup.getCsUnitGroup($.add("csugPerson", person.getCsupId()));
			...
			Double remain = commonMoneyService.getCoin(member.getCsmHost(),member.getCsmId(),new Date());
			
			CsUnitGroup group = CsUnitGroup.get(person.getCsupGroup());

			CsCreditCard csCreditCard = csCreditCardService.getCsCreditCard($.add(CsCreditCard.F.csccMember, member.getCsmId()).add(CsCreditCard.F.csccType, 1));
			...
			data.put("unitName", person.get$csupInfo() == null ? "" : person.get$csupInfo().getCsuiName$());
			data.put("deptName", group == null ? "" : group.getCsugName$());
			data.put("memberNum", person.getCsupName());
			data.put("coinRemain", remain);
			data.put("csupflag", (person.getCsupFlag() != null && person.getCsupFlag().indexOf("1") > 0) ? 1 : 0);
			data.put("isapproved", (person.getCsupFlag() != null && person.getCsupFlag().indexOf("3") > 0) ? 1 : 0);
	...
	public String getUserAmount() {
		try {
			...
			// 可用余额
			Double canUseMoney = $.or(commonMoneyService.getUsableAmount(member.getCsmId()), 0).doubleValue();
			...
			// 冻结资金
			Double freezeMoney = remainCharge + $.or(member.getCsmCoupon(), 0).doubleValue() - canUseMoney;
			...
			for (String host : hostmap.keySet()) {
				Double coin = commonMoneyService.getCoin(Long.valueOf(host),member.getCsmId(), Calendar.getInstance().getTime());
				Map<String, Object> data = new HashMap<String, Object>();
				data.put("host", host);
			...
			// 可用免费小时
			Double canUseFreeHour = commonMoneyService.getCanUseFreeHour(member.getCsmId(), new Date());
	...
	public String applyRefund() {
		try {
			...
			List<CsOrder> csOrderList = csOrderService.getCsOrderList(
					$.add(CsOrder.F.csoPayMember, member.getCsmId())
							.add(CsOrder.F.csoStatus, 4)
							.add("desc", CsOrder.C.csoRetTime), -1);
			// 没有定单
			if (csOrderList.size() == 0) {
				// 没有用车记录，3个工作日退款
				csRefundResult = doRefund(member);
				return $.SendHtml($.json(JsonFormat.success().setData($.add("map", putRefund(csRefundResult)))),CHARSET);
			} else {
				if (csOrderList.get(0).getCsoRetTime() != null) {
					...
					if (now.after(WidgetHelper.getWorkDay(7, csOrderList.get(0).getCsoRetTime()))) {
						...
						newCsRefund.setNotNull(member.getCsmHost(), member
								.getCsmId(), 0d, "如果不能自动退款，请联系会员确认退款帐号", member
								.getCsmName(), member.getCsmMobile(),
								new Date(), WidgetHelper.getWorkDay(1),
								WidgetHelper.getWorkDay(9, csOrderList.get(0).getCsoRetTime()), 
								WidgetHelper.getWorkDay(9, csOrderList.get(0).getCsoRetTime()), (short) 0);
	...
	private CsRefund doRefund(CsMember member) {
		...
		newCsRefund.setNotNull(member.getCsmHost(), member.getCsmId(), 0d,
				"如果不能自动退款，请联系会员确认退款帐号", member.getCsmName(),
				member.getCsmMobile(), new Date(), WidgetHelper.getWorkDay(1),
				WidgetHelper.getWorkDay(4), WidgetHelper.getWorkDay(4),
				(short) 0);
		newCsRefund.csrMobile(member.getCsmMobile());
	...
	public String saveDriverInfo() {
		...
		if (realName == null && certifyType == null && certifyNubmer == null
				&& certifyImageUrl == null && driverNum == null
				&& driverImageUrl == null) {
			return returnError("102", "参数缺失");
		}
		...
		try {
			if (driverNum != null && !driverNum.equals("")) {
				...
				// 驾驶证号码已存在
				if (existMemberInfo != null && !existMemberInfo.getCsmiMemberId().equals(member.getCsmId()))
			...
			csMemberInfoService.executeTransaction(new Function() {
				...
				public <T> T execute(Object... arg0) {
					...
					csMemberInfo.csmiMemberId(memberId)
							.csmiCertifyImage(certifyImageUrl)
							.csmiDriverImage(driverImageUrl)
							.csmiDriverNum(driverNum).csmiName(realName)
			...
			csMemberInfoService.executeTransaction(new Function() {
				...
				public <T> T execute(Object... arg0) {
					...
					if (member.getCsmInfo() != null && member.getCsmInfo() > 0) {
						newCsMemberInfo.setNotNull(member.getCsmHost(),member.getCsmId(), new Date(), new Date(),(short) 1);
	...
	public String getAuditInfo() {
		try {
			...
			// 找到（自己是审核人的那个）组
			CsUnitGroup csupg = csUnitGroupService.getCsUnitGroup($.add(CsUnitGroup.F.csugPerson, person.getCsupId()));
			if (csupg == null) {
				return returnError("102", "会员没有分配所属组");
			}

			Page<CsUnitOrder> page = csUnitOrderService.getCsUnitOrderPage($.getInteger("page", 0),5,
					$.add(CsUnitOrder.F.csuoUnitGroup, csupg.getCsugId())
							.add("definex","(" + CsUnitOrder.C.csuoSpecial + " is null or " + CsUnitOrder.C.csuoSpecial + "=0 )")
							.add(CsUnitOrder.F.csuoState,auditStatus == null ? null : Short.valueOf(auditStatus)));
	...
	public String getPersonOrders() {
		try {
			...
			List<CsUnitOrder> allResult = csUnitOrderService.getCsUnitOrderList($.add(CsUnitOrder.F.csuoPerson, person.getCsupId()).add("definex", "(csuo_state in (1,2))"),-1);
			...
			// 查询出用户“可远程控制”的订单
			List<CsOrder> orderList = csOrderService.getCsOrderList($.add(CsOrder.F.csoUseMember, member.getCsmId()).add("definex", "(cso_status in (0,1))"), -1);
	...
	boolean isCallCarOrder(CsUnitOrder data) {
		return data != null && data.getCsuoSpecial() != null && data.getCsuoSpecial().intValue() > 0;
	}
	...
	private Map<String, Object> getUnitOrderMap(CsUnitOrder data) {
		...
		mapdata.put("orderFee",data.get$csuoOrder().getCsoPayReal()+data.get$csuoOrder().getCsoPayCoin()-$.or(data.get$csuoOrder().getCsoPayTimeout(),0d));
	...
	public String getUnitOrderById() {
		try {
			...
			if (order != null) {
				...
				CsComplain csComplain = csComplainService.getCsComplain($.add(CsComplain.F.cscObject, "CsOrder@"+order.getCsoId()));
				...
				if(order.getCsoStatus() != 0 && order.getCsoStatus() != 1){
					...
					CsState state = csStateService.getCsState($.add(CsState.F.cssNumber, order.get$csoCar().getCscNumber()));
					if (state != null) {
	...
	public String getPersonalOrderById() {
		try {
			...
			if (CommonDisposeService.isOrderCluster(order)) {
				CsOrderCluster coc = CsOrderCluster.Get($.add(CsOrderCluster.F.csocId, $.or(order.getCsoSrc(), "").replace("CsOrderCluster@", "")));
				if (coc != null) {
					map.put("payNeed", order.getCsoPayNeed() + coc.getCsocPayNeed());
					map.put("payReal", order.getCsoPayReal() + coc.getCsocPayReal()+order.getCsoPayCoin());
				...
				map.put("timeoutFee", order.getCsoPayTimeout());//超时费用
				map.put("orderFee",(order.getCsoPayReal()+order.getCsoPayCoin()-$.or(order.getCsoPayTimeout(),0d)));
				//map.put("orderFee",(order.getCsoPayReal()+order.getCsoPayCoin()) > 0 ? $.or(order.getCsoPayRent(), 0d) : 0);//租金费用
			}
	...
	public String getCurrentOrder() {
		try {
			...
			// 已下单的订单
			CsUnitOrder order = csUnitOrderService.getCsUnitOrder($.add(CsUnitOrder.F.csuoState, 2).add(CsUnitOrder.F.csuoPerson, person.getCsupId()));
			if (order == null) {
	...
	public String submitAudit() {
		try {
			...
			CsUnitPerson unitPerson = CsUnitPerson.Get($.add(CsUnitPerson.F.csupMember, member.getCsmId()));
			...
			try {
				...
				CsUnitMt mt = new CsUnitMt(member.getCsmHost(),
						order.getCsuoUnitInfo(), order.getCsuoPerson(), null,
						null, 1, "CsUnitOrder@" + order.getCsuoId(), null,
						new Date(), new Date(), Short.valueOf("0"));

				commonUnitService.executeCheckUnitOrder( auditState.intValue() == 2 ? true : false, Long.valueOf(orderId), unitPerson.getCsupInfo(), "");
				...
				if (auditState == 2) { // 通过
					if (isCallCarOrder(order)) {
						...
						mt.setCsumContent("您的" + ApiBase.RENT_CAR + "订单" + order.getCsuoOrder() + "审批通过。");
					}
					...
					if (isCallCarOrder(order)) {
						...
						mt.setCsumContent("您的" + ApiBase.RENT_CAR + "订单" + order.getCsuoOrder() + "审批未通过。如需使用，请重新申请。");
					}
				...
				try {
					...
					extra.put("data",$.json($.add("messageId", mt.getCsumId()).add("unitOrderId", order.getCsuoId())));
					JpushClientHelper.sendAlertWithTag(useUnitPerson.getCsupMember().toString(), mt.getCsumTitle(), mt.getCsumContent(), extra, JpushConfig.OFC_APP);
				} catch (Exception e) {
					Logger.getRootLogger().error(e.getMessage(), e);
				}
			} catch (Exception e) {
	...
	public String getUnitOrders() {
		try {
			...
			if (type == null)
				type = 1;
			if (type == 1) {
				Page<CsUnitOrder> page = csUnitOrderService.getCsUnitOrderPage($.getInteger("page", 0),5,$.add(CsUnitOrder.F.csuoPerson, unitPerson.getCsupId())
								.add("definex","( (" + CsUnitOrder.C.csuoSpecial + " is null or " + CsUnitOrder.C.csuoSpecial + "=0 ))"));
				...
				Page<CsOrder> page = csOrderService.getCsOrderPage($.getInteger("page", 0),5,
								$.add(CsOrder.F.csoUseMember, member.getCsmId()).add("definex","(not exists(select 1 from cs_unit_order uo where uo.csuo_order = cso_id) ) "));
				LzMap pagemap = $.$("index", page.getIndex()).add("total", page.getTotal()).add("count", page.getCount()).add("size", page.getSize());
				return $.SendHtml($.json(JsonFormat.success().setData($.Map("list", assemUserOrders(page.getResult())).add("page", pagemap))), CHARSET);
			}
	...
	public String queryOrders(){
		try {
			...
			LzMap lzMap = $.add(CsOrder.F.csoUseMember, member.getCsmId()).add("definex","(cso_id like '%"+queryParam+"%' or cso_car_number like '%"+queryParam+"%')").add("desc", CsOrder.C.csoId);
			List<CsOrder> page = csOrderService.getCsOrderList(lzMap,-1);
			List<Map<String, Object>> dataList = assemUserOrders(page);
			for (Map<String, Object> mapdata : dataList) {
				CsUnitOrder unitOrder = csUnitOrderService.getCsUnitOrder($.add(CsUnitOrder.F.csuoOrder, mapdata.get("orderId")));
				if (unitOrder != null) {
					mapdata.put("state", unitOrder.getCsuoState());
					mapdata.put("id", unitOrder.getCsuoId());
	...
	/**
	 * carmodelname car carImg startOutletsName csoAddress csoLongitude
	 * csoLatitude csoName csoFlag finishOutletsName finishOutletsLat
	 * finishOutletsLng finishOutletsAddr orderId orderStatus startTime
	 * finishTime carno carnumber endurance payNeed payReal orderFee mealpFee
	 * timeoutFee
	 * 
	 * @param orderList
	 * @return
	...
	public List<Map<String, Object>> assemUserOrders(List<CsOrder> orderList) {
		...
		for (CsOrder data : orderList) {
			...
			if (CommonDisposeService.isOrderCluster(data)) {
				CsOrderCluster coc = CsOrderCluster.Get($.add(CsOrderCluster.F.csocId, $.or(data.getCsoSrc(), "").replace("CsOrderCluster@", "")));
				if (coc != null) {
					datamap.put("payNeed",data.getCsoPayNeed() + coc.getCsocPayNeed());
					datamap.put("payReal",data.getCsoPayReal() + coc.getCsocPayReal()+data.getCsoPayCoin());
				...
				datamap.put("timeoutFee", data.getCsoPayTimeout());//超时费用
				datamap.put("orderFee",(data.getCsoPayReal()+data.getCsoPayCoin()-$.or(data.getCsoPayTimeout(),0d)));
				//datamap.put("orderFee",(data.getCsoPayReal()+data.getCsoPayCoin()) > 0 ? $.or(data.getCsoPayRent(), 0d) : 0);//租金费用
			}

			CsComplain csComplain = csComplainService.getCsComplain($.add(CsComplain.F.cscObject, "CsOrder@"+data.getCsoId()));
	...
	public String getUserOrders() {
		try {
			...
			Page<CsOrder> page = csOrderService.getCsOrderPage($.getInteger("page", 0), 5,$.add(CsOrder.F.csoUseMember, member.getCsmId()));
	...
	/**
	 * 
	 * startOutletsName finishOutletsName finishOutletsLat finishOutletsLng
	 * finishOutletsAddr carmodelImage //车型图片 previewimg //车辆主图 carImg
	 * //车辆图片，区别是可能会取车型图片 orderStatus endurance
	 * 
	 * 加载公务车订单附加属性
	 * 
	 * @param result
	 */
	public List<Map<String, Object>> loadUnitOrderFeatures(List<CsUnitOrder> result, boolean loadStateData) {
		List<Map<String, Object>> dataList = new ArrayList<Map<String, Object>>();
		Map<Long, CsCar> carmap = new HashMap<Long, CsCar>();
		Map<Long, CsCarModel> carmodelmap = new HashMap<Long, CsCarModel>();
		...
		for (CsUnitOrder data : result) {
			...
			if(csOrder == null){
				...
				CsComplain csComplain = csComplainService.getCsComplain($.add(CsComplain.F.cscObject, "CsOrder@"+csOrder.getCsoId()));
				datamap.put("isComment", csComplain == null ? false : true);
				datamap.put("starLevel", csComplain == null ? 0 : csComplain.getCscLevel());
				datamap.put("starLevel1", csComplain == null ? 0 : csComplain.getCscLevel1());
				datamap.put("content", csComplain == null ? " " : csComplain.getCscContent());
				if (loadStateData) {
					//订单已预订使用中
					if((csOrder.getCsoStatus() == 0 || csOrder.getCsoStatus() == 1) && car != null){
						CsState state = csStateService.getCsState($.add(CsState.F.cssNumber, car.getCscNumber()));
						if (state != null) {
							datamap.put("endurance", state.getCssEndurance());
						} else {
	...
	public String getSpecials() {
		try {
			...
			ICsSpecialCarService csSpecialCarService = $
					.GetSpringBean("csSpecialCarService");
			...
			Page<CsSpecialCar> page = csSpecialCarService.getCsSpecialCarPage($.getInteger("page", 0), 5, $.add(CsSpecialCar.F.csscPerson,unitPerson.getCsupId().toString()));
			...
			for (CsSpecialCar data : page.getResult()) {
				...
				if (val != null) {
					datamap.put("share",
							Integer.valueOf(val.get("share").toString()));
	...
	public String cancelSpecialOrder() {
		try {
			...
			commonUnitService.executeCancelUnitOrder(unitPerson.getCsupInfo(),specialCar.getCsscUnitOrder(), "");
			return $.SendHtml($.json(JsonFormat.success()), CHARSET);
		} catch (Exception ex) {
			return returnError(ex);
		}
	...
	public String renewUnitOrder() {
		try {
			...
			if (newFinishTime == null) {
				try {
					newFinishTime = new DateUtil().StringtoDate($.getString("newFinishTime"),"yyyy MM-dd HH:mm:ss");
				} catch (Exception e) {
				}
			}
			...
			CsUnitOrder unitOrder = commonUnitService.executeReUnitOrder(person.getCsupInfo(), Long.valueOf(unitOrderId),newFinishTime);
			if (unitOrder != null) {
				return $.SendHtml($.json(JsonFormat.success().setData($.$("unitOrderId", unitOrder.getCsuoId()))),CHARSET);
			} else {
	...
	public String renewPersonalOrder() {
		try {
			...
			CsOrder reOrder = commonDisposeService.executeReOrder(order,newFinishDate, freehour, From.APP, null);
			if (reOrder != null) {
				return $.SendHtml($.json(JsonFormat.success().setData($.$("orderId", reOrder.getCsoId()))), CHARSET);
			} else {
				return returnError("110", "续订申请失败");
			}
	...
	public String getMessages() {
		try {
			...
			Map<String, Object> params = $.Map(CsUnitMt.F.csumStatus, readStatus).add(CsUnitMt.F.csumPerson, person.getCsupId());
			if(csumType != 0){
				params.put(CsUnitMt.F.csumType, csumType);
			}
			
			Page<CsUnitMt> page = csUnitMtService.getCsUnitMtPage($.getInteger("page", 0), 10, params);

			List<Map<String, Object>> dataList = new ArrayList<Map<String, Object>>();
			for (CsUnitMt data : page.getResult()) {
				String udata = data.getCsumData();
				if (udata != null && udata.indexOf("CsUnitOrder") != -1) {
					udata = udata.substring(udata.indexOf("CsUnitOrder") + "CsUnitOrder@".length(), udata.length());
				}
		...
		try {
			...
			if (state != null) {
				if (new Date().getTime() - state.getCssAddTime().getTime() <= 1000 * 60 * 10) {
					if (state.getCssOrder() != null && state.getCssOrder().intValue() > 0 && state.getCssOrder().intValue() != orderId.intValue()) {
			...
			// 公务订单判断审核状态
			CsUnitOrder unitOrder = CsUnitOrder.getCsUnitOrder($.add(CsUnitOrder.F.csuoOrder, csOrder.getCsoId()));
			if (unitOrder != null) {
				if (unitOrder.getCsuoState().intValue() != 2) {
					return returnError("108", "订单未通过审核");
				}
			}
			CsArgument takeCarTime = csArgumentService.getCsArgument($.add(CsArgument.F.csaFlag,ArgumentKey.TAKE_CAR_TIME));

			// 判断订单开始时间：订单前10分钟才可以取车
			Calendar calendar = Calendar.getInstance();
			calendar.setTime(csOrder.getCsoStartTime());
			calendar.add(Calendar.MINUTE,takeCarTime == null ?  -20 : Integer.parseInt(takeCarTime.getCsaValue()));
			...
			if (conType.equals("1")) { // 取车/开门
				...
				if (csOrder.getCsoStatus().equals(new Short("0")) || csOrder.getCsoStatus().equals(new Short("1"))) {
					...
					if(state != null){
						Double[] lng_lat = CoordtransformUtil.WGS84ToGCJ02(Double.parseDouble(state.getCssLongitude()), Double.parseDouble(state.getCssLatitude()));
						double distance = getDistance(lng_lat[0],lng_lat[1],csOrder.get$csoOutletsRet().getCsoLongitude(),csOrder.get$csoOutletsRet().getCsoLatitude());
						if (destance != null && !destance.equals("") && !destance.equals("0") && distance > Integer.parseInt(destance))
	...
	public String cancelUnitOrderSubmit() {
		try {
			...
			final CsOrder order = csOrderService.getCsOrderById(unitOrder
					.getCsuoOrder());
			// 验证当前订单是不是当前用户的
			if (order == null) {
				return returnError("101", "订单不存在");
			...
			csOrderService.executeTransaction(new Function() {
				...
				public <T> T execute(Object... arg0) {
					...
					order.csoSerial(order.getCsoUseMember() + "-"
							+ order.getCsoCar() + "-"
							+ order.getCsoStartTime().getTime() + "-"
							+ order.getCsoStatus() + "-"
					...
					SchedulingService.onlineCar(order.getCsoCar(), (short) 1,"异借还车订单取消车辆自动上线", null);
	...
	public String thirdPartyOrder() {
		try {
			...
			String fromLocate = LocateConverter.execute(fromLatitude + "," + fromLongitude, 5, null);
			...
			CsUnitOrder unitOrder = commonUnitService.executeAddSpecialOrder(
					person.getCsupId(), profile, startTime, finishTime, type,
					fromLatitude + ";" + fromLongitude, toLatitude + ";"
							+ toLongitude, fromAddr, toAddr, message, carLevel);

			// 获取预估价格
			ICsSpecialCarService csSpecialCarService = $.GetSpringBean("csSpecialCarService");
			CsSpecialCar csSpecialCar = csSpecialCarService.getCsSpecialCarById(unitOrder.getCsuoSpecial());
			...
			if (unitOrder.get$csuoSpecial().getCsscThird() == 1) {
				...
				if (price_Estimate != null) {
					...
					if (Double.valueOf(code.toString()).intValue() == 0 && msg.equals("SUCCESS")) {
						Map<String, Object> price_new = (Map<String, Object>) price_Estimate.get("data");
						String price = ((Map<String, Object>) price_new.get(DidiApi.carModelMapping(csSpecialCar))).get("price").toString();
				...
				if (price_Estimate != null) {
					...
					if (Double.valueOf(code.toString()).intValue() == 200) {
						String total_fee = ((Map<String, Object>) price_Estimate.get("result")).get("total_fee").toString();
			...
			try {
				...
				extra.put("data",$.json($.add("unitOrderId", unitOrder.getCsuoId())));
				JpushClientHelper.sendAlertWithTag(auditPerson.getCsupMember()
						.toString(), "待办提醒", "您有一笔" + ApiBase.CALL_CAR
						+ "申请待审批" + formatter.format(new Date()), extra,
						JpushConfig.OFC_APP);
			} catch (Exception e) {
				Logger.getRootLogger().error(e.getMessage(), e);
			}

			return $.SendHtml($.json(JsonFormat.success().setData($.$("unitOrderId", ""))), CHARSET);
		} catch (Exception ex) {
			return returnError(ex);
		}
	}
	...
	public String getSpecialOrderLocale() {
		try {
			...
			CsSpecialCar specialCar = csSpecialCarService.getCsSpecialCarById(Long.valueOf(special_id));
			if (specialCar == null) {
				return returnError("101", "订单号无效");
			}
			...
			if (specialCar.getCsscThird() != null) {
				if (specialCar.getCsscThird().intValue() == 1) { // 1:滴滴专车
					Map<String, Object> map = DidiApi.getOrderDetail(member.getCsmMobile(), specialCar.getCsscThirdNo());
					if (map != null) {
					...
					if (map != null) {
						...
						if (Double.valueOf(code.toString()).intValue() == 200 && msg.equals("success")) {
	...
	public String cancelPersonalOrder() {
		try {
			...
			commonDisposeService.executeCancelOrder(orderId, "会员自主取消订单",From.APP, "鹏龙app" + version);
			return $.SendHtml($.json(JsonFormat.success()), CHARSET);
		} catch (Exception e) {
			return returnError(e);
		}
	...
	public String modifyPass() {
		try {
			...
			if (SystemHelper.isNullOrEmpty(oldPass)
					|| SystemHelper.isNullOrEmpty(newPass)
		...
		try {
			...
			if (csUpdate != null) {
				return $.SendHtml($.json(JsonFormat.success().setData(
						$.$("newVersion", csUpdate.getCsuVersion())
						.add("newFile", csUpdate.getCsuFile()))), CHARSET);
			} else {
				return $.SendHtml($.json(JsonFormat.success()), CHARSET);
			}
		} catch (Exception ex) {
	...
	public String locateCar() {
		try {
				...
				if (order != null) {
					if (order.getCsoStatus().intValue() == 1) { // 如果订单已取车，获取车辆当前实时位置
						...
						if (state != null) {
							...
							// 如果数据是2个小时以内的
							if (new Date().getTime() - state.getCssAddTime().getTime() < 1000 * 60 * 60 * 2) {

								
								Double[] longlan = CoordtransformUtil.WGS84ToGCJ02(Double.parseDouble(state.getCssLatitude()), Double.parseDouble(state.getCssLongitude()));
								
								//
								return $.SendHtml($.json(JsonFormat.success().setData($.add("latitude",longlan[1]).add("longitude",longlan[0]))),CHARSET);
							}
						}
					}
				}
			...
			return $.SendHtml($.json(JsonFormat.success().setData($.add("latitude", car.getCscLatitude()).add("longitude", car.getCscLongitude()))),CHARSET);
		} catch (Exception ex) {
			return returnError(ex);
		}
	}
	...
	public String getPayUrl() {
		try {
			...
			String result = commonPayService.executeGetAppAlipayString(member.getCsmId(), amount, noticeUrl, app);
			return $.SendHtml($.json(JsonFormat.success().setData($.add("urlData", result))), CHARSET);
		} catch (Exception ex) {
			return returnError(ex);
		}
	}
		...
		for (String kvstr : paramArr) {
			for (int i = 0; i < kvstr.length(); i++) {
				if (kvstr.charAt(i) == '=' && (i + 1 != kvstr.length())
						&& kvstr.charAt(i + 1) == '"'
						&& kvstr.charAt(kvstr.length() - 1) == '"'
						&& i + 2 != kvstr.length()) {
					result.put(kvstr.substring(0, i),
							kvstr.substring(i + 2, kvstr.length() - 1));
				}
			}
		}
	...
	public String getRefund() {
		try {
			...
			CsRefund csRefund = csRefundService.getCsRefund($.add(CsRefund.F.csrMember, member.getCsmId()).add("definex","csr_status<2"));
	...
	public String getUserRecord() {
		try {
			...
			Page<CsRecord> page = csRecordService.getCsRecordPage($.getInteger("page", 0), 10, param);
			List<Map<String, Object>> dataList = new ArrayList<Map<String, Object>>();
	...
	public String getPrefCoupon() {
		try {
			...
			CsArgument csArgument = csArgumentService.getCsArgument($.add(CsArgument.F.csaFlag, "coupon_activity_end_time"));
			String endTime = csArgument == null ? "2018-12-31 23:59:59" : csArgument.getCsaValue();
			
			CsItem item = null;
			// 判断是否有资格购买“北京出行”的活动现金券
			if (Calendar.getInstance().getTime().before(new DateUtil().StringtoDate(endTime,NORMAL_DATE_FORMAT))) {
				item = csItemService.getCsItem($.add(CsItem.F.csiFlag,CouponBuyUtil.北京出行充值送活动优惠券).add(CsItem.F.csiStatus, 1));
				if (item != null) {
					// 判断用户是否是第一次购买
			...
			if (item == null) {
				item = csItemService.getCsItem($.add(CsItem.F.csiFlag,CouponBuyUtil.默认优惠券).add(CsItem.F.csiStatus, 1));
			}
			...
			LzMap data = $
					...
					.add("itemId", item.getCsiId())
					.add("price", couponBuy.getPrice())
					// .add("amount", couponBuy.getAmount())
					.add("amount", 5000)
					.add("totalFee", couponBuy.getNeedPayTotalFee())
					.add("count", couponBuy.getCount())
					.add("title", couponBuy.getText())
	...
	public String submitBuyCoupon() {
		...
		try {
			...
			if(eventLock.tryLock(lockReason, 3000)){
				...
				CsItem bitem = csItemService.getCsItem($.add(CsItem.F.csiId, itemId).add(CsItem.F.csiStatus, 1));
				if (bitem == null) {
					return returnError("102", "您购买的现金券已失效");
				}
	
				// 安全校验：判断一下如果已经购买过“北京出行充值送活动优惠券”，则使用购买默认优惠券
				if (bitem.getCsiFlag().equals(CouponBuyUtil.北京出行充值送活动优惠券) && isPurchasedItem(bitem, member.getCsmId())) {
					bitem = csItemService.getCsItem($.add(CsItem.F.csiFlag,CouponBuyUtil.默认优惠券).add(CsItem.F.csiStatus, 1));
				}
				...
				if (payType == 1) {
					...
					String result = commonPayService.executeGetAppAlipayString(member.getCsmId(), money, noticeUrl, app, out_trade_no);
					...
					return $.SendHtml($.json(JsonFormat.success().setData($.Map("urlData", result))), CHARSET);
					...
					String out_trade_no = $.date(new Date(), "yyyyMMddHHmmssSSS") + member.getCsmId();
					String result = cmbPayService.executeCMBPayMobileUrl(member.getCsmId(), money, null, app, out_trade_no);
					CsAlipayRecord csAlipayRecord = csAlipayRecordService.getCsAlipayRecord($.add("csarTradeNo", out_trade_no));
					csAlipayRecord.csarBuyItem(item.getCsiId());
					csAlipayRecord.csarBuyCount(couponBuy.getCount());
					csAlipayRecord.update();
	
					return $.SendHtml($.json(JsonFormat.success().setData($.add("urlData", result).add("callbackUrl",noticeUrl))), CHARSET);
				} else {
	...
	public String exchangeCoupon() {
		try {
			...
			// 开始兑换
			int result = commonMoneyService.executeRechangeCard(couponNumber,code, member.getCsmId(), "恭喜您，兑换充值卡成功");
			if (result == 104) {
	...
	public String getAppProtocol(){
		try {
			...
			CsContent content = csContentService.getCsContent($.add(CsContent.F.cscFlag, LICENSE_FLAG).add(CsContent.F.cscParent, root.getCscId()).add(CsContent.F.cscStatus, 1));
			if (content != null) {
				Map<String, Object> map = new HashMap<String, Object>();
...
//			}
			...
			// 首页BANNER
			if (type == 0 || type == 1) {
				List<CsContent> contentList = csContentService
						.getCsContentList(
								$.add(CsContent.F.cscFlag, BANNER_FLAG)
										.add(CsContent.F.cscParent,root.getCscId())
										.add(CsContent.F.cscStatus, 1)
										.add("asc", "csc_order"), 50);
				if (contentList != null) {
			...
			//红包帮助
			if(type == 0 || type == 5){
				CsContent content = csContentService.getCsContent($.add(CsContent.F.cscFlag, COIN_FLAG)
						.add(CsContent.F.cscParent, root.getCscId())
						.add(CsContent.F.cscStatus, 1));
				if(content != null){
					Map<String, Object> map = new HashMap<String, Object>();
			...
			if(type == 0 || type == 4){
				...
				try{
					CsArgument orderTimeMinArg = csArgumentService.getCsArgument($.add(CsArgument.F.csaFlag, "orderTimeMin"));
					CsArgument appTimerSepArg = csArgumentService.getCsArgument($.add(CsArgument.F.csaFlag, "appTimerSep"));
					CsArgument renewOrderTimeMinArg = csArgumentService.getCsArgument($.add(CsArgument.F.csaFlag, "renewOrderTimeMin"));
					CsArgument preOrderTimeMinArg = csArgumentService.getCsArgument($.add(CsArgument.F.csaFlag, "preOrderTimeMin"));
					
					orderTimeMin		 = 	(orderTimeMinArg!=null && orderTimeMinArg.getCsaValue()!=null) ? Integer.valueOf(orderTimeMinArg.getCsaValue()) : orderTimeMin;
					appTimerSep			 = 	(appTimerSepArg!=null && appTimerSepArg.getCsaValue()!=null) ? Integer.valueOf(appTimerSepArg.getCsaValue()) : appTimerSep;
					renewOrderTimeMin 	 = 	(renewOrderTimeMinArg!=null && renewOrderTimeMinArg.getCsaValue()!=null) ? Integer.valueOf(renewOrderTimeMinArg.getCsaValue()) :renewOrderTimeMin;
					preOrderTimeMin 	 = 	(preOrderTimeMinArg!=null && preOrderTimeMinArg.getCsaValue()!=null) ? Integer.valueOf(preOrderTimeMinArg.getCsaValue()) : preOrderTimeMin;
				}catch(Exception e){
					Logger.getRootLogger().error(e);
				}
			...
			Map<String ,Object> map = new HashMap<String,Object>();
			if(appType == 1){	// 1安卓
				CsUpdate update = csUpdateService.getCsUpdate($.add(CsUpdate.F.csuType, 0).add(CsUpdate.F.csuStatus, 1));
				if (update != null) {
				...
				int upgradeFlag 	= 0;
				CsUpdate update = csUpdateService.getCsUpdate($.add(CsUpdate.F.csuType, 1).add(CsUpdate.F.csuStatus, 1));
				if (update != null) {
					String versionList = getUpdateVersions(update.getCsuVersion$());
					if(versionList!=null && in(appVersion, versionList, ",")){	//匹配需要强制更新的版本如：2.3.0,2.4.0
						upgradeFlag = 1;
...
//			if(update!=null){
//				LzMap map = $.add("version", update.getCsuVersion()).add("fileUrl", update.getCsuFile());
//				data.put("update", map);
//			}
	...
	public String getCmbPayUrl() {
		try {
			...
			String noticeUrl = $.config("service.domain")
					+ "service/cmb.notice.do";
			String result = cmbPayService.executeCMBPayMobileUrl(
					member.getCsmId(), amount, null, app);
			return $.SendHtml(
					$.json(JsonFormat.success().setData(
							$.add("urlData", result).add("callbackUrl",
									noticeUrl))), CHARSET);
		} catch (Exception ex) {
			return returnError(ex);
		}
	...
	public String getRentConfig() {
		try {
			return $.SendHtml(
					$.json(JsonFormat.success().setData(
							$.$("mileagePrice", mileagePrice))), CHARSET);
		} catch (Exception ex) {
			return returnError(ex);
		}
	}
	...
	public String submitRentOrder() {
		try {
			...
			final Double amount = mileagePrice * milage
					* (isShare.intValue() == 1 ? 0.5 : 1); // 预估金额
			final String mobile = $.getString("mobile"); // 乘车人联系电话
			final String message = $.json($.Map("share", isShare).add(
					"profile", profile));

			final CsUnitPerson person = CsUnitPerson.getCsUnitPerson($.add(
					"csupMember", member.getCsmId()));
			if (person == null)
				return returnError("106", "用户未绑定企业用户");

			// 找到自己所在的组，给组长发消息用
			CsUnitGroup group = CsUnitGroup.getCsUnitGroupById(person
					.getCsupGroup());
			if (group == null)
				return returnError("107", "用户未绑定分组");
			...
			CsUnitOrder unitOrder = csStateService
					.executeTransaction(new Function() {

						@Override
						public <T> T execute(Object... object) {
							final CsUnitOrder unitOrder = commonUnitService
									.executeAddSpecialOrder(person.getCsupId(),
											profile, startTime, finishTime,
											type, fromLatitude + ";"
													+ fromLongitude, toLatitude
													+ ";" + toLongitude,
											fromAddr, toAddr, message,
											carLevel, mobile, null);
							unitOrder.setCsuoMileage(milage);
							unitOrder.setCsuoState((short) 2); // 直接审核通过
							unitOrder.update();

							new CsSpecialCar(unitOrder.getCsuoSpecial())
									.csscStatus((short) 2) // 状态是“已下单”
							...
							final String startTimeStr = new DateUtil()
									.dateToString(startTime, NORMAL_DATE_FORMAT);
							final String finishTimeStr = new DateUtil()
									.dateToString(finishTime,
											NORMAL_DATE_FORMAT);

							// 找到所有上线状态（并且订单时间上没有冲突）司机，发送订单信息
							ICsDriverService csDriverService = $
									.GetSpringBean("csDriverService");
							...
							// // sql.append("       and (");
							// //
							// sql.append("             (cssc.cssc_start_time <='").append(startTimeStr).append("' and cssc.cssc_finish_time >'").append(startTimeStr).append("')");
							// // sql.append("             or ");
							// //
							// sql.append("             (cssc.cssc_start_time >='").append(startTimeStr).append("' and cssc.cssc_finish_time >'").append(startTimeStr).append("')");
							// // sql.append("       )");
							...
							sql.append(" exists ( select 1 from cs_driver_car where csdc_driver=csd_id and csdc_state !=2 and csdc_status=1 ) ");

							final List<CsDriver> driverList = csDriverService
									.getCsDriverList(
											$.add("definex", sql.toString()),
											-1);
								...
								public void run() {
									...
									for (CsDriver driver : driverList) {
										...
										try {
											...
											extra.put("startTtime", String
													.valueOf(startTime
															.getTime())); // 开始时间
											extra.put("finishTime", String
													.valueOf(finishTime
															.getTime())); // 结束时间
											...
											extra.put("milage",
													String.valueOf(milage)); // 预估里程
											extra.put("isCarPool",
													String.valueOf(isShare)); // 是否拼车，1拼车0不拼车
											extra.put("profile",
													String.valueOf(isShare)); // 乘车需求
											extra.put("specId", String
													.valueOf(unitOrder
															.getCsuoSpecial())); // 订单ID
											// extra.put("distance",
											...
											Logger.getLogger("alipay")
													.error("订单ID ："
															...
															+ " 手机号:"
															+ driver.getCsdMobile());
											JpushClientHelper.sendMessageWithTag(
													driver.getCsdMobile(),
													message, extra,
													JpushConfig.OFC_DRIVER_APP);
										} catch (Exception e) {
											rentAppLogger.error(
													"submitRentOrder error! data:\t"
															+ $.json(extra), e);
	...
	public String calcCancelRentOrder() {
		try {
			...
			CsUnitPerson unitPerson = CsUnitPerson.Get($.add(
					CsUnitPerson.F.csupMember, member.getCsmId()));
			if (unitPerson == null) {
				return returnError("101", "用户无效");
			}
			...
			ICsSpecialCarService csSpecialCarService = $
					.GetSpringBean("csSpecialCarService");
			final CsSpecialCar specialCar = csSpecialCarService
					.getCsSpecialCarById(specialId);
			if (specialCar == null) {
				return returnError("102", "订单无效");
			}
			...
			UnitOrderInfo unitOrderInfo = commonUnitService
					.execulateCalcRentOrder(specialCar.getCsscUnitOrder(),
							new Date(), null, true);
			Double totalFee = 0d;
	...
	public String cancelRentOrder() {
		try {
			final CsMember member = OauthUtils.getOauth($.getString(
					"access_token", ""));
			if (member == null) {
				return returnError("100", "登录授权无效");
			}
			...
			csSpecialCarService.executeTransaction(new Function() {
				...
				public <T> T execute(Object... object) {
					// TODO Auto-generated method stub
					commonUnitService.executeCancelRentOrder(
							specialCar.getCsscUnitOrder(), member.getCsmId(),
							remark);
					// 已接单订单取消，推送给指定司机通知
					if (specialCar.getCsscStatus().intValue() == 3
							|| specialCar.getCsscStatus().intValue() == 8) {
						...
						try {
							JpushClientHelper.sendAlertWithTag(
									specialCar.get$csscDrivercar()
											.get$csdcDriver().getCsdMobile$(),
									"订单取消",
									specialCar.getCsscAddTime$()
											+ specialCar.getCsscStartAddr$()
											+ "订单已取消，请重新接单",
									JpushConfig.OFC_DRIVER_APP);
						} catch (Exception e) {
							rentAppLogger.error(
					...
					// 未必接单订单取消，推送给所有司机
					List<CsDriver> driverList = CacheStoreHelper
							.getDriverList(String.valueOf(specialId));
					if (driverList == null)
						return null;
					for (CsDriver csDriver : driverList) {
						...
						try {
							...
							Logger.getLogger("alipay").error(
									"取消订单ID ：" + specialId + " 手机号:"
											+ csDriver.getCsdMobile());
							JpushClientHelper.sendMessageWithTag(
									csDriver.getCsdMobile(),
									"{\"title\":\"订单取消\"}", extra,
									JpushConfig.OFC_DRIVER_APP);
	...
	public String getMeals() {
		try {
			...
			List<CsItem> itemList = csItemService.getCsItemList(
					$.add(CsItem.F.csiFlag, "201604MEAL")
							.add(CsItem.F.csiStatus, 1)
							.add("asc", "csi_data_number"), -1);
			...
			for (CsItem item : itemList) {
				...
				finishcal.setTime(orderList.get(orderList.size() - 1)
						.getCsoFinishTime());
				for (int i = 0; i < 30; i++) {
					Map<String, Object> map = new HashMap<String, Object>();
					if (i > 0) {
					...
					System.out.println("开始时间:"
							+ new DateUtil().dateToString(cal.getTime(),
									"yyyy-MM-dd HH:mm:ss")
							+ "\t"
							+ "结束时间:"
							+ new DateUtil().dateToString(finishcal.getTime(),
									"yyyy-MM-dd HH:mm:ss"));
					ordersList.add(map);
				}

				LzMap data = $.$("itemName", item.getCsiTitle())
						.add("itemId", item.getCsiId())
						.add("price", item.getCsiPrice())
						.add("title", item.getCsiTitle())
						.add("descript", item.getCsiRemark())
						.add("orders", ordersList)
						.add("time1", me.getFeature("time1"))
						.add("time2", me.getFeature("time2"));

				dataList.add(data);
			}

			return $.SendHtml($.json(JsonFormat.success().setData(
					$.add("list", dataList))), CHARSET);
		} catch (com.ccclubs.exception.MessageException oe) {
			oe.printStackTrace();
			Logger.getRootLogger().error(oe);
			return returnError("105", oe.getMessage());
	...
	public String otherBook() {
		...
		try {
			...
			CsArgument argument = csArgumentService.getCsArgument($.add(CsArgument.F.csaFlag,ArgumentKey.OTHER_BOOK_TIME));

			java.text.SimpleDateFormat formatter = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			java.util.Calendar calendar = java.util.Calendar.getInstance();
			calendar.setTime(takeTime);
			calendar.add(java.util.Calendar.MINUTE,argument != null ? Integer.parseInt(argument.getCsaValue()) : -15 );
			String dt_end = formatter.format(calendar.getTime());

			Page<CsOrder> page = csOrderService.getCsOrderPage($.getInteger("page", 0),5,$.add("definex","( (csuo.csuo_state is null || csuo.csuo_state = 2) and cso_use_member != '"
											+ member.getCsmId$()+ "' and cso_start_time BETWEEN  '" + dt_end + "'  and  '"+$.date(takeTime, "yyyy-MM-dd")+" 23:59:59'  )").add(CsOrder.F.csoOutlets, takeOutletsId)
									.add(CsOrder.F.csoStatus, 0).add("asc", "cso_start_time").add("join","LEFT JOIN cs_unit_order csuo on cso_id = csuo.csuo_order"));
			List<Map<String, Object>> dataList = new ArrayList<Map<String, Object>>();
			...
			LzMap pagemap = $.$("index", page.getIndex())
	...
	public String putComplain(){
		try {
			...
			complain.setCscObject(orderId == null || orderId.equals("") ? "" : "CsOrder@"+orderId);//关联订单
			complain.setCscLevel(starLevel == null || starLevel.equals("") ? 0 : Short.parseShort(starLevel));//星级
			complain.setCscLevel1(starLevel1 == null || starLevel1.equals("") ? 0 : Short.parseShort(starLevel1));
	...
	public String getMemberSumm(){
		try {
			...
			CsMemberSummary csMemberSummary = csMemberSummaryService.getCsMemberSummary($.add(CsMemberSummary.F.csmsMember, member.getCsmId()));
			
			LzMap data = new LzMap();
			data.add("join_time", member.getCsmAddTime());
			
			data.add("ord_total",  csMemberSummary == null ? 0 : csMemberSummary.getCsmsOrdTotal());
			data.add("unitord_total",  csMemberSummary == null ? 0 : csMemberSummary.getCsmsUnitordTotal());
			data.add("order_total",  csMemberSummary == null ? 0 : csMemberSummary.getCsmsOrdTotal()+ ( csMemberSummary == null ? 0 : csMemberSummary.getCsmsUnitordTotal()));
			
			Double d_ord_time_long = Double.parseDouble( csMemberSummary == null ? "0" : csMemberSummary.getCsmsOrdTimelong().toString());
			...
			Double d_unitord_time = Double.parseDouble( csMemberSummary == null ? "0" : csMemberSummary.getCsmsUnitordTimelong().toString());
			...
			data.add("ord_mileage",  csMemberSummary == null ? 0 : csMemberSummary.getCsmsOrdMileages());
			data.add("unitord_mileage",  csMemberSummary == null ? 0 : csMemberSummary.getCsmsUnitordMileages());
			data.add("mileage_total",  csMemberSummary == null ? 0 : csMemberSummary.getCsmsOrdMileages()+csMemberSummary.getCsmsUnitordMileages());
			
			data.add("ord_amount",  csMemberSummary == null ? 0 : csMemberSummary.getCsmsOrdAmount());
			data.add("unitord_amount",  csMemberSummary == null ? 0 : csMemberSummary.getCsmsUnitordAmount());
			data.add("amount_total",  csMemberSummary == null ? 0 : csMemberSummary.getCsmsOrdAmount()+csMemberSummary.getCsmsUnitordAmount());
			
			return $.SendHtml($.json(JsonFormat.success().setData($.$("member_summary", data))),CHARSET);
		} catch (Exception e) {
			// TODO: handle exception
			return returnError(e);
		}
	...
	public String getCoinList(){
		try {
			...
			if(type.equals("1")){
				coinList = csCoinService.getCsCoinList($.add(CsCoin.F.cscStatus, 1).add("definex", "( 1=1 and "+CsCoin.C.cscEnd+" >= '"+$.date(new Date(), "yyyy-MM-dd")+"' and csc_remain > 0)").add(CsCoin.F.cscMember, member.getCsmId()).add(CsCoin.F.cscHost, member.getCsmHost()).add("asc", CsCoin.C.cscEnd), -1);
			}else{
				coinList = csCoinService.getCsCoinList($.add("definex", "( 1=1 and "+CsCoin.C.cscEnd+" <= '"+$.date(new Date(), "yyyy-MM-dd")+"' and  csc_remain > 0)").add(CsCoin.F.cscMember, member.getCsmId()).add(CsCoin.F.cscHost, member.getCsmHost()).add("asc", CsCoin.C.cscEnd), -1);
			}
			...
			return $.SendHtml($.json(JsonFormat.success().setData($.add("list", dataList))), CHARSET);
		} catch (Exception e) {
			// TODO: handle exception
			return returnError(e);
		}
	...
	public String getCoinRecord(){
		try {
			...
			Page<CsUseRecord> page = csUseRecordService.getCsUseRecordPage($.getInteger("page", 0),10,$.add(CsUseRecord.F.csurMember, member.getCsmId()).add(CsUseRecord.F.csurHost, member.getCsmHost()));
			List<Map<String, Object>> dataList = new ArrayList<Map<String,Object>>();
			Map<String, Object> data;
	...
	public String getViolat(){
		try {
			...
			LzMap params = $.add(CsViolat.F.csvMember, member.getCsmId()).add("desc", "csv_happen_time");
			if(type == 2 ){
			...
			List<Map<String, Object>> mapList = new ArrayList<Map<String,Object>>(violatList.size());
			for (CsViolat csViolat : violatList) {
				...
				if(!$.empty(csViolat.getCsvDecipt$()) && csViolat.getCsvDecipt$().indexOf("-") != -1){
					String address = csViolat.getCsvDecipt().substring(0, csViolat.getCsvDecipt().lastIndexOf("-"));
					String decipt = csViolat.getCsvDecipt().substring(csViolat.getCsvDecipt().lastIndexOf("-")+1, csViolat.getCsvDecipt().length());
			...
			LzMap pagemap = $.add("count", mapList.size()).add("money", $(money)).add("score", score);
			
			return $.SendHtml($.json(JsonFormat.success().setData($.Map("list", mapList).add("summary", pagemap))),CHARSET);
	...
	public void setCsFavoriteAddressService(
			ICsFavoriteAddressService csFavoriteAddressService) {
